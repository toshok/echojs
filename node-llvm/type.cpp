...
#define LLVM_SET_PROTOTYPE_METHOD(obj, name)                                   \
    NODE_SET_PROTOTYPE_METHOD(obj, #name, Type::name)
...
#undef LLVM_SET_PROTOTYPE_METHOD
    ...
    target->Set(String::NewSymbol("Type"), s_func);
}
...
#define LLVM_TYPE_METHOD(name, llvm_ty)                                        \
    Handle<Value> Type::name(const Arguments &args) {                          \
        ...
        Type *result_type =                                                    \
            new Type(llvm::Type::llvm_ty(llvm::getGlobalContext()));           \
