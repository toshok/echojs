...
void GlobalVariable::Init(Handle<Object> target) {
    ...
    NODE_SET_PROTOTYPE_METHOD(s_ct, "setInitializer",
                              GlobalVariable::SetInitializer);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "setAlignment",
                              GlobalVariable::SetAlignment);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "toString", GlobalVariable::ToString);

    s_func = Persistent<Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("GlobalVariable"), s_func);
}

GlobalVariable::GlobalVariable(llvm::GlobalVariable *llvm_global)
...
Handle<::v8::Value> GlobalVariable::New(const Arguments &args) {
    ...
    if (args.Length()) {
        ...
        GlobalVariable *val = new GlobalVariable(new ::llvm::GlobalVariable(
