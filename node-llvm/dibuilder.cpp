...
DIBuilder::DIBuilder() : llvm_dibuilder(NULL) {}

DIBuilder::~DIBuilder() {}

void DIBuilder::Initialize() {}
...
Handle<v8::Value> DIBuilder::CreateCompileUnit(const Arguments &args) {
    ...
    Handle<v8::Value> result =
        DIDescriptor::New(dib->llvm_dibuilder->createCompileUnit(
            llvm::dwarf::DW_LANG_C99, *file, *dir, *producer, isOptimized,
...
Handle<v8::Value> DIBuilder::CreateFile(const Arguments &args) {
    ...
    membertypes.push_back(dib->llvm_dibuilder->createBasicType(
        "unsigned long long", 64, 64, llvm::dwarf::DW_ATE_unsigned));

    dib->ejsValueType = dib->llvm_dibuilder->createStructType(
        llvm_file, "ejsval", llvm_file, 0, 64, 64, 0, llvm::DIType(),
        dib->llvm_dibuilder->getOrCreateArray(membertypes));

    dib->ejsValuePointerType = dib->llvm_dibuilder->createPointerType(
        dib->ejsValueType, sizeof(void *) * 8);
#endif
...
llvm::DICompositeType DIBuilder::CreateDIFunctionType(llvm::DIFile file,
...
Handle<v8::Value> DIBuilder::CreateFunction(const Arguments &args) {
    ...
    Handle<v8::Value> result =
        DISubprogram::New(dib->llvm_dibuilder->createFunction(
...
Handle<v8::Value> DIBuilder::CreateLexicalBlock(const Arguments &args) {
    ...
    Handle<v8::Value> result = DILexicalBlock::New(
...
void DIDescriptor::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DIDescriptor"), s_func);
}
...
void DIType::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DIType"), s_func);
}
...
void DIScope::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DIScope"), s_func);
}
...
void DISubprogram::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DISubprogram"), s_func);
}
...
void DIFile::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DIFile"), s_func);
}
...
void DILexicalBlock::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DILexicalBlock"), s_func);
}

v8::Handle<v8::Value>
DILexicalBlock::New(llvm::DILexicalBlock llvm_dilexicalblock) {
    HandleScope scope;
    Local<Object> new_instance = DILexicalBlock::s_func->NewInstance();
    DILexicalBlock *new_dilexicalblock =
        new DILexicalBlock(llvm_dilexicalblock);
...
void DebugLoc::Init(Handle<Object> target) {
    ...
    s_func = Persistent<v8::Function>::New(s_ct->GetFunction());
    target->Set(String::NewSymbol("DebugLoc"), s_func);
}
...
Handle<v8::Value> DebugLoc::Get(const Arguments &args) {
    ...
    return scope.Close(
        DebugLoc::New(llvm::DebugLoc::get(line, column, discope, NULL)));
}
