...
Handle<v8::Value> ConstantArray::New(const Arguments &args) {
    return ThrowException(Exception::Error(
        String::New("ConstantArray is not meant to be instantiated.")));
}
...
Handle<v8::Value> ConstantArray::Get(const Arguments &args) {
    ...
    for (uint32_t i = 0; i < elements->Length(); i++) {
        element_constants.push_back(
            static_cast<llvm::Constant *>(Value::GetLLVMObj(elements->Get(i))));
    }

    Handle<v8::Value> result = Value::New(llvm::ConstantArray::get(
