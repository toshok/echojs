...
void BasicBlock::Init(Handle<Object> target) {
    ...
    s_ct->InstanceTemplate()->SetAccessor(String::NewSymbol("name"),
                                          BasicBlock::GetName);
    s_ct->InstanceTemplate()->SetAccessor(String::NewSymbol("parent"),
    ...
    target->Set(String::NewSymbol("BasicBlock"), s_func);
}
...
Handle<::v8::Value> BasicBlock::New(const Arguments &args) {
    ...
    if (args.Length()) {
        ...
        BasicBlock *bb = new BasicBlock(llvm::BasicBlock::Create(
            llvm::getGlobalContext(), *blockname, fun));
        bb->Wrap(args.This());
    }
...
Handle<v8::Value> BasicBlock::GetParent(Local<String> property,
...
Handle<v8::Value> BasicBlock::GetName(Local<String> property,
                                      const AccessorInfo &info) {
    HandleScope scope;
    BasicBlock *bb = ObjectWrap::Unwrap<BasicBlock>(info.This());
    Handle<v8::String> result = String::New(bb->llvm_bb->getName().data(),
                                            bb->llvm_bb->getName().size());
    return scope.Close(result);
}
