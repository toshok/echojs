...
void Constant::Init(Handle<Object> target) {
    ...
    target->Set(String::NewSymbol("Constant"), s_func);
}
...
Handle<v8::Value> Constant::New(const Arguments &args) {
    return ThrowException(Exception::Error(
        String::New("Constant is not meant to be instantiated.")));
}
...
Handle<v8::Value> Constant::GetBoolValue(const Arguments &args) {
    HandleScope scope;
    REQ_BOOL_ARG(0, b);
    Handle<v8::Value> result = Value::New(llvm::Constant::getIntegerValue(
...
Handle<v8::Value> Constant::GetIntegerValue(const Arguments &args) {
    ...
    if (args.Length() == 2)
        result = Value::New(llvm::Constant::getIntegerValue(
            ty, llvm::APInt(ty->getPrimitiveSizeInBits(), v)));
    else if (args.Length() == 3 && args[2]->IsNumber() &&
        ...
        return ThrowException(Exception::TypeError(
            String::New("Invalid parameters to Constant.getIntegerValue")));
    }
