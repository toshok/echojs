...
#define REQ_LLVM_VAL_ARG(I, VAR)                                               \
    if (args.Length() <= (I) ||                                                \
        !args[I]                                                               \
             ->IsObject() /* XXX || !jsllvm::Value::HasInstance(args[I]) */) { \
...
#define REQ_LLVM_CONST_ARG(I, VAR)                                                \
    if (args.Length() <= (I) ||                                                   \
        !args[I]                                                                  \
             ->IsObject() /* XXX || !jsllvm::Constant::HasInstance(args[I]) */) { \
        printf("in function %s\n", __PRETTY_FUNCTION__);                          \
        return ThrowException(Exception::TypeError(                               \
            String::New("Argument " #I " must be an llvm Constant")));            \
    }                                                                             \
    ::llvm::Constant *VAR =                                                       \
        static_cast<::llvm::Constant *>(jsllvm::Value::GetLLVMObj(args[I]));
...
#define REQ_NULLABLE_LLVM_CONST_ARG(I, VAR)                                       \
    ...
    if (args.Length() > (I) && args[I]->IsNull())                                 \
        ...
        VAR = static_cast<::llvm::Constant *>(                                    \
            jsllvm::Value::GetLLVMObj(args[I]));
...
#define REQ_LLVM_CONST_INT_ARG(I, VAR)                                            \
    ...
    ::llvm::ConstantInt *VAR = static_cast<::llvm::ConstantInt *>(                \
        jsllvm::Value::GetLLVMObj(args[I]));
...
#define REQ_LLVM_TYPE_ARG(I, VAR)                                              \
    if (args.Length() <= (I) ||                                                \
        !args[I]->IsObject() /* XXX || !jsllvm::Type::HasInstance(args[I]) */) \
        return ThrowException(Exception::TypeError(                            \
...
#define REQ_LLVM_BB_ARG(I, VAR)                                                     \
    if (args.Length() <= (I) ||                                                     \
        (!args[I]->IsNull() &&                                                      \
         !args[I]                                                                   \
              ->IsObject() /* XXX && !jsllvm::BasicBlock::HasInstance(args[I]) */)) \
        return ThrowException(Exception::TypeError(                                 \
...
#define REQ_LLVM_FUN_ARG(I, VAR)                                                  \
    if (args.Length() <= (I) ||                                                   \
        !args[I]                                                                  \
             ->IsObject() /* XXX || !jsllvm::Function::HasInstance(args[I]) */) { \
        printf("in function %s\n", __PRETTY_FUNCTION__);                          \
        return ThrowException(Exception::TypeError(                               \
...
#define REQ_LLVM_DIFILE_ARG(I, VAR)                                            \
    if (args.Length() <= (I) ||                                                \
        !args[I]                                                               \
             ->IsObject() /* XXX || !jsllvm::DIFile::HasInstance(args[I]) */)  \
        return ThrowException(Exception::TypeError(                            \
...
#define REQ_LLVM_DISCOPE_ARG(I, VAR)                                           \
    if (args.Length() <= (I) ||                                                \
        !args[I]                                                               \
             ->IsObject() /* XXX || !jsllvm::DIScope::HasInstance(args[I]) */) \
        return ThrowException(Exception::TypeError(                            \
...
#define REQ_LLVM_MDNODE_ARG(I, VAR)                                             \
    if (args.Length() <= (I) ||                                                 \
        !args[I]                                                                \
             ->IsObject() /* XXX || !jsllvm::MDNode::HasInstance(args[I]) */) { \
        printf("in function %s\n", __PRETTY_FUNCTION__);                        \
        return ThrowException(Exception::TypeError(                             \
