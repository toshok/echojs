...
static ejsval ArrayType_impl(ejsval env, ejsval _this, int argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

ejsval ArrayType_new(llvm::ArrayType *llvm_ty) {
    ejsval result =
        _ejs_object_new(_ejs_ArrayType_prototype, &_ejs_ArrayType_specops);
    ((ArrayType *)EJSVAL_TO_OBJECT(result))->type = llvm_ty;
    return result;
}

static ejsval ArrayType_get(ejsval env, ejsval _this, int argc, ejsval *args) {
...
ejsval ArrayType_prototype_toString(ejsval env, ejsval _this, int argc,
                                    ejsval *args) {
    std::string str;
...
ejsval ArrayType_prototype_dump(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
    ((ArrayType *)EJSVAL_TO_OBJECT(_this))->type->dump();
    return _ejs_undefined;
}

llvm::ArrayType *ArrayType_GetLLVMObj(ejsval val) {
    if (EJSVAL_IS_NULL(val))
        return NULL;
    return ((ArrayType *)EJSVAL_TO_OBJECT(val))->type;
}

void ArrayType_init(ejsval exports) {
    ...
    _ejs_ArrayType_prototype =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_ArrayType_specops);

    _ejs_ArrayType = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMArrayType", (EJSClosureFunc)ArrayType_impl,
        _ejs_ArrayType_prototype);

    _ejs_object_setprop_utf8(exports, "ArrayType", _ejs_ArrayType);

#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_ArrayType, x, ArrayType_##x)
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_ArrayType_prototype, x,                     \
                              ArrayType_prototype_##x)
