...
static ejsval BasicBlock_create(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
    ejsval F = _this;
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in BasicBlock[Symbol.create] is not a constructor");
...
static ejsval BasicBlock_impl(ejsval env, ejsval _this, int argc,
                              ejsval *args) {
    if (EJSVAL_IS_UNDEFINED(_this)) {
        ...
        bb->llvm_bb =
            llvm::BasicBlock::Create(llvm::getGlobalContext(), name, fun);
        return _this;
    }
}

ejsval BasicBlock_new(llvm::BasicBlock *llvm_bb) {
    ejsval result =
        _ejs_object_new(_ejs_BasicBlock_prototype, &_ejs_BasicBlock_specops);
    ((BasicBlock *)EJSVAL_TO_OBJECT(result))->llvm_bb = llvm_bb;
    return result;
}

ejsval BasicBlock_prototype_toString(ejsval env, ejsval _this, int argc,
                                     ejsval *args) {
    std::string str;
...
ejsval BasicBlock_prototype_dump(ejsval env, ejsval _this, int argc,
                                 ejsval *args) {
    ((BasicBlock *)EJSVAL_TO_OBJECT(_this))->llvm_bb->dump();
    return _ejs_undefined;
}

ejsval BasicBlock_prototype_get_name(ejsval env, ejsval _this, int argc,
                                     ejsval *args) {
    BasicBlock *bb = ((BasicBlock *)EJSVAL_TO_OBJECT(_this));
    return _ejs_string_new_utf8(bb->llvm_bb->getName().data());
}

ejsval BasicBlock_prototype_get_parent(ejsval env, ejsval _this, int argc,
                                       ejsval *args) {
    BasicBlock *bb = ((BasicBlock *)EJSVAL_TO_OBJECT(_this));
...
void BasicBlock_init(ejsval exports) {
    ...
    _ejs_BasicBlock_prototype =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_BasicBlock_specops);

    _ejs_BasicBlock = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMBasicBlock", (EJSClosureFunc)BasicBlock_impl,
        _ejs_BasicBlock_prototype);

    _ejs_object_setprop_utf8(exports, "BasicBlock", _ejs_BasicBlock);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_BasicBlock_prototype, x,                    \
                              BasicBlock_prototype_##x)
#define PROTO_ACCESSOR(x)                                                      \
    EJS_INSTALL_ATOM_GETTER(_ejs_BasicBlock_prototype, x,                      \
                            BasicBlock_prototype_get_##x)
...
#undef PROTO_ACCESSOR
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(
        _ejs_BasicBlock, create, BasicBlock_create, EJS_PROP_NOT_ENUMERABLE);
}
};
