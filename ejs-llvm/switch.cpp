...
static EJSObject *Switch_allocate() { return (EJSObject *)_ejs_gc_new(Switch); }

static ejsval Switch_impl(ejsval env, ejsval _this, int argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

ejsval Switch_new(llvm::SwitchInst *llvm_switch) {
    ejsval result =
        _ejs_object_new(_ejs_Switch_prototype, &_ejs_Switch_specops);
    ((Switch *)EJSVAL_TO_OBJECT(result))->llvm_switch = llvm_switch;
    return result;
}

ejsval Switch_prototype_toString(ejsval env, ejsval _this, int argc,
                                 ejsval *args) {
    std::string str;
...
ejsval Switch_prototype_dump(ejsval env, ejsval _this, int argc, ejsval *args) {
    ((Switch *)EJSVAL_TO_OBJECT(_this))->llvm_switch->dump();
    return _ejs_undefined;
}

ejsval Switch_prototype_addCase(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
...
llvm::SwitchInst *Switch_GetLLVMObj(ejsval val) {
    if (EJSVAL_IS_NULL(val))
        return NULL;
    return ((Switch *)EJSVAL_TO_OBJECT(val))->llvm_switch;
}

void Switch_init(ejsval exports) {
    ...
    _ejs_Switch_prototype =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_Switch_specops);

    _ejs_Switch = _ejs_function_new_utf8_with_proto(_ejs_null, "LLVMSwitch",
                                                    (EJSClosureFunc)Switch_impl,
                                                    _ejs_Switch_prototype);
    _ejs_object_setprop_utf8(exports, "Switch", _ejs_Switch);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_Switch_prototype, x, Switch_prototype_##x)
