...
static ejsval GlobalVariable_create(ejsval env, ejsval _this, int argc,
                                    ejsval *args) {
    ejsval F = _this;
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in GlobalVariable[Symbol.create] is not a constructor");
...
static ejsval GlobalVariable_impl(ejsval env, ejsval _this, int argc,
                                  ejsval *args) {
    if (EJSVAL_IS_UNDEFINED(_this)) {
        ...
        gv->llvm_global = new ::llvm::GlobalVariable(
            *module, type, false, visible ? llvm::GlobalValue::ExternalLinkage
...
ejsval GlobalVariable_new(llvm::GlobalVariable *llvm_global) {
...
ejsval GlobalVariable_prototype_toString(ejsval env, ejsval _this, int argc,
                                         ejsval *args) {
    std::string str;
    llvm::raw_string_ostream str_ostream(str);
    ((GlobalVariable *)EJSVAL_TO_OBJECT(_this))
        ->llvm_global->print(str_ostream);

    return _ejs_string_new_utf8(trim(str_ostream.str()).c_str());
}

ejsval GlobalVariable_prototype_dump(ejsval env, ejsval _this, int argc,
                                     ejsval *args) {
    ((GlobalVariable *)EJSVAL_TO_OBJECT(_this))->llvm_global->dump();
    return _ejs_undefined;
}

ejsval GlobalVariable_prototype_setInitializer(ejsval env, ejsval _this,
...
ejsval GlobalVariable_prototype_setAlignment(ejsval env, ejsval _this, int argc,
                                             ejsval *args) {
    GlobalVariable *global = (GlobalVariable *)EJSVAL_TO_OBJECT(_this);
...
void GlobalVariable_init(ejsval exports) {
    ...
    _ejs_GlobalVariable_prototype =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_Object_specops);

    _ejs_GlobalVariable = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMGlobalVariable", (EJSClosureFunc)GlobalVariable_impl,
        _ejs_GlobalVariable_prototype);

    _ejs_object_setprop_utf8(exports, "GlobalVariable", _ejs_GlobalVariable);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_GlobalVariable_prototype, x,                \
                              GlobalVariable_prototype_##x)
...
#undef PROTO_METHOD
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_GlobalVariable, create,
                                      GlobalVariable_create,
