...
static ejsval AllocaInst_impl(ejsval env, ejsval _this, int argc,
                              ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}
...
ejsval AllocaInst_new(llvm::AllocaInst *llvm_alloca) {
    ejsval result =
        _ejs_object_new(_ejs_AllocaInst_prototype, &_ejs_AllocaInst_specops);
    ((AllocaInst *)EJSVAL_TO_OBJECT(result))->llvm_alloca = llvm_alloca;
    return result;
}

ejsval AllocaInst_prototype_toString(ejsval env, ejsval _this, int argc,
                                     ejsval *args) {
    std::string str;
...
ejsval AllocaInst_prototype_dump(ejsval env, ejsval _this, int argc,
                                 ejsval *args) {
    ((AllocaInst *)EJSVAL_TO_OBJECT(_this))->llvm_alloca->dump();
    return _ejs_undefined;
}

ejsval AllocaInst_prototype_setAlignment(ejsval env, ejsval _this, int argc,
                                         ejsval *args) {
...
llvm::AllocaInst *AllocaInst_GetLLVMObj(ejsval val) {
    if (EJSVAL_IS_NULL(val))
        return NULL;
    return ((AllocaInst *)EJSVAL_TO_OBJECT(val))->llvm_alloca;
}

void AllocaInst_init(ejsval exports) {
    ...
    _ejs_AllocaInst_prototype =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_AllocaInst_specops);

    _ejs_AllocaInst = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMAllocaInst", (EJSClosureFunc)AllocaInst_impl,
        _ejs_AllocaInst_prototype);

    _ejs_object_setprop_utf8(exports, "AllocaInst", _ejs_AllocaInst);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_AllocaInst_prototype, x,                    \
                              AllocaInst_prototype_##x)
