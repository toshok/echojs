...
static ejsval DIBuilder_create(ejsval env, ejsval _this, int argc,
                               ejsval *args) {
    ejsval F = _this;
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in DIBuilder[Symbol.create] is not a constructor");
    EJSObject *F_ = EJSVAL_TO_OBJECT(F);
    // 2. Let obj be the result of calling OrdinaryCreateFromConstructor(F,
    // "%DatePrototype%", ([[DateData]]) ).
...
static ejsval DIBuilder_impl(ejsval env, ejsval _this, int argc, ejsval *args) {
...
static ejsval DIBuilder_prototype_createCompileUnit(ejsval env, ejsval _this,
                                                    int argc, ejsval *args) {
    REQ_UTF8_ARG(0, file);
    REQ_UTF8_ARG(1, dir);
    ...
    return DIDescriptor_new(dib->llvm_dibuilder->createCompileUnit(
        llvm::dwarf::DW_LANG_C99, file, dir, producer, isOptimized, flags,
        runtimeVersion));
}

static ejsval DIBuilder_prototype_createFile(ejsval env, ejsval _this, int argc,
                                             ejsval *args) {
    REQ_UTF8_ARG(0, file);
    REQ_UTF8_ARG(1, dir);
...
createDIFunctionType(llvm::DIBuilder *llvm_dibuilder, llvm::DIFile file,
                     llvm::FunctionType *fty) {
    // XXX add function parameter types
    llvm::DITypeArray param_types =
        llvm_dibuilder->getOrCreateTypeArray(llvm::None);
    return llvm_dibuilder->createSubroutineType(file, param_types);
}

static ejsval DIBuilder_prototype_createFunction(ejsval env, ejsval _this,
                                                 int argc, ejsval *args) {
    ...
    return DISubprogram_new(dib->llvm_dibuilder->createFunction(
        discope, name, linkageName, file, line_no,
        createDIFunctionType(dib->llvm_dibuilder, file, fn->getFunctionType()),
        isLocalToUnit, isDefinition, scopeLine, flags, isOptimized, fn));
}

static ejsval DIBuilder_prototype_createLexicalBlock(ejsval env, ejsval _this,
                                                     int argc, ejsval *args) {
    ...
    return DILexicalBlock_new(
        dib->llvm_dibuilder->createLexicalBlock(parentScope, file, line, col));
}

static ejsval DIBuilder_prototype_finalize(ejsval env, ejsval _this, int argc,
                                           ejsval *args) {
    DIBuilder *dib = ((DIBuilder *)EJSVAL_TO_OBJECT(_this));
...
void DIBuilder_init(ejsval exports) {
    ...
    _ejs_DIBuilder = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMDIBuilder", (EJSClosureFunc)DIBuilder_impl,
        _ejs_DIBuilder_prototype);

    _ejs_object_setprop_utf8(exports, "DIBuilder", _ejs_DIBuilder);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_DIBuilder_prototype, x,                     \
                              DIBuilder_prototype_##x)
...
#undef PROTO_METHOD
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_DIBuilder, create, DIBuilder_create,
...
ejsval DIDescriptor_new(llvm::DIDescriptor llvm_didescriptor) {
    ejsval result = _ejs_object_new(_ejs_DIDescriptor_prototype,
                                    &_ejs_DIDescriptor_specops);
    ((DIDescriptor *)EJSVAL_TO_OBJECT(result))->llvm_didescriptor =
        llvm_didescriptor;
    return result;
}

static ejsval DIDescriptor_impl(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval DIDescriptor_prototype_verify(ejsval env, ejsval _this, int argc,
                                            ejsval *args) {
...
void DIDescriptor_init(ejsval exports) {
    ...
    _ejs_DIDescriptor = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMDIDescriptor", (EJSClosureFunc)DIDescriptor_impl,
        _ejs_DIDescriptor_prototype);

    _ejs_object_setprop_utf8(exports, "DIDescriptor", _ejs_DIDescriptor);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_DIDescriptor_prototype, x,                  \
                              DIDescriptor_prototype_##x)
...
ejsval DISubprogram_new(llvm::DISubprogram llvm_disubprogram) {
    ejsval result = _ejs_object_new(_ejs_DISubprogram_prototype,
                                    &_ejs_DISubprogram_specops);
    ((DISubprogram *)EJSVAL_TO_OBJECT(result))->llvm_disubprogram =
        llvm_disubprogram;
    return result;
}
...
static ejsval DISubprogram_impl(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval DISubprogram_prototype_verify(ejsval env, ejsval _this, int argc,
                                            ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}
...
void DISubprogram_init(ejsval exports) {
    ...
    _ejs_DISubprogram = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMDISubprogram", (EJSClosureFunc)DISubprogram_impl,
        _ejs_DISubprogram_prototype);

    _ejs_object_setprop_utf8(exports, "DISubprogram", _ejs_DISubprogram);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_DISubprogram_prototype, x,                  \
                              DISubprogram_prototype_##x)
...
static EJSObject *DIFile_allocate() { return (EJSObject *)_ejs_gc_new(DIFile); }

ejsval DIFile_new(llvm::DIFile llvm_difile) {
    ejsval result =
        _ejs_object_new(_ejs_DIFile_prototype, &_ejs_DIFile_specops);
...
static ejsval DIFile_impl(ejsval env, ejsval _this, int argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval DIFile_prototype_verify(ejsval env, ejsval _this, int argc,
                                      ejsval *args) {
...
void DIFile_init(ejsval exports) {
    ...
    _ejs_DIFile = _ejs_function_new_utf8_with_proto(_ejs_null, "LLVMDIFile",
                                                    (EJSClosureFunc)DIFile_impl,
                                                    _ejs_DIFile_prototype);

    _ejs_object_setprop_utf8(exports, "DIFile", _ejs_DIFile);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_DIFile_prototype, x, DIFile_prototype_##x)
...
ejsval DILexicalBlock_new(llvm::DILexicalBlock llvm_dilexicalblock) {
    ejsval result = _ejs_object_new(_ejs_DILexicalBlock_prototype,
                                    &_ejs_DILexicalBlock_specops);
    ((DILexicalBlock *)EJSVAL_TO_OBJECT(result))->llvm_dilexicalblock =
        llvm_dilexicalblock;
    return result;
}
...
static ejsval DILexicalBlock_impl(ejsval env, ejsval _this, int argc,
                                  ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}
...
void DILexicalBlock_init(ejsval exports) {
    ...
    _ejs_DILexicalBlock = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMDILexicalBlock", (EJSClosureFunc)DILexicalBlock_impl,
        _ejs_DILexicalBlock_prototype);

    _ejs_object_setprop_utf8(exports, "DILexicalBlock", _ejs_DILexicalBlock);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_DILexicalBlock_prototype, x,                \
                              DILexicalBlock_prototype_##x)
...
ejsval DebugLoc_new(llvm::DebugLoc llvm_debugloc) {
    ejsval result =
        _ejs_object_new(_ejs_DebugLoc_prototype, &_ejs_DebugLoc_specops);
...
static ejsval DebugLoc_impl(ejsval env, ejsval _this, int argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval DebugLoc_get(ejsval env, ejsval _this, int argc, ejsval *args) {
    REQ_INT_ARG(0, line);
...
void DebugLoc_init(ejsval exports) {
    ...
    _ejs_DebugLoc = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMDebugLoc", (EJSClosureFunc)DebugLoc_impl,
        _ejs_DebugLoc_prototype);
