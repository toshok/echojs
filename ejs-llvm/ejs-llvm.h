...
#define REQ_LLVM_VAL_ARG(I, VAR)                                               \
    if (argc <= (I) ||                                                         \
        !EJSVAL_IS_OBJECT(                                                     \
            args[I]) /*|| !jsllvm::Value::HasInstance(args[I]) */) {           \
        printf("in function %s\n", __PRETTY_FUNCTION__);                       \
        abort();                                                               \
    }                                                                          \
...
#define REQ_LLVM_CONST_ARG(I, VAR)                                                   \
    if (argc <=                                                                      \
        (I) /*|| !args[I]->IsObject() || !jsllvm::Constant::HasInstance(args[I]) */) \
        abort();                                                                     \
    ::llvm::Constant *VAR =                                                          \
        static_cast<::llvm::Constant *>(Value_GetLLVMObj(args[I]));
...
#define REQ_NULLABLE_LLVM_CONST_ARG(I, VAR)                                    \
    ...
    if (argc > (I) && EJSVAL_IS_NULL(args[(I)]))                               \
        ...
        argc <= (I) ||                                                         \
        !EJSVAL_IS_OBJECT(                                                     \
            args[I]) /* XXX || !jsllvm::Constant::HasInstance(args[I]) */) {   \
        printf("in function %s\n", __PRETTY_FUNCTION__);                       \
        abort();                                                               \
    } else                                                                     \
...
#define REQ_LLVM_TYPE_ARG(I, VAR)                                                \
    if (argc <=                                                                  \
        (I) /*|| !args[I]->IsObject() || !jsllvm::Type::HasInstance(args[I]) */) \
        abort();                                                                 \
    ::llvm::Type *VAR = Type_GetLLVMObj(args[I]);

#define REQ_LLVM_BB_ARG(I, VAR)                                                                                \
    if (argc <=                                                                                                \
        (I) /* || (!args[I]->IsNull() && !args[I]->IsObject() && !jsllvm::BasicBlock::HasInstance(args[I]) */) \
        abort();                                                                                               \
    ::llvm::BasicBlock *VAR = BasicBlock_GetLLVMObj(args[I]);

#define REQ_LLVM_FUN_ARG(I, VAR)                                                      \
    if (argc <=                                                                       \
        (I) /* || !args[I]->IsObject() || !jsllvm::Function::HasInstance(args[I]) */) \
        abort();                                                                      \
    ::llvm::Function *VAR = Function_GetLLVMObj(args[I]);

#define REQ_LLVM_DIFILE_ARG(I, VAR)                                                 \
    if (argc <=                                                                     \
        (I) /* || !args[I]->IsObject() || !jsllvm::DIFile::HasInstance(args[I]) */) \
        abort();                                                                    \
