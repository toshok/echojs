...
static ejsval ConstantArray_impl(ejsval env, ejsval _this, int argc,
                                 ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval ConstantArray_get(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
    ...
    for (int i = 0; i < EJSARRAY_LEN(elements); i++) {
        element_constants.push_back(static_cast<llvm::Constant *>(
            Value_GetLLVMObj(EJSDENSEARRAY_ELEMENTS(elements)[i])));
    }

    return Value_new(llvm::ConstantArray::get(
...
void ConstantArray_init(ejsval exports) {
    ...
    _ejs_ConstantArray = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMConstantArray", (EJSClosureFunc)ConstantArray_impl,
        _ejs_ConstantArray_prototype);

    _ejs_object_setprop_utf8(exports, "ConstantArray", _ejs_ConstantArray);

#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_ConstantArray, x, ConstantArray_##x)
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_ConstantArray_prototype, x,                 \
                              ConstantArray_prototype_##x)
