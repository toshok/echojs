...
EJSObject *Type_alloc_instance() { return (EJSObject *)_ejs_gc_new(Type); }

ejsval Type_get_prototype() { return _ejs_Type_prototype; }

static ejsval Type_impl(ejsval env, ejsval _this, int argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

#define LLVM_TYPE_METHOD_PROXY(name) LLVM_TYPE_METHOD(name, name)
#define LLVM_TYPE_METHOD(name, llvm_ty)                                        \
    ejsval Type_##name(ejsval env, ejsval _this, int argc, ejsval *args) {     \
...
ejsval Type_prototype_pointerTo(ejsval env, ejsval _this, int argc,
                                ejsval *args) {
    return Type_new(((Type *)EJSVAL_TO_OBJECT(_this))->type->getPointerTo());
}

ejsval Type_prototype_isVoid(ejsval env, ejsval _this, int argc, ejsval *args) {
    return ((Type *)EJSVAL_TO_OBJECT(_this))->type->isVoidTy() ? _ejs_true
                                                               : _ejs_false;
}

ejsval Type_prototype_toString(ejsval env, ejsval _this, int argc,
                               ejsval *args) {
    std::string str;
...
ejsval Type_prototype_dump(ejsval env, ejsval _this, int argc, ejsval *args) {
    ((Type *)EJSVAL_TO_OBJECT(_this))->type->dump();
    return _ejs_undefined;
}

ejsval Type_new(llvm::Type *llvm_ty) {
...
llvm::Type *Type_GetLLVMObj(ejsval val) {
...
void Type_init(ejsval exports) {
    ...
    _ejs_Type = _ejs_function_new_utf8_with_proto(
        _ejs_null, "LLVMType", (EJSClosureFunc)Type_impl, _ejs_Type_prototype);
...
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_Type_prototype, x, Type_prototype_##x)
