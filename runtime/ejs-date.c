...
static ejsval _ejs_Date_impl(ejsval env, ejsval _this, uint32_t argc,
                             ejsval *args) {
    if (EJSVAL_IS_UNDEFINED(_this)) {
        // called as a function
        if (argc == 0) {
            // XXX we shouldn't be creating a date object here and immediately
            // throwing it away.
        ...
        if (argc < 2) {
            ...
            // there are all sorts of validation steps here that are missing
            // from ejs
...
static ejsval _ejs_Date_prototype_toString(ejsval env, ejsval _this,
...
static ejsval _ejs_Date_prototype_getTimezoneOffset(ejsval env, ejsval _this,
                                                    uint32_t argc,
                                                    ejsval *args) {
    EJSDate *date = (EJSDate *)EJSVAL_TO_OBJECT(_this);
    struct tm tm;
...
static ejsval _ejs_Date_prototype_getTime(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    return NUMBER_TO_EJSVAL(
        _ejs_date_get_time((EJSDate *)EJSVAL_TO_OBJECT(_this)));
}

static ejsval _ejs_Date_now(ejsval env, ejsval _this, uint32_t argc,
                            ejsval *args) {
    struct timeval tv;
    struct timezone tz;
    gettimeofday(&tv, &tz);
...
static ejsval _ejs_Date_create(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in Date[Symbol.create] is not a constructor");
...
void _ejs_date_init(ejsval global) {
    _ejs_Date = _ejs_function_new_without_proto(_ejs_null, _ejs_atom_Date,
                                                (EJSClosureFunc)_ejs_Date_impl);
...
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs_Date_prototype, x,                    \
                                    _ejs_Date_prototype_##x,                   \
                                    EJS_PROP_NOT_ENUMERABLE)
#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs_Date, x, _ejs_Date_##x,               \
                                    EJS_PROP_NOT_ENUMERABLE)
...
#undef PROTO_METHOD
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_Date, create, _ejs_Date_create,
                                      EJS_PROP_NOT_ENUMERABLE);
}
