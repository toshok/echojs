...
#define NATIVE_ERROR_CTOR(err)                                                 \
    static ejsval _ejs_##err##_impl(ejsval env, ejsval _this, uint32_t argc,   \
                                    ejsval *args) {                            \
        if (EJSVAL_IS_UNDEFINED(_this))                                        \
            _this =                                                            \
                _ejs_object_new(_ejs_##err##_prototype, &_ejs_Error_specops);  \
                                                                               \
        if (argc >= 1) {                                                       \
...
static ejsval _ejs_Error_prototype_toString(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    if (!EJSVAL_IS_OBJECT(_this)) {
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Error.prototype.toString called on non-object");
    }
...
static ejsval _ejs_Error_create(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in Error[Symbol.create] is not a constructor");
...
ejsval _ejs_nativeerror_new(EJSNativeErrorType err_type, ejsval msg) {
    ...
    case EJS_ERROR:
        ...
        break;
    case EJS_EVAL_ERROR:
        proto = _ejs_EvalError_prototype;
        break;
    case EJS_RANGE_ERROR:
        proto = _ejs_RangeError_prototype;
        break;
    case EJS_REFERENCE_ERROR:
        proto = _ejs_ReferenceError_prototype;
        break;
    case EJS_SYNTAX_ERROR:
        proto = _ejs_SyntaxError_prototype;
        break;
    case EJS_TYPE_ERROR:
        proto = _ejs_TypeError_prototype;
        break;
    case EJS_URI_ERROR:
        proto = _ejs_URIError_prototype;
        break;
    }
    ...
    switch (err_type) {
    case EJS_ERROR:
        _ejs_Error_impl(_ejs_null, exc, 1, &msg);
        break;
    case EJS_EVAL_ERROR:
        _ejs_EvalError_impl(_ejs_null, exc, 1, &msg);
        break;
    case EJS_RANGE_ERROR:
        _ejs_RangeError_impl(_ejs_null, exc, 1, &msg);
        break;
    case EJS_REFERENCE_ERROR:
        _ejs_ReferenceError_impl(_ejs_null, exc, 1, &msg);
        break;
    case EJS_SYNTAX_ERROR:
        _ejs_SyntaxError_impl(_ejs_null, exc, 1, &msg);
        break;
    case EJS_TYPE_ERROR:
        _ejs_TypeError_impl(_ejs_null, exc, 1, &msg);
        break;
    case EJS_URI_ERROR:
        _ejs_URIError_impl(_ejs_null, exc, 1, &msg);
        break;
    }
...
ejsval _ejs_nativeerror_new_utf8(EJSNativeErrorType err_type,
                                 const char *message) {
...
void _ejs_error_init(ejsval global) {
    ejsval toString =
        _ejs_function_new_native(_ejs_null, _ejs_atom_toString,
                                 (EJSClosureFunc)_ejs_Error_prototype_toString);
    _ejs_gc_add_root(&toString);

#define EJS_ADD_NATIVE_ERROR_TYPE(err)                                         \
    EJS_MACRO_START                                                            \
    _ejs_##err = _ejs_function_new_without_proto(                              \
        _ejs_null, _ejs_atom_##err, (EJSClosureFunc)_ejs_##err##_impl);        \
    _ejs_object_setprop(global, _ejs_atom_##err, _ejs_##err);                  \
    _ejs_##err##_prototype = _ejs_object_new(_ejs_null, &_ejs_Object_specops); \
    _ejs_object_setprop(_ejs_##err, _ejs_atom_prototype,                       \
                        _ejs_##err##_prototype);                               \
    _ejs_object_define_value_property(                                         \
        _ejs_##err##_prototype, _ejs_atom_constructor, _ejs_##err,             \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);  \
                                                                               \
    _ejs_object_setprop(_ejs_##err##_prototype, _ejs_atom_name,                \
                        _ejs_atom_##err);                                      \
    _ejs_object_setprop(_ejs_##err##_prototype, _ejs_atom_toString, toString); \
                                                                               \
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_##err, create, _ejs_Error_create,   \
...
void _ejs_throw_nativeerror_utf8(EJSNativeErrorType error_type,
                                 const char *message) {
...
void _ejs_throw_nativeerror(EJSNativeErrorType error_type, ejsval message) {
