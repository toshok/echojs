...
// length below which we eschew creating a rope and just create a flat string
// containing both primstrs
#define FLAT_ROPE_THRESHOLD 32

// length below which we eschew creating a dependent string and just create a
// flat string containing the slice
#define FLAT_DEP_THRESHOLD 32

int32_t ucs2_strcmp(const jschar *s1, const jschar *s2) {
    if (s1 == s2)
        return 0;
...
jschar *ucs2_strdup(const jschar *str) {
...
int32_t ucs2_strlen(const jschar *str) {
    int32_t rv = 0;
    while (*str++)
        rv++;
    return rv;
...
uint32_t ucs2_hash(const jschar *str, int32_t hash, int length) {
...
jschar *ucs2_strstr(const jschar *haystack, const jschar *needle) {
...
jschar *ucs2_strrstr(const jschar *haystack, const jschar *needle) {
...
static jschar utf8_to_ucs2(const unsigned char *input,
...
static int utf16_to_utf8_char(const jschar *utf16, char *utf8, int *utf16_adv) {
    jschar ucs2;
    ...
    *utf16_adv = 1;
    ...
    if (ucs2 >= 0x800 && ucs2 < 0xFFFF) {
        if (ucs2 >= 0xD800 && ucs2 <= 0xDFFF) {
            ...
            uint32_t combined =
                0x10000 + (((ucs2 - 0xD800) << 10) | (ucs2_2 - 0xDC00));
...
char *ucs2_to_utf8(const jschar *str) {
...
char *ucs2_to_utf8_buf(const jschar *str, char *buf, size_t buf_size) {
    int len = ucs2_strlen(str);
...
static ejsval _ejs_String_impl(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
...
static ejsval _ejs_String_prototype_toString(ejsval env, ejsval _this,
...
ejsval GetReplaceSubstitution(ejsval matched, ejsval string, int position,
    ...
    // unit elements from
    //     replacement to result while performing replacements as specified in
    //     Table 42. These $ replacements
    //     are done left-to-right, and, once such a replacement is performed,
    //     the new replacement text is not
    ...
    // 2 passes.  first pass calculates the length, the second pass builds the
    // string
        ...
        if (replacement_p[i] == '$') {
            ...
            case '9': {
                ...
                if (i < replacement_len - 2 && isdigit(replacement_p[i + 2])) {
                    ...
                    if (n == 0 || n > m)
                        ; // empty string goes here, so result_len doesn't
                          // change
                    else {
                        ejsval captured_str =
                            Get(captures, NUMBER_TO_EJSVAL(n));
        ...
        if (replacement_p[i] == '$') {
            ...
            case '$': // $$
                *result_p++ = '$';
                ...
                break;
            case '&':
                memmove(result_p, EJSVAL_TO_FLAT_STRING(matched),
                        matchLength * sizeof(jschar));
                result_p += matchLength;
                i++;
                break;
            case '\'':
                if (tailPos < stringLength) {
                    memmove(result_p, EJSVAL_TO_FLAT_STRING(string) + tailPos,
                            (stringLength - tailPos) * sizeof(jschar));
            ...
            case '9': {
                ...
                if (!EJSVAL_IS_UNDEFINED(captured_str)) {
                    memmove(result_p, EJSVAL_TO_FLAT_STRING(captured_str),
                            EJSVAL_TO_STRLEN(captured_str) * sizeof(jschar));
...
static ejsval _ejs_String_prototype_replace(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    ...
    // 11. Search string for the first occurrence of searchString and let pos be
    // the index within string
    //     of the first code unit of the matched substring and let matched be
    ...
    if (functionalReplace) {
        ...
        ejsval replValue =
            _ejs_invoke_closure(replaceValue, _ejs_undefined, 3, call_args);
    ...
    // 13. Else,
        ...
        // b. Let replStr be GetReplaceSubstitution(matched, string, pos,
        // captures, replaceValue).
        replStr = GetReplaceSubstitution(matched, string, pos, captures,
                                         replaceValue);
    }
    // 14. Let tailPos be pos + the number of code units in matched.
    int tailPos = pos + EJSVAL_TO_STRLEN(matched);

    // 15. Let newString be the String formed by concatenating the first pos
    // code units of string, replStr, and
    //     the trailing substring of string starting at index tailPos. If pos is
    //     0, the first element of the
    //     concatenation will be the empty String.
    ejsval newString = _ejs_string_concatv(
        pos == 0 ? _ejs_atom_empty : _ejs_string_new_substring(string, 0, pos),
        replStr, tailPos == EJSVAL_TO_STRLEN(string) - 1
                     ? _ejs_atom_empty
                     : _ejs_string_new_substring(
                           string, tailPos, EJSVAL_TO_STRLEN(string) - tailPos),
        _ejs_undefined);
...
jschar _ejs_string_ucs2_at(EJSPrimString *primstr, uint32_t offset) {
    ...
    case EJS_STRING_DEPENDENT:
        return _ejs_string_ucs2_at(primstr->data.dependent.dep,
                                   offset + primstr->data.dependent.off);
    case EJS_STRING_ROPE:
        if (offset < primstr->data.rope.left->length) {
            return _ejs_string_ucs2_at(primstr->data.rope.left, offset);
        } else {
            return _ejs_string_ucs2_at(primstr->data.rope.right,
                                       offset -
                                           primstr->data.rope.left->length);
        }
...
static ejsval _ejs_String_prototype_charAt(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args) {
    ejsval primStr;
...
static ejsval _ejs_String_prototype_charCodeAt(ejsval env, ejsval _this,
                                               uint32_t argc, ejsval *args) {
    ejsval primStr;
    ...
    return NUMBER_TO_EJSVAL(
        _ejs_string_ucs2_at(EJSVAL_TO_STRING(primStr), idx));
}

static ejsval _ejs_String_prototype_concat(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_String_prototype_indexOf(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    int idx = -1;
    if (argc == 0)
    ...
    if (EJSVAL_IS_STRING(haystack)) {
        ...
        haystack_cstr = EJSVAL_TO_FLAT_STRING(
            ((EJSString *)EJSVAL_TO_OBJECT(haystack))->primStr);
    }
    ...
    if (EJSVAL_IS_STRING(needle)) {
        ...
        needle_cstr = EJSVAL_TO_FLAT_STRING(
            ((EJSString *)EJSVAL_TO_OBJECT(needle))->primStr);
    }
...
static ejsval _ejs_String_prototype_lastIndexOf(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    int idx = -1;
    if (argc == 0)
        return NUMBER_TO_EJSVAL(idx);
...
static ejsval _ejs_String_prototype_localeCompare(ejsval env, ejsval _this,
                                                  uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_String_prototype_match(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    ...
    // 7. Let rx be the result of the abstract operation RegExpCreate(regexp,
    // undefined) (see 21.2.3.3)
...
static ejsval _ejs_String_prototype_search(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args) {
    ejsval regexp = _ejs_undefined;
    if (argc > 0)
        regexp = args[0];
...
static ejsval _ejs_String_prototype_substring(ejsval env, ejsval _this,
                                              uint32_t argc, ejsval *args) {
    ...
    /* 10. Return a String whose length is to - from, containing characters from
     * S, namely the characters with indices  */
    /*     from through to-1, in ascending order. */
    return _ejs_string_new_substring(S, from, to - from);
}

// ECMA262: B.2.3
static ejsval _ejs_String_prototype_substr(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args) {
    ejsval start = _ejs_undefined;
    ...
    /* 8. Return a String containing Result(6) consecutive characters from
     * Result(1) beginning with the character at  */
    /*    position Result(5). */
    return _ejs_string_new_substring(Result1, Result5, Result6);
}

static ejsval _ejs_String_prototype_toLowerCase(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    /* 1. Call CheckObjectCoercible passing the this value as its argument. */
    /* 2. Let S be the result of calling ToString, giving it the this value as
     * its argument. */
     ...
     * lowercase equivalent of the corresponding  */
    /*    character of S or the actual corresponding character of S if no
...
static ejsval _ejs_String_prototype_toLocaleLowerCase(ejsval env, ejsval _this,
                                                      uint32_t argc,
                                                      ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_String_prototype_toUpperCase(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    /* 1. Call CheckObjectCoercible passing the this value as its argument. */
    /* 2. Let S be the result of calling ToString, giving it the this value as
     * its argument. */
...
static ejsval _ejs_String_prototype_toLocaleUpperCase(ejsval env, ejsval _this,
                                                      uint32_t argc,
                                                      ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static EJSBool IsWhitespace(jschar c) {
...
static ejsval _ejs_String_prototype_trim(ejsval env, ejsval _this,
                                         uint32_t argc, ejsval *args) {
    // 1. Let O be RequireObjectCoercible(this value).
    ...
    ejsval T = _ejs_string_new_substring(S, leading,
                                         flat->length - leading - trailing);

    // 5. Return T.
    return T;
}

static ejsval _ejs_String_prototype_valueOf(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

// ECMA262: 15.5.4.14
typedef enum { MATCH_RESULT_FAILURE, MATCH_RESULT_SUCCESS } MatchResultType;
...
static MatchResultState SplitMatch(ejsval S, int q, ejsval R) {
    /* 1. If R is a RegExp object (its [[Class]] is "RegExp"), then */
    if (EJSVAL_IS_REGEXP(R)) {
        /*    a. Call the [[Match]] internal method of R giving it the arguments
         * S and q, and return the MatchResult  */
        /*       result. */
        EJS_NOT_IMPLEMENTED();
    }
     ...
     * such that the character at position q+i of S */
    /*    is different from the character at position i of R, then return
     * failure. */
...
static ejsval _ejs_String_prototype_split(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    ...
    // 10. If limit is undefined, let lim = 2^53-1; else let lim =
    // ToLength(limit).
    int64_t lim =
    ...
    // 20. Let T be a String value equal to the substring of S consisting of the
...
static ejsval _ejs_String_prototype_slice(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    // assert argc >= 1
    ...
    // If end is undefined, let intEnd be len; else let intEnd be
    // ToInteger(end).
    int intEnd = EJSVAL_IS_UNDEFINED(end) ? len : ToInteger(end);

    // If intStart is negative, let from be max(len + intStart,0); else let from
    // be min(intStart, len).
    ...
    // Return a String containing span consecutive characters from S beginning
    // with the character at position from.
    return _ejs_string_new_substring(S, from, span);
}

static ejsval _ejs_String_fromCharCode(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
    int length = argc;
...
static int codepoint_to_codeunits(int64_t codepoint, jschar *units) {
...
static ejsval _ejs_String_fromCodePoint(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
    if (argc == 0)
        return _ejs_atom_empty;
    ...
    jschar *elements =
        (jschar *)malloc(sizeof(jschar) * 2 * length); // a safe maximum
    jschar *el = elements;
    ...
    while (nextIndex < length) {
        ...
        if (ToDouble(nextCP) != ToInteger(nextCP)) {
            free(elements);
            ejsval msg = _ejs_string_concat(
                _ejs_string_new_utf8("Invalid code point: "), ToString(nextCP));
        ...
        //    f. Append the elements of the UTF-16Encoding (10.1.1) of nextCP to
        //    the end of elements.
...
static ejsval _ejs_String_raw(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ejsval callsite = _ejs_undefined;
    if (argc > 0)
        callsite = args[0];

    // 1. Let substitutions be a List consisting of all of the arguments passed
    // to this function, starting with the second argument. If fewer than two
    // arguments were passed, the List is empty.
    ...
    while (EJS_TRUE) {
        ...
        if (nextIndex + 1 == literalSegments)
            //        i. Return the string value whose elements are, in order,
            //        the elements in the List stringElements. If stringElements
...
static ejsval _ejs_String_prototype_startsWith(ejsval env, ejsval _this,
                                               uint32_t argc, ejsval *args) {
    ...
    // 6. Let searchStr be ToString(searchString).   XXX specbug redundant step
    // here
    // 7. ReturnIfAbrupt(searchStr).
    ejsval searchStr = ToString(searchString);

    // 8. Let pos be ToInteger(position). (If position is undefined, this step
    ...
    // 14. If the searchLength sequence of elements of S starting at start is
    // the same as the full element sequence of searchStr, return true.
...
static ejsval _ejs_String_prototype_endsWith(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ejsval searchString = _ejs_undefined;
...
// NOTE Returns a nonnegative integer Number less than 1114112 (0x110000) that
// is the UTF-16 encoded code
// point value starting at the string element at position pos in the String
// resulting from converting this object to a String. If
// there is no element at that position, the result is undefined. If a valid
// UTF-16 surrogate pair does not begin at pos, the
// result is the code unit at pos.
static ejsval _ejs_String_prototype_codePointAt(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    ...
    return NUMBER_TO_EJSVAL(((first - 0xD800) * 1024) + (second - 0xDC00) +
                            0x10000);
}

// ECMA262: 21.1.3.13 String.prototype.repeat ( count )
static ejsval _ejs_String_prototype_repeat(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args) {
...
static ejsval _ejs_String_prototype_includes(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ejsval searchString = _ejs_undefined;
    ejsval position = _ejs_undefined;
    if (argc > 0)
    ...
    // 7. Let pos be ToInteger(position). (If position is undefined, this step
    // produces the value 0).
    ...
    return ucs2_strstr(prim_S->data.flat + start, prim_searchStr->data.flat)
               ? _ejs_true
               : _ejs_false;

    // 11. Let searchLen be the number of elements in searchStr.
    // 12. If there exists any integer k not smaller than start such that k +
    // searchLen is not greater than len,
    //     and for all nonnegative integers j less than searchLen, the character
    //     at position k+j of S is the same
    //     as the character at position j of searchStr, return true; but if
    //     there is no such integer k, return false.
}

static ejsval _ejs_String_prototype_iterator(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    /* 1. Let O be CheckObjectCoercible(this value). */
    if (!EJSVAL_IS_STRING(_this) && !EJSVAL_IS_OBJECT(_this) &&
        !EJSVAL_IS_NULL(_this))
    ...
    /* 4. Return the result of calling the CreateStringIterator abstract
     * operation with argument S. */
    return _ejs_string_iterator_new(S);
}

/* 21.1.5.1 CreateStringIterator Abstract Operation */
ejsval _ejs_string_iterator_new(ejsval string) {
    ...
    _ejs_init_object((EJSObject *)iterator, _ejs_StringIterator_prototype,
                     &_ejs_StringIterator_specops);
...
static ejsval _ejs_StringIterator_impl(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
    return _this;
}

/* 21.1.5.2.1 %StringIteratorPrototype%.next () */
static ejsval _ejs_StringIterator_prototype_next(ejsval env, ejsval _this,
                                                 uint32_t argc, ejsval *args) {
    ...
    if (!EJSVAL_IS_STRINGITERATOR(O))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, ".next called on non-StringIterator instance");
    ...
    /* 9. Let first be the code unit value of the element at index position in
     * s. */
    ...
    // 10. If first < 0xD800 or first > 0xDBFF or position+1 = len then let
    // resultString be the string consisting of the single code unit first.
    ...
    // 11. Else,
        ...
        chars[1] =
            _ejs_string_ucs2_at(EJSVAL_TO_STRING(sPrimStr), position + 1);
        //      b. If second < 0xDC00 or second > 0xDFFF, then let resultString
    ...
    /* 13. Set the value of the [[StringIteratorNextIndex]] internal slot of O
     * to position+ resultSize. */
...
static void _ejs_string_init_proto() {
    ...
    _ejs_object_define_value_property(
        OBJECT_TO_EJSVAL(__proto__), _ejs_atom_name, _ejs_atom_empty,
...
void _ejs_string_init(ejsval global) {
    ...
    _ejs_String = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_String, (EJSClosureFunc)_ejs_String_impl);
    _ejs_object_setprop(global, _ejs_atom_String, _ejs_String);

    _ejs_object_setprop(_ejs_String, _ejs_atom_prototype,
                        _ejs_String_prototype);

#define OBJ_METHOD(x) EJS_INSTALL_ATOM_FUNCTION(_ejs_String, x, _ejs_String_##x)
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_String_prototype, x,                        \
                              _ejs_String_prototype_##x)
#define PROTO_METHOD_LEN(x, l)                                                 \
    EJS_INSTALL_ATOM_FUNCTION_LEN_FLAGS(                                       \
        _ejs_String_prototype, x, _ejs_String_prototype_##x, l,                \
    ...
    ejsval _iterator = _ejs_function_new_native(
        _ejs_null, _ejs_Symbol_iterator,
        (EJSClosureFunc)_ejs_String_prototype_iterator);
    _ejs_object_define_value_property(_ejs_String_prototype,
                                      _ejs_Symbol_iterator, _iterator,
                                      EJS_PROP_NOT_ENUMERABLE);

    _ejs_object_define_value_property(
        _ejs_String_prototype, _ejs_atom_constructor, _ejs_String,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);

#undef OBJ_METHOD
#undef PROTO_METHOD

    _ejs_StringIterator = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_String, (EJSClosureFunc)_ejs_StringIterator_impl);

    _ejs_gc_add_root(&_ejs_StringIterator_prototype);
    _ejs_StringIterator_prototype =
        _ejs_string_iterator_new(_ejs_String_prototype);
    EJSVAL_TO_OBJECT(_ejs_StringIterator_prototype)
        ->proto = _ejs_Object_prototype;
    _ejs_object_define_value_property(
        _ejs_StringIterator, _ejs_atom_prototype, _ejs_StringIterator_prototype,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE |
            EJS_PROP_NOT_WRITABLE);
    _ejs_object_define_value_property(
        _ejs_StringIterator_prototype, _ejs_atom_constructor,
        _ejs_StringIterator,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);

#define PROTO_ITER_METHOD(x)                                                   \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_StringIterator_prototype, x, _ejs_StringIterator_prototype_##x,   \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE)
...
static void _ejs_string_iterator_specop_scan(EJSObject *obj,
...
ejsval _ejs_string_new_utf8(const char *str) {
    ...
    size_t value_size =
        EJS_PRIMSTR_FLAT_ALLOC_SIZE + sizeof(jschar) * (str_len + 1);
...
ejsval _ejs_string_new_utf8_len(const char *str, int len) {
    size_t value_size =
        EJS_PRIMSTR_FLAT_ALLOC_SIZE + sizeof(jschar) * (len + 1);
    EJSBool ool_buffer = EJS_FALSE;
...
ejsval _ejs_string_new_substring(ejsval str, int off, int len) {
    ...
    if (len < FLAT_DEP_THRESHOLD) {
        rv = _ejs_gc_new_primstr(EJS_PRIMSTR_FLAT_ALLOC_SIZE +
                                 sizeof(jschar) * (len + 1));
        EJS_PRIMSTR_SET_TYPE(rv, EJS_STRING_FLAT);

        rv->data.flat = (jschar *)((char *)rv + EJS_PRIMSTR_FLAT_ALLOC_SIZE);

        // reuse the flatten machinery by using a stack allocated dep string
        // that we flatten into the rv
        EJSPrimString dep = {0};
...
static void flatten_dep(jschar **p, EJSPrimString *n, int *off, int *len) {
    ...
    // first step is to locate the node that contains the start of our
    // characters
    if (*off >= 0) {
        switch (EJS_PRIMSTR_GET_TYPE(n)) {
        case EJS_STRING_FLAT: {
            if (*off < n->length) {
                ...
                memmove(*p, n->data.flat + *off,
                        length_to_append * sizeof(jschar));
                *p += length_to_append;
...
static uint32_t hash_dep(int hash, EJSPrimString *n, int *off, int *len) {
...
static uint32_t _ejs_primstring_hash_inner(EJSPrimString *primstr,
                                           int cur_hash) {
    switch (EJS_PRIMSTR_GET_TYPE(primstr)) {
    case EJS_STRING_FLAT:
    ...
    case EJS_STRING_ROPE: {
        int hash =
            _ejs_primstring_hash_inner(primstr->data.rope.left, cur_hash);
        return _ejs_primstring_hash_inner(primstr->data.rope.right, hash);
    }
    }
}

uint32_t _ejs_primstring_hash(EJSPrimString *primstr) {
...
jschar _ejs_string_char_code_at(EJSPrimString *primstr, int i) {
    if (i >= primstr->length) {
    ...
    case EJS_STRING_ROPE: {
        if (i >= primstr->data.rope.left->length) {
            ...
            return _ejs_string_char_code_at(
                primstr->data.rope.right, i - primstr->data.rope.left->length);
        }
    }
    case EJS_STRING_DEPENDENT: {
        return _ejs_string_char_code_at(primstr->data.dependent.dep,
                                        i + primstr->data.dependent.off);
    }
    default:
        EJS_NOT_IMPLEMENTED();
    }
    ...
    for (int i = 0; i < primstr->length; i++) {
        ...
        if (ucs2 == (jschar)-1) {
            ...
            snprintf(error_buf, sizeof(error_buf),
                     "error converting ucs2 to utf8, index %d\n", i);
...
void _ejs_string_init_literal(const char *name, ejsval *val, EJSPrimString *str,
    ...
    str->length = length;
    str->hash = 0;
    str->gc_header = (EJS_STRING_FLAT | EJS_PRIMSTR_HAS_OOL_BUFFER_MASK)
                     << EJS_GC_USER_FLAGS_SHIFT;
    str->data.flat = ucs2_data;
    *val = STRING_TO_EJSVAL(str);
}

char *_ejs_string_describe(ejsval str) {
    return ucs2_to_utf8(EJSVAL_TO_FLAT_STRING(str));
}

char *_ejs_string_describe_prim(EJSPrimString *str) {
