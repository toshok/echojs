...
#define EJS_LIST_HEADER(t)                                                     \
    t *prev;                                                                   \
    t *next
#define EJS_LIST_INIT(v)                                                       \
    EJS_MACRO_START(v)->prev = (v)->next = NULL;                               \
    EJS_MACRO_END

#define EJS_LIST_PREPEND(v, l)                                                 \
    EJS_MACRO_START                                                            \
    v->next = l;                                                               \
    if (l)                                                                     \
        l->prev = v;                                                           \
...
#define EJS_LIST_INSERT_BEFORE(v, l, bef)                                      \
    EJS_MACRO_START                                                            \
    EJS_ASSERT(l);                                                             \
    if ((bef) == NULL) {                                                       \
        EJS_LIST_PREPEND(v, l);                                                \
...
#define EJS_SLIST_HEADER(t) t *next
#define EJS_SLIST_INIT(v)                                                      \
    EJS_MACRO_START                                                            \
    v->next = NULL;                                                            \
    EJS_MACRO_END

#define EJS_SLIST_ATTACH(v, l)                                                 \
    EJS_MACRO_START                                                            \
    v->next = l;                                                               \
    l = v;                                                                     \
    EJS_MACRO_END

#define EJS_SLIST_DETACH_HEAD(v, l)                                            \
    EJS_MACRO_START                                                            \
    l = l->next;                                                               \
    EJS_MACRO_END
...
extern char *ucs2_to_utf8_buf(const jschar *str, char *buf, size_t buf_size);
...
void _ejs_list_insert_node_sorted(EJSList *list, EJSListNode *node,
                                  EJSCompareFunc compare);
void _ejs_list_pop_head(EJSList *list);

int _ejs_list_length(EJSList *list);

#define EJS_LIST_FOREACH(l, t, v, b)                                           \
    for (t * (v) = (t *)(l)->head; (v); (v) = (t *)((EJSListNode *)(v))->next) \
    b
