...
static ejsval _ejs_Number_impl(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
...
static double thisNumberValue(ejsval value) {
...
static ejsval _ejs_Number_prototype_toString(ejsval env, ejsval _this,
...
static ejsval _ejs_Number_prototype_valueOf(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    EJSNumber *num = (EJSNumber *)EJSVAL_TO_OBJECT(_this);
...
static ejsval _ejs_Number_prototype_toFixed(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
#if notyet
    ...
    // 3. Let f be ToInteger(fractionDigits). (If fractionDigits is undefined,
    ...
    if (f < 0 || f > 20)
        _ejs_throw_nativeerror_utf8(
            EJS_RANGE_ERROR,
            "fractionDigits must be in the range of 0 <= f < 21");
    ...
    // 10. Else x < 10^21
        ...
        //     n / 10^f – x is as close to zero as
        //         possible. If there are two such n, pick the larger n.

        //     b. If n = 0, let m be the String "0". Otherwise, let m be the
        //     String consisting of the digits of the
        //        decimal representation of n (in order, with no leading
        //        zeroes).
        if (n == 0)
            m = _ejs_atom_0;
        else
            EJS_NOT_IMPLEMENTED(); // XXX let m be the String consisting of the
                                   // digits of the decimal representation of n
        ...
        if (f != 0) {
            ...
            //        iii. Let a be the first k–f elements of m, and let b be
            //        the remaining f elements of m.
...
static ejsval _ejs_Number_prototype_toPrecision(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
#if notyet
    ...
    if (p < 1 || p > 21) {
        _ejs_throw_nativeerror_utf8(
            EJS_RANGE_ERROR, "precision must be in the range of 1 <= p <= 21");
    }
    ...
    if (x == 0) {
        ...
        //     unit 0x0030 (the Unicode character ‘0’).
        m = zeros[p];
    ...
    // 12. Else x != 0,
        ...
        //     a. Let e and n be integers such that 10^(p–1) <= n < 10^p and for
        //     which the exact mathematical value of n * 10^(e–p+1) – x is as
        ...
        //        iii. Let m be the concatenation of the three Strings a, ".",
        //        and b.
        //        iv. If e > 0, then
        if (e > 0)
            //           1. Let c = "+".
        ...
        //        zeroes).
        //        vii. Return the concatenation of the five Strings s, m, "e",
        //        c, and d.
        return _ejs_string_concatv(s, m, _ejs_string_new_utf8("e"), c, d,
                                   _ejs_null);
    }
    ...
    // 14. If e >= 0, then
    if (e >= 0) {
        //     a. Let m be the concatenation of the first e+1 elements of m, the
        //     code unit 0x002E (Unicode character ‘.’), and the remaining p–
        //     (e+1) elements of m.
    }
    // 15. Else e < 0,
    else {
        //     a. Let m be the concatenation of the String "0.", –(e+1)
        //     occurrences of code unit 0x0030 (the Unicode character ‘0’), and
        //     the String m.
        int z = -(e + 1);
        ejsval zstr;
        if (z > 0 && z <= 21)
...
static ejsval _ejs_Number_isFinite(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
...
static ejsval _ejs_Number_toInteger(ejsval env, ejsval _this, uint32_t argc,
                                    ejsval *args) {
...
static ejsval _ejs_Number_isInteger(ejsval env, ejsval _this, uint32_t argc,
                                    ejsval *args) {
    ejsval number = _ejs_undefined;
    if (argc > 0)
        number = args[0];
...
static ejsval _ejs_Number_isSafeInteger(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
    ejsval number = _ejs_undefined;
    if (argc > 0)
        number = args[0];
...
static ejsval _ejs_Number_isNaN(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    ejsval number = _ejs_undefined;
    if (argc > 0)
        number = args[0];
...
static ejsval _ejs_Number_create(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in Number[Symbol.create] is not a constructor");
...
void _ejs_number_init(ejsval global) {
    _ejs_Number = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Number, (EJSClosureFunc)_ejs_Number_impl);
    ...
    _ejs_init_object((EJSObject *)prototype, _ejs_Object_prototype,
                     &_ejs_Number_specops);
    prototype->number = 0;
    _ejs_Number_prototype = OBJECT_TO_EJSVAL(prototype);

    _ejs_object_setprop(_ejs_Number, _ejs_atom_prototype,
                        _ejs_Number_prototype);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_Number_prototype, x,                        \
                              _ejs_Number_prototype_##x)
#define OBJ_METHOD(x) EJS_INSTALL_ATOM_FUNCTION(_ejs_Number, x, _ejs_Number_##x)
    ...
    _ejs_object_setprop(_ejs_Number, _ejs_atom_EPSILON,
                        NUMBER_TO_EJSVAL(nextafter(1, INFINITY) - 1));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_MAX_SAFE_INTEGER,
                        NUMBER_TO_EJSVAL(EJS_MAX_SAFE_INTEGER));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_MIN_SAFE_INTEGER,
                        NUMBER_TO_EJSVAL(EJS_MIN_SAFE_INTEGER));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_MAX_VALUE,
                        NUMBER_TO_EJSVAL(DBL_MAX));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_MIN_VALUE,
                        NUMBER_TO_EJSVAL(DBL_MIN));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_NaN,
                        NUMBER_TO_EJSVAL(nan("7734")));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_NEGATIVE_INFINITY,
                        NUMBER_TO_EJSVAL(-INFINITY));
    _ejs_object_setprop(_ejs_Number, _ejs_atom_POSITIVE_INFINITY,
                        NUMBER_TO_EJSVAL(INFINITY));

    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_Number, create, _ejs_Number_create,
