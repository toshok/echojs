...
static int num_builtin_modules =
    sizeof(builtin_modules) / sizeof(builtin_modules[0]);

ejsval _ejs_require EJSVAL_ALIGNMENT;

EJSBool require_builtin_module(const char *name, ejsval *module) {
    for (int i = 0; i < num_builtin_modules; i++) {
        if (!strcmp(builtin_modules[i].name, name)) {
            if (EJSVAL_IS_NULL(builtin_modules[i].cached_module_obj)) {
                ...
                builtin_modules[i].cached_module_obj =
...
EJSBool require_external_module(const char *name, ejsval *module) {
    for (int i = 0; i < _ejs_num_external_modules; i++) {
        if (!strcmp(_ejs_external_modules[i].name, name)) {
            if (EJSVAL_IS_NULL(_ejs_external_modules[i].cached_module_obj)) {
                ...
                _ejs_external_modules[i].cached_module_obj =
                    _ejs_object_new(_ejs_null, &_ejs_Object_specops);
                _ejs_external_modules[i].func(
                    _ejs_external_modules[i].cached_module_obj);
            }
...
EJSBool require_user_module(const char *name, ejsval *module) {
...
ejsval _ejs_module_get(ejsval arg) {
...
void _ejs_require_init(ejsval global) {
    ...
    for (i = 0; i < _ejs_num_modules; i++) {
        _ejs_init_object((EJSObject *)_ejs_modules[i], _ejs_Object_prototype,
                         &_ejs_Module_specops);
    }
}
