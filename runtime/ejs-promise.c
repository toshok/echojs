...
static EJSBool GetIteratorP(ejsval *iterator, ejsval iterable) {
    EJS_NOT_IMPLEMENTED();
}

static EJSBool IteratorStepP(ejsval *next, ejsval iterator) {
    EJS_NOT_IMPLEMENTED();
}

static EJSBool IteratorValueP(ejsval *value, ejsval iterResult) {
    EJS_NOT_IMPLEMENTED();
}

// ECMA262 25.4.5.4.0 Identity Functions
static ejsval identity(ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
...
static ejsval thrower(ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
    ejsval x = _ejs_undefined;
    if (argc > 0)
        x = args[0];
    _ejs_throw(x);
...
static void PromiseResolveThenableTask(ejsval promiseToResolve, ejsval thenable,
                                       ejsval then);
...
static void reaction_arg_dtor(void *data) {
    ReactionTaskArg *arg = (ReactionTaskArg *)data;
...
static void EnqueuePromiseReactionTask(EJSPromiseReaction *reaction,
                                       ejsval value) {
...
static void call_promise_resolve_thenable_task(void *data) {
...
static void resolve_thenable_arg_dtor(void *data) {
    ResolveThenableTaskArg *arg = (ResolveThenableTaskArg *)data;
    /* unroot the arg's ejsvals */
...
static void EnqueuePromiseResolveThenableTask(ejsval promiseToResolve,
                                              ejsval thenable, ejsval then) {
    ...
    _ejs_runloop_add_task(call_promise_resolve_thenable_task, arg,
                          resolve_thenable_arg_dtor);
}
...
static ejsval RejectPromise(ejsval promise, ejsval reason) {
    ...
    for (EJSPromiseReaction *reaction = reactions; reaction;
         reaction = reaction->next) {
        //       a. Perform EnqueueTask("PromiseTasks", PromiseReactionTask,
        //       (reaction, reason).
...
static ejsval FulfillPromise(ejsval promise, ejsval resolutionValue) {
    ...
    // 4. Set the value of promise's [[PromiseFulfullReactions]] internal slot
    // to undefined.
    // XXX we need to free our listnodes
    _promise->fulfillReactions = NULL;
...
static ejsval reject(ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
    ...
    // 3. Let alreadyResolved by be the value of F's [[AlreadyResolved]]
    // internal slot.
...
static ejsval resolve(ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
    ...
    *_ejs_closureenv_get_slot_ref(env, 0) = _ejs_true;
    ...
    if (SameValue(resolution, promise)) {
        ...
        ejsval selfResolutionError =
            _ejs_nativeerror_new_utf8(EJS_TYPE_ERROR, ""); // XXX
    ...
    // 12. Perform EnqueueTask ("PromiseTasks", PromiseResolveThenableTask,
...
static ejsval capabilitiesExecutor(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
...
static void CreateResolvingFunctions(ejsval promise, ejsval *out_resolve,
                                     ejsval *out_reject) {
...
static ejsval CreatePromiseCapabilityRecord(ejsval promise,
                                            ejsval constructor) {
    // 1. Assert: promise is an uninitialized object created as if by invoking
    ...
    ejsval executor =
        _ejs_function_new_anon(promiseCapability, capabilitiesExecutor);

    // 5. Set the [[Capability]] internal slot of executor to promiseCapability.

    // 6. Let constructorResult be the result of calling the [[Call]] internal
    // method of constructor, passing promise and (executor) as the arguments.
    // 7. ReturnIfAbrupt(constructorResult).
    ejsval constructorResult =
        _ejs_invoke_closure(constructor, promise, 1, &executor);

    // 8. If IsCallable(promiseCapability.[[Resolve]]) is false, then throw a
    // TypeError exception.
    ...
    if (EJSVAL_IS_OBJECT(constructorResult) &&
...
static ejsval NewPromiseCapability(ejsval C) {
...
static void PromiseReactionTask(EJSPromiseReaction *reaction, ejsval argument) {
    ...
    // 6. Else, Let let handlerResult be the result of calling the [[Call]]
    // internal method of handler passing undefined as thisArgument and
    // (argument) as argumentsList.
    else
        success = _ejs_invoke_closure_catch(&handlerResult, handler,
                                            _ejs_undefined, 1, &argument);
    ...
    if (!success) {
        ...
        //    thisArgument and (handlerResult.[[value]]) as argumentsList.
        success = _ejs_invoke_closure_catch(
            &status, EJS_CAPABILITY_GET_REJECT(promiseCapability),
            _ejs_undefined, 1, &handlerResult);

        //    b. NextTask status.
        return; // EJS_NOT_IMPLEMENTED();
    }
    // 8. Let handlerResult be handlerResult.[[value]].
    // 9. Let status be the result of calling the [[Call]] internal method of
    // promiseCapability.[[Resolve]] passing undefined as thisArgument and
    // (handlerResult) as argumentsList.
    success = _ejs_invoke_closure_catch(
        &status, EJS_CAPABILITY_GET_RESOLVE(promiseCapability), _ejs_undefined,
        1, &handlerResult);
...
static void PromiseResolveThenableTask(ejsval promiseToResolve, ejsval thenable,
    ...
    CreateResolvingFunctions(promiseToResolve, &resolvingFunctions_resolve,
                             &resolvingFunctions_reject);

    // 2. Let thenCallResult be the result of calling the [[Call]] internal
    // method of then passing thenable as the thisArgument and
    // (resolvingFunctions.[[Resolve]], resolvingFunctions.[[Reject]]) as
    // argumentsList.
    ejsval thenCallResult;
    ejsval args[] = {resolvingFunctions_resolve, resolvingFunctions_reject};
    EJSBool success =
        _ejs_invoke_closure_catch(&thenCallResult, then, thenable, 2, args);

    // 3. If thenCallResult is an abrupt completion,
    if (!success) {
        //    a. Let status be the result of calling the [[Call]] internal
        //    method of resolvingFunctions.[[Reject]] passing undefined as the
        //    thisArgument and (thenCallResult.[[value]]) as argumentsList.
        ejsval status;
        success = _ejs_invoke_closure_catch(&status, resolvingFunctions_reject,
                                            _ejs_undefined, 1, &thenCallResult);
...
static ejsval _ejs_Promise_impl(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    ...
    // 3. If promise does not have a [[PromiseState]] internal slot, then throw
    ...
    if (_promise->state != PROMISE_STATE_UNINITIALIZED)
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Promise constructor called on previously created object");
    ...
    //    1. Assert: promise has a [[PromiseState]] internal slot and it’s value
    //    is undefined.
    ...
    CreateResolvingFunctions(promise, &resolvingFunctions_resolve,
                             &resolvingFunctions_reject);

    //    7. Let completion be the result of calling the [[Call]] internal
    //    method of executor with undefined as thisArgument and
    //    (resolvingFunctions.[[Resolve]], resolvingFunctions.[[Reject]]) a
    //    argumentsList.
    ejsval completion;
    ejsval executor_args[] = {resolvingFunctions_resolve,
                              resolvingFunctions_reject};

    EJSBool success = _ejs_invoke_closure_catch(
        &completion, executor, _ejs_undefined, 2, executor_args);
    ...
    if (!success) {
        ...
        _ejs_invoke_closure(resolvingFunctions_reject, _ejs_undefined, 1,
                            &completion);
    }
...
static ejsval _ejs_Promise_prototype_catch(ejsval env, ejsval _this,
    ...
    return _ejs_invoke_closure(Get(promise, _ejs_atom_then), promise, 2,
                               thenargs);
}

static EJSPromiseReaction *_ejs_promise_reaction_new(ejsval capability,
                                                     ejsval handler) {
    EJSPromiseReaction *reaction =
        (EJSPromiseReaction *)malloc(sizeof(EJSPromiseReaction));
...
static ejsval _ejs_Promise_prototype_then(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    ...
    if (!EJSVAL_IS_CALLABLE(onFulfilled)) {
        //    a. Let onFulfilled be "Identity" a new Identity Function (see
        //    25.4.5.3.1).
        onFulfilled = _ejs_identity_function; // XXX does it really need to be a
                                              // newly instantiated one?
                                              // realm-specific?  we don't
                                              // instantiate a new one
    }

    // 4. If IsCallable(onRejected) is undefined or nullfalse, then
    if (!EJSVAL_IS_CALLABLE(onRejected)) {
        //    a. Let onRejected be "Thrower"a new Thrower Function (see
        //    25.4.5.3.3).
        onRejected = _ejs_thrower_function; // XXX does it really need to be a
                                            // newly instantiated one?
                                            // realm-specific?  we don't
                                            // instantiate a new one
    }
    // 5. If IsCallable(onFulfilled) is false or if IsCallable(onRejected) is
    // false, then throw a TypeError exception.
    if (!EJSVAL_IS_CALLABLE(onFulfilled) || !EJSVAL_IS_CALLABLE(onRejected)) {
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "1: shouldn't happen.  runtime error?");
    }
    ...
    if (_promise->state == PROMISE_STATE_PENDING) {
        ...
        EJSPromiseReaction *fulfillReaction =
            _ejs_promise_reaction_new(promiseCapability, onFulfilled);
        // 11. Let rejectReaction  be the PromiseReaction { [[Capabilities]]:
        // promiseCapability, [[Handler]]: onRejected  }.
        EJSPromiseReaction *rejectReaction =
            _ejs_promise_reaction_new(promiseCapability, onRejected);

        //        a. Append fulfillReaction as the last element of the List that
        //        is the value of promise's [[PromiseFulfillReactions]] internal
        //        slot.
        EJS_LIST_APPEND(EJSPromiseReaction, fulfillReaction,
                        _promise->fulfillReactions);
        //        b. Append rejectReaction as the last element of the List that
        //        is the value of promise's [[PromiseRejectReactions]] internal
        //        slot.
        EJS_LIST_APPEND(EJSPromiseReaction, rejectReaction,
                        _promise->rejectReactions);
    }
    ...
    // "fulfilled",
        ...
        //     a. Let value be the value of promise's [[PromiseResult]] internal
        //     slot.
        ejsval value = _promise->result;
        //     b. Perform EnqueueTask("PromiseTasks", PromiseReactionTask,
    ...
    // "rejected",
        ...
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "2: shouldn't happen.  runtime error?");
    }
    // 15. Return promiseCapability.[[Promise]].
    return EJS_CAPABILITY_GET_PROMISE(promiseCapability);
}

static ejsval _ejs_Promise_get_species(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
    return _ejs_Promise;
}

// ECMA262 25.4.4.6 Promise [ @@create ] ( )
static ejsval _ejs_Promise_create(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    if (!EJSVAL_IS_UNDEFINED(F)) {
        if (!EJSVAL_IS_CONSTRUCTOR(F))
            _ejs_throw_nativeerror_utf8(
                EJS_TYPE_ERROR,
                "'this' in Promise[Symbol.create] is not a constructor");
...
static ejsval resolve_element(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    // 1. If the value of F's [[AlreadyCalled]] internal slot is true, then
    // return undefined.
    ...
    // 6. Let remainingElementsCount be the value of F's [[RemainingElements]]
    // internal slot.

    // XXX remainingElementsCount needs to be a boxed value so that
    // resolve_element functions can update it
    // XXX maybe an EJSNumber?  supposed to be immutable, but we could fudge..
    // or another closure.
    ...
    //     promiseCapability.[[Resolve]] with undefined as thisArgument and
...
static ejsval _ejs_Promise_all(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    EJSBool success;
    ...
    if (!success) {
        _ejs_invoke_closure(EJS_CAPABILITY_GET_REJECT(promiseCapability),
                            _ejs_undefined, 1, &iterator);
        return EJS_CAPABILITY_GET_PROMISE(promiseCapability);
    }
    ...
    while (EJS_TRUE) {
        ...
        if (EJSVAL_IS_BOOLEAN(next) && !EJSVAL_TO_BOOLEAN(next)) {
            ...
            if (remainingElementsCount == 0) {
                ...
                success = _ejs_invoke_closure_catch(
                    &resolveResult,
                    EJS_CAPABILITY_GET_RESOLVE(promiseCapability),
                    _ejs_undefined, 1, &values);
                //           2. ReturnIfAbrupt(resolveResult).
                if (!success) {
                    _ejs_invoke_closure(
                        EJS_CAPABILITY_GET_REJECT(promiseCapability),
                        _ejs_undefined, 1, &resolveResult);
                    return EJS_CAPABILITY_GET_PROMISE(promiseCapability);
                }
            }
        ...
        if (!success) {
            _ejs_invoke_closure(EJS_CAPABILITY_GET_REJECT(promiseCapability),
                                _ejs_undefined, 1, &nextValue);
            return EJS_CAPABILITY_GET_PROMISE(promiseCapability);
        }
        //    f. Let nextPromise be Invoke(C, "resolve", (nextValue)).
        ejsval nextPromise;
        success = _ejs_invoke_closure_catch(
            &nextPromise, Get(C, _ejs_atom_resolve), C, 1, &nextValue);

        //    g. IfAbruptRejectPromise(nextPromise, promiseCapability).
        if (!success) {
            _ejs_invoke_closure(EJS_CAPABILITY_GET_REJECT(promiseCapability),
                                _ejs_undefined, 1, &nextPromise);
            return EJS_CAPABILITY_GET_PROMISE(promiseCapability);
        }

        //    h. Let resolveElement be a new built-in function object as defined
        //    in Promise.all Resolve Element Functions.
        ejsval resolvingElement_env = _ejs_closureenv_new(6);
        ejsval resolveElement =
            _ejs_function_new_anon(resolvingElement_env, resolve_element);

        //    i. Set the [[AlreadyCalled]] internal slot of resolveElement to
        //    false.
        ...
        EJS_RESOLVEELEMENT_SET_INDEX(resolvingElement_env,
        ...
        //    promiseCapability.
        EJS_RESOLVEELEMENT_SET_CAPABILITIES(resolvingElement_env,
                                            promiseCapability);
        //    m. Set the [[RemainingElements]] internal slot of resolveElement
        //    to remainingElementsCount.
        EJS_RESOLVEELEMENT_SET_REMAINING_ELEMENTS(
            resolvingElement_env, NUMBER_TO_EJSVAL(remainingElementsCount));
        //    n. Set remainingElementsCount.[[value]] to
        //    remainingElementsCount.[[value]] + 1.
        remainingElementsCount++;
        //    o. Let result be Invoke(nextPromise, "then", (resolveElement,
        //    promiseCapability.[[Reject]])).
        ejsval thenargs[] = {resolveElement,
                             EJS_CAPABILITY_GET_REJECT(promiseCapability)};
        ejsval result;
        success =
            _ejs_invoke_closure_catch(&result, Get(nextPromise, _ejs_atom_then),
                                      nextPromise, 2, thenargs);
        //    p. IfAbruptRejectPromise(result, promiseCapability).
        if (!success) {
            _ejs_invoke_closure(EJS_CAPABILITY_GET_REJECT(promiseCapability),
                                _ejs_undefined, 1, &result);
            return EJS_CAPABILITY_GET_PROMISE(promiseCapability);
        }
...
static ejsval _ejs_Promise_race(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    EJSBool success;
    ...
    while (EJS_TRUE) {
        ...
        //    f. Let nextPromise be Invoke(C, "resolve", (nextValue)).
        ejsval nextPromise;
        success = _ejs_invoke_closure_catch(&nextPromise, C, _ejs_atom_resolve,
                                            1, &nextValue);
        //    g. IfAbruptRejectPromise(nextPromise, promiseCapability).
        if (!success) {
            _ejs_invoke_closure(EJS_CAPABILITY_GET_REJECT(promiseCapability),
        ...
        ejsval result;
        ejsval args[] = {EJS_CAPABILITY_GET_RESOLVE(promiseCapability),
                         EJS_CAPABILITY_GET_REJECT(promiseCapability)};
        success = _ejs_invoke_closure_catch(&result, nextPromise,
                                            _ejs_atom_then, 2, args);
...
static ejsval _ejs_Promise_reject(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    // of promiseCapability.[[Reject]] with undefined as thisArgument and (r) as
    // argumentsList.
...
static ejsval _ejs_Promise_resolve(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    ejsval x = _ejs_undefined;
    if (argc > 0)
        x = args[0];
    ...
    // method of promiseCapability.[[Resolve]] with undefined as thisArgument
    // and (x) as argumentsList.
    // 6. ReturnIfAbrupt(resolveResult).
    _ejs_invoke_closure(EJS_CAPABILITY_GET_RESOLVE(promiseCapability),
                        _ejs_undefined, 1, &x);
...
void _ejs_promise_init(ejsval global) {
    _ejs_Promise = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Promise, (EJSClosureFunc)_ejs_Promise_impl);
    ...
    _ejs_object_setprop(_ejs_Promise, _ejs_atom_prototype,
                        _ejs_Promise_prototype);
    _ejs_object_define_value_property(
        _ejs_Promise_prototype, _ejs_atom_constructor, _ejs_Promise,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs_Promise_prototype, x,                 \
                                    _ejs_Promise_prototype_##x,                \
                                    EJS_PROP_NOT_ENUMERABLE)
#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs_Promise, x, _ejs_Promise_##x,         \
                                    EJS_PROP_NOT_ENUMERABLE)

#define PROTO_METHOD_LEN(x, l)                                                 \
    EJS_INSTALL_ATOM_FUNCTION_LEN_FLAGS(_ejs_Promise_prototype, x,             \
                                        _ejs_Promise_prototype_##x, l,         \
                                        EJS_PROP_NOT_ENUMERABLE)
#define OBJ_METHOD_LEN(x, l)                                                   \
    EJS_INSTALL_ATOM_FUNCTION_LEN_FLAGS(_ejs_Promise, x, _ejs_Promise_##x, l,  \
                                        EJS_PROP_NOT_ENUMERABLE)
    ...
    _ejs_object_define_value_property(
        _ejs_Promise_prototype, _ejs_Symbol_toStringTag, _ejs_atom_Promise,
...
#undef PROTO_METHOD
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_Promise, create, _ejs_Promise_create,
                                      EJS_PROP_NOT_ENUMERABLE);
...
static EJSObject *_ejs_promise_specop_allocate() {
    return (EJSObject *)_ejs_gc_new(EJSPromise);
}

static void _ejs_promise_specop_scan(EJSObject *obj, EJSValueFunc scan_func) {
    EJSPromise *promise = (EJSPromise *)obj;

    scan_func(promise->result);
    scan_func(promise->constructor);

    for (EJSPromiseReaction *reaction = promise->fulfillReactions; reaction;
         reaction = reaction->next) {
        scan_func(reaction->capabilities);
        scan_func(reaction->handler);
    }

    for (EJSPromiseReaction *reaction = promise->rejectReactions; reaction;
         reaction = reaction->next) {
        scan_func(reaction->capabilities);
        scan_func(reaction->handler);
    }
