...
static ejsval _ejs_Reflect_apply(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ...
    if (!EJSVAL_IS_ARRAY(argumentsList))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "argumentsList is not an array");

    // sparse arrays kinda suck here...
    if (!EJSVAL_IS_DENSE_ARRAY(argumentsList))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "argumentsList is not a dense array");

    // 6. Perform the PrepareForTailCall abstract operation.

    // 7. Return the result of calling the [[Call]] internal method of obj with
    // arguments thisArgument and args.
    return _ejs_invoke_closure(obj, thisArgument, EJS_ARRAY_LEN(argumentsList),
                               EJS_DENSE_ARRAY_ELEMENTS(argumentsList));
}

static ejsval _ejs_Reflect_construct(ejsval env, ejsval _this, uint32_t argc,
                                     ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_Reflect_defineProperty(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_Reflect_deleteProperty(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_Reflect_enumerate(ejsval env, ejsval _this, uint32_t argc,
                                     ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

// ECMA262: 26.1.6 Reflect.get ( target, propertyKey [ , receiver ])
static ejsval _ejs_Reflect_get(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    ejsval target = _ejs_undefined;
    ...
    // 6. Return the result of calling the [[Get]] internal method of obj with
    // arguments key, and receiver
    return OP(EJSVAL_TO_OBJECT(obj), Get)(obj, key, receiver);
}

// ECMA262: 26.1.7 Reflect.getOwnPropertyDescriptor ( target, propertyKey )
static ejsval _ejs_Reflect_getOwnPropertyDescriptor(ejsval env, ejsval _this,
                                                    uint32_t argc,
                                                    ejsval *args) {
    ejsval target = _ejs_undefined;
    ejsval propertyKey = _ejs_undefined;
...
static ejsval _ejs_Reflect_getPrototypeOf(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    ejsval target = _ejs_undefined;
    if (argc > 0)
        target = args[0];
...
static ejsval _ejs_Reflect_has(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    ejsval target = _ejs_undefined;
    ejsval propertyKey = _ejs_undefined;
...
static ejsval _ejs_Reflect_isExtensible(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
    ejsval target = _ejs_undefined;
    if (argc > 0)
        target = args[0];
...
static ejsval _ejs_Reflect_ownKeys(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    ejsval target = _ejs_undefined;
    if (argc > 0)
        target = args[0];
...
static ejsval _ejs_Reflect_preventExtensions(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ejsval target = _ejs_undefined;
    if (argc > 0)
        target = args[0];
    ...
    // 3. Return the result of calling the [[PreventExtensions]] internal method
    // of obj.
...
static ejsval _ejs_Reflect_set(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    ejsval target = _ejs_undefined;
    ejsval propertyKey = _ejs_undefined;
    ...
    // 6. Return the result of calling the [[Set]] internal method of obj with
    // arguments key, V, and receiver.
    return BOOLEAN_TO_EJSVAL(
        OP(EJSVAL_TO_OBJECT(obj), Set)(obj, key, V, receiver));
}

// ECMA262: 26.1.14 Reflect.setPrototypeOf ( target, proto )
static ejsval _ejs_Reflect_setPrototypeOf(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    ejsval target = _ejs_undefined;
    ...
    if (!EJSVAL_IS_OBJECT(proto) && !EJSVAL_IS_NULL(proto))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "prototype argument must be an object or null");

    // 4. Return the result of calling the [[SetPrototypeOf]] internal method of
    // obj with argument proto.
    return BOOLEAN_TO_EJSVAL(
        OP(EJSVAL_TO_OBJECT(obj), SetPrototypeOf)(target, proto));
}
...
#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_Reflect, x, _ejs_Reflect_##x)
