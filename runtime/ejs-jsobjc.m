...
- (EJSPrimString *)jsString {
    if (!_jsstr) {
        ...
        _jsstr = EJSVAL_TO_STRING(
            _ejs_string_new_ucs2_len((jschar *)buf, [_nsstr length]));
...
- (NSString *)nsString {
    if (!_nsstr) {
        ...
        _nsstr = [[NSString alloc] initWithCharacters:(unichar *)chars
                                               length:length];
    }
...
+ (id)numberValue:(double)num {
    return
        [[[CKValue alloc] initWithJSValue:NUMBER_TO_EJSVAL(num)] autorelease];
}

+ (id)objectValue:(CKObject *)obj {
    return [[[CKValue alloc]
        initWithJSValue:(obj == NULL ? _ejs_null
                                     : OBJECT_TO_EJSVAL([obj jsObject]))]
        autorelease];
}

+ (id)jsStringValue:(CKString *)str {
    return [[[CKValue alloc] initWithJSValue:STRING_TO_EJSVAL([str jsString])]
        autorelease];
}

+ (id)nsStringValue:(NSString *)str {
    return [[[CKValue alloc]
        initWithJSValue:STRING_TO_EJSVAL([[CKString stringWithNSString:str]
                            jsString])] autorelease];
}

+ (id)utf8StringValue:(const char *)str {
    return [[[CKValue alloc]
        initWithJSValue:STRING_TO_EJSVAL([[CKString stringWithUTF8CString:str]
                            jsString])] autorelease];
}
...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"<CKValue val=\"%@\">", [self nsStringValue]];
}
...
#if spidermonkey
- (void)defineClass:(NSString *)name
          withClass:(JSClass *)classp
        constructor:(JSNative)ctor
       ctorArgCount:(uintN)nargs
         properties:(JSPropertySpec *)properties
          functions:(JSFunctionSpec *)functions
   staticProperties:(JSPropertySpec *)staticProperties
    staticFunctions:(JSFunctionSpec *)staticFunctions {
    JS_InitClass([_ctx jsContext], _obj, NULL, classp, ctor, nargs, properties,
                 functions, staticProperties, staticFunctions);
}
...
+ (CKObject *)makeFunction:(CKString *)name
              withCallback:(EJSClosureFunc)callback
                  argCount:(uint32_t)nargs {
    return [self makeFunctionNS:[name nsString]
                   withCallback:callback
                       argCount:nargs];
}

+ (CKObject *)makeFunctionNS:(NSString *)name
                withCallback:(EJSClosureFunc)callback
                    argCount:(uint32_t)nargs {
    return [[[CKObject alloc]
        initWithJSObject:EJSVAL_TO_OBJECT(_ejs_function_new_utf8(
                             _ejs_null, [name UTF8String], callback))]
        autorelease];
}
...
- (CKValue *)valueForProperty:(CKString *)name {
    ...
    return [CKValue
        valueWithJSValue:_ejs_object_getprop_utf8(objval, [name UTF8String])];
}
...
- (void)defineProperty:(CKString *)name
                 value:(CKValue *)val
            attributes:(uint32_t)attrs {
    _ejs_object_define_value_property(OBJECT_TO_EJSVAL(_obj),
                                      STRING_TO_EJSVAL([name jsString]),
                                      [val jsValue], attrs);
}
...
- (void)defineProperty:(CKString *)name
    ...
    _ejs_object_define_value_property(OBJECT_TO_EJSVAL(_obj),
                                      STRING_TO_EJSVAL([name jsString]),
                                      OBJECT_TO_EJSVAL([obj jsObject]), attrs);
}

- (void)definePropertyNS:(NSString *)name
                   value:(CKValue *)val
              attributes:(uint32_t)attrs {
    _ejs_object_define_value_property(
        OBJECT_TO_EJSVAL(_obj),
        STRING_TO_EJSVAL([[CKString stringWithNSString:name] jsString]),
        [val jsValue], attrs);
}
...
- (BOOL)isArray {
    return _obj->ops == &_ejs_Array_specops ||
           _obj->ops == &_ejs_sparsearray_specops;
}
...
- (CKPropertyNameArray *)initWithObjectProperties:(CKObject *)obj {
    ...
    if (_count == 0) {
        ...
        uint32_t i = 0;
        for (_EJSPropertyMapEntry *s = map->head_insert; s;
             s = s->next_insert) {
            ejsval name = s->name;
...
+ (CKPropertyNameArray *)arrayWithObjectProperties:(CKObject *)obj {
    return [[[CKPropertyNameArray alloc] initWithObjectProperties:obj]
        autorelease];
}
...
- (CKInvocation *)initWithFunction:(CKObject *)func
                          argCount:(NSUInteger)argCount
                        thisObject:(CKObject *)obj {
    self = [super init];
    _func = [func retain];
    _thisObj = [obj retain];
    isFuncCtor = NO;
    _argCount = argCount;
    _args = _argCount == 0 ? NULL
                           : (CKValue **)calloc(_argCount, sizeof(CKValue *));
    return self;
}

- (CKInvocation *)initWithConstructor:(CKObject *)ctor
                             argCount:(NSUInteger)argCount {
    self = [super init];
    _func = [ctor retain];
    _thisObj = nil;
...
+ (CKInvocation *)invocationWithFunction:(CKObject *)func
                                argCount:(NSUInteger)argCount
                              thisObject:(CKObject *)obj {
    return [[[CKInvocation alloc] initWithFunction:func
                                          argCount:argCount
                                        thisObject:obj] autorelease];
}

+ (CKInvocation *)invocationWithConstructor:(CKObject *)ctor
                                   argCount:(NSUInteger)argCount {
    return [[[CKInvocation alloc] initWithConstructor:ctor argCount:argCount]
        autorelease];
}
...
- (CKValue *)invoke {
    ...
    ejsval *jsargs =
        _argCount == 0 ? NULL : (ejsval *)malloc(_argCount * sizeof(ejsval));
    NSUInteger i;
    ...
    if (isFuncCtor) {
        ejsval o =
            _ejs_object_create(OBJECT_TO_EJSVAL([[_func prototype] jsObject]));
        _ejs_invoke_closure(OBJECT_TO_EJSVAL([_func jsObject]), o, _argCount,
                            jsargs);
        rv = o;
    } else {
