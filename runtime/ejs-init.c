...
_ejs_root_builtin_globals(void)
{
    ...
    _ejs_gc_add_root (&_ejs_AggregateError);
    _ejs_gc_add_root (&_ejs_AggregateError_prototype);
...
// ES6 19.2.4.1: every builtin function has an own .length
// (non-writable, non-enumerable, configurable).  Most natives are
// installed without one; this table (generated by introspecting a
// conforming engine) stamps the spec arity onto every builtin
// reachable by a global dot-path.
static void
_ejs_install_builtin_arities (void)
{
    static const struct { const char* path; int len; } arities[] = {
#include "ejs-builtin-arities.h"
    };
    for (unsigned i = 0; i < sizeof(arities)/sizeof(arities[0]); i++) {
        char buf[128];
        strncpy (buf, arities[i].path, sizeof(buf)-1);
        buf[sizeof(buf)-1] = 0;
        ejsval cur = _ejs_global;
        char* save = NULL;
        for (char* seg = strtok_r (buf, ".", &save); seg; seg = strtok_r (NULL, ".", &save)) {
            if (!EJSVAL_IS_OBJECT(cur)) { cur = _ejs_undefined; break; }
            cur = _ejs_object_getprop (cur, _ejs_string_new_utf8 (seg));
        }
        if (EJSVAL_IS_FUNCTION(cur))
            _ejs_object_define_value_property (cur, _ejs_atom_length, NUMBER_TO_EJSVAL(arities[i].len),
...
_ejs_init(int argc, char** argv)
{
    ...
    // ES 19.1: undefined/NaN/Infinity are non-writable, non-enumerable,
    // non-configurable value properties of the global object
    _ejs_object_define_value_property (_ejs_global, _ejs_atom_undefined, _ejs_undefined,
                                       EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE | EJS_PROP_NOT_WRITABLE);
    _ejs_object_define_value_property (_ejs_global, _ejs_atom_NaN, _ejs_nan,
                                       EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE | EJS_PROP_NOT_WRITABLE);
    _ejs_object_define_value_property (_ejs_global, _ejs_atom_Infinity, _ejs_Infinity,
                                       EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE | EJS_PROP_NOT_WRITABLE);
    ...
    _ejs_iterator_helpers_init(_ejs_global);
    ...
    _ejs_resources_init(_ejs_global);

    _ejs_typedarrays_init(_ejs_global);
    _ejs_atomics_init(_ejs_global);
#if IOS
...
#undef GLOBAL_METHOD
    ...
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs__ejs, detachArrayBuffer, _ejs_detachArrayBuffer_impl, 0);
