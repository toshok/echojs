...
const ejsval _ejs_undefined EJSVAL_ALIGNMENT =
    STATIC_BUILD_EJSVAL(EJSVAL_TAG_UNDEFINED, 0);
ejsval _ejs_nan;
const ejsval _ejs_Infinity EJSVAL_ALIGNMENT =
    STATIC_BUILD_DOUBLE_EJSVAL(HUGE_VAL);
const ejsval _ejs_null EJSVAL_ALIGNMENT =
    STATIC_BUILD_EJSVAL(EJSVAL_TAG_NULL, 0);
const ejsval _ejs_true EJSVAL_ALIGNMENT = STATIC_BUILD_BOOLEAN_EJSVAL(EJS_TRUE);
const ejsval _ejs_false EJSVAL_ALIGNMENT =
    STATIC_BUILD_BOOLEAN_EJSVAL(EJS_FALSE);
...
ejsval _ejs_unhandledException(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
...
static void _ejs_init_classes() {
    ...
    _ejs_Class_initialize(&_ejs_Uint8ClampedArray_specops,
                          &_ejs_Object_specops);
...
#if IOS
    _ejs_Class_initialize(&_ejs_WebGLRenderingContext_specops,
                          &_ejs_Object_specops);
    ...
    _ejs_Class_initialize(&_ejs_WebGLRenderbuffer_specops,
                          &_ejs_Object_specops);
    ...
    _ejs_Class_initialize(&_ejs_WebGLUniformLocation_specops,
                          &_ejs_Object_specops);
#endif
#if IOS || OSX
...
void _ejs_init(int argc, char **argv) {
    ...
    _ejs_object_setprop(
        _ejs_global, _ejs_atom_eval,
        _ejs_function_new_native(_ejs_undefined, _ejs_atom_eval, _ejs_eval));
...
#define GLOBAL_METHOD(x)                                                       \
    EJS_MACRO_START                                                            \
    _ejs_##x = _ejs_function_new_native(_ejs_null, _ejs_atom_##x,              \
                                        (EJSClosureFunc)_ejs_##x##_impl);      \
...
#undef GLOBAL_METHOD
    ...
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs__ejs, unhandledException,
                                    _ejs_unhandledException, 0);
}
