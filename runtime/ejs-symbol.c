...
#define EJSVAL_IS_SYMBOL(v)                                                    \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Symbol_specops))
#define EJSVAL_TO_SYMBOL(v) ((EJSSymbol *)EJSVAL_TO_OBJECT(v))

// ECMA262: 19.4.2.2 Symbol.for ( key )
static ejsval _ejs_Symbol_for(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ...
    // 4. Assert: GlobalSymbolRegistry does not current contain an entry for
    // stringKey.
    // 5. Let newSymbol be a new unique Symbol value whose [[Description]] is
    // stringKey.
    ejsval newSymbol = _ejs_null; // XXX

    // 6. Append the record { [[key]]: stringKey, [[symbol]]: newSymbol) to the
...
static ejsval _ejs_Symbol_keyFor(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ...
    if (!EJSVAL_IS_SYMBOL(sym))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Symbol.keyFor called with non-symbol argument");

    // 2. For each element e of the GlobalSymbolRegistry List (see 19.4.2.2),
    //    a. If SameValue(e.[[symbol]], sym) is true, then return e.[[key]].
    // 3. Assert: GlobalSymbolRegistry does not current contain an entry for
    // sym.
...
static ejsval _ejs_Symbol_prototype_toString(ejsval env, ejsval _this,
    ...
    // 3. Else,
    //    a. If s does not have a [[SymbolData]] internal slot, then throw a
    //    TypeError exception.
    //    b. Let sym be the value of s’s [[SymbolData]] internal slot.
    else
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Symbol.prototype.toString called with non-symbol this");
    ...
    // 7. Let result be the result of concatenating the strings "Symbol(", desc,
    // and ")".
    ejsval result =
        _ejs_string_concatv(_ejs_atom_Symbol, _ejs_string_new_utf8("("), desc,
...
// ECMA262: 19.4.1
static ejsval _ejs_Symbol_impl(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
...
void _ejs_Symbol_create_impl(ejsval env, ejsval _this, uint32_t argc,
                             ejsval *args) {
    _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR, "1"); // XXX
}

void _ejs_symbol_init(ejsval global) {
    _ejs_Symbol = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Symbol, (EJSClosureFunc)_ejs_Symbol_impl);
    _ejs_object_setprop(global, _ejs_atom_Symbol, _ejs_Symbol);

    _ejs_gc_add_root(&_ejs_Symbol_prototype);
    _ejs_Symbol_prototype =
        _ejs_object_new(_ejs_null, &_ejs_Object_specops); // XXX
    _ejs_object_setprop(_ejs_Symbol, _ejs_atom_prototype,
                        _ejs_Symbol_prototype);
    ...
    _ejs_object_define_value_property(
        _ejs_Symbol_prototype, _ejs_Symbol_toStringTag, _ejs_atom_Symbol,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_WRITABLE |
            EJS_PROP_CONFIGURABLE);

    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(
        _ejs_Symbol, create, _ejs_Symbol_create_impl, EJS_PROP_NOT_ENUMERABLE);
}

ejsval _ejs_symbol_new(ejsval description) {
    EJSSymbol *rv = _ejs_gc_new(EJSSymbol);

    _ejs_init_object((EJSObject *)rv, _ejs_Symbol_prototype,
                     &_ejs_Symbol_specops);

    rv->description =
        EJSVAL_IS_UNDEFINED(description) ? description : ToString(description);

    return OBJECT_TO_EJSVAL(rv);
}

uint32_t _ejs_symbol_hash(ejsval obj) {
...
static EJSObject *_ejs_symbol_specop_allocate() {
