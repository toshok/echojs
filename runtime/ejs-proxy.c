...
ejsval _ejs_Proxy_create(ejsval env, ejsval this, uint32_t argc, ejsval *args) {
    if (argc == 0)
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "create requires more than 0 arguments");
    if (!EJSVAL_IS_OBJECT(args[0]))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "argument 0 is not a non-null object");

    EJS_NOT_IMPLEMENTED();
}

ejsval _ejs_Proxy_createFunction(ejsval env, ejsval this, uint32_t argc,
                                 ejsval *args) {
    if (argc <= 1)
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "create requires more than 1 arguments");
    if (!EJSVAL_IS_OBJECT(args[0]))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "argument 0 is not a non-null object");
...
static ejsval _ejs_Proxy_impl(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(target))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "Proxy target argument must be an object");

    // 2. If Type(handler) is not Object, throw a TypeError Exception.
    if (!EJSVAL_IS_OBJECT(handler))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "Proxy handler argument must be an object");

    // 3. Let P be a newly created object.
    // 4. Set P’s essential internal methods to the definitions specified in
    // 9.5.
...
static ejsval _ejs_Proxy_create_impl(ejsval env, ejsval _this, uint32_t argc,
                                     ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in Proxy[Symbol.create] is not a constructor");

    EJSObject *F_ = EJSVAL_TO_OBJECT(F);

    // 2. Let obj be the result of calling OrdinaryCreateFromConstructor(F,
    // "%ObjectPrototype%", ([[ProxyTarget]], [[ProxyHandler]]) ).
...
void _ejs_proxy_init(ejsval global) {
    _ejs_Proxy = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Proxy, (EJSClosureFunc)_ejs_Proxy_impl);
...
#undef OBJ_METHOD
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(
        _ejs_Proxy, create, _ejs_Proxy_create_impl, EJS_PROP_NOT_ENUMERABLE);
}

static ejsval _ejs_proxy_specop_get_prototype_of(ejsval O) {
    ...
    if (EJSVAL_IS_NULL(handler))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "null ProxyHandler in getPrototypeOf");
    ...
    // 7. Let handlerProto be the result of calling the [[Call]] internal method
    // of trap with handler as the this value and a new List containing target.
    ...
    if (!EJSVAL_IS_OBJECT(handlerProto) && EJSVAL_IS_NULL(handlerProto))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "getPrototypeOf trap returned invalid prototype");

    EJSObject *_target = EJSVAL_TO_OBJECT(
        target); // XXX do we need to verify that it actually is an object here?
    ...
    if (!SameValue(handlerProto, targetProto))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "inconsistent prototypes from target and handler");

    // 16. Return handlerProto.
    return handlerProto;
}

static EJSBool _ejs_proxy_specop_set_prototype_of(ejsval O, ejsval V) {
    EJSProxy *proxy = EJSVAL_TO_PROXY(O);
    ...
    if (EJSVAL_IS_NULL(handler))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "null ProxyHandler in setPrototypeOf");
    ...
    // 8. Let trapResult be the result of calling the [[Call]] internal method
    // of trap with handler as the this value and a new List containing target
    // and V.
    ...
    if (booleanTrapResult && SameValue(V, targetProto)) {
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "cycle detected in prototype chain");
    }
...
// 9.5.3
static EJSBool _ejs_proxy_specop_is_extensible(ejsval O) {
    EJSProxy *proxy = EJSVAL_TO_PROXY(O);
    ...
    if (EJSVAL_IS_NULL(handler))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "null ProxyHandler in isExtensible");
    ...
    ejsval args[] = {target};
    EJSBool booleanTrapResult =
        ToEJSBool(_ejs_invoke_closure(trap, handler, 2, args));
    ...
    if (booleanTrapResult != targetResult)
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "return value mismatch in isExtensible");
...
// es6 rev 38 04-16-2015
// 9.5.4
static EJSBool _ejs_proxy_specop_prevent_extensions(ejsval O) {
    EJSProxy *proxy = EJSVAL_TO_PROXY(O);
    ...
    if (EJSVAL_IS_NULL(handler))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "null ProxyHandler in preventExtensions");
    ...
    if (booleanTrapResult) {
        ...
        if (targetIsExtensible)
            _ejs_throw_nativeerror_utf8(
                EJS_TYPE_ERROR,
                "extensible invariant mismatch in preventExtensions");
    }
...
static ejsval _ejs_proxy_specop_get(ejsval obj, ejsval propertyName,
    ...
    EJSPropertyDesc *targetDesc =
        OP(_target, GetOwnProperty)(target, propertyName, NULL);

    // 12. If targetDesc is not undefined, then
    if (!targetDesc) {
        //     a. If IsDataDescriptor(targetDesc) and
        //     targetDesc.[[Configurable]] is false and targetDesc.[[Writable]]
        //     is false, then
        if (IsDataDescriptor(targetDesc) &&
            !_ejs_property_desc_is_configurable(targetDesc) &&
            !_ejs_property_desc_is_writable(targetDesc)) {
            //        i. If SameValue(trapResult, targetDesc.[[Value]]) is
            //        false, then throw a TypeError exception.
            if (!SameValue(trapResult,
                           _ejs_property_desc_get_value(targetDesc)))
                _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR, "1"); // XXX
        }
        //     b. If IsAccessorDescriptor(targetDesc) and
        //     targetDesc.[[Configurable]] is false and targetDesc.[[Get]] is
        //     undefined, then
        if (IsAccessorDescriptor(targetDesc) &&
            !_ejs_property_desc_is_configurable(targetDesc) &&
            EJSVAL_IS_UNDEFINED(_ejs_property_desc_get_getter(targetDesc))) {
...
static EJSPropertyDesc *_ejs_proxy_specop_get_own_property(ejsval O, ejsval P,
                                                           ejsval *exc) {
    // 1. Assert: IsPropertyKey(P) is true.
    EJSProxy *proxy = EJSVAL_TO_PROXY(O);
    ...
    if (EJSVAL_IS_NULL(handler))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "null ProxyHandler in getOwnProperty");
    ...
    // method of trap with handler as the this value and a new List containing
    // target and P.
    // 9. ReturnIfAbrupt(trapResultObj).
    ejsval args[] = {target, P};
    ejsval trapResultObj = _ejs_invoke_closure(trap, handler, 2, args);

    // 10. If Type(trapResultObj) is neither Object nor Undefined, then throw a
    // TypeError exception.
    if (!EJSVAL_IS_OBJECT(trapResultObj) &&
        !EJSVAL_IS_UNDEFINED(trapResultObj)) {
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "return value from getOwnProperty trap "
...
static EJSBool _ejs_proxy_specop_set(ejsval obj, ejsval propertyName,
    ...
    // 7. If trap is undefined, then
    if (EJSVAL_IS_UNDEFINED(trap)) {
        // a. Return the result of calling the [[Set]] internal method of target
        // with arguments P, V, and Receiver.
    ...
    if (targetDesc) {
        ...
        if (IsAccessorDescriptor(targetDesc) &&
            !_ejs_property_desc_is_configurable(targetDesc)) {
            //        i. If targetDesc.[[Set]] is undefined, then throw a
            //        TypeError exception.
...
static EJSBool _ejs_proxy_specop_has_property(ejsval O, ejsval P) {
    // 1. Assert: IsPropertyKey(P) is true.
    ...
    if (!booleanTrapResult) {
        ...
        EJSPropertyDesc *targetDesc =
            OP(_target, GetOwnProperty)(target, P, NULL);
...
static EJSBool _ejs_proxy_specop_delete(ejsval O, ejsval P,
                                        EJSBool unusedflag) {
    // 1. Assert: IsPropertyKey(P) is true.
...
static EJSBool _ejs_proxy_specop_define_own_property(ejsval O, ejsval P,
    ...
    // 10. Let trapResult be the result of calling the [[Call]] internal method
    // of trap with handler as the this value and a new List containing target,
    // P, and descObj.
    ...
    // 18. If Desc has a [[Configurable]] field and if Desc.[[Configurable]] is
    // false, then
    ...
    // 21. Else targetDesc is not undefined,
        ...
        //     b. If settingConfigFalse is true and targetDesc.[[Configurable]]
        //     is true, then throw a TypeError exception.
        EJS_NOT_IMPLEMENTED();
    }
    // 22. Return true.
    return EJS_TRUE;
...
EJS_DEFINE_CLASS(Proxy, _ejs_proxy_specop_get_prototype_of,
                 ...
                 _ejs_proxy_specop_has_property, _ejs_proxy_specop_get,
                 _ejs_proxy_specop_set, _ejs_proxy_specop_delete,
