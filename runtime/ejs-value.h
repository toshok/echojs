...
#define EJSVAL_IS_PRIMITIVE(v)                                                 \
    (EJSVAL_IS_NUMBER(v) || EJSVAL_IS_STRING(v) || EJSVAL_IS_BOOLEAN(v) ||     \
     EJSVAL_IS_UNDEFINED(v))

#define EJSVAL_IS_OBJECT(v) EJSVAL_IS_OBJECT_IMPL(v)
#define EJSVAL_IS_ARRAY(v)                                                     \
    (EJSVAL_IS_OBJECT(v) &&                                                    \
     (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Array_specops ||                       \
      EJSVAL_TO_OBJECT(v)->ops == &_ejs_sparsearray_specops))
#define EJSVAL_IS_FUNCTION(v)                                                  \
    (EJSVAL_IS_OBJECT(v) &&                                                    \
     (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Function_specops))
#define EJSVAL_IS_CALLABLE(v)                                                  \
    (EJSVAL_IS_OBJECT(v) &&                                                    \
     (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Function_specops ||                    \
      EJSVAL_TO_OBJECT(v)->ops == &_ejs_Proxy_specops))
#define EJSVAL_IS_CONSTRUCTOR(v)                                               \
    (EJSVAL_IS_OBJECT(v) &&                                                    \
     (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Function_specops ||                    \
      EJSVAL_TO_OBJECT(v)->ops == &_ejs_Proxy_specops)) // XXX this is lacking
#define EJSVAL_IS_DATE(v)                                                      \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Date_specops))
#define EJSVAL_IS_REGEXP(v)                                                    \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_RegExp_specops))
#define EJSVAL_IS_NUMBER_OBJECT(v)                                             \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Number_specops))
#define EJSVAL_IS_STRING_OBJECT(v)                                             \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_String_specops))
#define EJSVAL_IS_BOOLEAN_OBJECT(v)                                            \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Boolean_specops))
...
#define EJSVAL_IS_NULL_OR_UNDEFINED(v)                                         \
    (EJSVAL_IS_NULL(v) || EJSVAL_IS_UNDEFINED(v))
#define EJSVAL_IS_NO_ITER_VALUE_MAGIC(v)                                       \
    (EJSVAL_IS_MAGIC_IMPL(v) && EJSVAL_TO_INT32_IMPL(v) == EJS_NO_ITER_VALUE)
#define EJSVAL_IS_ARRAY_HOLE_MAGIC(v)                                          \
    (EJSVAL_IS_MAGIC_IMPL(v) && EJSVAL_TO_INT32_IMPL(v) == EJS_ARRAY_HOLE)

#define EJSOBJECT_IS_ARRAY(v)                                                  \
    ((v)->ops == &_ejs_Array_specops || (v)->ops == &_ejs_sparsearray_specops)
