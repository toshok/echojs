...
// ES6: 23.1.3.1
// Map.prototype.clear ()
ejsval _ejs_Set_prototype_clear(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.clear called with non-object this.");

    // 3. If S does not have a [[SetData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_SET(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.clear called with non-Set this.");

    // 4. If S’s [[SetData]] internal slot is undefined, then throw a TypeError
    // exception.

    // 5. Let entries be the List that is the value of S’s [[SetData]] internal
    // slot.
    ...
    for (EJSSetValueEntry *e = entries; e; e = e->next_insert) {
        //    a. Replace the element of entries whose value is e with an element
...
ejsval _ejs_set_delete(ejsval S, ejsval value) {
    // our caller should have already validated and thrown appropriate
    // TypeErrors
...
ejsval _ejs_Set_prototype_delete(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Set.prototype.delete called with non-object this.");

    // 3. If S does not have a [[SetData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_SET(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.delete called with non-Set this.");
...
ejsval _ejs_Set_prototype_entries(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    // 1. Let S be the this value.
    ejsval S = _this;

    // 2. Return the result of calling the CreateSetIterator abstract operation
    // with arguments S and "key+value".
    return _ejs_set_iterator_new(S, EJS_SET_ITER_KIND_KEYVALUE);
}

// ES6: 23.2.3.6 Set.prototype.forEach ( callbackfn , thisArg = undefined )
ejsval _ejs_Set_prototype_forEach(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Set.prototype.forEach called with non-object this.");

    // 3. If S does not have a [[SetData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_SET(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.forEach called with non-Set this.");
    ...
    if (!EJSVAL_IS_CALLABLE(callbackfn))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Set.prototype.forEach callbackfn isn't a function.");
    ...
    // 7. Let entries be the List that is the value of S’s [[SetData]] internal
    // slot.
    EJSSetValueEntry *entries = set->head_insert;

    // 8. Repeat for each e that is an element of entries, in original insertion
    // order
    for (EJSSetValueEntry *e = entries; e; e = e->next_insert) {
        ...
        //       internal method of callbackfn with T as thisArgument and a List
        //       containing e, e, and S as argumentsList.
...
ejsval _ejs_set_has(ejsval S, ejsval value) {
    // our caller should have already validated and thrown appropriate
    // TypeErrors
    EJS_ASSERT(EJSVAL_IS_SET(S));
    ...
    // 6. Repeat for each e that is an element of entries,
    for (EJSSetValueEntry *e = entries; e; e = e->next_insert) {
        // a. If e is not empty and SameValueZero(e, value) is true, then return
        // true.
        if (SameValueZero(e->value, value))
            return _ejs_true;
    }
...
ejsval _ejs_Set_prototype_has(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ejsval value = _ejs_undefined;
    if (argc > 0)
        value = args[0];
    ...
    if (!EJSVAL_IS_OBJECT(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.has called with non-object this.");

    // 3. If S does not have a [[SetData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_SET(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.has called with non-Set this.");
...
ejsval _ejs_set_add(ejsval S, ejsval value) {
    // our caller should have already validated and thrown appropriate
    // TypeErrors
    EJS_ASSERT(EJSVAL_IS_SET(S));
    ...
    if (EJSVAL_IS_NUMBER(value) &&
        EJSDOUBLE_IS_NEGZERO(EJSVAL_TO_NUMBER(value)))
...
ejsval _ejs_Set_prototype_add(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ejsval value = _ejs_undefined;
    if (argc > 0)
        value = args[0];
    ...
    if (!EJSVAL_IS_OBJECT(S)) {
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.add called with non-object this.");
    }
    ...
    if (!EJSVAL_IS_SET(S))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set.prototype.set called with non-Set this.");
...
static ejsval _ejs_Set_prototype_get_size(ejsval env, ejsval _this,
    ...
    if (!EJSVAL_IS_OBJECT(S)) {
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set size getter called with non-object this.");
    }
    ...
    if (!EJSVAL_IS_SET(S)) {
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set size getter called with non-Set this.");
    }
...
ejsval _ejs_Set_prototype_values(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    // 1. Let S be the this value.
    ejsval S = _this;

    // 2. Return the result of calling the CreateSetIterator abstract operation
    // with argument S and "value".
    return _ejs_set_iterator_new(S, EJS_SET_ITER_KIND_VALUE);
}

// ES6: 23.2.1.1 Set ( [ iterable ] )
static ejsval _ejs_Set_impl(ejsval env, ejsval _this, uint32_t argc,
                            ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(set))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set constructor called with non-object this.");

    // 3. If set does not have a [[SetData]] internal slot, then throw a
    // TypeError exception.
    if (!EJSVAL_IS_SET(set))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Set constructor called with non-Set this.");
    ...
    if (_set->head_insert)
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Set constructor called with an already initialized Set");
    ...
    // 8. If the value of sets’s [[SetData]] internal slot is not undefined,
    ...
    for (;;) {
        ...
        //    method of adder with set as thisArgument
...
static ejsval _ejs_Set_get_species(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    return _ejs_Set;
}
...
// ECMA262: 23.2.2.2
// Set[ @@create ] ( )
static ejsval _ejs_Set_create(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in Set[Symbol.create] is not a constructor");
...
ejsval _ejs_set_iterator_new(ejsval set, EJSSetIteratorKind kind) {
    ...
    _ejs_init_object((EJSObject *)iter, _ejs_SetIterator_prototype,
                     &_ejs_SetIterator_specops);
...
static ejsval _ejs_SetIterator_impl(ejsval env, ejsval _this, uint32_t argc,
                                    ejsval *args) {
    return _this;
}

static ejsval _ejs_SetIterator_prototype_next(ejsval env, ejsval _this,
                                              uint32_t argc, ejsval *args) {
    ...
    if (!EJSVAL_IS_SETITERATOR(O))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    ".next called on non-SetIterator instance");
    ...
    /* 6. Let itemKind be the value of the [[SetIterationKind]] internal slot of
     * O. */
    ...
    /* 8. Assert: s has a [[SetData]] internal slot and s has been initialized
     * so the value of
     * [[SetData]] is not undefined. */

    /* 9. Let entries be the List that is the value of the [[SetData]] internal
     * slot of s. */
    EJSSetValueEntry *entries = EJSVAL_TO_SET(s)->head_insert;

    /* 10. Repeat while index is less than the total number of elements of
     * entries. The number of elements must
    ...
    for (EJSSetValueEntry *entry = entries; entry; entry = entry->next_insert) {
        ...
        /*  (see EJSVAL_IS_NO_ITER_VALUE_MAGIC check at the beginning of the
         * loop */

        /*      i. If itemKind is "key+value" then, */
        if (itemKind == EJS_SET_ITER_KIND_KEYVALUE) {
            /* 1. Let result be the result of performing ArrayCreate(2). */
            /* 2. Assert: result is a new, well-formed Array object so the
...
void _ejs_set_init(ejsval global) {
    _ejs_Set = _ejs_function_new_without_proto(_ejs_null, _ejs_atom_Set,
                                               (EJSClosureFunc)_ejs_Set_impl);
...
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_Set_prototype, x, _ejs_Set_prototype_##x,                         \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE)
#define PROTO_GETTER(x)                                                        \
    EJS_INSTALL_ATOM_GETTER(_ejs_Set_prototype, x, _ejs_Set_prototype_get_##x)
    ...
    // expand PROTO_METHOD(values) here so that we can install the function for
    // both keys and @@iterator below
    ejsval _values = _ejs_function_new_native(
        _ejs_null, _ejs_atom_values, (EJSClosureFunc)_ejs_Set_prototype_values);
    _ejs_object_define_value_property(_ejs_Set_prototype, _ejs_atom_values,
                                      _values, EJS_PROP_NOT_ENUMERABLE |
                                                   EJS_PROP_FLAGS_WRITABLE |
                                                   EJS_PROP_CONFIGURABLE);
    _ejs_object_define_value_property(
        _ejs_Set_prototype, _ejs_atom_keys, _values,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE);

    _ejs_object_define_value_property(_ejs_Set_prototype, _ejs_Symbol_iterator,
                                      _values, EJS_PROP_NOT_ENUMERABLE);
    _ejs_object_define_value_property(
        _ejs_Set_prototype, _ejs_Symbol_toStringTag, _ejs_atom_Set,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_WRITABLE |
            EJS_PROP_CONFIGURABLE);

    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_Set, create, _ejs_Set_create,
...
#undef PROTO_METHOD
    ...
    _ejs_SetIterator = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Set, (EJSClosureFunc)_ejs_SetIterator_impl);

    _ejs_gc_add_root(&_ejs_SetIterator_prototype);
    _ejs_SetIterator_prototype =
        _ejs_set_iterator_new(_ejs_Set_prototype, EJS_SET_ITER_KIND_VALUE);
    EJSVAL_TO_OBJECT(_ejs_SetIterator_prototype)->proto = _ejs_Object_prototype;
    _ejs_object_define_value_property(
        _ejs_SetIterator, _ejs_atom_prototype, _ejs_SetIterator_prototype,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE |
            EJS_PROP_NOT_WRITABLE);
    _ejs_object_define_value_property(
        _ejs_SetIterator_prototype, _ejs_atom_constructor, _ejs_SetIterator,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);

#define PROTO_ITER_METHOD(x)                                                   \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_SetIterator_prototype, x, _ejs_SetIterator_prototype_##x,         \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE)
    PROTO_ITER_METHOD(next);
#undef PROTO_ITER_METHOD
}

static EJSObject *_ejs_set_specop_allocate() {
    return (EJSObject *)_ejs_gc_new(EJSSet);
}

static void _ejs_set_specop_finalize(EJSObject *obj) {
...
static void _ejs_set_specop_scan(EJSObject *obj, EJSValueFunc scan_func) {
    EJSSet *set = (EJSSet *)obj;
...
EJS_DEFINE_CLASS(Set,
                 ...
                 _ejs_set_specop_allocate, _ejs_set_specop_finalize,
                 _ejs_set_specop_scan)

static void _ejs_set_iterator_specop_scan(EJSObject *obj,
