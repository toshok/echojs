...
static id get_peer(ejsval obj) {
    return ((EJSWebGLObject *)EJSVAL_TO_OBJECT(obj))->peer;
}

static void finalize_release_private_data(EJSObject *obj) {
...
static ejsval webglactiveinfo_get_size(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
...
static ejsval webglactiveinfo_get_type(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
    WebGLActiveInfo *info = (WebGLActiveInfo *)get_peer(_this);
...
static ejsval webglactiveinfo_get_name(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
    WebGLActiveInfo *info = (WebGLActiveInfo *)get_peer(_this);
...
#define CHECK_GL                                                               \
    do {                                                                       \
        if ((glerr = glGetError()) != GL_NO_ERROR) {                           \
            NSLog(@"GL error = %d, at %s, line %d", glerr,                     \
...
#define JSMETHOD(meth)                                                         \
    static ejsval JSMETHODNAME(meth)(ejsval env, ejsval _this, uint32_t argc,  \
                                     ejsval * args)
#define THROW_ARG_COUNT_EXCEPTION(expected_count)                              \
    _ejs_throw_nativeerror_utf8(EJS_ERROR /*XXX*/, "argument count mismatch")
...
JSMETHOD(attachShader) {
    ...
    SPEW(NSLog(@"glAttachShader (program = %d, shader = %d)", [program glId],
               [shader glId]);)
...
//    void blendColor(GLclampf red, GLclampf green, GLclampf blue, GLclampf
//    alpha);
JSMETHOD(blendColor) {
    if (argc != 4)
...
JSMETHOD(bufferData) {
    ...
    if (EJSVAL_IS_NUMBER(args[1])) {
        ...
        if (EJSOBJECT_IS_TYPEDARRAY(obj)) {
            ...
            // SPEW(NSLog (@"  using [%lu-%lu]", [bufferView byteOffset],
...
/*
 J3D    void bufferSubData(GLenum target, GLintptr offset, ArrayBufferView
 data);
...
//    void clearColor(GLclampf red, GLclampf green, GLclampf blue, GLclampf
//    alpha);
JSMETHOD(clearColor) {
    if (argc != 4)
        THROW_ARG_COUNT_EXCEPTION(1);
...
//    void colorMask(GLboolean red, GLboolean green, GLboolean blue, GLboolean
//    alpha);
JSMETHOD(colorMask) {
    if (argc != 4)
        THROW_ARG_COUNT_EXCEPTION(4);
...
/*
 ...
 GLint border);
 void copyTexSubImage2D(GLenum target, GLint level, GLint xoffset, GLint
 yoffset,
 GLint x, GLint y, GLsizei width, GLsizei height);
...
JSMETHOD(createBuffer) {
    ...
    _ejs_init_object((EJSObject *)obj, _ejs_Object__proto__,
                     &_ejs_WebGLBuffer_specops);
...
JSMETHOD(createRenderbuffer) {
    ...
    EJSWebGLObject *obj = _ejs_gc_new(EJSWebGLObject);
    _ejs_init_object((EJSObject *)obj, _ejs_Object__proto__,
                     &_ejs_WebGLRenderbuffer_specops);
...
//    void framebufferTexture2D(GLenum target, GLenum attachment, GLenum
//    textarget,
...
JSMETHOD(framebufferTexture2D) {
    ...
    glFramebufferTexture2D(target, attachment, textarget, [texture glId],
                           level);
    CHECK_GL;
...
JSMETHOD(getActiveAttrib) {
    ...
    glGetProgramiv([program glId], GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &bufSize);
    CHECK_GL;
    ...
    glGetActiveAttrib([program glId], index, bufSize, NULL, &size, &type, name);
    CHECK_GL;

    SPEW(NSLog(@"getActiveAttrib index = %d, name = %s, type = %d, size = %d",
               index, name, type, size);)

    EJSWebGLObject *obj = _ejs_gc_new(EJSWebGLObject);
    _ejs_init_object((EJSObject *)obj, WebGLActiveInfo__proto__,
                     &_ejs_WebGLActiveInfo_specops);
...
JSMETHOD(getActiveUniform) {
    ...
    glGetProgramiv([program glId], GL_ACTIVE_UNIFORM_MAX_LENGTH, &bufSize);
    CHECK_GL;
    ...
    glGetActiveUniform([program glId], index, bufSize, NULL, &size, &type,
                       name);
    CHECK_GL;
...
JSMETHOD(getProgramInfoLog) {
    ...
    glGetProgramiv([program glId], GL_INFO_LOG_LENGTH, &info_log_length);
    CHECK_GL;

    program_log = (GLchar *)calloc(1, (size_t)(info_log_length + 1));

    glGetProgramInfoLog([program glId], info_log_length, NULL, program_log);
    CHECK_GL;
...
JSMETHOD(getProgramParameter) {
    ...
    glGetProgramiv([program glId], pname, &param);
    CHECK_GL;
    ...
    case GL_ACTIVE_ATTRIBUTES:
        SPEW(NSLog(@"program has %d active %s", param,
                   pname == GL_ATTACHED_SHADERS ? "shaders" : "attributes");)
        return NUMBER_TO_EJSVAL(param);
    case GL_ACTIVE_UNIFORMS:
        SPEW(NSLog(@"program id = %d, GL_ACTIVE_UNIFORMS = %d", [program glId],
                   param);)
        return NUMBER_TO_EJSVAL(param);
    default:
...
JSMETHOD(getShaderInfoLog) {
    ...
    glGetShaderiv([shader glId], GL_INFO_LOG_LENGTH, &info_log_length);
    CHECK_GL;

    shader_log = (GLchar *)calloc(1, (size_t)(info_log_length + 1));

    glGetShaderInfoLog([shader glId], info_log_length, NULL, shader_log);
    CHECK_GL;
...
JSMETHOD(getShaderParameter) {
    ...
    glGetShaderiv([shader glId], pname, &param);
    CHECK_GL;
...
JSMETHOD(getShaderSource) {
    ...
    glGetShaderiv([shader glId], GL_SHADER_SOURCE_LENGTH, &source_length);
    CHECK_GL;

    shader_source = (GLchar *)calloc(1, (size_t)(source_length + 1));

    glGetShaderSource([shader glId], source_length, NULL, shader_source);
    CHECK_GL;
...
//    WebGLUniformLocation getUniformLocation(WebGLProgram program, DOMString
//    name);
JSMETHOD(getUniformLocation) {
    ...
    GLint loc = glGetUniformLocation([program glId], name);
    SPEW(NSLog(@"glGetUniformLocation (%d, %s) => %d", [program glId], name,
               loc);)
    ...
    EJSWebGLObject *obj = _ejs_gc_new(EJSWebGLObject);
    _ejs_init_object((EJSObject *)obj, _ejs_Object__proto__,
                     &_ejs_WebGLUniformLocation_specops);
...
JSMETHOD(shaderSource) {
    ...
    glShaderSource([shader glId], 1, (const GLchar **)&source, NULL);
    CHECK_GL;
...
JSMETHOD(texImage2D) {
    ...
    if (argc == 6) {
        ...
        UIImage *uiimage = (UIImage *)get_objc_id(
            [[CKValue valueWithJSValue:args[5]] objectValue]);
        ...
        SPEW(NSLog(@"creating bitmapContext with CGImageRef = %p, colorspace = "
                   @"%p, width = %d, height = %d",
                   cgimage, CGImageGetColorSpace(cgimage), width, height);)
        SPEW(NSLog(@"colorspace model = %d",
                   CGColorSpaceGetModel(CGImageGetColorSpace(cgimage)));)
        SPEW(NSLog(@"bits per component = %lu",
                   CGImageGetBitsPerComponent(cgimage));)
        SPEW(NSLog(@"bytes per row = %lu", CGImageGetBytesPerRow(cgimage));)

        bitmapContext = CGBitmapContextCreate(
            NULL, (size_t)width, (size_t)height,
            ...
            format == GL_RGBA
                ...
                : (CGBitmapInfo)
                      kCGImageAlphaNoneSkipLast); // CGImageGetBitmapInfo(cgimage));
        ...
        CGContextDrawImage(bitmapContext, CGRectMake(0, 0, width, height),
                           cgimage);
        ...
        if (EJSVAL_IS_NULL(args[8])) {
            pixels = calloc(
                1, (size_t)(width * height *
                            4)); // XXX we don't always need 32bpp, do we?
            free_pixels = YES;
            ...
            if (type == GL_UNSIGNED_BYTE &&
                EJSTYPEDARRAY_ELEMENT_TYPE(obj) != EJS_TYPEDARRAY_UINT8)
                _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR, "");
    ...
    SPEW(NSLog(@"glTexImage2D (target=%d format=%d type=%d internalFormat=%d "
               @"w=%d, h=%d)",
               target, format, type, internalFormat, width, height);)
...
static ejsval uniform_f(size_t c, uint32_t argc, ejsval *args) {
    int loc = -1;
...
// if (EJSVAL_IS_OBJECTOfClass (ctx, args[0], WebGLUniformLocationClass)) {
    ...
    if (c > 0)
        gl_args[0] = (GLfloat)EJSVAL_TO_NUMBER(args[1]);
    if (c > 1)
        gl_args[1] = (GLfloat)EJSVAL_TO_NUMBER(args[2]);
    if (c > 2)
        gl_args[2] = (GLfloat)EJSVAL_TO_NUMBER(args[3]);
    if (c > 3)
        gl_args[3] = (GLfloat)EJSVAL_TO_NUMBER(args[4]);

    switch (c) {
    case 1:
        glUniform1f(loc, gl_args[0]);
        break;
    case 2:
        glUniform2f(loc, gl_args[0], gl_args[1]);
        break;
    case 3:
        glUniform3f(loc, gl_args[0], gl_args[1], gl_args[2]);
        break;
    case 4:
        glUniform4f(loc, gl_args[0], gl_args[1], gl_args[2], gl_args[3]);
        break;
    default:
        abort();
    }
...
//    GLint w);
static ejsval uniform_i(size_t c, uint32_t argc, ejsval *args) {
    int loc = -1;
...
// if (EJSVAL_IS_OBJECTOfClass (ctx, args[0], WebGLUniformLocationClass)) {
    ...
    if (c > 0)
        gl_args[0] = (GLint)EJSVAL_TO_NUMBER(args[1]);
    if (c > 1)
        gl_args[1] = (GLint)EJSVAL_TO_NUMBER(args[2]);
    if (c > 2)
        gl_args[2] = (GLint)EJSVAL_TO_NUMBER(args[3]);
    if (c > 3)
        gl_args[3] = (GLint)EJSVAL_TO_NUMBER(args[4]);

    switch (c) {
    case 1:
        glUniform1i(loc, gl_args[0]);
        break;
    case 2:
        glUniform2i(loc, gl_args[0], gl_args[1]);
        break;
    case 3:
        glUniform3i(loc, gl_args[0], gl_args[1], gl_args[2]);
        break;
    case 4:
        glUniform4i(loc, gl_args[0], gl_args[1], gl_args[2], gl_args[3]);
        break;
    default:
        abort();
    }
...
static ejsval uniform_fv(size_t c, uint32_t argc, ejsval *args) {
    int loc = -1;
...
// if (EJSVAL_IS_OBJECTOfClass (ctx, args[0], WebGLUniformLocationClass)) {
    ...
    if (EJSOBJECT_IS_TYPEDARRAY(arrayObj) &&
        EJSTYPEDARRAY_ELEMENT_TYPE(arrayObj) == EJS_TYPEDARRAY_FLOAT32) {
        bufferView = arrayObj;
    } else if (EJSOBJECT_IS_ARRAY(arrayObj)) {
        bufferView = EJSVAL_TO_OBJECT(_ejs_typedarray_new_from_array(
            EJS_TYPEDARRAY_FLOAT32, OBJECT_TO_EJSVAL(arrayObj)));
    } else {
    ...
    SPEW(NSLog(@"glUniform%zufv (loc = %d, count = %d, array_buffer = %p", c,
               loc, count, array_buffer);)
    SPEW(for (int i = 0; i < count; i++) {
        switch (c) {
        case 1:
            NSLog(@"%g", array_buffer[i]);
            break;
        case 2:
            NSLog(@"%g %g", array_buffer[i * 2 + 0], array_buffer[i * 2 + 1]);
            break;
        case 3:
            NSLog(@"%g %g %g", array_buffer[i * 3 + 0], array_buffer[i * 3 + 1],
                  array_buffer[i * 3 + 2]);
            break;
        case 4:
            NSLog(@"%g %g %g %g", array_buffer[i * 4 + 0],
                  array_buffer[i * 4 + 1], array_buffer[i * 4 + 2],
                  array_buffer[i * 4 + 3]);
            break;
        }
    });
...
static ejsval uniform_iv(size_t c, uint32_t argc, ejsval *args) {
    int loc = -1;
...
// if (EJSVAL_IS_OBJECTOfClass (ctx, args[0], WebGLUniformLocationClass)) {
    ...
    EJSObject *bufferView;
    if (EJSOBJECT_IS_TYPEDARRAY(arrayObj) &&
        EJSTYPEDARRAY_ELEMENT_TYPE(arrayObj) == EJS_TYPEDARRAY_INT32) {
        bufferView = arrayObj;
    } else if (EJSOBJECT_IS_ARRAY(arrayObj)) {
        bufferView = EJSVAL_TO_OBJECT(_ejs_typedarray_new_from_array(
            EJS_TYPEDARRAY_INT32, OBJECT_TO_EJSVAL(arrayObj)));
    } else {
        NSLog(@"non array-like object passed to uniform*fv");
        abort();
    }
    ...
    switch (c) {
    case 1:
        glUniform1iv(loc, count, array_buffer);
        break;
    case 2:
        glUniform2iv(loc, count, array_buffer);
        break;
    case 3:
        glUniform3iv(loc, count, array_buffer);
        break;
    case 4:
        glUniform4iv(loc, count, array_buffer);
        break;
    default:
        abort();
    }
...
//    void uniformMatrix4fv(WebGLUniformLocation location, GLboolean transpose,
//                          float[] value);
static ejsval uniformMatrix_fv(size_t c, uint32_t argc, ejsval *args) {
    int loc;
...
// if (EJSVAL_IS_OBJECTOfClass (ctx, args[0], WebGLUniformLocationClass)) {
    ...
    if (EJSVAL_IS_OBJECT(args[0])) {
        ...
        const char *arg_as_str =
            [[CKValue valueWithJSValue:args[0]] utf8StringValue];
    ...
    if (EJSOBJECT_IS_TYPEDARRAY(arrayObj) &&
        ...
        NSLog(@"non array-like object passed to uniform*fv");
        const char *arg2_as_str =
            [[CKValue valueWithJSValue:args[2]] utf8StringValue];
    ...
    SPEW(NSLog(@"glUniformMatrix%zufv(loc = %d, count = %d, transpose = %d, "
               @"array_buffer = %p)",
               c, loc, count, transpose, array_buffer);)
    SPEW({
        NSLog(@"matrix = ");
        int i;
...
JSMETHOD(vertexAttribPointer) {
    ...
    SPEW(NSLog(@"glVertexAttribPointer (%d, %d, %d, %d, %d, %ld)", indx, size,
...
#if notyet
    if (!JSValueIsInstanceOfConstructor(
            ctx, args[0], [WebGLProgram jsConstructor], exception))
...
static EJSBool webglrenderingcontext_specop_has_property(ejsval O, ejsval P) {
    char *prop = ucs2_to_utf8(EJSVAL_TO_FLAT_STRING(P));

    webgl_constant *constant_info = (webgl_constant *)bsearch(
        prop, constants, sizeof(constants) / sizeof(webgl_constant),
        sizeof(webgl_constant), compare_constants);
...
static ejsval webglrenderingcontext_specop_get(ejsval O, ejsval P,
                                               ejsval receiver) {
    char *prop = ucs2_to_utf8(EJSVAL_TO_FLAT_STRING(P));
...
ejsval _ejs_objc_allocateWebGLRenderingContext(ejsval env, ejsval _this,
                                               uint32_t argc, ejsval *args) {
    EAGLContext *eagl_context = (EAGLContext *)get_objc_id(
        [[CKValue valueWithJSValue:args[0]] objectValue]);

    SPEW(NSLog(@"EAGLContext = %p\n", eagl_context);)

    EJSWebGLObject *glContext = _ejs_gc_new(EJSWebGLObject);
    _ejs_init_object((EJSObject *)glContext, _ejs_Object__proto__,
                     &_ejs_WebGLRenderingContext_specops);

    glContext->peer = eagl_context; // XXX retain?

    ejsval obj = OBJECT_TO_EJSVAL(glContext);

#define WEBGL_FUNC(name, nArgs)                                                \
    EJS_INSTALL_FUNCTION_FLAGS(obj, #name, JSMETHODNAME(name),                 \
...
#define EJS_WEBGL_OBJ(n)                                                       \
    EJS_DEFINE_CLASS(n, OP_INHERIT, OP_INHERIT, OP_INHERIT, OP_INHERIT,        \
                     OP_INHERIT, OP_INHERIT, OP_INHERIT, OP_INHERIT,           \
                     OP_INHERIT, OP_INHERIT, OP_INHERIT, OP_INHERIT,           \
                     OP_INHERIT, finalize_release_private_data, OP_INHERIT)
...
void _ejs_webgl_init(ejsval global) {
    ...
    EJS_INSTALL_GETTER(WebGLActiveInfo__proto__, "size",
                       webglactiveinfo_get_size);
    EJS_INSTALL_GETTER(WebGLActiveInfo__proto__, "type",
                       webglactiveinfo_get_type);
    EJS_INSTALL_GETTER(WebGLActiveInfo__proto__, "name",
                       webglactiveinfo_get_name);
}
