...
static EJSBool json_value_to_ejsval(JSON_Value *v, ejsval *rv) {
    ...
    case JSONObject: {
        ...
        *rv = _ejs_object_new(_ejs_null, &_ejs_Object_specops);
        ...
        for (int i = 0; i < count; i++) {
            ...
            ejsval propval;
            if (!json_value_to_ejsval(json_object_get_value(obj, propkey),
                                      &propval))
                return EJS_FALSE;
...
/* 15.12.2 */
static ejsval _ejs_JSON_parse(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ...
    /* 2. Parse JText using the grammars in 15.12.1. Throw a SyntaxError
       exception if JText did not conform to the
    ...
    /* 3. Let unfiltered be the result of parsing and evaluating JText as if it
       was the source text of an ECMAScript
       Program but using JSONString in place of StringLiteral. Note that since
       JText conforms to the JSON
       grammar this result will be either a primitive value or an object that is
       defined by either an ArrayLiteral or
       an ObjectLiteral. */
    ...
    if (EJSVAL_IS_CALLABLE(reviver)) {
        ...
        EJS_NOT_IMPLEMENTED();
        /*    a. Let root be a new object created as if by the expression new
           Object(), where Object is the
              standard built-in constructor with that name. */
        /*    b. Call the [[DefineOwnProperty]] internal method of root with the
           empty String, the
              PropertyDescriptor {[[Value]]: unfiltered, [[Writable]]: true,
           [[Enumerable]]: true,
              [[Configurable]]: true}, and false as arguments. */
        /*    c. Return the result of calling the abstract operation Walk,
           passing root and the empty String. The
...
// all state for JSON.stringify that isn't pass as parameters.  stack allocated
// at the beginning of
// _stringify;
typedef struct {
    // XXX stack goes here
...
static ejsval JA(StringifyState *state, ejsval value) {
    ...
    /* 1. If stack contains value then throw a TypeError exception because the
    ...
    /* 6. Let len be the result of calling the [[Get]] internal method of value
    ...
    /* 10. Else */
        ...
        if (EJSVAL_EQ(state->gap, _ejs_atom_empty)) {
            /*        i. Let properties be a String formed by concatenating all
               the element Strings of partial with
                         each adjacent pair of Strings separated with the comma
               character. A comma is not inserted
            ...
            for (int i = 1; i < EJS_ARRAY_LEN(partial); i++) {
                properties = _ejs_string_concatv(
                    properties, comma_str, EJS_DENSE_ARRAY_ELEMENTS(partial)[i],
                    _ejs_null);
            }

            /*        ii. Let final be the result of concatenating "[",
             * properties, and "]". */
            final = _ejs_string_concatv(_ejs_string_new_utf8("["), properties,
                                        _ejs_string_new_utf8("]"), _ejs_null);
        }
        /*     b. Else */
        else {
            /*        i. Let separator be the result of concatenating the comma
               character, the line feed character,
                      and indent. */
            ejsval separator =
                _ejs_string_concat(_ejs_string_new_utf8(",\n"), state->indent);
            /*        ii. Let properties be a String formed by concatenating all
               the element Strings of partial with
                          each adjacent pair of Strings separated with
               separator. The separator String is not inserted
                          either before the first String or after the last
               String. */
            ejsval properties = EJS_DENSE_ARRAY_ELEMENTS(partial)[0];
            for (int i = 1; i < EJS_ARRAY_LEN(partial); i++) {
                properties = _ejs_string_concatv(
                    properties, separator, EJS_DENSE_ARRAY_ELEMENTS(partial)[i],
                    _ejs_null);
            }
            /*        iii. Let final be the result of concatenating "[", the
               line feed character, indent, properties, the
...
static ejsval JO(StringifyState *state, ejsval value) {
    ...
    for (int kk = 0; kk < EJS_ARRAY_LEN(K); kk++) {
        ...
        if (!EJSVAL_IS_UNDEFINED(strP)) {
            ...
            /*       ii. Let member be the concatenation of member and the colon
             * character. */
...
static ejsval Quote(StringifyState *state, ejsval value) {
    ...
    EJSPrimString *prim_value = _ejs_primstring_flatten(
        EJSVAL_TO_STRING(value)); // this is pretty terrible.. we need a way to
                                  // simply iterate over characters in a string
    ...
    for (int vi = 0; vi < len; vi++) {
         ...
         */
        if (C == '\"' || C == '\\') {
            /*       i. Let product be the concatenation of product and the
             * backslash character. */
            product[pi++] = '\\';
        ...
        /*    b. Else if C is backspace, formfeed, newline, carriage return, or
           tab */
             ...
             * backslash character. */
            product[pi++] = '\\';
            /*       ii. Let abbrev be the character corresponding to the value
             * of C as follows: */
            /*           backspace "b" */
            jschar abbrev;
            if (C == '\b')
            ...
            if (C == '\t')
                abbrev = 't';
            /*       iii. Let product be the concatenation of product and
             * abbrev. */
            product[pi++] = abbrev;
        }
        /*    c. Else if C is a control character having a code unit value less
           than the space character */
            ...
            product[pi++] = 'u';
            /*       iii. Let hex be the result of converting the numeric code
               unit value of C to a String of four
                     hexadecimal digits. */
...
static ejsval Str(StringifyState *state, ejsval key, ejsval holder) {
    ...
    if (EJSVAL_IS_OBJECT(value)) {
        /*    a. Let toJSON be the result of calling the [[Get]] internal method
         * of value with argument "toJSON". */
        ejsval toJSON =
            OP(EJSVAL_TO_OBJECT(holder), Get)(holder, _ejs_atom_toJSON, holder);
        /*    b. If IsCallable(toJSON) is true */
        if (EJSVAL_IS_CALLABLE(toJSON)) {
            /*       i. Let value be the result of calling the [[Call]] internal
               method of toJSON passing value as the
    ...
    if (!EJSVAL_IS_UNDEFINED(state->ReplacerFunction)) {
        /*    a. Let value be the result of calling the [[Call]] internal method
           of ReplacerFunction passing holder as
              the this value and with an argument list consisting of key and
           value. */
        EJS_NOT_IMPLEMENTED();
    }
    ...
    if (EJSVAL_IS_OBJECT(value)) {
        ...
        /*    b. Else if the [[Class]] internal property of value is "String"
           then, */
        else if (EJSVAL_IS_STRING_OBJECT(value)) {
            /*       i. Let value be ToString(value). */
            value = ToString(value);
        }
        /*    c. Else if the [[Class]] internal property of value is "Boolean"
           then, */
    ...
    /* 8. If Type(value) is String, then return the result of calling the
    ...
    if (EJSVAL_IS_OBJECT(value) && !EJSVAL_IS_CALLABLE(value)) {
        ...
        if (EJSVAL_IS_ARRAY(value)) {
            ...
            /*     b. Else, return the result of calling the abstract operation
             * JO with argument value. */
...
/* 15.12.3 */
static ejsval _ejs_JSON_stringify(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    if (EJSVAL_IS_OBJECT(replacer)) {
        ...
        /*    b. Else if the [[Class]] internal property of replacer is "Array",
           then */
            ...
            /*       ii. For each value v of a property of replacer that has an
               array index property name. The
                         properties are enumerated in the ascending array index
            ...
            for (int i = 0; i < EJS_ARRAY_LEN(replacer); i++) {
                ...
                /*           4. Else if Type(v) is Object then, */
                    ...
                    /*              a If the [[Class]] internal property of v is
                       "String" or "Number" then let
                                    item be ToString(v). */
                    if (EJSVAL_IS_STRING_OBJECT(v) ||
                        EJSVAL_IS_NUMBER_OBJECT(v))
                        item = ToString(v);
                }
                /*           5. If item is not undefined and item is not
                 * currently an element of PropertyList then, */
                if (!EJSVAL_IS_UNDEFINED(
                        item) /* XXX && not currently in PropertyList */) {
    ...
    if (EJSVAL_IS_OBJECT(space)) {
        ...
        /*    b. Else if the [[Class]] internal property of space is "String"
           then, */
    ...
    if (EJSVAL_IS_NUMBER(space)) {
        ...
        /*    b. Set gap to a String containing space space characters. This
           will be the empty String if space is less
              than 1. */
        if (space_int < 1) {
            state.gap = _ejs_atom_empty;
        } else {
    ...
    /* 7. Else if Type(space) is String */
        ...
        /*    a. If the number of characters in space is 10 or less, set gap to
           space otherwise set gap to a String
...
void _ejs_json_init(ejsval global) {
    ...
    _ejs_object_define_value_property(
        _ejs_JSON, _ejs_Symbol_toStringTag, _ejs_atom_JSON,
