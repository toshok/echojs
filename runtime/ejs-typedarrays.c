...
static EJSBool IsDetachedBuffer(ejsval arrayBuffer) {
    ...
    if (!EJSVAL_IS_ARRAYBUFFER(arrayBuffer))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "value not an array buffer"); // assertion error?
...
static ejsval ValidateTypedArray(ejsval O) {
    ...
    if (IsDetachedBuffer(buffer))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "typed array's buffer is detached");

    return buffer;
}

ejsval _ejs_arraybuffer_new(int size) {
    EJSArrayBuffer *rv = _ejs_gc_new(EJSArrayBuffer);

    _ejs_init_object((EJSObject *)rv, _ejs_ArrayBuffer_prototype,
                     &_ejs_ArrayBuffer_specops);
...
ejsval _ejs_arraybuffer_new_slice(ejsval bufferval, int offset, int size) {
    EJSArrayBuffer *rv = _ejs_gc_new(EJSArrayBuffer);
...
static ejsval _ejs_ArrayBuffer_impl(ejsval env, ejsval _this, uint32_t argc,
                                    ejsval *args) {
...
static ejsval _ejs_ArrayBuffer_create(ejsval env, ejsval _this, uint32_t argc,
                                      ejsval *args) {
    ejsval F = _this;
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in ArrayBuffer[Symbol.create] is not a constructor");

    EJSObject *F_ = EJSVAL_TO_OBJECT(F);

    // 1. Let obj be the result of calling
    // OrdinaryCreateFromConstructor(constructor, "%ArrayBufferPrototype%", (
...
static ejsval _ejs_ArrayBuffer_get_species(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args) {
    return _ejs_ArrayBuffer;
}

static ejsval _ejs_ArrayBuffer_isView(ejsval env, ejsval _this, uint32_t argc,
                                      ejsval *args) {
...
static ejsval _ejs_ArrayBuffer_prototype_slice(ejsval env, ejsval _this,
                                               uint32_t argc, ejsval *args) {
    EJSArrayBuffer *buffer = (EJSArrayBuffer *)EJSVAL_TO_OBJECT(_this);
...
static ejsval _ejs_DataView_impl(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    if (EJSVAL_IS_UNDEFINED(_this)) {
    ...
    _ejs_object_define_value_property(_this, _ejs_atom_byteLength,
                                      DOUBLE_TO_EJSVAL_IMPL(view->byteLength),
                                      EJS_PROP_FLAGS_ENUMERABLE);
    _ejs_object_define_value_property(_this, _ejs_atom_byteOffset,
                                      DOUBLE_TO_EJSVAL_IMPL(view->byteOffset),
                                      EJS_PROP_FLAGS_ENUMERABLE);
    _ejs_object_define_value_property(_this, _ejs_atom_buffer, view->buffer,
                                      EJS_PROP_FLAGS_ENUMERABLE);

    return _this;
}

static ejsval _ejs_DataView_create(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in DataView[Symbol.create] is not a constructor");
...
static inline EJSBool needToSwap(EJSBool littleEndian) {
...
#define EJS_SWAP_2BYTES(x) (((x >> 8) & 0x00FF) | ((x << 8) & 0xFF00))
...
static void swapBytes(void *value, int elementSizeInBytes) {
...
#define EJS_DATA_VIEW_METHOD_IMPL(ElementType, elementtype,                    \
                                  elementSizeInBytes)                          \
    static ejsval _ejs_DataView_prototype_get##ElementType##_impl(             \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {               \
        if (argc < 1) {                                                        \
        ...
        return NUMBER_TO_EJSVAL(val);                                          \
                                                                               ...
                                                                               \
    static void _ejs_DataView_prototype_set##ElementType##_impl(               \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {               \
        if (argc < 2) {                                                        \
            _ejs_log("wrong number of arguments\n");                           \
            EJS_NOT_IMPLEMENTED();                                             \
...
#define EJS_TYPED_ARRAY(EnumType, ArrayType, arraytype, elementtype,             \
                        elementSizeInBytes)                                      \
    static ejsval _ejs_##ArrayType##Array_impl(ejsval env, ejsval _this,         \
                                               uint32_t argc, ejsval *args) {    \
        if (EJSVAL_IS_UNDEFINED(_this))                                          \
            _ejs_throw_nativeerror_utf8(                                         \
                EJS_TYPE_ERROR, "Constructor cannot be called as a function");   \
                                                                                 \
        ...
        if (EJSVAL_IS_OBJECT(args[0])) {                                         \
            if (EJSVAL_IS_TYPEDARRAY(args[0])) {                                 \
                /* TypedArray(TypedArray array) */                               \
                EJSTypedArray *typed_array =                                     \
                    (EJSTypedArray *)EJSVAL_TO_OBJECT(args[0]);                  \
                                                                                 \
                ...
                arr->buffer = _ejs_arraybuffer_new(arr->byteLength);             \
                                                                                 \
                _ejs_log("need to copy the existing data from the typed "        \
                         "array to this array\n");                               \
                EJS_NOT_IMPLEMENTED();                                           \
                ...
                arr->buffer = _ejs_arraybuffer_new(arr->byteLength);             \
                                                                                 \
                void *buf_data =                                                 \
                    ((EJSArrayBuffer *)EJSVAL_TO_OBJECT(arr->buffer))            \
                        ->data.alloced_buf;                                      \
                if (EJSVAL_IS_DENSE_ARRAY(args[0])) {                            \
                    EJSObject *arr = EJSVAL_TO_OBJECT(args[0]);                  \
                    int i;                                                       \
                    for (i = 0; i < EJSARRAY_LEN(arr); i++) {                    \
                        ((elementtype *)buf_data)[i] =                           \
                            (elementtype)EJSVAL_TO_NUMBER(                       \
                                EJSDENSEARRAY_ELEMENTS(arr)[i]);                 \
                    }                                                            \
                } else {                                                         \
                    _ejs_log("need to implement normal array object copying "    \
                             "for sparse arrays.  or do we?\n");                 \
                    EJS_NOT_IMPLEMENTED();                                       \
                }                                                                \
            } else if (EJSVAL_IS_ARRAYBUFFER(args[0])) {                         \
                EJSArrayBuffer *buffer =                                         \
                    (EJSArrayBuffer *)EJSVAL_TO_OBJECT(args[0]);                 \
                /* TypedArray(ArrayBuffer buffer) */                             \
                ...
                if (byteOffset + byteLength > buffer->size) {                    \
                    if (lengthSpecified)                                         \
                        _ejs_throw_nativeerror_utf8(                             \
                            EJS_RANGE_ERROR, "Length is out of range.");         \
                    else                                                         \
                ...
                if ((byteOffset % sizeof(elementtype)) != 0)                     \
                    _ejs_throw_nativeerror_utf8(                                 \
                        EJS_RANGE_ERROR,                                         \
                        "Byte offset / length is not aligned.");                 \
        ...
        construct_from_array_len : {                                             \
            ...
            EJS_NOT_IMPLEMENTED();                                               \
                                                                                 ...
                                                                                 \
        _ejs_object_define_value_property(_this, _ejs_atom_length,               \
                                          DOUBLE_TO_EJSVAL_IMPL(arr->length),    \
                                          EJS_PROP_FLAGS_ENUMERABLE);            \
        _ejs_object_define_value_property(                                       \
            _this, _ejs_atom_byteOffset,                                         \
            DOUBLE_TO_EJSVAL_IMPL(arr->byteOffset),                              \
            EJS_PROP_FLAGS_ENUMERABLE);                                          \
        _ejs_object_define_value_property(                                       \
            _this, _ejs_atom_byteLength,                                         \
            DOUBLE_TO_EJSVAL_IMPL(arr->byteLength),                              \
            EJS_PROP_FLAGS_ENUMERABLE);                                          \
        _ejs_object_define_value_property(                                       \
            _this, _ejs_atom_buffer, arr->buffer, EJS_PROP_FLAGS_ENUMERABLE);    \
                                                                                 \
        return _this;                                                            \
    }                                                                            \
                                                                                 \
    static ejsval _ejs_##ArrayType##array_specop_get(                            \
        ejsval obj, ejsval propertyName, ejsval receiver) {                      \
        /* check if propertyName is an integer, or a string that we can          \
         * convert to an int */                                                  \
        ...
        if (EJSVAL_IS_STRING(propertyName) &&                                    \
            !ucs2_strcmp(_ejs_ucs2_length,                                       \
                         EJSVAL_TO_FLAT_STRING(propertyName))) {                 \
        ...
        return _ejs_Object_specops.Get(obj, propertyName, receiver);             \
                                                                                 ...
                                                                                 \
    static EJSPropertyDesc *_ejs_##ArrayType##array_specop_get_own_property(     \
        ejsval obj, ejsval propertyName, ejsval *exc) {                          \
        ...
        return _ejs_Object_specops.GetOwnProperty(obj, propertyName, exc);       \
                                                                                 ...
                                                                                 \
    static EJSBool _ejs_##ArrayType##array_specop_set(                           \
        ejsval obj, ejsval propertyName, ejsval val, ejsval receiver) {          \
        /* check if propertyName is an integer, or a string that we can          \
         * convert to an int */                                                  \
        EJSBool is_index = EJS_FALSE;                                            \
        ejsval idx_val = ToNumber(propertyName);                                 \
        ...
        return EJS_TRUE; /* XXX */                                               \
                                                                                 ...
                                                                                 \
    static EJSBool _ejs_##ArrayType##array_specop_define_own_property(           \
        ejsval obj, ejsval propertyName, EJSPropertyDesc *propertyDescriptor,    \
        EJSBool flag) {                                                          \
        return _ejs_Object_specops.DefineOwnProperty(                            \
            obj, propertyName, propertyDescriptor, flag);                        \
    }                                                                            \
                                                                                 \
    static EJSBool _ejs_##ArrayType##array_specop_has_property(                  \
        ejsval obj, ejsval propertyName) {                                       \
        EJS_NOT_IMPLEMENTED();                                                   \
    }                                                                            \
                                                                                 \
    /* Rev 38 Final Draft, April 14, 2015 */                                     \
    ...
    static ejsval _ejs_##ArrayType##Array_prototype_set_array(                   \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        ejsval array;                                                            \
        ...
        if (srcLength + targetOffset > targetLength)                             \
            _ejs_throw_nativeerror_utf8(                                         \
                EJS_RANGE_ERROR, "srcLength + targetOffset > targetLength");     \
                                                                                 \
        /* 21. Let targetByteIndex be targetOffset × targetElementSize +        \
         * targetByteOffset. */                                                  \
        /* Impl note: the buffer data already comes adjusted for the typed       \
         * array */                                                              \
        ...
        while (targetIndex < limit) {                                            \
            ...
            ejsval kNumber = ToNumber(Get(src, Pk));                             \
                                                                                 \
            /* d. If IsDetachedBuffer(targetBuffer) is true, throw a TypeError   \
             * exception. */                                                     \
            if (IsDetachedBuffer(targetBuffer))                                  \
                _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,                      \
                                            "typed array buffer is detached");   \
            ...
            ((elementtype *)data)[targetIndex] =                                 \
                (elementtype)EJSVAL_TO_NUMBER(kNumber);                          \
                                                                                 \
            /* f. Set k to k + 1. */                                             \
            k++;                                                                 \
                                                                                 \
    ...
    static ejsval _ejs_##ArrayType##Array_prototype_set_typedarray(              \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        ejsval typedArray = args[0];                                             \
        EJSTypedArray *typedArrayObj =                                           \
            (EJSTypedArray *)EJSVAL_TO_OBJECT(typedArray);                       \
                                                                                 \
        ejsval offset = _ejs_undefined;                                          \
        if (argc >= 2)                                                           \
            offset = args[1];                                                    \
        ...
        int32_t targetLength = targetObj->length;                                \
                                                                                 \
        /* 12. Let srcBuffer be the value of typedArray’s                      \
        ...
        if (IsDetachedBuffer(srcBuffer))                                         \
            _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,                          \
                                        "source array buffer is detached");      \
                                                                                 \
         ...
         * [[TypedArrayName]] internal slot. */                                  \
        /* 19. Let srcType be the string value of the Element Type value in      \
         * Table 49 for srcName . */                                             \
        EJSTypedArrayType srcType = typedArrayObj->element_type;                 \
                                                                                 \
        /* 20. Let srcElementSize be the Number value of the Element Size        \
         * value specified in Table 49 for srcName. */                           \
                                                                                 \
        ...
        if (srcType != targetType) {                                             \
             ...
             * srcType). */                                                      \
            /*  ii. Perfrom SetValueInBuffer (targetBuffer, targetByteIndex,     \
             * targetType, value). */                                            \
             ...
             * must be performed */                                              \
            /* in a manner that preserves the bit-level encoding of the source   \
             * data. */                                                          \
            /* b. Repeat, while targetByteIndex < limit */                       \
            while (targetIndex < limit) {                                        \
                /* Impl note: since we have good enough direct access to the     \
                 * buffers                                                       \
                 * and their respective types, we don't do the byte-by-byte      \
                 * read/write */                                                 \
                                                                                 \
                /* i. Let value be GetValueFromBuffer(srcBuffer, srcByteIndex,   \
                 * "Uint8"). */                                                  \
                ejsval value = _ejs_typedarray_get_at(                           \
                    (EJSObject *)typedArrayObj, srcIndex);                       \
                                                                                 \
                /* ii. Perform SetValueInBuffer (targetBuffer,                   \
                 * targetByteIndex, "Uint8", value). */                          \
                void *data = _ejs_typedarray_get_data((EJSObject *)targetObj);   \
                ((elementtype *)data)[targetIndex] =                             \
                    (elementtype)EJSVAL_TO_NUMBER(value);                        \
                                                                                 \
    ...
    static ejsval _ejs_##ArrayType##Array_prototype_set_impl(                    \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        ...
        if (argc >= 1)                                                           \
            overloaded = args[0];                                                \
                                                                                 \
        /* 1. Assert: array is any ECMAScript language value other than an       \
        ...
        if (EJSVAL_IS_TYPEDARRAY(overloaded))                                    \
            return _ejs_##ArrayType##Array_prototype_set_typedarray(             \
                env, _this, argc, args);                                         \
        else                                                                     \
            return _ejs_##ArrayType##Array_prototype_set_array(env, _this,       \
                                                               argc, args);      \
                                                                                 \
        EJS_NOT_REACHED();                                                       \
    }                                                                            \
                                                                                 \
    static ejsval _ejs_##ArrayType##Array_prototype_get_impl(                    \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        EJS_NOT_IMPLEMENTED();                                                   \
    }                                                                            \
                                                                                 \
    static ejsval _ejs_##ArrayType##Array_prototype_subarray_impl(               \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        ...
        if (!EJSVAL_IS_OBJECT(O))                                                \
            _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,                          \
                                        "argument is not an Object");            \
                                                                                 \
        /* 3. If O does not have a [[TypedArrayName]] internal slot, then        \
         * throw a TypeError exception. */                                       \
        if (!EJSVAL_IS_TYPEDARRAY(O))                                            \
            _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,                          \
                                        "argument is not a typed array");        \
        ...
        if (EJSVAL_IS_UNDEFINED(buffer))                                         \
            _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,                          \
                                        "typed arrray's buffer is undefined");   \
                                                                                 \
         ...
         * [[TypedArrayName]] internal slot. */                                  \
        /* 20. Let elementType be the string value of the Element Type value     \
        ...
        ejsval argumentsList[3] = {buffer, NUMBER_TO_EJSVAL(beginByteOffset),    \
                                   NUMBER_TO_EJSVAL(newLength)};                 \
                                                                                 \
        EJSTypedArray *rv = _ejs_gc_new(EJSTypedArray);                          \
        _ejs_init_object((EJSObject *)rv, _ejs_##ArrayType##Array_prototype,     \
                         _ejs_typed_array_specops[EJS_TYPEDARRAY_##EnumType]);   \
                                                                                 \
        /* 28. Return the result of calling the [[Construct]] internal method    \
         * of constructor with argument argumentsList. */                        \
        ejsval ctor = _ejs_##ArrayType##Array;                                   \
        return _ejs_invoke_closure(ctor, OBJECT_TO_EJSVAL((EJSObject *)rv), 3,   \
                                   argumentsList);                               \
    }                                                                            \
                                                                                 \
    /* this should be a single getter reused by all typed-arrays */              \
    static ejsval _ejs_##ArrayType##Array_prototype_get_toStringTag(             \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        if (!EJSVAL_IS_TYPEDARRAY(_this))                                        \
            _ejs_throw_nativeerror_utf8(                                         \
                EJS_TYPE_ERROR, "get toStringTag called on non-type array");     \
                                                                                 \
        EJSTypedArray *arr = EJSVAL_TO_TYPEDARRAY(_this);                        \
        switch (arr->element_type) {                                             \
        case EJS_TYPEDARRAY_INT8:                                                \
            return _ejs_atom_Int8Array;                                          \
        case EJS_TYPEDARRAY_UINT8:                                               \
            return _ejs_atom_Uint8Array;                                         \
        case EJS_TYPEDARRAY_UINT8CLAMPED:                                        \
            return _ejs_atom_Uint8ClampedArray;                                  \
        case EJS_TYPEDARRAY_INT16:                                               \
            return _ejs_atom_Int16Array;                                         \
        case EJS_TYPEDARRAY_UINT16:                                              \
            return _ejs_atom_Uint16Array;                                        \
        case EJS_TYPEDARRAY_INT32:                                               \
            return _ejs_atom_Int32Array;                                         \
        case EJS_TYPEDARRAY_UINT32:                                              \
            return _ejs_atom_Uint32Array;                                        \
        case EJS_TYPEDARRAY_FLOAT32:                                             \
            return _ejs_atom_Float32Array;                                       \
        case EJS_TYPEDARRAY_FLOAT64:                                             \
            return _ejs_atom_Float64Array;                                       \
        default:                                                                 \
            EJS_NOT_IMPLEMENTED();                                               \
        }                                                                        \
    }                                                                            \
                                                                                 \
    static ejsval _ejs_##ArrayType##Array_get_species(                           \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        return _ejs_##ArrayType##Array;                                          \
    }                                                                            \
                                                                                 \
    static ejsval _ejs_##ArrayType##Array_create(                                \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        /* 1. Let F be the this value. */                                        \
        ...
        if (!EJSVAL_IS_CONSTRUCTOR(F))                                           \
            _ejs_throw_nativeerror_utf8(                                         \
                EJS_TYPE_ERROR, "'this' in " #ArrayType                          \
                                "Array[Symbol.create] is not a constructor");    \
                                                                                 \
        EJSObject *F_ = EJSVAL_TO_OBJECT(F);                                     \
                                                                                 \
        /* 3. Let proto be GetPrototypeFromConstructor(F,                        \
         * "%TypedArrayPrototype%").  */                                         \
        ...
        _ejs_init_object((EJSObject *)obj, proto,                                \
                         _ejs_typed_array_specops[EJS_TYPEDARRAY_##EnumType]);   \
        /* 6. Add a [[ViewedArrayBuffer]] internal slot to obj and set its       \
         * initial value to undefined.  */                                       \
        obj->buffer = _ejs_undefined;                                            \
        /* 7. Add a [[TypedArrayName]] internal slot to obj and set its          \
         * initial value to undefined .  */                                      \
        /* XXX element type? */                                                  \
        /* 8. Add a [[ByteLength]] internal slot to obj and set its initial      \
         * value to 0. */                                                        \
        obj->byteLength = 0;                                                     \
        /* 9. Add a [[ByteOffset]] internal slot to obj and set its initial      \
         * value to 0. */                                                        \
        obj->byteOffset = 0;                                                     \
        /* 10. Add an [[ArrayLength]] internal slot to obj and set its initial   \
         * value to 0. */                                                        \
        obj->length = 0;                                                         \
        /* 11. Return obj. */                                                    \
        return OBJECT_TO_EJSVAL((EJSObject *)obj);                               \
                                                                                 ...
                                                                                 \
    static ejsval _ejs_##ArrayType##Array_of_impl(                               \
        ejsval env, ejsval _this, uint32_t argc, ejsval *args) {                 \
        ejsval newObj;                                                           \
                                                                                 \
        /* 1. Let len be the actual number of arguments passed to this           \
         * function. */                                                          \
        uint32_t len = argc;                                                     \
                                                                                 \
        ...
        if (EJSVAL_IS_CONSTRUCTOR(C))                                            \
            /* a. Let newObj be the result of calling the [[Construct]]          \
             * internal method of C with argument «len». */                    \
...
static ejsval _ejs_TypedArray_prototype_every(ejsval env, ejsval _this,
                                              uint32_t argc, ejsval *args) {
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.every called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.every called on non typed-array object");

    /* 1. 1. Let O be the result of calling ToObject passing the this value as
     * the argument. */
    ...
    while (k < len) {
        ...
        ejsval testResult =
            _ejs_invoke_closure(callbackfn, T, 3, callbackfn_args);
...
static ejsval _ejs_TypedArray_prototype_fill(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.fill called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.fill called on non typed-array object");
    ...
    /* 8. If relativeStart < 0, let k be max((len + relativeStart),0); else let
     * k be min(relativeStart, len). */
    int32_t k;
    if (relativeStart < 0)
...
static ejsval _ejs_TypedArray_prototype_find(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.find called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.find called on non typed-array object");
    ...
    while (k < len) {
        ...
        ejsval testResult =
            _ejs_invoke_closure(predicate, T, 3, predicate_args);
...
static ejsval _ejs_TypedArray_prototype_findIndex(ejsval env, ejsval _this,
                                                  uint32_t argc, ejsval *args) {
    ejsval predicate = _ejs_undefined;
    ejsval thisArg = _ejs_undefined;
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.findIndex called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.findIndex called on non typed-array object");
...
static ejsval _ejs_TypedArray_prototype_forEach(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    ejsval callbackfn = _ejs_undefined;
    ejsval thisArg = _ejs_undefined;
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.forEach called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.forEach called on non typed-array object");
    ...
    while (k < len) {
        ...
        /*  ii. Call the [[Call]] internal method of callbackfn with T as the
...
static ejsval _ejs_TypedArray_prototype_indexOf(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.indexOf called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.indexOf called on non typed-array object");
    ...
    /* 7. If argument fromIndex was passed let n be ToInteger(fromIndex); else
     * let n be 0. */
    ...
    while (k < len) {
        /* (We optimize for non-null, non-sparse access, invariant of typed
         * arrays */
        ...
        /*  iii. Let same be the result of performing Strict Equality Comparison
...
static ejsval _ejs_TypedArray_prototype_join(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ...
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.join called on non typed-array object");
    ...
    /* (Since R will be used as starting point, copy it now to the internal
     * buffer */
...
static ejsval _ejs_TypedArray_prototype_keys(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    /* 1. Let O be the this value. */
    ejsval O = _this;
...
static ejsval _ejs_TypedArray_prototype_lastIndexOf(ejsval env, ejsval _this,
                                                    uint32_t argc,
                                                    ejsval *args) {
    ejsval searchElement = _ejs_undefined;
    ejsval fromIndex = _ejs_undefined;
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.lastIndexOf called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR, "TypedArray.prototype."
                                                    "lastIndexOf called on non "
                                                    "typed-array object");
...
static ejsval _ejs_TypedArray_prototype_reduce(ejsval env, ejsval _this,
                                               uint32_t argc, ejsval *args) {
    ejsval callbackfn = _ejs_undefined;
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.reduce called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.reduce called on non typed-array object");
    ...
    if (len == 0 && EJSVAL_IS_UNDEFINED(initialValue))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "array is empty and the initial value was not provided");
    ...
    while (k < len) {
        ...
        /*  iii. Let accumulator be Call(callbackfn, undefined, «accumulator,
         * kValue, k, O»). */
        ejsval callbackfn_args[4] = {accumulator, kValue, NUMBER_TO_EJSVAL(k),
                                     O};
        accumulator =
            _ejs_invoke_closure(callbackfn, _ejs_undefined, 4, callbackfn_args);
...
static ejsval _ejs_TypedArray_prototype_reduceRight(ejsval env, ejsval _this,
                                                    uint32_t argc,
                                                    ejsval *args) {
    ejsval callbackfn = _ejs_undefined;
    ejsval initialValue = _ejs_undefined;
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.reduceRight called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR, "TypedArray.prototype."
                                                    "reduceRight called on non "
                                                    "typed-array object");
...
static ejsval _ejs_TypedArray_prototype_reverse(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    /*  ValidateTypedArray is applied to the this value prior to evaluating the
     * algorithm. */
...
static ejsval _ejs_TypedArray_prototype_some(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    ejsval callbackfn = _ejs_undefined;
    ejsval thisArg = _ejs_undefined;
    ...
    if (EJSVAL_IS_NULL_OR_UNDEFINED(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.some called on null or undefined");

    /* This function is not generic. */
    if (!EJSVAL_IS_TYPEDARRAY(_this))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "TypedArray.prototype.some called on non typed-array object");
...
static ejsval _ejs_TypedArray_prototype_values(ejsval env, ejsval _this,
                                               uint32_t argc, ejsval *args) {
    /* 1. Let O be the this value. */
    ejsval O = _this;
...
static ejsval _ejs_TypedArray_prototype_toString(ejsval env, ejsval _this,
                                                 uint32_t argc, ejsval *args) {
...
ejsval _ejs_typedarray_new(EJSTypedArrayType element_type, uint32_t length) {
    ...
    _ejs_init_object((EJSObject *)rv, _ejs_typed_array_protos[element_type],
                     _ejs_typed_array_specops[element_type]);
...
ejsval _ejs_typedarray_new_from_array(EJSTypedArrayType element_type,
    ...
    for (i = 0; i < arrlen; i++) {
        ...
        switch (element_type) {
        case EJS_TYPEDARRAY_INT8:
            ((int8_t *)data)[i] = (int8_t)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_UINT8:
            ((uint8_t *)data)[i] = (uint8_t)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_UINT8CLAMPED:
            EJS_NOT_IMPLEMENTED();
        case EJS_TYPEDARRAY_INT16:
            ((int16_t *)data)[i] = (int16_t)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_UINT16:
            ((uint16_t *)data)[i] = (uint16_t)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_INT32:
            ((int32_t *)data)[i] = (int32_t)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_UINT32:
            ((uint32_t *)data)[i] = (uint32_t)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_FLOAT32:
            ((float *)data)[i] = (float)EJSVAL_TO_NUMBER(item);
            break;
        case EJS_TYPEDARRAY_FLOAT64:
            ((double *)data)[i] = (double)EJSVAL_TO_NUMBER(item);
            break;
        default:
            EJS_NOT_REACHED();
        }
...
ejsval _ejs_typedarray_get_at(EJSObject *array, int index) {
    void *data = _ejs_typedarray_get_data(array);

    switch (((EJSTypedArray *)array)->element_type) {
    case EJS_TYPEDARRAY_INT8:
        return NUMBER_TO_EJSVAL(((int8_t *)data)[index]);
    case EJS_TYPEDARRAY_UINT8:
        return NUMBER_TO_EJSVAL(((uint8_t *)data)[index]);
    case EJS_TYPEDARRAY_UINT8CLAMPED:
        EJS_NOT_IMPLEMENTED();
    case EJS_TYPEDARRAY_INT16:
        return NUMBER_TO_EJSVAL(((int16_t *)data)[index]);
    case EJS_TYPEDARRAY_UINT16:
        return NUMBER_TO_EJSVAL(((uint16_t *)data)[index]);
    case EJS_TYPEDARRAY_INT32:
        return NUMBER_TO_EJSVAL(((int32_t *)data)[index]);
    case EJS_TYPEDARRAY_UINT32:
        return NUMBER_TO_EJSVAL(((uint32_t *)data)[index]);
    case EJS_TYPEDARRAY_FLOAT32:
        return NUMBER_TO_EJSVAL(((float *)data)[index]);
    case EJS_TYPEDARRAY_FLOAT64:
        return NUMBER_TO_EJSVAL(((double *)data)[index]);
    default:
        EJS_NOT_REACHED();
    }
}

void _ejs_typedarray_set_at(EJSObject *array, int index, ejsval value) {
    void *data = _ejs_typedarray_get_data(array);

    switch (((EJSTypedArray *)array)->element_type) {
    case EJS_TYPEDARRAY_INT8:
        ((int8_t *)data)[index] = (int8_t)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_UINT8:
        ((uint8_t *)data)[index] = (uint8_t)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_UINT8CLAMPED:
        EJS_NOT_IMPLEMENTED();
    case EJS_TYPEDARRAY_INT16:
        ((int16_t *)data)[index] = (int16_t)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_UINT16:
        ((uint16_t *)data)[index] = (uint16_t)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_INT32:
        ((int32_t *)data)[index] = (int32_t)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_UINT32:
        ((uint32_t *)data)[index] = (uint32_t)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_FLOAT32:
        ((float *)data)[index] = (float)EJSVAL_TO_NUMBER(value);
        break;
    case EJS_TYPEDARRAY_FLOAT64:
        ((double *)data)[index] = (double)EJSVAL_TO_NUMBER(value);
        break;
    default:
        EJS_NOT_REACHED();
    }
...
void *_ejs_arraybuffer_get_data(EJSObject *buf) {
    EJSArrayBuffer *array_buffer = (EJSArrayBuffer *)buf;

    if (array_buffer->dependent) {
        return _ejs_arraybuffer_get_data(
                   EJSVAL_TO_OBJECT(array_buffer->data.dependent.buf)) +
               array_buffer->data.dependent.offset;
    }
...
void *_ejs_typedarray_get_data(EJSObject *arr) {
    ...
    void *buffer_data =
        _ejs_arraybuffer_get_data(EJSVAL_TO_OBJECT(typed_array->buffer));
...
void *_ejs_dataview_get_data(EJSObject *view) {
    ...
    void *buffer_data =
        _ejs_arraybuffer_get_data(EJSVAL_TO_OBJECT(data_view->buffer));
...
#define OBJ_METHOD_IMPL(t, x)                                                  \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_##t, x, _ejs_##t##_##x##_impl)
#define PROTO_METHOD(t, x)                                                     \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_##t##_prototype, x, _ejs_##t##_prototype_##x)
#define PROTO_METHOD_IMPL_GENERIC(t, x)                                        \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_##t##_prototype, x,                         \
                              _ejs_TypedArray_prototype_##x)
#define PROTO_METHOD_IMPL(t, x)                                                \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_##t##_prototype, x,                         \
                              _ejs_##t##_prototype_##x##_impl)
#define PROTO_GETTER(t, x)                                                     \
    EJS_INSTALL_SYMBOL_GETTER(_ejs_##t##_prototype, x,                         \
                              _ejs_##t##_prototype_get_##x)

    // ArrayBuffer
    {
        _ejs_ArrayBuffer = _ejs_function_new_without_proto(
            _ejs_null, _ejs_atom_ArrayBuffer,
            (EJSClosureFunc)_ejs_ArrayBuffer_impl);
        _ejs_object_setprop(global, _ejs_atom_ArrayBuffer, _ejs_ArrayBuffer);

        _ejs_gc_add_root(&_ejs_ArrayBuffer_prototype);
        _ejs_ArrayBuffer_prototype =
            _ejs_object_new(_ejs_null, &_ejs_Object_specops);
        _ejs_object_setprop(_ejs_ArrayBuffer, _ejs_atom_prototype,
                            _ejs_ArrayBuffer_prototype);
        ...
        _ejs_object_define_value_property(
            _ejs_ArrayBuffer_prototype, _ejs_Symbol_toStringTag,
            _ejs_atom_ArrayBuffer,
            EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_WRITABLE |
                EJS_PROP_CONFIGURABLE);
        EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_ArrayBuffer, create,
                                          _ejs_ArrayBuffer_create,
                                          EJS_PROP_NOT_ENUMERABLE);
        EJS_INSTALL_SYMBOL_GETTER(_ejs_ArrayBuffer, species,
                                  _ejs_ArrayBuffer_get_species);
    }

    // DataView
    {
        _ejs_DataView = _ejs_function_new_without_proto(
            _ejs_null, _ejs_atom_DataView, (EJSClosureFunc)_ejs_DataView_impl);
        _ejs_object_setprop(global, _ejs_atom_DataView, _ejs_DataView);

        _ejs_gc_add_root(&_ejs_DataView_prototype);
        _ejs_DataView_prototype =
            _ejs_object_new(_ejs_null, &_ejs_Object_specops);
        _ejs_object_setprop(_ejs_DataView, _ejs_atom_prototype,
                            _ejs_DataView_prototype);
        ...
        _ejs_object_define_value_property(
            _ejs_DataView_prototype, _ejs_Symbol_toStringTag,
            _ejs_atom_DataView,
            EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_WRITABLE |
                EJS_PROP_CONFIGURABLE);

        EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_DataView, create,
                                          _ejs_DataView_create,
                                          EJS_PROP_NOT_ENUMERABLE);
    }

#define ADD_TYPEDARRAY(EnumType, ArrayType, arraytype, elementSizeInBytes)     \
    EJS_MACRO_START                                                            \
    _ejs_##ArrayType##Array = _ejs_function_new_without_proto(                 \
        _ejs_null, _ejs_atom_##ArrayType##Array,                               \
        (EJSClosureFunc)_ejs_##ArrayType##Array_impl);                         \
    _ejs_object_setprop(global, _ejs_atom_##ArrayType##Array,                  \
                        _ejs_##ArrayType##Array);                              \
                                                                               \
    _ejs_gc_add_root(&_ejs_##ArrayType##Array_prototype);                      \
    _ejs_##ArrayType##Array_prototype =                                        \
        _ejs_object_new(_ejs_null, &_ejs_Object_specops);                      \
    _ejs_object_setprop(_ejs_##ArrayType##Array, _ejs_atom_prototype,          \
                        _ejs_##ArrayType##Array_prototype);                    \
                                                                               \
    /* make sure ctor.BYTES_PER_ELEMENT is defined */                          \
    _ejs_object_define_value_property(                                         \
        _ejs_##ArrayType##Array, _ejs_atom_BYTES_PER_ELEMENT,                  \
        NUMBER_TO_EJSVAL(elementSizeInBytes), EJS_PROP_FLAGS_ENUMERABLE);      \
                                                                               \
    _ejs_typed_array_elsizes[EJS_TYPEDARRAY_##EnumType] = elementSizeInBytes;  \
    _ejs_typed_array_protos[EJS_TYPEDARRAY_##EnumType] =                       \
        _ejs_##ArrayType##Array_prototype;                                     \
    _ejs_typed_array_specops[EJS_TYPEDARRAY_##EnumType] =                      \
        &_ejs_##ArrayType##Array_specops;                                      \
                                                                               \
    ...
    PROTO_METHOD_IMPL_GENERIC(ArrayType##Array, toString);                     \
                                                                               \
    PROTO_GETTER(ArrayType##Array, toStringTag); /* XXX needs to be            \
                                                    enumerable: false,         \
                                                    configurable: true */      \
                                                                               \
    /* Manually expand values() so we can use it for the iterator */           \
    ejsval _values = _ejs_function_new_native(                                 \
        _ejs_null, _ejs_atom_values,                                           \
        (EJSClosureFunc)_ejs_TypedArray_prototype_values);                     \
    _ejs_object_define_value_property(                                         \
        _ejs_##ArrayType##Array_prototype, _ejs_atom_values, _values,          \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE);  \
    _ejs_object_define_value_property(                                         \
        _ejs_##ArrayType##Array_prototype, _ejs_Symbol_iterator, _values,      \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE);  \
                                                                               \
    OBJ_METHOD_IMPL(ArrayType##Array, of);                                     \
                                                                               \
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_##ArrayType##Array, create,         \
                                      _ejs_##ArrayType##Array_create,          \
                                      EJS_PROP_NOT_ENUMERABLE);                \
    EJS_INSTALL_SYMBOL_GETTER(_ejs_##ArrayType##Array, species,                \
                              _ejs_##ArrayType##Array_get_species);            \
                                                                               \
    EJS_MACRO_END
...
static ejsval _ejs_arraybuffer_specop_get(ejsval obj, ejsval propertyName,
    ...
    // we also handle the length getter here
    if (EJSVAL_IS_STRING(propertyName) &&
        !ucs2_strcmp(_ejs_ucs2_byteLength,
                     EJSVAL_TO_FLAT_STRING(propertyName))) {
...
_ejs_arraybuffer_specop_get_own_property(ejsval obj, ejsval propertyName,
    ...
    // XXX we need to handle the length property here (see EJSArray's
    // get_own_property)

    return _ejs_Object_specops.GetOwnProperty(obj, propertyName, exc);
}

static EJSBool _ejs_arraybuffer_specop_set(ejsval obj, ejsval propertyName,
                                           ejsval val, ejsval receiver) {
    // check if propertyName is a uint32, or a string that we can convert to an
    // uint32
    int idx = -1;
    if (EJSVAL_IS_NUMBER(propertyName)) {
        double n = EJSVAL_TO_NUMBER(propertyName);
    ...
    if (idx != -1) {
        if (idx >= EJS_DENSE_ARRAY_ALLOC(obj)) {
            ...
            memmove(new_elements, EJS_DENSE_ARRAY_ELEMENTS(obj),
                    EJS_DENSE_ARRAY_ALLOC(obj) * sizeof(ejsval));
...
static EJSBool _ejs_arraybuffer_specop_has_property(ejsval obj,
                                                    ejsval propertyName) {
    // check if propertyName is a uint32, or a string that we can convert to an
    // uint32
    int idx = -1;
...
static EJSBool _ejs_arraybuffer_specop_delete(ejsval obj, ejsval propertyName,
                                              EJSBool flag) {
    // check if propertyName is a uint32, or a string that we can convert to an
    // uint32
    int idx = -1;
...
static void _ejs_arraybuffer_specop_scan(EJSObject *obj,
...
EJS_DEFINE_CLASS(ArrayBuffer,
                 ...
                 _ejs_arraybuffer_specop_get, _ejs_arraybuffer_specop_set,
                 _ejs_arraybuffer_specop_delete,
                 OP_INHERIT, // [[Enumerate]]
                 OP_INHERIT, // [[OwnPropertyKeys]]
                 _ejs_arraybuffer_specop_allocate,
                 _ejs_arraybuffer_specop_finalize, _ejs_arraybuffer_specop_scan)
...
static void _ejs_typedarray_specop_scan(EJSObject *obj,
                                        EJSValueFunc scan_func) {
    EJSTypedArray *arr = (EJSTypedArray *)obj;
...
static EJSBool _ejs_dataview_specop_set(ejsval obj, ejsval propertyName,
                                        ejsval val, ejsval receiver) {
    EJSBool is_index = EJS_FALSE;
    ejsval idx_val = ToNumber(propertyName);
    int idx;
...
static EJSBool _ejs_dataview_specop_has_property(ejsval obj,
                                                 ejsval propertyName) {
    // check if propertyName is a uint32, or a string that we can convert to an
    // uint32
    int idx = -1;
...
static EJSBool _ejs_dataview_specop_delete(ejsval obj, ejsval propertyName,
                                           EJSBool flag) {
    int idx = -1;
    if (EJSVAL_IS_NUMBER(propertyName)) {
        double n = EJSVAL_TO_NUMBER(propertyName);
...
static EJSBool
_ejs_dataview_specop_define_own_property(ejsval obj, ejsval propertyName,
                                         EJSPropertyDesc *propertyDescriptor,
                                         EJSBool flag) {
    return _ejs_Object_specops.DefineOwnProperty(obj, propertyName,
                                                 propertyDescriptor, flag);
}
...
EJS_DEFINE_CLASS(DataView,
                 ...
                 _ejs_dataview_specop_set, _ejs_dataview_specop_delete,
                 OP_INHERIT, // [[Enumerate]]
                 OP_INHERIT, // [[OwnPropertyKeys]]
