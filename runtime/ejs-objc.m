...
static ejsval _ejs_ObjcHandle_impl(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

ejsval _ejs_objc_handle_new(id handle) {
    if (handle == nil)
        _ejs_throw_nativeerror_utf8(
            EJS_ERROR, "_ejs_objc_handle_new passed a nil objc id");

    EJSObjcHandle *rv = _ejs_gc_new(EJSObjcHandle);
    _ejs_init_object((EJSObject *)rv, _ejs_ObjcHandle_proto,
                     &_ejs_objchandle_specops);
...
CKObject *create_objc_handle_object(id objc_id) {
...
static ejsval _ejs_CoffeeKitObject_impl(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}

static ejsval _ejs_CoffeeKitObject_setHandle(ejsval env, ejsval _this,
                                             uint32_t argc, ejsval *args) {
    CKObject *thisObject =
        [CKObject objectWithJSObject:EJSVAL_TO_OBJECT(_this)];

    SPEW(NSLog(@"made it to coffeekit_object_set_handle, argumentCount = %u",
               argc);)

    EJSObjcHandle *handleArg = (EJSObjcHandle *)EJSVAL_TO_OBJECT(args[0]);

    if (handleArg->handle == nil) {
        _ejs_throw_nativeerror_utf8(EJS_ERROR,
                                    "setHandle called with nil handle");
    }

    [thisObject
        definePropertyNS:@"_ck_handle"
...
static ejsval _ejs_CoffeeKitObject_prototype_toString(ejsval env, ejsval _this,
                                                      uint32_t argc,
                                                      ejsval *args) {
    CKObject *thisObject =
        [CKObject objectWithJSObject:EJSVAL_TO_OBJECT(_this)];
...
void _ejs_objc_init(ejsval global) {
    ...
    _ejs_ObjcHandle_proto =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_objchandle_specops);

    _ejs_ObjcHandle = _ejs_function_new_native(_ejs_null, _ejs_atom_ObjcHandle,
                                               _ejs_ObjcHandle_impl);

    _ejs_object_setprop(_ejs_ObjcHandle, _ejs_atom_prototype,
                        _ejs_ObjcHandle_proto);
    _ejs_object_setprop(_ejs_ObjcHandle_proto, _ejs_atom_constructor,
                        _ejs_ObjcHandle);
    ...
    _ejs_CoffeeKitObject_proto =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_coffeekitobject_specops);

    _ejs_CoffeeKitObject =
        _ejs_function_new_native(_ejs_null, _ejs_atom_PirouetteObject,
                                 (EJSClosureFunc)_ejs_CoffeeKitObject_impl);

    _ejs_object_setprop(_ejs_CoffeeKitObject, _ejs_atom_prototype,
                        _ejs_CoffeeKitObject_proto);
    _ejs_object_setprop(_ejs_CoffeeKitObject_proto, _ejs_atom_constructor,
                        _ejs_CoffeeKitObject);

    EJS_INSTALL_ATOM_FUNCTION(_ejs_CoffeeKitObject, setHandle,
                              _ejs_CoffeeKitObject_setHandle);
    EJS_INSTALL_ATOM_FUNCTION(_ejs_CoffeeKitObject_proto, toString,
                              _ejs_CoffeeKitObject_prototype_toString);

    _ejs_object_setprop(global, _ejs_atom_PirouetteObject,
                        _ejs_CoffeeKitObject);
}

static ejsval _ejs_objc_requireFramework(ejsval env, ejsval _this,
                                         uint32_t argc, ejsval *args) {
    // this needs to go away, replaced by a pragma that the compiler can react
    // to
    return _ejs_undefined;
}

static ejsval _ejs_objc_allocInstance(ejsval env, ejsval _this, uint32_t argc,
                                      ejsval *args) {
    if (argc == 0)
        return _ejs_null;
    ...
    if (!EJSVAL_IS_STRING(clsname))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "args0 for allocInstance must be a string");
...
static ejsval _ejs_objc_staticCall(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    const char *clsname_cstr =
        [[CKString stringWithJSString:EJSVAL_TO_STRING(args[0])] UTF8String];
    const char *selector_cstr =
        [[CKString stringWithJSString:EJSVAL_TO_STRING(args[1])] UTF8String];

    Class cls = objc_getClass(clsname_cstr);
    SEL sel = sel_getUid(selector_cstr);

    return OBJECT_TO_EJSVAL(
        [create_objc_handle_object(objc_msgSend(cls, sel)) jsObject]);
}

static ejsval _ejs_objc_getInstanceVariable(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    ...
    id handle =
        get_objc_id([CKObject objectWithJSObject:EJSVAL_TO_OBJECT(args[0])]);
    ...
    if (ivar_value)
        return OBJECT_TO_EJSVAL(
            [create_objc_handle_object(ivar_value) jsObject]);
    else
        return _ejs_null;
}

static ejsval _ejs_objc_setInstanceVariable(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    ...
    id value_handle =
        get_objc_id([[CKValue valueWithJSValue:args[2]] objectValue]);
...
static ejsval invokeSelectorFromJS(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args);

static ejsval _ejs_objc_selectorInvoker(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
    if (!EJSVAL_IS_STRING(args[0])) {
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "non-string passed to objc.selectorInvoker");
    }
    ...
    CKObject *func = [CKObject
        makeFunctionNS:[NSString stringWithFormat:@"__ejs_invoke_%s_from_js",
                                                  selname_utf8]
          withCallback:(EJSClosureFunc)invokeSelectorFromJS
              argCount:0];
...
static ejsval _ejs_objc_getTypeEncoding(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
    EJS_NOT_IMPLEMENTED();
}
...
- (id)initWithName:(NSString *)name_str
          selector:(SEL)selector
           typeSig:(const char *)type_str;
- (NSMethodSignature *)resolveSigWithClass:(Class)cls
                              staticMethod:(BOOL)static_method;
- (NSString *)name;
- (SEL)sel;
- (NSString *)sig;
...
- (NSMethodSignature *)resolveSigWithClass:(Class)cls
    ...
    //  NSLog (@"resolveSigWithClass %@ (static = %s)", _name, static_method ?
    //  "YES" : "NO");
    Method meth = static_method ? class_getClassMethod(cls, _sel)
                                : class_getInstanceMethod(cls, _sel);

    if (!meth) {
        NSString *str = [NSString
            stringWithFormat:@"failed to locate method `%s' on class %@",
                             sel_getName(_sel), cls];
        _ejs_throw_nativeerror_utf8(EJS_ERROR, [str UTF8String]);
    }

    //  NSLog (@"method type encoding = %s", method_getTypeEncoding (meth));

    _methodSig = [[NSMethodSignature
        signatureWithObjCTypes:method_getTypeEncoding(meth)] retain];
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<MethodInfo name=%@ sel=%s sig=%@>",
                                      _name, sel_getName(_sel), _sig];
}
@end

static const char *get_selector_name_from_function(CKObject *function) {
    return [function utf8StringForPropertyNS:@"_ck_sel"];
}

static const char *get_type_sig_from_function(CKObject *function) {
    return [function utf8StringForPropertyNS:@"_ck_typeSig"];
}

static const char *get_type_encoding_from_function(CKObject *function) {
    return [function utf8StringForPropertyNS:@"_ck_typeEncoding"];
}

static SEL get_selector_from_function(CKObject *function) {
    return sel_getUid(get_selector_name_from_function(function));
}

static bool is_function_exported(CKObject *func) {
    CKValue *propval = [func valueForPropertyNS:@"_ck_exported"];

    return [propval isBool] && [propval boolValue];
}

static CKObject *get_object_prototype_method(NSString *name) {
    EJS_NOT_IMPLEMENTED();
#if notyet
    CKObject *object_proto =
        [[[ctx globalObject] objectForPropertyNS:@"Object"] prototype];
    return [object_proto objectForPropertyNS:name];
#endif
}

static CKValue *marshal_id_as_jsvalue(id objc_id, BOOL protect) {
    if (objc_id == nil) {
    ...
    CKInvocation *inv =
        [CKInvocation invocationWithConstructor:ctor_obj argCount:1];

    [inv setArgument:handle_val atIndex:0];

    // and this will create a ref to the handle_val so that the lifetime of the
    // objc_id will be at least
    // as long as the lifetime of jspeer.
    CKValue *rv = [inv invoke];
    CKObject *jspeer = [rv objectValue];
    if (jspeer == NULL)
        [NSException raise:@"Exception marshaling id as jsvalue"
                    format:@"%@", [inv exception]];
...
static CKValue *marshal_cgrect_as_jsvalue(CGRect *rect) {
    ...
    CKObject *peer = [CKObject
        objectWithJSObject:EJSVAL_TO_OBJECT(_ejs_object_create(_ejs_null))];

    [peer defineProperty:x_
                   value:[CKValue numberValue:rect->origin.x]
              attributes:RO_DONT_ENUM_PERMANENT];

    [peer defineProperty:y_
                   value:[CKValue numberValue:rect->origin.y]
              attributes:RO_DONT_ENUM_PERMANENT];

    [peer defineProperty:width_
                   value:[CKValue numberValue:rect->size.width]
              attributes:RO_DONT_ENUM_PERMANENT];

    [peer defineProperty:height_
                   value:[CKValue numberValue:rect->size.height]
              attributes:RO_DONT_ENUM_PERMANENT];

    return [CKValue objectValue:peer];
}

static NSArray *marshal_jsarray_as_nsarray(CKObject *o) {
    ...
    for (i = 0; i < length; i++) {
        CKObject *el =
            [o objectForPropertyNS:[NSString stringWithFormat:@"%d", i]];
...
static ejsval invokeSelectorFromJS(ejsval env, ejsval _this, uint32_t argc,
    ...
    if (handle == nil) {
        const char *register_cstr =
            [thisObj utf8StringForPropertyNS:@"_ck_register"];
    ...
    if (!typeEncoding) {
        ...
        if (!typeEncoding) {
            NSString *str = [NSString
                stringWithFormat:@"failed to locate selector `%s'.  this is "
                                 @"usually due to a framework not being "
                                 @"linked against.",
                                 sel_getName(sel)];
            _ejs_throw_nativeerror_utf8(EJS_ERROR, [str UTF8String]);
        }
    ...
    if (handle == NULL) {
        _ejs_throw_nativeerror_utf8(EJS_ERROR,
                                    "NULL handle in invokeSelectorFromJS");
    }

    if (sel == NULL) {
        _ejs_throw_nativeerror_utf8(EJS_ERROR,
                                    "NULL selector in invokeSelectorFromJS");
    }
    ...
    if ((argc + 2 - 1 /* the -1 to get rid of the selector passed as arg0 */) !=
        [sig numberOfArguments]) {
        NSLog(@"Incorrect number of arguments to objective-C method %s on %@ "
              @"(type encoding %s).  expected = %lu, offered = %u",
              sel_getName(sel), handle, typeEncoding,
              (unsigned long)[sig numberOfArguments], argc + 2);
        return _ejs_null;
    }
    ...
    for (i = 0; i < argc - 1; i++) {
        ...
        case _C_ID: {
            ...
            if ([val isString]) {
                ...
                //            NSLog (@"arg %d: string marshalling of %@", i,
                //            arg_ptr);
            } else if ([val isNull] || [val isUndefined]) {
                arg_ptr = nil;
                //            NSLog (@"arg %d: object marshalling of %@", i,
                //            arg_ptr);
                ...
                if (arg_ptr == NULL) {
                    if ([o isArray]) {
                        // let's assume we marshal this as an NSArray for now...
                        // XXX
                ...
                NSLog(
                    @"arg %d: skipping @ marshaling arg (unhandled JS type %d)",
                    i, JSValueGetType(cx, JS_ARGV(cx, vp)[i]));
                NSLog(@"selector: %s", sel_getName(sel));
                int j;
                for (j = 0; j < argc; j++) {
                    NSLog(@"arg %d: JS type %d", j,
                          JSValueGetType(cx, JS_ARGV(cx, vp)[i]));
                }
#else
        ...
        case _C_SEL: {
            if ([val isObject] && [[val objectValue] isFunction]) {
                ...
                SPEW(NSLog(@"arg %d: marshaling selector %s", i,
...
#if notyet_spidermonkey
                NSLog(@"arg %d: skipping selector marshaling arg (unhandled JS "
                      @"type %d)",
                      i, JSValueGetType(cx, JS_ARGV(cx, vp)[i]));
#else
        ...
        case _C_FLT: {
            ...
            SPEW(NSLog(@"arg %d: marshaling float = %g", i,
                       v);)[inv setArgument:&v atIndex:objc_arg_num];
            break;
        }

        case _C_DBL: {
            double v = (double)[val numberValue];
            SPEW(NSLog(@"arg %d: marshaling double = %g", i,
                       v);)[inv setArgument:&v atIndex:objc_arg_num];
            break;
        }
        ...
        case _C_INT: {
            ...
            SPEW(NSLog(@"arg %d: marshaling int = %d", i,
                       v);)[inv setArgument:&v atIndex:objc_arg_num];
            break;
        }
        ...
        case _C_UINT: {
            ...
            SPEW(NSLog(@"arg %d: marshaling uint = %u", i,
                       v);)[inv setArgument:&v atIndex:objc_arg_num];
            break;
        }
        ...
        case _C_STRUCT_B: {
            // marshal by duck typing, not by the JS type, so we can accept
            // object literals
            ...
            if (!strncmp(&arg_type[1], "CGRect", strlen("CGRect")) ||
                ...
                NSLog(@"marshaling rectangle %g %g %g %g", rect->origin.x,
...
#if IOS
            else if (!strncmp(&arg_type[1], "UIEdgeInsets",
                              strlen("UIEdgeInsets"))) {

                CKObject *insetsobj = [val objectValue];

                UIEdgeInsets *insets =
                    (UIEdgeInsets *)malloc(sizeof(UIEdgeInsets));
                ...
                offset->horizontal =
                    [offsetobj floatForPropertyNS:@"horizontal"];
                ...
                NSLog(@"skipping marshaling arg %d (unhandled struct type %s)",
                      objc_arg_num, arg_type);
                abort();
            }
            break;
        }
        ...
        default:
            NSLog(@"skipping marshaling arg %d (unhandled objc type %s)",
                  objc_arg_num, arg_type);
            abort();
            break;
        }
    ...
    SPEW(NSLog(@" start objc invoke");)[inv invoke];
    ...
    case _C_ID: {
        SPEW(NSLog(@"@ return type, creating JS peer, return_buffer = %p, "
                   @"value = %p",
                   return_buffer, *(id *)return_buffer);)
    ...
    case _C_STRUCT_B: {
        if (!strncmp(&return_type[1], "CGRect", strlen("CGRect")) ||
            ...
            NSString *str =
                [NSString stringWithFormat:@"unhandled struct return type `%s'",
                                           return_type];
            _ejs_throw_nativeerror_utf8(EJS_ERROR, [str UTF8String]);
        }
        break;
    ...
    default: {
        NSString *str = [NSString
            stringWithFormat:@"unhandled return type `%s'", return_type];
        _ejs_throw_nativeerror_utf8(EJS_ERROR, [str UTF8String]);
    }
    }
...
static CKObject *get_js_peer(id obj, id *class_out, BOOL *static_method) {
...
static void *coffeekit_method_tramp(id obj, SEL sel, ...) {
    ...
    NSMutableDictionary *method_map =
        objc_getAssociatedObject(cls, JSMETHODMAP_KEY);
    if (method_map == NULL) {
    ...
    NSMethodSignature *sig =
        [meth_info resolveSigWithClass:cls staticMethod:static_method];

    SPEW(NSLog(@"method_map[%@] = %@, looking up on jspeer %@", sel_nsstr,
               name_nsstr, jspeer);)
    ...
    CKInvocation *inv = [CKInvocation invocationWithFunction:func
                                                    argCount:jsarg_count
                                                  thisObject:jspeer];
    ...
    for (jsarg_num = 0; jsarg_num < jsarg_count; jsarg_num++) {
        ...
        case _C_ID: {
            ...
            [inv setArgument:marshal_id_as_jsvalue(objc_id, YES)
                     atIndex:jsarg_num];
            break;
        }
        case _C_STRUCT_B: {
            if (!strncmp(&arg_type[1], "CGRect", strlen("CGRect")) ||
                ...
                [inv setArgument:marshal_cgrect_as_jsvalue(&rect)
                         atIndex:jsarg_num];
            } else {
                NSString *str = [NSString
                    stringWithFormat:
                        @"unhandled tramp arg marshalling for struct type `%s'",
                        arg_type];
                _ejs_throw_nativeerror_utf8(EJS_ERROR, [str UTF8String]);
            }
            break;
    ...
    if (rv == NULL) {
        ...
        NSLog(@"WTF");
        NSLog(@"WTF");
        [NSException raise:@"Exception calling JS method"
                    format:@"%@", [inv exception]];
    }
    ...
    // bail out early (and ignore the JS return value) if the return type is
    // void
    if (return_type[0] == _C_VOID)
        return NULL;
    ...
    case _C_ID: {
        if ([rv isString])
            return [[rv stringValue] nsString];
        return [rv isObject] ? get_objc_id([rv objectValue]) : NULL;
    }
    case _C_CHR: {
        return [rv isBool] ? (void *)(NSInteger)[rv boolValue]
                           : (void *)(NSInteger)[rv numberValue];
    }
    case _C_INT:
    case _C_LNG:
    case _C_LNG_LNG: {
        return [rv isNumber] ? (void *)(NSInteger)[rv numberValue]
                             : (void *)(NSInteger)0;
    }
    case _C_UINT:
    case _C_ULNG:
    case _C_ULNG_LNG: {
        return [rv isNumber] ? (void *)(NSUInteger)[rv numberValue]
                             : (void *)(NSUInteger)0;
    }
    case _C_CLASS: {
        ...
        if (!register_cstr) {
            NSLog(@"invalid or missing _ck_register on a object returned for "
                  @"where Class JS->objc marshalling was assumed");
            return NULL;
        }
...
static id coffeekit_ctor_tramp(id obj, SEL sel, ...) {
    ...
    CKInvocation *inv =
        [CKInvocation invocationWithConstructor:js_ctor argCount:1];
    ...
    CKValue *rv = [inv invoke];
    if (rv == NULL)
        [NSException raise:@"Exception creating new JS wrapper"
                    format:@"%@", [inv exception]];
    if (![rv isObject])
        [NSException
             raise:@"Constructor returned something other than an object"
            format:@"%@", [inv exception]];
...
static CKObject *register_members(Class cls, CKObject *obj,
                                  NSMutableDictionary *method_map) {
    CKObject *ctor = NULL;
    EJSObject *_obj = [obj jsObject];

    for (_EJSPropertyMapEntry *s = _obj->map->head_insert; s;
         s = s->next_insert) {
        if (_ejs_property_desc_has_getter(s->desc) ||
            ...
            if (_ejs_property_desc_has_getter(s->desc)) {
                ...
                CKObject *getter = [CKObject
                    objectWithJSObject:EJSVAL_TO_OBJECT(s->desc->getter)];
                ...
                if ([ck_ivar isString]) {
                    ...
                    if (NO == class_addIvar(cls, ivar_name, sizeof(id),
                                            log2(sizeof(id)), "@")) {
            ...
            if (!propobj || ![propobj isFunction]) {
                SPEW(NSLog(@"property %@ isn't a function, skipping for now",
                           name_nsstr);)
                continue;
            }
            ...
            if (NSOrderedSame == [name_nsstr compare:@"constructor"]) {
                ...
                class_addMethod(cls, sel_getUid("init"),
                                (IMP)coffeekit_ctor_tramp, "@@:");
            } else {
                if (!is_function_exported(propobj)) {
                    SPEW(NSLog(@"function %@ isn't exported, skipping",
                               name_nsstr);)
                    continue;
                }
                ...
                SPEW(NSLog(@"adding method %s/%s in class %s", sel_name,
                           type_sig, class_getName(cls));)

                MethodInfo *info = [[MethodInfo alloc] initWithName:name_nsstr
                                                           selector:sel
                                                            typeSig:type_sig];
                [method_map setObject:info
                               forKey:[NSString stringWithUTF8String:sel_name]];
                [info release];

                class_addMethod(cls, sel, (IMP)coffeekit_method_tramp,
                                type_sig);
            }
        }
    }
...
static Class register_js_class(CKObject *proto, const char *register_name,
    ...
    if (cls) {
        ...
        if (objc_getAssociatedObject(cls, MONKEYPATCHED_RETAIN_RELEASE) ==
            NULL) {
            //                      Method retain = class_getInstanceMethod
            //                      (cls, sel_getUid ("retain"));
            //                      Method release = class_getInstanceMethod
            //                      (cls, sel_getUid ("release"));

            objc_setAssociatedObject(cls, MONKEYPATCHED_RETAIN_RELEASE, (id)1,
                                     OBJC_ASSOCIATION_ASSIGN);

            //                      class_replaceMethod(cls,
            //                      sel_getUid("retainCount"),
            //                      (IMP)monkey_retainCount, "I@:");
            //                      class_replaceMethod(cls,
            //                      sel_getUid("retain"), (IMP)monkey_retain,
            //                      "@@:");
            //                      class_replaceMethod(cls,
            //                      sel_getUid("release"), (IMP)monkey_release,
            //                      "v@:");
        }
    ...
    SPEW(NSLog(@"register_name = %s, super_name = %s", register_name,
               super_register_name);)
    ...
    objc_setAssociatedObject(cls, JSMETHODMAP_KEY, method_map,
                             OBJC_ASSOCIATION_RETAIN);
    ...
    */
    ...
    SPEW(NSLog(@"associated ctor %p with class %s/%p", ctor, register_name,
               cls);)

    return cls;
}

static ejsval _ejs_objc_registerJSClass(ejsval env, ejsval _this, uint32_t argc,
                                        ejsval *args) {
...
#if IOS
static ejsval _ejs_objc_UIApplicationMain(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
    NSString *delegate_name =
        [[CKString stringWithJSString:EJSVAL_TO_STRING(args[2])] nsString];

    NSLog(@"About to call UIApplicationMain (..., %@)!", delegate_name);
    UIApplicationMain(0, NULL, nil,
                      delegate_name); // XXX get argv/argc/principal from @args
    exit(0);
}

#else
static ejsval _ejs_objc_NSApplicationMain(ejsval env, ejsval _this,
                                          uint32_t argc, ejsval *args) {
...
ejsval _ejs_objc_module_func(ejsval exports) {
    EJS_INSTALL_FUNCTION(exports, "requireFramework",
                         _ejs_objc_requireFramework);
    ...
    EJS_INSTALL_FUNCTION(exports, "getInstanceVariable",
                         _ejs_objc_getInstanceVariable);
    EJS_INSTALL_FUNCTION(exports, "setInstanceVariable",
                         _ejs_objc_setInstanceVariable);
...
#if IOS
    EJS_INSTALL_FUNCTION(exports, "allocateWebGLRenderingContext",
                         _ejs_objc_allocateWebGLRenderingContext);
    EJS_INSTALL_FUNCTION(exports, "UIApplicationMain",
                         _ejs_objc_UIApplicationMain);
#else
    EJS_INSTALL_FUNCTION(exports, "NSApplicationMain",
                         _ejs_objc_NSApplicationMain);
#endif
