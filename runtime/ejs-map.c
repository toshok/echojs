...
ejsval _ejs_map_new() {
    ...
    _ejs_init_object((EJSObject *)map, _ejs_Object_prototype,
                     &_ejs_Map_specops);
...
// ES6: 23.1.3.1
// Map.prototype.clear ()
ejsval _ejs_Map_prototype_clear(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.clear called with non-object this.");

    // 3. If M does not have a [[MapData]] internal slot throw a TypeError
    // exception.
    // 4. If M’s [[MapData]] internal slot is undefined, then throw a TypeError
    // exception.

    // 5. Let entries be the List that is the value of M’s [[MapData]] internal
    // slot.
...
ejsval _ejs_map_delete(ejsval map, ejsval key) {
    ...
    // 5. If M’s [[MapComparator]] internal slot is undefined, then let same be
    // the abstract operation SameValueZero.
    // 6. Else, let same be the abstract operation SameValue.
    // 7. Let entries be the List that is the value of M’s [[MapData]] internal
    // slot.
...
ejsval _ejs_Map_prototype_delete(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Map.prototype.delete called with non-object this.");

    // 3. If M does not have a [[MapData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_MAP(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.delete called with non-Map this.");
...
ejsval _ejs_Map_prototype_entries(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    // 1. Let M be the this value.
    ejsval M = _this;
    ...
    if (!EJSVAL_IS_OBJECT(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.get called with non-object this.");

    // 3. Return the result of calling the CreateMapIterator abstract operation
    // with arguments M and "key+value".
    return _ejs_map_iterator_new(M, EJS_MAP_ITER_KIND_KEYVALUE);
}

// ES6: 23.1.3.5
// Map.prototype.forEach ( callbackfn , thisArg = undefined )
ejsval _ejs_Map_prototype_forEach(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Map.prototype.forEach called with non-object this.");

    // 3. If M does not have a [[MapData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_MAP(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.forEach called with non-Map this.");
    ...
    if (!EJSVAL_IS_CALLABLE(callbackfn))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "Map.prototype.forEach callbackfn isn't a function.");
    ...
    // 7. Let entries be the List that is the value of M’s [[MapData]] internal
    // slot.
    // 8. Repeat for each Record {[[key]], [[value]]} e that is an element of
    // entries, in original key insertion order
    //    a. If e.[[key]] is not empty, then
    //       i. Let funcResult be the result of calling the [[Call]] internal
...
ejsval _ejs_Map_prototype_get(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ejsval key = _ejs_undefined;
    if (argc > 0)
        key = args[0];
    ...
    if (!EJSVAL_IS_MAP(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.get called with non-Map this.");
...
ejsval _ejs_Map_prototype_has(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ejsval key = _ejs_undefined;
    if (argc > 0)
        key = args[0];
    ...
    if (!EJSVAL_IS_OBJECT(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.has called with non-object this.");

    // 3. If M does not have a [[MapData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_MAP(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.has called with non-Map this.");
...
// ES6: 23.1.3.8
// Map.prototype.keys ( )
ejsval _ejs_Map_prototype_keys(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    // 1. Let M be the this value.
    ejsval M = _this;

    // 2. Return the result of calling the CreateMapIterator abstract operation
    // with arguments M and "key".
...
ejsval _ejs_Map_prototype_set(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ejsval key = _ejs_undefined;
    ...
    if (!EJSVAL_IS_OBJECT(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.set called with non-object this.");

    // 3. If M does not have a [[MapData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_MAP(M))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map.prototype.set called with non-Map this.");
...
static ejsval _ejs_Map_prototype_get_size(ejsval env, ejsval _this,
...
ejsval _ejs_Map_prototype_values(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    // 1. Let M be the this value.
    ejsval M = _this;

    // 2. Return the result of calling the CreateMapIterator abstract operation
    // with arguments M and "value".
    return _ejs_map_iterator_new(M, EJS_MAP_ITER_KIND_VALUE);
}

// ES6: 23.1.1.1
// Map (iterable = undefined , comparator = undefined )
static ejsval _ejs_Map_impl(ejsval env, ejsval _this, uint32_t argc,
                            ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(map))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map constructor called with non-object this.");

    // 3. If map does not have a [[MapData]] internal slot, then throw a
    // TypeError exception.
    if (!EJSVAL_IS_MAP(map))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "Map constructor called with non-Map this.");
    ...
    if (!EJSVAL_IS_UNDEFINED(comparator)) {
        ...
        if (!EJSVAL_TO_BOOLEAN(_ejs_op_strict_eq(comparator, _ejs_atom_is)))
            _ejs_throw_nativeerror_utf8(
                EJS_RANGE_ERROR, "Map constructor argument 'comparator' must "
                                 "be either undefined or the string 'is'");
    }
    EJSMap *_map = EJSVAL_TO_MAP(map);

    // 9. If the value of map’s [[MapData]] internal slot is not undefined, then
...
static ejsval _ejs_Map_get_species(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    return _ejs_Map;
}

static ejsval _ejs_Map_create(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in Map[Symbol.create] is not a constructor");
...
ejsval _ejs_map_iterator_new(ejsval map, EJSMapIteratorKind kind) {
    ...
    _ejs_init_object((EJSObject *)iterator, _ejs_MapIterator_prototype,
                     &_ejs_MapIterator_specops);
...
static ejsval _ejs_MapIterator_impl(ejsval env, ejsval _this, uint32_t argc,
                                    ejsval *args) {
    return _this;
}

static ejsval _ejs_MapIterator_prototype_next(ejsval env, ejsval _this,
                                              uint32_t argc, ejsval *args) {
    ...
    /* 6. Let itemKind be the value of the [[MapIterationKind]] internal slot of
     * O. */
    ...
    /* 8. Assert: m has a [[MapData]] internal slot and m has been initialized
     * so the value of
     * [[MapData]] is not undefined. */

    /* 9. Let entries be the List that is the value of the [[MapData]] internal
     * slot of m. */
    EJSKeyValueEntry *entries = EJSVAL_TO_MAP(m)->head_insert;
    ...
    for (EJSKeyValueEntry *entry = entries; entry; entry = entry->next_insert) {
        ...
        /*  iii. Else, */
            ...
            /* 3. Assert: result is a new, well-formed Array object so the
...
void _ejs_map_init(ejsval global) {
    _ejs_Map = _ejs_function_new_without_proto(_ejs_null, _ejs_atom_Map,
                                               (EJSClosureFunc)_ejs_Map_impl);
...
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_Map_prototype, x, _ejs_Map_prototype_##x,                         \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE)
#define PROTO_GETTER(x)                                                        \
    EJS_INSTALL_ATOM_GETTER(_ejs_Map_prototype, x, _ejs_Map_prototype_get_##x)
    ...
    // expand PROTO_METHOD(entries) here so we can install the function for
    // @@iterator below
    ejsval _entries =
        _ejs_function_new_native(_ejs_null, _ejs_atom_entries,
                                 (EJSClosureFunc)_ejs_Map_prototype_entries);
    _ejs_object_define_value_property(
        _ejs_Map_prototype, _ejs_atom_entries, _entries,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_FLAGS_WRITABLE |
            EJS_PROP_FLAGS_CONFIGURABLE);

    _ejs_object_define_value_property(
        _ejs_Map_prototype, _ejs_Symbol_iterator, _entries,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_FLAGS_WRITABLE |
            EJS_PROP_FLAGS_CONFIGURABLE);
    _ejs_object_define_value_property(
        _ejs_Map_prototype, _ejs_Symbol_toStringTag, _ejs_atom_Map,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_WRITABLE |
            EJS_PROP_CONFIGURABLE);

    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_Map, create, _ejs_Map_create,
...
#undef PROTO_METHOD
    ...
    _ejs_MapIterator = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Map, (EJSClosureFunc)_ejs_MapIterator_impl);

    _ejs_gc_add_root(&_ejs_MapIterator_prototype);
    _ejs_MapIterator_prototype =
        _ejs_map_iterator_new(_ejs_Map_prototype, EJS_MAP_ITER_KIND_VALUE);
    EJSVAL_TO_OBJECT(_ejs_MapIterator_prototype)->proto = _ejs_Object_prototype;
    _ejs_object_define_value_property(
        _ejs_MapIterator, _ejs_atom_prototype, _ejs_MapIterator_prototype,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE |
            EJS_PROP_NOT_WRITABLE);
    _ejs_object_define_value_property(
        _ejs_MapIterator_prototype, _ejs_atom_constructor, _ejs_MapIterator,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);

#define PROTO_ITER_METHOD(x)                                                   \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_MapIterator_prototype, x, _ejs_MapIterator_prototype_##x,         \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE)
...
EJS_DEFINE_CLASS(Map,
                 ...
                 _ejs_map_specop_allocate, _ejs_map_specop_finalize,
                 _ejs_map_specop_scan)

static void _ejs_map_iterator_specop_scan(EJSObject *obj,
