...
// arenas are reserved in ARENA_PAGES * PAGE_SIZE chunks.  ARENA_PAGES=2048
// gives us an arena size of 8MB
...
#define PTR_TO_ARENA_PAGE_BASE(ptr)                                            \
    ((void *)ALIGN(PTR_TO_ARENA(ptr) + sizeof(Arena), PAGE_SIZE))
#define PTR_TO_ARENA_PAGE_INDEX(ptr)                                           \
    ((((uintptr_t)(ptr) & ~PTR_TO_ARENA_MASK) -                                \
      ((uintptr_t)PTR_TO_ARENA_PAGE_BASE(ptr) & ~PTR_TO_ARENA_MASK)) /         \
     PAGE_SIZE)

#define PTR_TO_CELL(ptr, info)                                                 \
    (((char *)(ptr) - (char *)(info)->page_start) / (info)->cell_size)
...
static void _ejs_gc_worklist_init() {
    work_list.list = NULL;
    work_list.free_list = NULL;
}

static void _ejs_gc_worklist_push(GCObjectPtr obj) {
    if (obj == NULL)
        return;

    WorkListSegment *segment;

    if (EJS_UNLIKELY(!work_list.list ||
                     work_list.list->size == MAX_WORKLIST_SEGMENT_SIZE)) {
...
static GCObjectPtr _ejs_gc_worklist_pop() {
    if (work_list.list == NULL ||
        work_list.list->size ==
            0 /* shouldn't happen, since we push the page to the free list if we hit 0 */)
        return NULL;
...
#define WORKLIST_PUSH_AND_GRAY(x)                                              \
    EJS_MACRO_START                                                            \
...
#define WORKLIST_PUSH_AND_GRAY_CELL(x, cell)                                   \
    EJS_MACRO_START                                                            \
    if (IS_WHITE(cell)) {                                                      \
        _ejs_gc_worklist_push((GCObjectPtr)(x));                               \
        SET_GRAY(cell);                                                        \
...
static void *alloc_from_os(size_t size, size_t align) {
    if (align == 0) {
        size = MAX(size, PAGE_SIZE);
        void *res = mmap(NULL, size, PROT_READ | PROT_WRITE,
                         MAP_ANON | MAP_PRIVATE, MAP_FD, 0);
        SPEW(2, _ejs_log("mmap for 0 alignment = %p\n", res));
        return res == MAP_FAILED ? NULL : res;
    }

    void *res = mmap(NULL, size * 2, PROT_READ | PROT_WRITE,
                     MAP_ANON | MAP_PRIVATE, MAP_FD, 0);
    if (res == MAP_FAILED) {
        return NULL;
    }
    ...
    if (((uintptr_t)res % align) == 0) {
        ...
        // the area after
        munmap(aligned_res + size,
               (uintptr_t)res + size * 2 - (uintptr_t)(aligned_res + size));
        res = aligned_res;
...
#define SET_GRAY(cell)                                                         \
    EJS_MACRO_START                                                            \
    BitmapCell _bc;                                                            \
    do {                                                                       \
        _bc = (cell);                                                          \
    } while (!__sync_bool_compare_and_swap(                                    \
                 &cell, _bc, (_bc & ~CELL_COLOR_MASK) | CELL_GRAY_MASK));      \
    EJS_MACRO_END

#define SET_WHITE(cell)                                                        \
    EJS_MACRO_START                                                            \
    BitmapCell _bc;                                                            \
    do {                                                                       \
        _bc = (cell);                                                          \
    } while (!__sync_bool_compare_and_swap(                                    \
                 &cell, _bc, (_bc & ~CELL_COLOR_MASK) | white_mask));          \
    EJS_MACRO_END

#define SET_BLACK(cell)                                                        \
    EJS_MACRO_START                                                            \
    BitmapCell _bc;                                                            \
    do {                                                                       \
        _bc = (cell);                                                          \
    } while (!__sync_bool_compare_and_swap(                                    \
                 &cell, _bc, (_bc & ~CELL_COLOR_MASK) | black_mask));          \
    EJS_MACRO_END

#define SET_FREE(cell)                                                         \
    EJS_MACRO_START                                                            \
    BitmapCell _bc;                                                            \
    do {                                                                       \
        _bc = (cell);                                                          \
...
#define OBJECT_SIZE_LOW_LIMIT_BITS                                             \
    4 // smallest object we'll allocate (1<<4 = 16)
#define OBJECT_SIZE_HIGH_LIMIT_BITS                                            \
    11 // max object size for the non-LOS allocator = 2048

#define HEAP_PAGELISTS_COUNT                                                   \
    (OBJECT_SIZE_HIGH_LIMIT_BITS - OBJECT_SIZE_LOW_LIMIT_BITS) +               \
        1 // +1 because we're inclusive on 11
...
uintptr_t ptr_to_arena_page_index(void *ptr) {
    return PTR_TO_ARENA_PAGE_INDEX(ptr);
}
uintptr_t ptr_to_cell(void *ptr, PageInfo *info) {
    return PTR_TO_CELL(ptr, info);
}

#if sanity
static void verify_arena(Arena *arena) {
...
static Arena *arena_new() {
    ...
    if (insert_point == -1)
        insert_point = num_arenas;
    if (num_arenas - insert_point > 0)
        memmove(&heap_arenas[insert_point + 1], &heap_arenas[insert_point],
                (num_arenas - insert_point) * sizeof(Arena *));
...
static PageInfo *alloc_page_info_from_arena(Arena *arena, void *page_data,
                                            size_t cell_size) {
    // FIXME allocate the PageInfo and bitmap from the arena as well
    PageInfo *info = (PageInfo *)calloc(
        1, sizeof(PageInfo) + (sizeof(BitmapCell) * PAGE_SIZE /
                               (1 << OBJECT_SIZE_LOW_LIMIT_BITS)));
    EJS_LIST_INIT(info);
    ...
    // allocate a bitmap large enough to store any sized object so we can reuse
    // the bitmap
...
static PageInfo *alloc_page_from_arena(Arena *arena, size_t cell_size) {
    ...
    if (arena->free_pages) {
        ...
        info->num_free_cells = info->num_cells;
        info->bump_ptr = info->page_start;
        memset(info->page_bitmap, CELL_FREE,
               info->num_cells * sizeof(BitmapCell));
        SPEW(3, _ejs_log("alloc_page_from_arena from free pages for cell size "
                         "%zd = %p\n",
                         info->cell_size, info));
        return info;
    } else if (page_data < arena->end) {
        PageInfo *info =
            alloc_page_info_from_arena(arena, page_data, cell_size);
        int page_idx = arena->num_pages++;
        arena->pos = page_data + PAGE_SIZE;
        arena->pages[page_idx] = page_data;
        arena->page_infos[page_idx] = info;
        SPEW(3, _ejs_log("alloc_page_from_arena from bump pointer for cell "
                         "size %zd = %p\n",
                         info->cell_size, info));
        return info;
    } else {
...
static int compare_ptrs(const void *v1, const void *v2) {
    Arena **a1 = (Arena **)v1;
    Arena **a2 = (Arena **)v2;
...
static Arena *find_arena(GCObjectPtr ptr) {
...
static Arena *find_arena_in_array(GCObjectPtr ptr, Arena **array, int length) {
    void *arena_ptr = PTR_TO_ARENA(ptr);
    Arena **bsearch_rv = (Arena **)bsearch(&arena_ptr, array, length,
                                           sizeof(Arena *), compare_ptrs);
    return bsearch_rv ? *bsearch_rv : NULL;
}

static PageInfo *find_page_and_cell_from_arena(GCObjectPtr ptr,
                                               uint32_t *cell_idx,
                                               Arena *arena) {
...
static void set_black(GCObjectPtr ptr) {
    uint32_t cell_idx;
    PageInfo *page = find_page_and_cell(ptr, &cell_idx);
    if (!page)
        return;
...
static EJSBool is_white(GCObjectPtr ptr) {
    uint32_t cell_idx;
    PageInfo *page = find_page_and_cell(ptr, &cell_idx);
    if (!page)
        return EJS_FALSE;
...
static PageInfo *alloc_new_page(size_t cell_size) {
    ...
    // need a new arena
    SPEW(2, _ejs_log(
                "unable to find page in current arenas, allocating a new one"));
    LOCK_ARENAS();
...
static void _ejs_finalize_obj(GCObjectPtr ptr, Arena *arena, PageInfo *info,
                              uint32_t cell_idx) {
    EJS_ASSERT(info);
    ...
    if (info->num_free_cells == info->num_cells) {
        ...
        if (info->num_free_cells == info->num_cells) {
            if (info->los_info) {
                ...
                EJS_ASSERT(arena);
                SPEW(2,
                     _ejs_log("page %p is empty, putting it on the free list\n",
                              info));
                LOCK_PAGE(info);
...
void _ejs_gc_allocate_oom_exceptions() {
    _ejs_gc_add_root(&los_allocation_failed_exc);
    los_allocation_failed_exc =
        _ejs_nativeerror_new_utf8(EJS_ERROR, "LOS allocation failed");

    _ejs_gc_add_root(&page_allocation_failed_exc);
    page_allocation_failed_exc =
        _ejs_nativeerror_new_utf8(EJS_ERROR, "page allocation failed");
}

static void _scan_ejsvalue(ejsval val) {
    if (!EJSVAL_IS_TRACEABLE_IMPL(val))
        return;
...
static void _scan_from_ejsobject(EJSObject *obj) {
    OP(obj, Scan)(obj, _scan_ejsvalue);
}

static void _scan_from_ejsprimstr(EJSPrimString *primStr) {
    EJSPrimStringType strtype = EJS_PRIMSTR_GET_TYPE(primStr);

    switch (strtype) {
    case EJS_STRING_ROPE:
        // inline _scan_ejsvalue's push logic here to save creating an ejsval
        // from the primStr only to destruct
...
static void _scan_from_ejsclosureenv(EJSClosureEnv *env) {
...
void _ejs_gc_mark_thread_stack_bottom(GCObjectPtr *btm) { stack_bottom = btm; }

static void mark_pointers_in_range(GCObjectPtr *low, GCObjectPtr *high) {
    GCObjectPtr *p;
    for (p = low; p < high - 1; p++) {
        GCObjectPtr gcptr;

#if OSX
        // really a 64 bit check here, since for 64 bit systems, ejsvals can be
        // stuck in registers, so we need to check if it's a valid
        ...
        if (!page)
            continue; // skip values outside our heap.

        // XXX more checks before we start treating the pointer like a
        // GCObjectPtr?
        BitmapCell cell = page->page_bitmap[cell_idx];
        if (IS_FREE(cell))
            continue; // skip free cells
        if (!IS_WHITE(cell))
            continue; // skip pointers to gray/black cells

        WORKLIST_PUSH_AND_GRAY_CELL(gcptr, page->page_bitmap[cell_idx]);
    }
}

static void mark_ejsvals_in_range(void *low, void *high) {
    void *p = low;
#if IOS
    ...
    for (; p < high - sizeof(ejsval); p += sizeof(ejsval)) {
        ...
        if (EJSVAL_IS_GCTHING_IMPL(candidate_val)) {
            GCObjectPtr gcptr =
                (GCObjectPtr)EJSVAL_TO_GCTHING_IMPL(candidate_val);
            ...
            if (page) {
                ...
                if (EJSVAL_IS_STRING(candidate_val)) {
                    ...
                    WORKLIST_PUSH_AND_GRAY_CELL(gcptr,
                                                page->page_bitmap[cell_idx]);
                } else {
                    // SPEW(_ejs_log ("found ptr to %p(%s) on stack\n",
                    // EJSVAL_TO_OBJECT(candidate_val),
                    // CLASSNAME(EJSVAL_TO_OBJECT(candidate_val))));
                    WORKLIST_PUSH_AND_GRAY_CELL(gcptr,
                                                page->page_bitmap[cell_idx]);
                }
            }
            ...
            if (info->num_free_cells == info->num_cells) {
                ...
                for (int c = 0, ce = info->num_cells; c < ce; c++) {
                    ...
                    if (IS_WHITE(cell)) {
                        ...
                        GCObjectPtr gcobj = (GCObjectPtr)(info->page_start +
                                                          c * info->cell_size);
...
#define MARK_REGISTERS                                                         \
    EJS_MACRO_START                                                            \
    GCObjectPtr __r0, __r1, __r2, __r3, __r4, __r5, __r6, __r7, __r8, __r9,    \
        __r10, __r11, __r12;                                                   \
    __asm("str r0, %0; str r1, %1; str r2, %2; str r3, %3; str r4, %4; str "   \
          "r5, %5; str r6, %6;"                                                \
          "str r7, %7; str r8, %8; str r9, %9; str r10, %10; str r11, %11; "   \
          "str r12, %12;"                                                      \
...
#define MARK_REGISTERS                                                         \
    EJS_MACRO_START                                                            \
    GCObjectPtr __rax, __rbx, __rcx, __rdx, __rsi, __rdi, __rbp, __rsp, __r8,  \
        __r9, __r10, __r11, __r12, __r13, __r14, __r15;                        \
    __asm("movq %%rax, %0; movq %%rbx, %1; movq %%rcx, %2; movq %%rdx, %3; "   \
          "movq %%rsi, %4;"                                                    \
          "movq %%rdi, %5; movq %%rbp, %6; movq %%rsp, %7; movq %%r8, %8;  "   \
          "movq %%r9, %9;"                                                     \
          "movq %%r10, %10; movq %%r11, %11; movq %%r12, %12; movq %%r13, "    \
          "%13; movq %%r14, %14; movq %%r15, %15;"                             \
          : "=m"(__rax), "=m"(__rbx), "=m"(__rcx), "=m"(__rdx), "=m"(__rsi),   \
            "=m"(__rdi), "=m"(__rbp), "=m"(__rsp), "=m"(__r8), "=m"(__r9),     \
            "=m"(__r10), "=m"(__r11), "=m"(__r12), "=m"(__r13), "=m"(__r14),   \
            "=m"(__r15));                                                      \
                                                                               \
...
static void mark_thread_stack() {
    MARK_REGISTERS;

    GCObjectPtr stack_top = NULL;

    mark_ejsvals_in_range(((void *)&stack_top) + sizeof(GCObjectPtr),
                          stack_bottom);
}
...
void _ejs_gc_push_generator(EJSGenerator *gen) {
    generators[generator_count++] = gen;
}

void _ejs_gc_pop_generator() { generator_count--; }

static void mark_generator_stacks() {
...
static void _ejs_gc_collect_inner(EJSBool shutting_down) {
#if gc_timings > 1
    ...
    if (shutting_down) {
        ...
        SPEW(1, _ejs_log("final gc page statistics:\n");
             for (int hp = 0; hp < HEAP_PAGELISTS_COUNT; hp++) {
                 ...
                 EJS_LIST_FOREACH(&heap_pages[hp], PageInfo, page, { len++; });
...
static GCObjectPtr alloc_from_page(PageInfo *info) {
    ...
    if (info->bump_ptr) {
        ...
        // check if we can service the next alloc request from the bump_ptr.  if
        // we can't, switch
    ...
    SPEW(2, _ejs_log("allocated obj %p from page %p (cell size %zd), free "
                     "cells remaining %zd\n",
...
static GCObjectPtr alloc_from_los(size_t size, EJSScanType scan_type) {
    // allocate enough space for the object, our header, and our bitmap.  leave
    // room enough to align the return value
    LargeObjectInfo *rv = alloc_from_os(size + sizeof(LargeObjectInfo) + 16, 0);
    if (rv == NULL)
        return NULL;

    rv->page_info.page_bitmap =
        (char *)((void *)rv + sizeof(LargeObjectInfo)); // our bitmap comes
                                                        // right after the
                                                        // header
    rv->page_info.page_start =
        (void *)ALIGN((void *)rv + sizeof(LargeObjectInfo) + 8, 8);
...
GCObjectPtr _ejs_gc_alloc(size_t size, EJSScanType scan_type) {
    GCObjectPtr rv = NULL;
    ...
    switch (scan_type) {
    case EJS_SCAN_TYPE_PRIMSTR:
        num_primstr_allocs++;
        break;
    case EJS_SCAN_TYPE_OBJECT:
        num_object_allocs++;
        break;
    case EJS_SCAN_TYPE_CLOSUREENV:
        num_closureenv_allocs++;
        break;
    }
    ...
    if (!gc_disabled &&
        ((num_allocs == 400000 ||
          (alloc_size - alloc_size_at_last_gc) >= 60 * 1024 * 1024) ||
         (collect_every_alloc && collect_every_alloc == num_allocs))) {
        //        if (num_allocs == 400000) _ejs_log ("collecting due to
        //        num_allocs == %d, alloc_size = %d, alloc_size size last gc =
        //        %d\n", num_allocs, alloc_size, alloc_size -
        //        alloc_size_at_last_gc);
        //        if (alloc_size - alloc_size_at_last_gc >= 40*1024*1024)
        //        _ejs_log ("collecting due to allocs since last gc\n");
        _ejs_gc_collect();
...
retry_allocation : {
    ...
    *((GCObjectHeader *)rv) = scan_type;
    ...
    if (info->num_free_cells == 0) {
        // if the page is full, bump it to the end of the list (if there's more
        // than 1 page in the list)
...
void _ejs_gc_add_root(ejsval *root) {
...
void _ejs_gc_remove_root(ejsval *root) {
...
void _ejs_gc_mark_conservative_range(void *low, void *high) {
    mark_ejsvals_in_range(low, high);
}

static int page_list_count(PageInfo *page) {
    int count = 0;
    while (page) {
        count++;
        page = page->next;
...
void _ejs_gc_dump_heap_stats() {
    ...
    for (int i = 0; i < HEAP_PAGELISTS_COUNT; i++) {
        ...
        _ejs_log("heap_pages[%d, size %d] : %d pages\n", i,
                 1 << (i + OBJECT_SIZE_LOW_LIMIT_BITS),
                 _ejs_list_length(&heap_pages[i]));
#if gc_timings > 3
        EJS_LIST_FOREACH(&heap_pages[i], PageInfo, page, {
            GCObjectPtr p = page->page_start;
            for (int c = 0; c < CELLS_IN_PAGE(page);
                 c++, p += page->cell_size) {
                if (IS_FREE(page->page_bitmap[c]))
                    continue;
                GCObjectHeader *headerp = (GCObjectHeader *)p;
                if ((*headerp & EJS_SCAN_TYPE_OBJECT) != 0)
                    ...
                    _ejs_log(((*headerp >> EJS_GC_USER_FLAGS_SHIFT) & 0x10) != 0
                                 ? "s"
                                 : "S");
    ...
    if (los_list) {
        ...
        for (LargeObjectInfo *lobj = los_list; lobj; lobj = lobj->next) {
            GCObjectHeader *headerp =
                (GCObjectHeader *)lobj->page_info.page_start;
            if ((*headerp & EJS_SCAN_TYPE_OBJECT) != 0)
                _ejs_log("O");
            else if ((*headerp & EJS_SCAN_TYPE_CLOSUREENV) != 0)
                _ejs_log("C");
...
static ejsval _ejs_GC_collect(ejsval env, ejsval _this, uint32_t argc,
                              ejsval *args) {
    _ejs_gc_collect();
    return _ejs_undefined;
}

static ejsval _ejs_GC_dumpAllocationStats(ejsval env, ejsval _this,
...
static ejsval _ejs_GC_dumpLiveStrings(ejsval env, ejsval _this, uint32_t argc,
                                      ejsval *args) {
    _ejs_log("strings:\n");
    for (int i = 0; i < HEAP_PAGELISTS_COUNT; i++) {
        EJS_LIST_FOREACH(&heap_pages[i], PageInfo, page, {
            ...
            for (int c = 0; c < CELLS_IN_PAGE(page);
                ...
                case EJS_STRING_DEPENDENT:
                    _ejs_log(" dependent %p, off %d : ",
                             primstr->data.dependent.dep,
                             primstr->data.dependent.off);
                    break;
                case EJS_STRING_ROPE:
                    _ejs_log(" rope %p, %p : ", primstr->data.rope.left,
                             primstr->data.rope.right);
                    break;
                case EJS_STRING_FLAT:
                    _ejs_log("%s ",
                             EJS_PRIMSTR_HAS_OOL_BUFFER(primstr) ? " ool" : "");
                    break;
                }
...
void _ejs_GC_init(ejsval ejs_obj) {
