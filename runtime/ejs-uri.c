...
static ejsval Encode(ejsval string, ejsval unescaped) {
    ...
    for (;;) {
        ...
        /* d. Else C is not in unescapedSet, */
            ...
            /* i. If the code unit value of C is not less than 0xDC00 and not
             * greater than 0xDFFF, throw a URIError exception. */
            if (C >= 0xDC00 && C <= 0xDFFF)
                _ejs_throw_nativeerror_utf8(EJS_URI_ERROR, "URI malformed");

            /* ii. If the code unit value of C is less than 0xD800 or greater
             * than 0xDBFF, then Let V be the code unit value of C. */
            jschar V;
            ...
            /* iii. Else, */
                ...
                /* 3. Let kChar be the code unit value of the code unit at index
                 * k within string. */
                jschar kChar = _ejs_string_ucs2_at(stringObj, k);

                /* 4. If kChar is less than 0xDC00 or greater than 0xDFFF, throw
                 * a URIError exception. */
                if (kChar < 0xDC00 || kChar > 0xDFFF)
                    _ejs_throw_nativeerror_utf8(EJS_URI_ERROR, "URI malformed");

                /* 5. Let V be (((the code unit value of C) – 0xD800) × 0x400 +
                 * (kChar – 0xDC00) + 0x10000). */
                V = (C - 0xD800) * 0x400 + (kChar - 0xDC00) + 0x10000;
            }

            /* iv. Let Octets be the array of octets resulting by applying the
             * UTF-8 transformation to V, and let L be the array size. */
            char octets[4];
            ...
            while (j < L) {
                ...
                /* 2. Let S be a String containing three code units “%XY” where
                 * XY are two uppercase hexadecimal
                 * digits encoding the value of jOctet. */
                char buff[4];
                sprintf(buff, "%%%X", jOctet);
                ejsval S = _ejs_string_new_utf8(buff);

                /* 3. Let R be a new String value computed by concatenating the
                 * previous value of R and S. */
                R = _ejs_string_concat(R, S);
...
ejsval _ejs_encodeURIComponent_impl(ejsval env, ejsval _this, uint32_t argc,
    ...
    /* 3. Let unescapedURIComponentSet be a String containing one instance of
     * each code unit valid in uriUnescaped. */
...
void _ejs_uri_init(ejsval global) {
    ...
    /* a 'funny' thing is that we are flatting the string whenever we are
     * encoding it. It would be nice to have it
     * in that state always! */
    char *uriUnescaped = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0"
                         "123456789-_.!~*'()";
