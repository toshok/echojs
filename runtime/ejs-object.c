...
ejsval Get(ejsval O, ejsval P) {
    ...
    // 3. Return the result of calling the [[Get]] internal method of O passing
    // P and O as the arguments
...
ejsval Put(ejsval O, ejsval P, ejsval V, EJSBool Throw) {
    ...
    // 4. Let success be the result of calling the [[Set]] internal method of O
    // passing P, V, and O as the arguments.
...
ejsval GetV(ejsval V, ejsval P) {
    ...
    // 5. Return the result of calling the [[Get]] internal method of box
...
ejsval GetMethod(ejsval O, ejsval P) {
    ...
    if (!EJSVAL_IS_CALLABLE(func))
        _ejs_throw_nativeerror(
            EJS_TYPE_ERROR,
            _ejs_string_concat(_ejs_atom_error_not_callable, ToString(P)));
...
EJSBool HasProperty(ejsval argument, ejsval property) {
    // 1. Assert: Type(O) is Object.
    ...
    // 3. Return the result of calling the [[HasProperty]] internal method of O
    // with argument P.
    return OP(EJSVAL_TO_OBJECT(argument), HasProperty)(argument, property);
}

ejsval DeletePropertyOrThrow(ejsval argument, ejsval property) {
    /* 1. Assert: Type(O) is Object. */
    if (!EJSVAL_IS_OBJECT(argument))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR, "O is not an Object");
    ...
    /* 3. Let success be the result of calling the [[Delete]] internal method of
     * O passing P as the argument. */
    /* 4. ReturnIfAbrupt(success). */
    EJSBool success =
        OP(EJSVAL_TO_OBJECT(argument), Delete)(argument, property, EJS_TRUE);
...
static EJSBool DefinePropertyOrThrow(ejsval O, ejsval P, EJSPropertyDesc *desc,
                                     ejsval *exc) {
    // 1. Assert: Type(O) is Object.
    // 2. Assert: IsPropertyKey(P) is true.
    ...
    // 4. ReturnIfAbrupt(success).
    EJSBool success =
        OP(EJSVAL_TO_OBJECT(O), DefineOwnProperty)(O, P, desc, EJS_FALSE);
    // 5. If success is false, then throw a TypeError exception.
    if (!success)
...
static uint32_t PropertyKeyHash(ejsval argument) {
...
EJSBool IsAccessorDescriptor(EJSPropertyDesc *Desc) {
    ...
    if (!_ejs_property_desc_has_getter(Desc) &&
        !_ejs_property_desc_has_setter(Desc))
        return EJS_FALSE;
...
EJSBool IsDataDescriptor(EJSPropertyDesc *Desc) {
    ...
    if (!_ejs_property_desc_has_value(Desc) &&
        !_ejs_property_desc_has_writable(Desc))
        return EJS_FALSE;
...
// ECMA262: 6.2.4.3
static EJSBool IsGenericDescriptor(EJSPropertyDesc *Desc) {
    /* 1. If Desc is undefined, then return false. */
    if (!Desc)
        return EJS_FALSE;
...
ejsval FromPropertyDescriptor(EJSPropertyDesc *Desc) {
    ...
    if (_ejs_property_desc_has_value(Desc)) {
        //    a. Call OrdinaryDefineOwnProperty with arguments obj, "value", and
        //    PropertyDescriptor{[[Value]]: Desc.[[Value]], [[Writable]]: true,
        //    [[Enumerable]]: true, [[Configurable]]: true}
        EJSPropertyDesc value_desc = {
            .value = Desc->value,
            .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                     EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
    ...
    if (_ejs_property_desc_has_writable(Desc)) {
        // a. Call OrdinaryDefineOwnProperty with arguments obj, "writable", and
        // PropertyDescriptor{[[Value]]: Desc.[[Writable]], [[Writable]]: true,
        // [[Enumerable]]: true, [[Configurable]]: true}.
        EJSPropertyDesc writable_desc = {
            .value = BOOLEAN_TO_EJSVAL(_ejs_property_desc_is_writable(Desc)),
            .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                     EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
    ...
    if (_ejs_property_desc_has_getter(Desc)) {
        ...
        //    PropertyDescriptor{[[Value]]: Desc.[[Get]], [[Writable]]: true,
        //    [[Enumerable]]: true, [[Configurable]]: true}.
        EJSPropertyDesc get_desc = {
            .value = _ejs_property_desc_get_getter(Desc),
            .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                     EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
    ...
    if (_ejs_property_desc_has_getter(Desc)) {
        ...
        //    PropertyDescriptor{[[Value]]: Desc.[[Set]], [[Writable]]: true,
        //    [[Enumerable]]: true, [[Configurable]]: true}.
        EJSPropertyDesc set_desc = {
            .value = _ejs_property_desc_get_setter(Desc),
            .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                     EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
    ...
    if (_ejs_property_desc_has_enumerable(Desc)) {
        ...
        //    [[Configurable]]: true}.
        EJSPropertyDesc enumerable_desc = {
            .value = BOOLEAN_TO_EJSVAL(_ejs_property_desc_is_enumerable(Desc)),
            .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                     EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
    ...
    if (_ejs_property_desc_has_configurable(Desc)) {
        ...
        //    [[Configurable]]: true}.
        EJSPropertyDesc configurable_desc = {
            .value =
                BOOLEAN_TO_EJSVAL(_ejs_property_desc_is_configurable(Desc)),
            .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                     EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
...
// ECMA262: 6.2.4.5
void ToPropertyDescriptor(ejsval Obj, EJSPropertyDesc *desc) {
    // 1. ReturnIfAbrupt(Obj).
    // 2. If Type(Obj) is not Object throw a TypeError exception.
    if (!EJSVAL_IS_OBJECT(Obj)) {
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "ToPropertyDescriptor called on non-object");
    }
    ...
    if (OP(obj, HasProperty)(Obj, _ejs_atom_enumerable)) {
        ...
        _ejs_property_desc_set_enumerable(
            desc, EJSVAL_TO_BOOLEAN(
                      ToBoolean(OP(obj, Get)(Obj, _ejs_atom_enumerable, Obj))));
    }
    ...
    if (OP(obj, HasProperty)(Obj, _ejs_atom_configurable)) {
        ...
        _ejs_property_desc_set_configurable(
            desc, EJSVAL_TO_BOOLEAN(ToBoolean(
                      OP(obj, Get)(Obj, _ejs_atom_configurable, Obj))));
    }
    ...
    if (OP(obj, HasProperty)(Obj, _ejs_atom_value)) {
        ...
        _ejs_property_desc_set_value(desc,
                                     OP(obj, Get)(Obj, _ejs_atom_value, Obj));
    }
    ...
    if (OP(obj, HasProperty)(Obj, _ejs_atom_writable)) {
        ...
        _ejs_property_desc_set_writable(
            desc, EJSVAL_TO_BOOLEAN(
                      ToBoolean(OP(obj, Get)(Obj, _ejs_atom_writable, Obj))));
    }
    ...
    if (_ejs_property_desc_has_getter(desc) ||
        _ejs_property_desc_has_setter(desc)) {
        /*    a. If either desc.[[Value]] or desc.[[Writable]] are present, then
         * throw a TypeError exception. */
        if (_ejs_property_desc_has_value(desc) ||
            _ejs_property_desc_has_writable(desc)) {
            _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                        "Invalid property.  A property cannot "
                                        "both have accessors and be writable "
                                        "or have a value");
        }
    }
...
void _ejs_propertydesc_free(EJSPropertyDesc *desc) { free(desc); }

static int primes[] = {5, 17, 31, 67, 131, 257, 521, 1031, 2053, 4099};
...
void _ejs_propertymap_free(EJSPropertyMap *map) {
    //_ejs_log ("%p: free\n", map);
...
void _ejs_propertymap_foreach_value(EJSPropertyMap *map,
                                    EJSValueFunc foreach_func) {
...
void _ejs_propertymap_foreach_property(EJSPropertyMap *map,
                                       EJSPropertyDescFunc foreach_func,
                                       void *data) {
    for (_EJSPropertyMapEntry *s = map->head_insert; s; s = s->next_insert) {
...
void _ejs_propertymap_remove(EJSPropertyMap *map, ejsval name) {
    //_ejs_log ("%p: remove (%s)\n", map,
    // ucs2_to_utf8(EJSVAL_TO_FLAT_STRING(name)));
    if (map->inuse == 0) {
    ...
    while (s) {
        if (s->hash == hashcode &&
            EJSVAL_TO_BOOLEAN(_ejs_op_strict_eq(s->name, name))) {
            ...
            if (map->head_insert == s) {
                ...
                // FIXME do we need a doubly linked list for the insert-list
                // instead of this second traversal?
                _EJSPropertyMapEntry *prev_insert;
                for (prev_insert = map->head_insert; prev_insert;
                     prev_insert = prev_insert->next_insert) {
...
EJSPropertyDesc *_ejs_propertymap_lookup(EJSPropertyMap *map, ejsval name) {
    ...
    for (_EJSPropertyMapEntry *s = map->buckets[bucket]; s;
         s = s->next_bucket) {
        if (s->hash != hashcode)
            continue;
...
static void _ejs_propertymap_rehash(EJSPropertyMap *map) {
    ...
    map->buckets = (_EJSPropertyMapEntry **)calloc(
        sizeof(_EJSPropertyMapEntry *), map->nbuckets);
...
void _ejs_propertymap_insert(EJSPropertyMap *map, ejsval name,
...
static EJSObject *_ejs_property_iterator_specop_allocate() {
    return (EJSObject *)_ejs_gc_new(EJSPropertyIterator);
}

static void _ejs_property_iterator_specop_finalize(EJSObject *obj) {
    EJSPropertyIterator *iterator = (EJSPropertyIterator *)obj;
    free(iterator->keys);
    _ejs_Object_specops.Finalize(obj);
}

static void _ejs_property_iterator_specop_scan(EJSObject *obj,
...
static EJS_DEFINE_CLASS(_EJSPropertyIterator,
    ...
    static EJSBool name_in_keys(ejsval name, ejsval *keys, int num) {
    for (int i = 0; i < num; i++) {
        if (!ucs2_strcmp(EJSVAL_TO_FLAT_STRING(name),
                         EJSVAL_TO_FLAT_STRING(keys[i])))
            return EJS_TRUE;
    }
    return EJS_FALSE;
}

static void collect_keys(ejsval objval, int *num, int *alloc, ejsval **keys) {
    ...
    for (_EJSPropertyMapEntry *s = obj->map->head_insert; s;
         s = s->next_insert) {
        if (_ejs_property_desc_is_enumerable(s->desc) &&
            !name_in_keys(s->name, *keys, *num)) {
            if (*num == *alloc - 1) {
...
ejsval _ejs_property_iterator_new(ejsval forVal) {
    // the iterator-using code for for..in should handle a null iterator return
    // value,
    // which would save us this allocation.
    ejsval iter =
        _ejs_object_new(_ejs_null, &_ejs__EJSPropertyIterator_specops);
    EJSPropertyIterator *iterator =
        (EJSPropertyIterator *)EJSVAL_TO_OBJECT(iter);

    iterator->current = -1;

    if (EJSVAL_IS_PRIMITIVE(forVal) || EJSVAL_IS_NULL(forVal) ||
        EJSVAL_IS_UNDEFINED(forVal)) {
        iterator->num = 0;
...
ejsval _ejs_property_iterator_current(ejsval iter) {
    EJSPropertyIterator *iterator =
        (EJSPropertyIterator *)EJSVAL_TO_OBJECT(iter);
    if (iterator->current == -1) {
        printf("_ejs_property_iterator_current called before "
...
EJSBool _ejs_property_iterator_next(ejsval iter, EJSBool free_on_end) {
    EJSPropertyIterator *iterator =
        (EJSPropertyIterator *)EJSVAL_TO_OBJECT(iter);
    iterator->current++;
    if (iterator->current == iterator->num) {
...
void _ejs_property_iterator_free(ejsval iter) {
...
#define MAYBE_INHERIT_DISALLOW_NULL(p)                                         \
    EJS_MACRO_START                                                            \
    MAYBE_INHERIT(p);                                                          \
    EJS_ASSERT(child->p);                                                      \
    EJS_MACRO_END
...
ejsval _ejs_number_new(double value) { return NUMBER_TO_EJSVAL(value); }

ejsval _ejs_object_setprop(ejsval val, ejsval key, ejsval value) {
...
ejsval _ejs_object_getprop(ejsval obj, ejsval key) {
    if (EJSVAL_IS_NULL(obj) || EJSVAL_IS_UNDEFINED(obj)) {
        ...
        snprintf(msg_buf, sizeof(msg_buf) - 1,
                 "Cannot read property '%s' of %s", key_utf8,
                 EJSVAL_IS_NULL(obj) ? "null" : "undefined");
        free(key_utf8);
    ...
    if (EJSVAL_IS_PRIMITIVE(obj)) {
        if (EJSVAL_IS_STRING(obj)) {
            if (!ucs2_strcmp(EJSVAL_TO_FLAT_STRING(ToString(key)),
                             _ejs_ucs2_length))
...
EJSBool _ejs_object_define_value_property(ejsval obj, ejsval key, ejsval value,
                                          uint32_t flags) {
    EJSObject *_obj = EJSVAL_TO_OBJECT(obj);
    EJSPropertyDesc desc = {.value = value,
                            .flags = flags | EJS_PROP_FLAGS_VALUE_SET};
    return OP(_obj, DefineOwnProperty)(obj, key, &desc, EJS_FALSE);
}

EJSBool _ejs_object_define_accessor_property(ejsval obj, ejsval key, ejsval get,
                                             ejsval set, uint32_t flags) {
    EJSObject *_obj = EJSVAL_TO_OBJECT(obj);
    EJSPropertyDesc desc = {.getter = get,
                            .setter = set,
                            .flags = flags | EJS_PROP_FLAGS_SETTER_SET |
                                     EJS_PROP_FLAGS_GETTER_SET};
    return OP(_obj, DefineOwnProperty)(obj, key, &desc, EJS_FALSE);
}

ejsval _ejs_object_setprop_utf8(ejsval val, const char *key, ejsval value) {
...
static ejsval _ejs_Object_impl(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
...
static ejsval _ejs_Object_getPrototypeOf(ejsval env, ejsval _this,
    ...
    // 3. Return the result of calling the [[GetPrototypeOf]] internal method of
    // obj.
...
static ejsval _ejs_Object_setPrototypeOf(ejsval env, ejsval _this,
                                         uint32_t argc, ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(O))
        return O;
    // 5. Let status be the result of calling the [[SetPrototypeOf]] internal
    // method of O with argument proto.
...
// externally visible (for the compiler) wrapper around above native
// implementation.
...
static ejsval _ejs_Object_getOwnPropertyDescriptor(ejsval env, ejsval _this,
                                                   uint32_t argc,
                                                   ejsval *args) {
    ejsval O = _ejs_undefined;
    ...
    EJSPropertyDesc *desc =
        OP(EJSVAL_TO_OBJECT(obj), GetOwnProperty)(obj, key, NULL);
...
static ejsval _ejs_Object_getOwnPropertyNames(ejsval env, ejsval _this,
                                              uint32_t argc, ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
     ...
     * expression new Array () where Array is the standard built-in constructor
     * with that name. */
    ...
    for (_EJSPropertyMapEntry *s = O_->map->head_insert; s;
         s = s->next_insert) {
        if (!_ejs_property_desc_is_enumerable(s->desc))
            continue;
        ...
        if (!EJSVAL_IS_SYMBOL(name)) {
            /*    b. Call the [[DefineOwnProperty]] internal method of array
               with arguments ToString(n), the
                  PropertyDescriptor {[[Value]]: name, [[Writable]]: true,
...
// ECMA262: 19.1.2.8
static ejsval _ejs_Object_getOwnPropertySymbols(ejsval env, ejsval _this,
                                                uint32_t argc, ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
    ...
    /* 2. Let array be the result of creating a new object as if by the
       expression new Array () where Array is the
...
static ejsval _ejs_Object_assign(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ...
    // 4. Let sourceList be the List of argument vales starting with the second
    // argument.
    ...
    for (int source_i = 1; source_i < argc; source_i++) {
        ...
        if (!EJSVAL_IS_OBJECT(from)) {
            _ejs_throw_nativeerror_utf8(
                EJS_TYPE_ERROR, "null/undefined passed to Object.assign");
        }
        ...
        for (_EJSPropertyMapEntry *s = from_->map->head_insert; s;
             s = s->next_insert) {
            //       i. Let nextKey be Get(keysArray, ToString(nextIndex)).
            //       ii. ReturnIfAbrupt(nextKey).
            ejsval nextKey = s->name;

            //       iii. Let desc be the result of calling the
            //       [[GetOwnProperty]] internal method of from with argument
            //       nextKey.
            ...
            //       v. Else if desc is not undefined and desc.[[Enumerable]] is
            //       true, then
...
static ejsval _ejs_Object_defineProperties(ejsval env, ejsval _this,
                                           uint32_t argc, ejsval *args);

// ECMA262: 15.2.3.5
/* Object.create ( O [, Properties] ) */
static ejsval _ejs_Object_create(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ejsval O = _ejs_undefined;
     ...
     * properties to obj as if by calling the  */
    /*    standard built-in function Object.defineProperties with arguments obj
     * and Properties. */
...
// a simple externally visible wrapper around the above native impl that only
// supplied the prototype argument
...
static ejsval _ejs_Object_defineProperty(ejsval env, ejsval _this,
                                         uint32_t argc, ejsval *args) {
    ejsval O = _ejs_undefined;
    ejsval P = _ejs_undefined;
    ...
    if (!EJSVAL_IS_OBJECT(O)) {
        ...
        snprintf(msg, sizeof(msg) - 1,
                 "defineProperty(%s) called on non-object", utf8_name);
        free(utf8_name);
...
static ejsval _ejs_Object_defineProperties(ejsval env, ejsval _this,
    ...
    int names_len = 0;
    for (_EJSPropertyMapEntry *s = props_obj->map->head_insert; s;
         s = s->next_insert) {
        if (_ejs_property_desc_is_enumerable(s->desc))
            names_len++;
    }
    ...
    DefinePropertiesPair *descriptors =
        malloc(sizeof(DefinePropertiesPair) * names_len);
    ...
    for (int n = 0; n < names_len; n++) {
        ...
        /* c. Append the pair (a two element List) consisting of P and desc to
    ...
    for (int d = 0; d < names_len; d++) {
        ...
        /*    c. Call the [[DefineOwnProperty]] internal method of O with
...
typedef enum { INTEGRITY_SEALED, INTEGRITY_FROZEN } IntegrityLevel;

// ECMA262: 7.3.11 SetIntegrityLevel (O, level)
static ejsval SetIntegrityLevel(ejsval O, IntegrityLevel level) {
    // 1. Assert: Type(O) is Object.
    EJSObject *O_ = EJSVAL_TO_OBJECT(O);
    ...
    // 8. Else level is "frozen",
        ...
        for (int i = 0; i < n; i++) {
            ejsval k = keys[i];
            //       i. Let status be the result of calling the
            //       [[GetOwnProperty]] internal method of O with k.
            ejsval exc;
            EJSPropertyDesc *currentDesc =
                OP(EJSVAL_TO_OBJECT(O), GetOwnProperty)(O, k, &exc);
            //       ii. If status is an abrupt completion, then
            ...
            //       iii. Else,
                ...
                if (currentDesc) {
                    ...
                    //               a. If IsAccessorDescriptor(currentDesc) is
                    //               true, then
                    ...
                    //               c. Let status be DefinePropertyOrThrow(O,
                    //               k, desc).
                    EJSBool status = DefinePropertyOrThrow(O, k, &desc, &exc);

                    //               d. If status is an abrupt completion, then
                    if (!status) {
                        //                  i. If pendingException is undefined,
                        //                  then set pendingException to status.
                        if (EJSVAL_IS_UNDEFINED(pendingException))
                            pendingException = exc;
                    }
                }
    ...
    if (!EJSVAL_IS_UNDEFINED(pendingException))
        _ejs_throw(pendingException); // returning an abrupt completion =
...
static ejsval TestIntegrityLevel(ejsval O, IntegrityLevel level) {
    // 1. Assert: Type(O) is Object.
    // 2. Assert: level is either "sealed" or "frozen".
    ...
    //     b. If status is an abrupt completion, then
    //        i. If pendingException is undefined, then set pendingException to
    //        status.
...
static ejsval _ejs_Object_seal(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
...
static ejsval _ejs_Object_freeze(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
...
static ejsval _ejs_Object_preventExtensions(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
    ...
    // 2. Let status be the result of calling the [[PreventExtensions]] internal
    // method of O.
    // 3. ReturnIfAbrupt(status).
    ...
    if (!status)
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "[[PreventExtensions]] returned false");
...
static ejsval _ejs_Object_is(ejsval env, ejsval _this, uint32_t argc,
                             ejsval *args) {
...
static ejsval _ejs_Object_isSealed(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
...
static ejsval _ejs_Object_isFrozen(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
...
// ECMA262: 15.2.3.13
static ejsval _ejs_Object_isExtensible(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
...
// ECMA262: 19.1.2.14
static ejsval _ejs_Object_keys(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    ejsval O = _ejs_undefined;
    if (argc > 0)
        O = args[0];
    ...
    return _ejs_Object_getOwnPropertyNames(_ejs_undefined, _ejs_undefined, 1,
                                           &O);
}

// ECMA262: 19.1.3.6
ejsval _ejs_Object_prototype_toString(ejsval env, ejsval _this, uint32_t argc,
                                      ejsval *args) {
    ...
    // 7. Else, if O is an exotic arguments object, then let builtinTag be
    // "Arguments".
    else if (EJSVAL_IS_ARGUMENTS(O)) {
        builtinTag = _ejs_atom_Arguments;
    }
    // 8. Else, if O is an ECMAScript function object, a built-in function
    // object, or a bound function exotic object, then let builtinTag be
    // "Function".
    else if (EJSVAL_IS_FUNCTION(O)) {
        builtinTag = _ejs_atom_Function;
    }
    // 9. Else, if O has an [[ErrorData]] internal slot, then let builtinTag be
    // "Error".
    else if (EJSVAL_IS_ERROR(O)) {
        builtinTag = _ejs_atom_Error;
    }
    // 10. Else, if O has a [[BooleanData]] internal slot, then let builtinTag
    // be "Boolean".
    else if (EJSVAL_IS_BOOLEAN_OBJECT(O)) {
        builtinTag = _ejs_atom_Boolean;
    }
    // 11. Else, if O has a [[NumberData]] internal slot, then let builtinTag be
    // "Number".
    else if (EJSVAL_IS_NUMBER_OBJECT(O)) {
        builtinTag = _ejs_atom_Number;
    }
    // 12. Else, if O has a [[DateValue]] internal slot, then let builtinTag be
    // "Date".
    else if (EJSVAL_IS_DATE(O)) {
        builtinTag = _ejs_atom_Date;
    }
    // 13. Else, if O has a [[RegExpMatcher]] internal slot, then let builtinTag
    // be "RegExp".
    ...
    EJSBool hasTag =
        OP(EJSVAL_TO_OBJECT(O), HasProperty)(O, _ejs_Symbol_toStringTag);
    ...
    // 18. Else,
        ...
        //     "Error", "Function", "Number", "RegExp", or
        //        "String" and SameValue(tag, builtinTag) is false, then let tag
        //        be the string value "~" concatenated with
        //        the current value of tag.
        if (check_for_subclasses && !SameValue(tag, builtinTag))
            tag = _ejs_string_concat(_ejs_atom_tilde, tag);
    }
    // 19. Return the String value that is the result of concatenating the three
    // Strings "[object ", tag, and "]".
    return _ejs_string_concatv(_ejs_atom_toString_prefix, tag,
                               _ejs_atom_toString_suffix, _ejs_null);
}

// ECMA262: 15.2.4.3
static ejsval _ejs_Object_prototype_toLocaleString(ejsval env, ejsval _this,
                                                   uint32_t argc,
                                                   ejsval *args) {
    ...
    if (!EJSVAL_IS_CALLABLE(toString))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "toString property is not callable");
...
static ejsval _ejs_Object_prototype_valueOf(ejsval env, ejsval _this,
                                            uint32_t argc, ejsval *args) {
    return ToObject(_this);
}

// ECMA262: 15.2.4.5
static ejsval _ejs_Object_prototype_hasOwnProperty(ejsval env, ejsval _this,
                                                   uint32_t argc,
                                                   ejsval *args) {
    ...
    return BOOLEAN_TO_EJSVAL(OP(EJSVAL_TO_OBJECT(_this),
                                GetOwnProperty)(_this, needle, NULL) != NULL);
}

// ECMA262: 15.2.4.6
static ejsval _ejs_Object_prototype_isPrototypeOf(ejsval env, ejsval _this,
                                                  uint32_t argc, ejsval *args) {
...
// ECMA262: 15.2.4.7
static ejsval _ejs_Object_prototype_propertyIsEnumerable(ejsval env,
                                                         ejsval _this,
                                                         uint32_t argc,
                                                         ejsval *args) {
    ejsval V = _ejs_undefined;
...
void _ejs_object_init_proto() {
    ...
    _ejs_init_object((EJSObject *)__proto__, _ejs_Object_prototype,
                     &_ejs_Function_specops);
    __proto__->func = _ejs_Function_empty;
    __proto__->env = _ejs_null;

    _ejs_object_define_value_property(
        _ejs_Object__proto__, _ejs_atom_name, _ejs_atom_empty,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE |
            EJS_PROP_NOT_WRITABLE);
}

void _ejs_object_init(ejsval global) {
    _ejs_Object = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_Object, (EJSClosureFunc)_ejs_Object_impl);
    _ejs_object_setprop(global, _ejs_atom_Object, _ejs_Object);

    // ECMA262 15.2.3.1
    _ejs_object_define_value_property(
        _ejs_Object, _ejs_atom_prototype, _ejs_Object_prototype,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE |
            EJS_PROP_NOT_WRITABLE);
    // ECMA262: 15.2.4.1
    _ejs_object_define_value_property(
        _ejs_Object_prototype, _ejs_atom_constructor, _ejs_Object,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_CONFIGURABLE | EJS_PROP_WRITABLE);

#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs_Object, x, _ejs_Object_##x,           \
                                    EJS_PROP_NOT_ENUMERABLE)
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_Object_prototype, x, _ejs_Object_prototype_##x,                   \
...
static ejsval _ejs_object_specop_get_prototype_of(ejsval obj) {
    return EJSVAL_TO_OBJECT(obj)->proto;
}

// [[SetPrototypeOf]] ECMA262: 9.1.1
static EJSBool _ejs_object_specop_set_prototype_of(ejsval O, ejsval V) {
    EJSObject *O_ = EJSVAL_TO_OBJECT(O);

    // When the [[SetPrototypeOf]] internal method of O is called with argument
    ...
    if (!EJSVAL_IS_NULL(V)) {
        ...
        while (!EJSVAL_IS_NULL(p)) {
            ...
            if (SameValue(p, O))
                return EJS_FALSE;
            //       ii.  Let nextp be the result of calling the
            //       [[GetPrototypeOf]] internal method of p with no arguments.
    ...
    // toshok -- why are re-doing this?  steps 4 and 5 above appear to already
    // have taken care of these cases.
...
static ejsval _ejs_object_specop_get(ejsval O, ejsval P, ejsval Receiver) {
    // 1. Assert: IsPropertyKey(P) is true.
    ejsval pname = ToPropertyKey(
        P); // XXX this shouldn't be necessary, but ejs passes numbers here

    if (EJSVAL_IS_STRING(pname) &&
        !ucs2_strcmp(_ejs_ucs2___proto__, EJSVAL_TO_FLAT_STRING(pname)))
    ...
    if (desc == NULL) {
        ...
        if (EJSVAL_IS_NULL(parent)) {
            // _ejs_log ("property lookup on a %s object, propname %s =>
            // undefined\n", CLASSNAME(EJSVAL_TO_OBJECT(O)),
            // EJSVAL_TO_FLAT_STRING(pname));
            return _ejs_undefined;
        }
    ...
    // 8. Return the result of calling the [[Call]] internal method of getter
    // with Receiver as the thisArgument and an empty List as argumentsList.
    return _ejs_invoke_closure(getter, Receiver, 0, NULL);
}

// ECMA262: 8.12.1
static EJSPropertyDesc *_ejs_object_specop_get_own_property(ejsval obj,
                                                            ejsval propertyName,
                                                            ejsval *exc) {
...
// ECMA262: 9.1.9
static EJSBool _ejs_object_specop_set(ejsval O, ejsval P, ejsval V,
                                      ejsval Receiver) {
    EJSPropertyDesc undefined_desc = {
        .value = _ejs_undefined,
        .flags = EJS_PROP_FLAGS_VALUE_SET | EJS_PROP_WRITABLE |
                 EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE};
    ...
    EJSPropertyDesc *ownDesc =
        OP(EJSVAL_TO_OBJECT(O), GetOwnProperty)(O, P, NULL);
    ...
    if (!ownDesc) {
        ...
        if (!EJSVAL_IS_NULL(parent)) {
            //       i. Return the result of calling the [[Set]] internal method
            //       of parent with arguments P, V, and Receiver.
    ...
    if (IsDataDescriptor(ownDesc)) {
        ...
        EJSPropertyDesc *existingDescriptor =
            OP(EJSVAL_TO_OBJECT(Receiver), GetOwnProperty)(Receiver, P, NULL);
        ...
        if (existingDescriptor) {
            ...
            EJSPropertyDesc valueDesc = {.value = V,
                                         .flags = EJS_PROP_FLAGS_VALUE_SET};
            return OP(EJSVAL_TO_OBJECT(Receiver), DefineOwnProperty)(
                Receiver, P, &valueDesc,
                EJS_FALSE); /* XXX not sure about last parameter here, it's an
                               ES5-ism */
        }
        //    f. Else Receiver does not currently have a property P,
        else {
            //       i. Return CreateDataProperty(Receiver, P, V).
            return _ejs_object_define_value_property(
                Receiver, P, V, EJS_PROP_ENUMERABLE | EJS_PROP_CONFIGURABLE |
                                    EJS_PROP_WRITABLE);
        }
    }
    ...
    if (IsAccessorDescriptor(ownDesc)) {
        ...
        //    c. Let setterResult be the result of calling the [[Call]] internal
        //    method of setter providing Receiver as thisArgument and a new List
        //    containing V as argumentsList.
        //    d. ReturnIfAbrupt(setterResult).
        /* unused ejsval setterResult = */ _ejs_invoke_closure(setter, Receiver,
                                                               1, &V);
    }
    // e. Return true.
    return EJS_TRUE;
...
static EJSBool _ejs_object_specop_has_property(ejsval O, ejsval P) {
    // 1. Assert: IsPropertyKey(P) is true.

    // 2. Let hasOwn be the result of calling the [[GetOwnProperty]] internal
    // method of O with argument P.
    // 3. ReturnIfAbrupt(hasOwn).
    EJSPropertyDesc *hasOwn =
        OP(EJSVAL_TO_OBJECT(O), GetOwnProperty)(O, P, NULL);
    ...
    // 5. Let parent be the result of calling the [[GetPrototypeOf]] internal
    // method of O.
    ...
    if (!EJSVAL_IS_NULL(parent)) {
        //    a. Return the result of calling the [[HasProperty]] internal
...
static EJSBool _ejs_object_specop_delete(ejsval O, ejsval P, EJSBool Throw) {
    EJSObject *obj = EJSVAL_TO_OBJECT(O);
...
// ECMA262: 8.12.9
static EJSBool _ejs_object_specop_define_own_property(ejsval O, ejsval P,
                                                      EJSPropertyDesc *Desc,
                                                      EJSBool Throw) {
#define REJECT(reason)                                                         \
    ...
    if (Throw)                                                                 \
        _ejs_throw_nativeerror(                                                \
            EJS_TYPE_ERROR,                                                    \
            _ejs_string_concat(P, _ejs_string_new_utf8(reason)));              \
    return EJS_FALSE;                                                          \
    EJS_MACRO_END
    ...
    if (!current && extensible) {
        ...
        if (IsGenericDescriptor(Desc) || IsDataDescriptor(Desc)) {
             ...
             * described by Desc. If the value of */
            /*          an attribute field of Desc is absent, the attribute of
             * the newly created property is set to its  */
            /*          default value. */
            if (_ejs_property_desc_has_value(Desc))
                _ejs_property_desc_set_value(
                    dest, _ejs_property_desc_get_value(Desc));
            if (_ejs_property_desc_has_configurable(Desc))
                _ejs_property_desc_set_configurable(
                    dest, _ejs_property_desc_is_configurable(Desc));
            if (_ejs_property_desc_has_enumerable(Desc))
                _ejs_property_desc_set_enumerable(
                    dest, _ejs_property_desc_is_enumerable(Desc));
            if (_ejs_property_desc_has_writable(Desc))
                _ejs_property_desc_set_writable(
                    dest, _ejs_property_desc_is_writable(Desc));
        }
        /*    b. Else, Desc must be an accessor Property Descriptor so, */
            ...
            /*          default value. */
            if (_ejs_property_desc_has_getter(Desc))
                _ejs_property_desc_set_getter(
                    dest, _ejs_property_desc_get_getter(Desc));
            if (_ejs_property_desc_has_setter(Desc))
                _ejs_property_desc_set_setter(
                    dest, _ejs_property_desc_get_setter(Desc));
            if (_ejs_property_desc_has_configurable(Desc))
                _ejs_property_desc_set_configurable(
                    dest, _ejs_property_desc_is_configurable(Desc));
            if (_ejs_property_desc_has_enumerable(Desc))
     ...
     * value of every field in Desc is the same  */
    /*    value as the corresponding field in current when compared using the
     * SameValue algorithm (9.12). */
    if ((Desc->flags & EJS_PROP_FLAGS_SET_MASK) ==
        (current->flags & EJS_PROP_FLAGS_SET_MASK)) {
        EJSBool match = EJS_TRUE;

        if (match)
            match = !_ejs_property_desc_has_enumerable(Desc) ||
                    (_ejs_property_desc_is_enumerable(Desc) ==
                     _ejs_property_desc_is_enumerable(current));
        if (match)
            match = !_ejs_property_desc_has_configurable(Desc) ||
                    (_ejs_property_desc_is_configurable(Desc) ==
                     _ejs_property_desc_is_configurable(current));
        if (match)
            match = !_ejs_property_desc_has_writable(Desc) ||
                    (_ejs_property_desc_is_writable(Desc) ==
                     _ejs_property_desc_is_writable(current));
        if (match)
            match = !_ejs_property_desc_has_value(Desc) ||
                    (EJSVAL_EQ(_ejs_property_desc_get_value(Desc),
                               _ejs_property_desc_get_value(current)));
        if (match)
            match = !_ejs_property_desc_has_getter(Desc) ||
                    (EJSVAL_EQ(_ejs_property_desc_get_getter(Desc),
                               _ejs_property_desc_get_getter(current)));
        if (match)
            match = !_ejs_property_desc_has_setter(Desc) ||
    ...
    if (!_ejs_property_desc_is_configurable(current)) {
        ...
        if (_ejs_property_desc_is_configurable(Desc))
            REJECT("_ejs_property_desc_is_configurable(Desc)");

        /*    b. Reject, if the [[Enumerable]] field of Desc is present and the
         * [[Enumerable]] fields of current and  */
        /*       Desc are the Boolean negation of each other. */
        if (_ejs_property_desc_has_enumerable(Desc) &&
            _ejs_property_desc_is_enumerable(Desc) !=
                _ejs_property_desc_is_enumerable(current))
            REJECT("_ejs_property_desc_has_enumerable (Desc) && "
                   "_ejs_property_desc_is_enumerable(Desc) != "
                   "_ejs_property_desc_is_enumerable(current)");
    }
    ...
    /* 9. Else, if IsDataDescriptor(current) and IsDataDescriptor(Desc) have
       different results, then */
        ...
        if (!_ejs_property_desc_is_configurable(current))
            REJECT("!_ejs_property_desc_is_configurable(current)");
        /*    b. If IsDataDescriptor(current) is true, then */
        if (IsDataDescriptor(current)) {
            /*       i. Convert the property named P of object O from a data
             * property to an accessor property.  */
        ...
        /*    c. Else, */
             ...
             * accessor property to a data property.  */
            /*          Preserve the existing values of the converted property‘s
             * [[Configurable]] and */
            /*          [[Enumerable]] attributes and set the rest of the
             * property‘s attributes to their default values. */
    ...
    /* 10. Else, if IsDataDescriptor(current) and IsDataDescriptor(Desc) are
       both true, then */
        ...
        if (!_ejs_property_desc_is_configurable(current)) {
            ...
            if (!_ejs_property_desc_is_writable(current)) {
                /*            1. Reject, if the [[Value]] field of Desc is
                ...
                if (_ejs_property_desc_has_value(Desc) &&
                    !EJSVAL_EQ(_ejs_property_desc_get_value(Desc),
                               _ejs_property_desc_get_value(current)))
                    REJECT(
                        ": property is not writable and also not configurable");
            }
        }
    ...
    /* 11. Else, IsAccessorDescriptor(current) and IsAccessorDescriptor(Desc)
       are both true so, */
        ...
        if (!_ejs_property_desc_is_configurable(current)) {
             ...
             * SameValue(Desc.[[Set]], current.[[Set]]) is  */
            /*           false. */
            if (_ejs_property_desc_has_setter(Desc) &&
                !EJSVAL_EQ(_ejs_property_desc_get_setter(Desc),
                           _ejs_property_desc_get_setter(current)))
     ...
     * correspondingly named attribute of the property  */
    /*     named P of object O to the value of the field. */
    EJSPropertyDesc *dest = _ejs_propertymap_lookup(obj->map, P);

    if (_ejs_property_desc_has_getter(Desc))
        _ejs_property_desc_set_getter(dest,
                                      _ejs_property_desc_get_getter(Desc));
    if (_ejs_property_desc_has_setter(Desc))
        _ejs_property_desc_set_setter(dest,
                                      _ejs_property_desc_get_setter(Desc));
    if (_ejs_property_desc_has_value(Desc))
    ...
    if (_ejs_property_desc_has_enumerable(Desc))
        _ejs_property_desc_set_enumerable(
            dest, _ejs_property_desc_is_enumerable(Desc));
    if (_ejs_property_desc_has_writable(Desc))
        _ejs_property_desc_set_writable(dest,
                                        _ejs_property_desc_is_writable(Desc));
...
EJSObject *_ejs_object_specop_allocate() { return _ejs_gc_new(EJSObject); }

void _ejs_object_specop_finalize(EJSObject *obj) {
    // printf ("_ejs_propertymap_free(obj->map = %p)\n", obj->map);
    _ejs_propertymap_free(obj->map);
    obj->map = NULL;
}

static void scan_property(ejsval name, EJSPropertyDesc *desc,
                          EJSValueFunc scan_func) {
    scan_func(name);
...
static void _ejs_object_specop_scan(EJSObject *obj, EJSValueFunc scan_func) {
    _ejs_propertymap_foreach_property(
        obj->map, (EJSPropertyDescFunc)scan_property, scan_func);
    scan_func(obj->proto);
}

// ECMA262: 9.1.3 [[IsExtensible]] ( )
static EJSBool _ejs_object_specop_is_extensible(ejsval O) {
...
static EJSBool _ejs_object_specop_preventextensions(ejsval O) {
...
static ejsval _ejs_object_specop_enumerate(ejsval O) {
    // 1. Let proto be the result of calling the [[GetPrototypeOf]] internal
    // method of O with no arguments.
    // 2. ReturnIfAbrupt(proto).
    // 3. If proto is the value null, then
    //    a. Let propList be a new empty List.
    // 4. Else
    //    a. Let propList be the result of calling the [[Enumerate]] internal
    //    method of proto.
    ...
    //       iii. If name is an element of propList, then remove name as an
    //       element of propList.
    //       iv. If desc.[[Enumerable]] is true, then add name as an element of
    //       propList.
...
static ejsval _ejs_object_specop_own_property_keys(ejsval O) {
    EJSObject *O_ = EJSVAL_TO_OBJECT(O);
    ...
    for (_EJSPropertyMapEntry *s = O_->map->head_insert; s;
        ...
        if (EJSVAL_IS_STRING(s->name)) {
            ...
            if (EJSVAL_IS_NUMBER(idx_val)) {
                ...
                if (floor(n) == n) {
                    // 2. For each own property key P of O that is an integer
                    // index, in ascending numeric index order
                    //    a. Add P as the last element of keys.
                    numberkeys[num_numberkeys++] = s->name;
                    continue;
                }
            }
            // 3. For each own property key P of O that is a String but is not
    ...
    ejsval keys = _ejs_array_new(
        num_numberkeys + num_stringkeys + num_symbolkeys, EJS_FALSE);
    ejsval *elements = EJS_DENSE_ARRAY_ELEMENTS(keys);

    memmove(elements, numberkeys, num_numberkeys * sizeof(ejsval));
    memmove(elements + num_numberkeys * sizeof(ejsval), stringkeys,
            num_stringkeys * sizeof(ejsval));
    memmove(elements + (num_numberkeys + num_stringkeys) * sizeof(ejsval),
            symbolkeys, num_symbolkeys * sizeof(ejsval));
...
EJS_DEFINE_CLASS(
    Object, _ejs_object_specop_get_prototype_of,
    _ejs_object_specop_set_prototype_of, _ejs_object_specop_is_extensible,
    _ejs_object_specop_preventextensions, _ejs_object_specop_get_own_property,
    _ejs_object_specop_define_own_property, _ejs_object_specop_has_property,
    _ejs_object_specop_get, _ejs_object_specop_set, _ejs_object_specop_delete,
    _ejs_object_specop_enumerate, _ejs_object_specop_own_property_keys,
    _ejs_object_specop_allocate, _ejs_object_specop_finalize,
