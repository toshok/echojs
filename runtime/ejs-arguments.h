...
#define EJSVAL_IS_ARGUMENTS(v)                                                 \
    (EJSVAL_IS_OBJECT(v) &&                                                    \
     (EJSVAL_TO_OBJECT(v)->ops == &_ejs_Arguments_specops))
#define EJSVAL_TO_ARGUMENTS(v) ((EJSArguments *)EJSVAL_TO_OBJECT(v))

#define EJS_ARGUMENTS_HAS_OOL_BUFFER_MASK 0x10
#define EJS_ARGUMENTS_HAS_OOL_BUFFER_MASK_SHIFTED                              \
    (EJS_ARGUMENTS_HAS_OOL_BUFFER_MASK << EJS_GC_USER_FLAGS_SHIFT)
#define EJS_ARGUMENTS_HAS_OOL_BUFFER(s)                                        \
    ((((EJSPrimString *)(s))->gc_header &                                      \
      EJS_ARGUMENTS_HAS_OOL_BUFFER_MASK_SHIFTED) >>                            \
     EJS_GC_USER_FLAGS_SHIFT) != 0
#define EJS_ARGUMENTS_SET_HAS_OOL_BUFFER(s)                                    \
    ((((EJSPrimString *)(s))->gc_header |=                                     \
      EJS_ARGUMENTS_HAS_OOL_BUFFER_MASK_SHIFTED))
