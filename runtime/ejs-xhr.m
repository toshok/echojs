...
typedef enum { UNSENT, OPENED, HEADERS_RECEIVED, LOADING, DONE } ReadyState;
...
- (NSString *)responseText {
    ...
    return [[NSString alloc]
        initWithData:responseData
            encoding:
                NSUTF8StringEncoding /*XXX this should come from the response headers... */];
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    state = DONE;
    [self performSelectorOnMainThread:@selector(proxyDidFinishLoading)
                           withObject:nil
                        waitUntilDone:NO];
}
...
- (void)invokeReadyStateChange {
    if (readystatechange) {
        CKInvocation *inv =
            [CKInvocation invocationWithFunction:readystatechange
                                        argCount:0
                                      thisObject:NULL];
...
static ejsval _ejs_XMLHttpRequest_prototype_open(ejsval env, ejsval _this,
     ...
     * void open(DOMString method, DOMString url, boolean async, DOMString?
     * user);
     * void open(DOMString method, DOMString url, boolean async, DOMString?
     * user, DOMString? password);
     */

    // Note: we only allow async mode.  if the user requests sync mode we throw
    // an exception
...
static ejsval _ejs_XMLHttpRequest_prototype_send(ejsval env, ejsval _this,
                                                 uint32_t argc, ejsval *args) {
    /* the forms the user can invoke this method using:
     *
     * void send();
    ...
    @try {
        if (argc == 0) {
            ...
            if (EJSVAL_IS_STRING(args[0])) {
                ...
                EJS_NOT_IMPLEMENTED(); // FIXME handle this... NSXmlDocument
                                       // case
            }
        }
...
static ejsval _ejs_XMLHttpRequest_prototype_setRequestHeader(ejsval env,
                                                             ejsval _this,
                                                             uint32_t argc,
                                                             ejsval *args) {
    /*
     * When the setRequestHeader(header, value) method is invoked, the user
     ...
     *
     * If any code point in value is higher than U+00FF LATIN SMALL LETTER Y
     * WITH DIAERESIS or after deflating value it does not match the field-value
     * production raise a SYNTAX_ERR exception and terminate these steps.
     ...
     * XMLHttpRequest.
     *
     * If header is not in the author request headers list append header with
     * its associated value to the list and terminate these steps.
     *
...
static ejsval _ejs_XMLHttpRequest_prototype_abort(ejsval env, ejsval _this,
                                                  uint32_t argc, ejsval *args) {
    /*
     * When the abort() method is invoked, the user agent must run these steps
     ...
     * responsible.
     *
     * If there are any tasks from the object's XMLHttpRequest task source in
     ...
     *
     * If the state is UNSENT, OPENED with the send() flag being false, or DONE
     * go to the next step.
     *
...
static ejsval _ejs_XMLHttpRequest_prototype_getResponseHeader(ejsval env,
                                                              ejsval _this,
                                                              uint32_t argc,
                                                              ejsval *args) {
    /*
     * When the getResponseHeader(header) is invoked, the user agent must run
     * these steps:
     *
     ...
     *
     * If any code point in header is higher than U+00FF LATIN SMALL LETTER Y
     * WITH DIAERESIS return null and terminate these steps.
     *
     * Let header be the result of deflating header.
     *
     * If header is a case-insensitive match for Set-Cookie or Set-Cookie2
     * return null and terminate these steps.
     *
     * If header is a case-insensitive match for multiple HTTP response headers,
     * return the inflated values of these headers as a single concatenated
     * string separated from each other by a U+002C COMMA U+0020 SPACE character
     * pair and terminate these steps.
     *
...
static ejsval _ejs_XMLHttpRequest_prototype_getAllResponseHeaders(
    ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
    /*
     * When the getAllResponseHeaders() method is invoked, the user agent must
     ...
     * case-insensitive match for Set-Cookie or Set-Cookie2, inflated, as a
     * single string, with each header line separated by a U+000D CR U+000A LF
     * pair, excluding the status line, and with each header name and header
     * value separated by a U+003A COLON U+0020 SPACE pair.
     */
...
static ejsval _ejs_XMLHttpRequest_prototype_get_readyState(ejsval env,
                                                           ejsval _this,
                                                           uint32_t argc,
                                                           ejsval *args) {
    XmlHttpRequest *xhr = (XmlHttpRequest *)get_peer(_this);
...
static ejsval _ejs_XMLHttpRequest_prototype_get_status(ejsval env, ejsval _this,
                                                       uint32_t argc,
                                                       ejsval *args) {
    /*
...
static ejsval _ejs_XMLHttpRequest_prototype_get_statusText(ejsval env,
                                                           ejsval _this,
                                                           uint32_t argc,
                                                           ejsval *args) {
    /*
...
static ejsval _ejs_XMLHttpRequest_prototype_get_responseText(ejsval env,
                                                             ejsval _this,
                                                             uint32_t argc,
                                                             ejsval *args) {
    /*
...
static ejsval _ejs_XMLHttpRequest_prototype_get_responseXML(ejsval env,
                                                            ejsval _this,
                                                            uint32_t argc,
                                                            ejsval *args) {
    EJS_NOT_IMPLEMENTED();
...
static ejsval _ejs_XMLHttpRequest_prototype_get_onreadystatechange(
    ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
    XmlHttpRequest *xhr = (XmlHttpRequest *)get_peer(_this);
    CKObject *readystatechange = [xhr readystatechange];

    return readystatechange ? OBJECT_TO_EJSVAL([readystatechange jsObject])
                            : _ejs_null;
}

static ejsval _ejs_XMLHttpRequest_prototype_set_onreadystatechange(
    ejsval env, ejsval _this, uint32_t argc, ejsval *args) {
    XmlHttpRequest *xhr = (XmlHttpRequest *)get_peer(_this);
...
static ejsval _ejs_XMLHttpRequest_impl(ejsval env, ejsval _this, uint32_t argc,
                                       ejsval *args) {
...
static ejsval _ejs_XMLHttpRequest_create(ejsval env, ejsval _this,
                                         uint32_t argc, ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in XMLHttpRequest[Symbol.create] is not a constructor");

    EJSObject *F_ = EJSVAL_TO_OBJECT(F);

    // 2. Let obj be the result of calling OrdinaryCreateFromConstructor(F,
    // "%XMLHttpRequestPrototype%", ([[XHRData]]) ).
...
void _ejs_xmlhttprequest_init(ejsval global) {
    _ejs_XMLHttpRequest = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_XMLHttpRequest,
        (EJSClosureFunc)_ejs_XMLHttpRequest_impl);
    _ejs_object_setprop(global, _ejs_atom_XMLHttpRequest, _ejs_XMLHttpRequest);

    _ejs_gc_add_root(&_ejs_XMLHttpRequest_prototype);
    _ejs_XMLHttpRequest_prototype =
        _ejs_object_new(_ejs_null, &_ejs_Object_specops);
    _ejs_object_setprop(_ejs_XMLHttpRequest, _ejs_atom_prototype,
                        _ejs_XMLHttpRequest_prototype);

#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(_ejs_XMLHttpRequest_prototype, x,          \
                                    _ejs_XMLHttpRequest_prototype_##x,         \
                                    EJS_PROP_NOT_ENUMERABLE)
#define PROTO_GETTER(x)                                                        \
    EJS_INSTALL_ATOM_GETTER(_ejs_XMLHttpRequest_prototype, x,                  \
                            _ejs_XMLHttpRequest_prototype_get_##x)
#define PROTO_ACCESSORS(x)                                                     \
    EJS_INSTALL_ATOM_ACCESSORS(_ejs_XMLHttpRequest_prototype, x,               \
                               _ejs_XMLHttpRequest_prototype_get_##x,          \
                               _ejs_XMLHttpRequest_prototype_set_##x)
...
#undef PROTO_ACCESSORS
    ...
    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_XMLHttpRequest, create,
                                      _ejs_XMLHttpRequest_create,
                                      EJS_PROP_NOT_ENUMERABLE);
}
