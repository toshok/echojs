...
#define EJSVAL_IS_WEAKSET(v)                                                   \
    (EJSVAL_IS_OBJECT(v) && (EJSVAL_TO_OBJECT(v)->ops == &_ejs_WeakSet_specops))
...
ejsval _ejs_WeakSet_prototype_add(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    if (!EJSVAL_IS_OBJECT(value))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "add called with non-Object value.");
    ...
    if (!EJSVAL_IS_SET(iset))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "[[WeakSetData]] internal error");

    _ejs_set_add(iset, S);
    return S;
#else
// 7. Let entries be the List that is the value of S’s [[WeakSetData]] internal
// slot.
...
ejsval _ejs_WeakSet_prototype_delete(ejsval env, ejsval _this, uint32_t argc,
                                     ejsval *args) {
    ejsval value = _ejs_undefined;
    if (argc > 0)
        value = args[0];
    ...
    if (!EJSVAL_IS_WEAKSET(S))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "delete called with non-WeakSet this.");
...
// 6. Let entries be the List that is the value of M’s [[WeakSetData]] internal
// slot.
    ...
    //    a. If e is not empty and SameValue(e, value) is true, then
    //       i. Replace the element of entries whose value is e with an element
...
ejsval _ejs_WeakSet_prototype_has(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ejsval value = _ejs_undefined;
    if (argc > 0)
        value = args[0];
    ...
    // 3. If M does not have a [[WeakSetData]] internal slot throw a TypeError
    // exception.
    if (!EJSVAL_IS_WEAKSET(S))
        _ejs_throw_nativeerror_utf8(EJS_TYPE_ERROR,
                                    "has called with non-WeakSet this.");
...
// ES6: 23.1.1.1
// WeakSet ()
static ejsval _ejs_WeakSet_impl(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    ...
    if (EJSVAL_IS_UNDEFINED(set)) {
        ...
        _ejs_init_object(obj, _ejs_WeakSet_prototype, &_ejs_WeakSet_specops);
    ...
    if (!EJSVAL_IS_OBJECT(set))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR, "WeakSet constructor called with non-object this.");
    ...
    if (!EJSVAL_IS_WEAKSET(set))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "WeakSet constructor called with non-WeakSet this.");

    // 4. If set’s [[WeakSetData]] internal slot is not undefined, then throw a
    // TypeError exception.
    ...
    // 8. If the value of sets’s [[WeakSetData]] internal slot is not undefined,
...
static ejsval _ejs_WeakSet_create(ejsval env, ejsval _this, uint32_t argc,
                                  ejsval *args) {
    ...
    if (!EJSVAL_IS_CONSTRUCTOR(F))
        _ejs_throw_nativeerror_utf8(
            EJS_TYPE_ERROR,
            "'this' in WeakSet[Symbol.create] is not a constructor");
...
void _ejs_weakset_init(ejsval global) {
    ...
    _ejs_WeakSet = _ejs_function_new_without_proto(
        _ejs_null, _ejs_atom_WeakSet, (EJSClosureFunc)_ejs_WeakSet_impl);
    _ejs_object_setprop(global, _ejs_atom_WeakSet, _ejs_WeakSet);

    _ejs_gc_add_root(&_ejs_WeakSet_prototype);
    _ejs_WeakSet_prototype =
        _ejs_object_new(_ejs_Object_prototype, &_ejs_Object_specops);
    _ejs_object_setprop(_ejs_WeakSet, _ejs_atom_prototype,
                        _ejs_WeakSet_prototype);

#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_WeakSet, x, _ejs_WeakSet_##x)
#define PROTO_METHOD(x)                                                        \
    EJS_INSTALL_ATOM_FUNCTION_FLAGS(                                           \
        _ejs_WeakSet_prototype, x, _ejs_WeakSet_prototype_##x,                 \
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_WRITABLE | EJS_PROP_CONFIGURABLE)
#define PROTO_GETTER(x)                                                        \
    EJS_INSTALL_ATOM_GETTER(_ejs_WeakSet_prototype, x,                         \
                            _ejs_WeakSet_prototype_get_##x)
    ...
    _ejs_object_define_value_property(
        _ejs_WeakSet_prototype, _ejs_Symbol_toStringTag, _ejs_atom_WeakSet,
        EJS_PROP_NOT_ENUMERABLE | EJS_PROP_NOT_WRITABLE |
            EJS_PROP_CONFIGURABLE);

    EJS_INSTALL_SYMBOL_FUNCTION_FLAGS(_ejs_WeakSet, create, _ejs_WeakSet_create,
