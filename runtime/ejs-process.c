...
static ejsval _ejs_Process_get_env(ejsval env, ejsval _this, uint32_t argc,
                                   ejsval *args) {
    ...
    while (*p) {
        ...
        *eq = '\0';
        ...
        _ejs_object_define_value_property(
            env_obj, k, v, EJS_PROP_ENUMERABLE | EJS_PROP_NOT_CONFIGURABLE |
...
static ejsval _ejs_Process_exit(ejsval env, ejsval _this, uint32_t argc,
                                ejsval *args) {
    int exit_status = 0;
...
static ejsval _ejs_Process_chdir(ejsval env, ejsval _this, uint32_t argc,
                                 ejsval *args) {
...
static ejsval _ejs_Process_cwd(ejsval env, ejsval _this, uint32_t argc,
                               ejsval *args) {
    char cwd[MAXPATHLEN];
...
void _ejs_process_init(ejsval global, uint32_t argc, char **argv) {
    ...
    for (int i = 0; i < argc; i++)
        _ejs_object_setprop(_argv, NUMBER_TO_EJSVAL(i),
                            _ejs_string_new_utf8(argv[i]));

#define OBJ_PROP(x)                                                            \
    EJS_INSTALL_ATOM_GETTER(_ejs_Process, x, _ejs_Process_get_##x)
#define OBJ_METHOD(x)                                                          \
    EJS_INSTALL_ATOM_FUNCTION(_ejs_Process, x, _ejs_Process_##x)
