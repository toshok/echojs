...
| types-bench3 | the P4.6 polymorphic microbenchmark: the bench2 kernel with two receiver classes ({x,y} / {z,x,y}) alternating at one site — the 2-way guard chain (`shapePolyGuards=4`) runs 0.31s, PARITY with the monomorphic twin, vs 1.67s declined (-fno-poly-shape-guards) and 3.64s flag-off (2026-07-24, M-series) | 4 (poly) | match |
