...
describe ('LLVM bindings', function() {
	...
	it('llvm.FunctionType.get()', function() {
	    ...
	    assert.equal ('i8 (i8)*', llvm.FunctionType.get(int8ty, [int8ty]).pointerTo().toString());
	});
    });

    describe ('Constant', function() {
	it('llvm.Constant.getIntegerValue()', function() {
	    assert.equal ('i8 0', llvm.Constant.getIntegerValue(llvm.Type.getInt8Ty(), 0).toString());
	    assert.equal ('i32 512', llvm.Constant.getIntegerValue(llvm.Type.getInt32Ty(), 512).toString());
	});
    });
	...
	it('llvm.IRBuilder.createAlloca()', function() {
	    assert.equal ('%local = alloca i8', llvm.IRBuilder.createAlloca(llvm.Type.getInt8Ty(), 'local'));
	});
    });
	...
	it('should add an extern reference to the module', function() {
	    var module = new llvm.Module('test_module');
	    module.getOrInsertExternalFunction('puts', llvm.Type.getInt32Ty(), [llvm.Type.getInt8Ty().pointerTo()]);
	...
	it('llvm.IRBuilder#createCall', function() {
	    var module = new llvm.Module('test_module');
	    var puts = module.getOrInsertExternalFunction('puts', llvm.Type.getInt32Ty(), [llvm.Type.getInt8Ty().pointerTo()]);

	    var puts_call = llvm.IRBuilder.createCall(puts, [llvm.Constant.getNull(llvm.Type.getInt8Ty().pointerTo())], "puts_rv");
