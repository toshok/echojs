    ...
    visitFunction (n) {
        ...
        if (n.params.length > 0 && n.params[n.params.length - 1].type == b.RestElement) {
            ...
            if (rest_argument.type !== b.Identifier)
                reportError(Error, "we assume rest elements are of the form: ...Identifier", this.filename, rest_argument.argument.loc);
            let rest_name = rest_argument.name;
            let rest_declaration = b.letDeclaration(b.identifier(rest_argument.name), b.callExpression(arrayFromRest_id, [b.literal(rest_argument.name), b.literal(n.params.length)]));
