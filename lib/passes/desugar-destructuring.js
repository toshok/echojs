...
function createObjectPatternBindings(id, pattern, bindings) {
    for (let prop of pattern.properties) {
        ...
        if (prop.value.type === b.Identifier) {
            ...
            } else bindings.push({ key: prop.value, value: memberexp });
...
function createArrayPatternBindingsUsingIterator(id, pattern, bindings) {
    ...
    bindings.push({
        key: iter_id,
        value: b.callExpression(b.memberExpression(id, Symbol_iterator, true), []),
        need_decl: true,
    });
    bindings.push({
        key: wrapper_id,
    ...
    for (let el of pattern.elements) {
        ...
        if (el == null) {
            bindings.push({
                key: fresh() /*unused*/,
                value: b.callExpression(b.memberExpression(wrapper_id, getNextValue_id), []),
            });
            ...
            bindings.push({
                key: el.argument,
                value: b.callExpression(b.memberExpression(wrapper_id, getRest_id), []),
            });
            seen_spread = true;
        } else throw new Error(`createArrayPatternBindingsUsingIterator ${el.type}`);
    }
}
    ...
    visitVariableDeclaration(n) {
        ...
        for (let decl of n.declarations) {
            if (decl.id.type === b.ObjectPattern) {
                ...
                reportError(
                    Error,
                    `unhandled type of variable declaration in DesugarDestructuring ${decl.id
                        .type}`,
                    this.filename,
                    n.loc
                );
