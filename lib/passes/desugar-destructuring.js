...
export class DesugarDestructuring extends TransformPass {
    ...
    desugarForHead(n) {
        let head = n.left;
        let bindStmt = null;
        if (head.type === b.VariableDeclaration) {
            let d = head.declarations[0];
            if (head.declarations.length === 1 && d.id.type !== b.Identifier) {
                let tmp = fresh();
                let inner = b.variableDeclaration(head.kind, d.id, tmp);
                bindStmt = this.visit(inner);
                n.left = b.letDeclaration(tmp, null);
                // strip the placeholder init: a for-of/for-in head
                // declaration has no initializer
                n.left.declarations[0].init = null;
            }
        } else if (head.type !== b.Identifier) {
            // ObjectPattern/ArrayPattern assignment form, or a member
            // expression target
            let tmp = fresh();
            let assign = b.expressionStatement(b.assignmentExpression(head, "=", tmp));
        ...
        if (bindStmt) {
            let stmts = Array.isArray(bindStmt) ? bindStmt : [bindStmt];
    ...
    visitCatchClause(n) {
        if (n.param && n.param.type !== b.Identifier) {
            let tmp = fresh();
            let bindDecl = this.visit(b.letDeclaration(n.param, tmp));
            n.param = tmp;
            n.body = this.visit(n.body);
    ...
    visitFunction(n) {
        ...
        if (n.body.type === b.BlockStatement) {
            n.body.body = new_decls.concat(n.body.body);
        } else if (new_decls.length > 0) {
            n.body = b.blockStatement(new_decls.concat([b.returnStatement(n.body)]));
            n.expression = false;
        }
        n.params = new_params;
        n.body = this.visit(n.body);
