...
import { Identifier, ComputedPropertyKey } from '../ast-builder';
import { superid, setPrototypeOf_id } from '../common-ids';
...
function createSuperReference (is_static, id) {
    let obj = is_static ? superid : b.memberExpression(superid, b.identifier('prototype'));
...
class DesugarClasses extends TransformPass {
        constructor () {
                super();
        ...
        visitCallExpression (n) {
            if (n.callee.type === Identifier && n.callee.name === "super")
                n.callee = createSuperReference(this.method_stack.top.static, this.method_stack.top.key);
            else
        ...
        generateClassIIFE (n) {
            ...
            methods.forEach ((m, mkey) => {
                ...
                if (mkey === 'constructor') {
                    ctor = m;
                    let ctor_func = this.create_constructor(m, n);
                    ...
                    if (n.superClass) {
                        ...
                        let l = b.memberExpression(ctor_func.id,b.identifier("prototype"));
                        let r = b.callExpression(objectCreate_id, [b.memberExpression(superid, b.identifier("prototype"))]);
	    ...
	    if (static_props)
            ...
            if (!ctor)
                class_init_iife_body.unshift(this.create_default_constructor(n));
        ...
        create_default_constructor (ast_class) {
            ...
            return b.functionDeclaration(ast_class.id, [], b.blockStatement(), [], b.identifier('args'));
	}
                
        create_proto_method (ast_method, ast_class) {
            let proto_member = b.memberExpression(b.memberExpression(ast_class.id, b.identifier('prototype')), (ast_method.key.type === ComputedPropertyKey ? ast_method.key.expression : ast_method.key), ast_method.key.type === ComputedPropertyKey);
	    ...
	    let target;
            ...
            return b.expressionStatement(b.callExpression(b.memberExpression(b.identifier("Object"), b.identifier("defineProperties")), [target, propdescs_literal]));
	}
}
