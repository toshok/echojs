...
import { arrayFromSpread_id, setConstructorKindDerived_id, setConstructorKindBase_id, superid, constructSuper_id, constructSuperApply_id, setPrototypeOf_id, objectCreate_id, call_id, prototype_id, constructor_id, get_id, set_id, Object_id, proto_id, defineProperty_id, enumerable_id, value_id, defineProperties_id } from '../common-ids';
                ...
                if (class_element.kind === "set") {
                    if (class_element.value.params.length > 0) {
                        ...
                        if (last_param.type == b.RestElement)
                            reportError(SyntaxError, "Setters aren't allowed to have a rest", this.filename, last_param.loc);
                    }
                }
