...
export class DesugarClasses extends TransformPass {
    ...
    create_proto_method (ast_method, ast_class) {
        let proto_member = b.memberExpression(b.identifier('proto'),
                                              ast_method.key.type === b.ComputedPropertyKey ? ast_method.key.expression : ast_method.key,
                                              ast_method.key.type === b.ComputedPropertyKey);
    ...
    create_static_method (ast_method, ast_class) {
        ...
        return b.expressionStatement(b.assignmentExpression(b.memberExpression(ast_class.id, (ast_method.key.type === b.ComputedPropertyKey ? ast_method.key.expression : ast_method.key), ast_method.key.type === b.ComputedPropertyKey), "=", method));
    }
