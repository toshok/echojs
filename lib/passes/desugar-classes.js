...
function createSuperReference(is_static, id) {
    if (id && id.name === "constructor") return superid;
...
export class DesugarClasses extends TransformPass {
    ...
    generateClassIIFE(n) {
        ...
        class_init_iife_body.push(
            b.letDeclaration(b.identifier("proto"), b.memberExpression(n.id, prototype_id))
        );
        ...
        if (n.superClass) {
            class_init_iife_body.unshift(
                b.expressionStatement(
                    b.assignmentExpression(
                        b.memberExpression(b.memberExpression(n.id, prototype_id), constructor_id),
                        "=",
                        n.id
                    )
                )
            ...
            class_init_iife_body.unshift(
                b.expressionStatement(
                    b.callExpression(setPrototypeOf_id, [
                        b.memberExpression(ctor_func.id, prototype_id),
                        b.callExpression(objectCreate_id, [
                            b.memberExpression(superid, prototype_id),
                        ]),
                    ])
                )
            );
            ...
            class_init_iife_body.unshift(
                b.expressionStatement(b.callExpression(setPrototypeOf_id, [ctor_func.id, superid]))
            );

            class_init_iife_body.unshift(
                b.expressionStatement(intrinsic(setConstructorKindDerived_id, [ctor_func.id]))
            );
        } else {
            class_init_iife_body.unshift(
                b.expressionStatement(intrinsic(setConstructorKindBase_id, [ctor_func.id]))
            );
        }
    ...
    create_default_constructor(ast_class) {
        ...
        let functionBody = b.blockStatement(
            ast_class.superClass
                ? [b.expressionStatement(intrinsic(constructSuperApply_id, [superid, args_id]))]
                : []
        );
    ...
    create_proto_method(ast_method, ast_class) {
        ...
        return b.expressionStatement(
            b.callExpression(Object_defineProperty, [proto_id, method_key, defineProperty_args])
        );
    }
    ...
    create_static_method(ast_method, ast_class) {
        ...
        return b.expressionStatement(
            b.callExpression(Object_defineProperty, [ast_class.id, method_key, defineProperty_args])
        );
    }
    ...
    create_properties(properties, ast_class, are_static) {
        ...
        let target;
        if (are_static) target = ast_class.id;
