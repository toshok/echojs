...
export class IIFEIdioms extends TransformPass {
    ...
    maybeInlineIIFE(candidate, n) {
        ...
        for (let i = 0; i < arity; i++) {
            replacement.body.push(
                b.letDeclaration(
                    candidate.arguments[0].arguments[1].params[i],
                    i < arg_count ? candidate.arguments[i + 1] : b.undefinedLit()
                )
            );
        }
        ...
        this.function_stack.top.scratch_size = Math.max(
            ...
            candidate.arguments[0].arguments[1].scratch_size
        );
        ...
        if (is_intrinsic(n.expression, "%setSlot")) {
            ...
            is_intrinsic(n.expression, "%setGlobal") ||
            is_intrinsic(n.expression, "%setLocal")
        ) {
    ...
    maybeInlineIIFECall(candidate, n) {
        if (candidate.arguments.length !== 2 || candidate.arguments[1].type !== b.ThisExpression)
            return n;
    ...
    visitExpressionStatement(n) {
        let isMakeClosure = (a) =>
            is_intrinsic(a, "%makeClosure") ||
            is_intrinsic(a, "%makeAnonClosure") ||
            is_intrinsic(a, "%makeClosureNoEnv");

        let candidate;
        // bail out early if we know we aren't in the right place
        if (is_intrinsic(n.expression, "%invokeClosure")) candidate = n.expression;
        else if (is_intrinsic(n.expression, "%setSlot")) candidate = n.expression.arguments[2];
        else if (
            is_intrinsic(n.expression, "%setGlobal") ||
            is_intrinsic(n.expression, "%setLocal")
        )
        ...
        if (isMakeClosure(candidate.arguments[0])) {
            ...
            candidate.arguments[0].type === b.MemberExpression &&
            isMakeClosure(candidate.arguments[0].object) &&
            candidate.arguments[0].property.name === "call"
        ) {
