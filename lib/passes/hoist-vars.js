...
export class HoistVars extends TransformPass {
    ...
    visitVariableDeclaration(n) {
        ...
        n.declarations.forEach((decl) => {
            if (decl.init)
                assignments.push(
                    b.assignmentExpression(b.identifier(decl.id.name), "=", this.visit(decl.init))
                );
        });
        ...
        for (let decl of n.declarations) {
            if (this.scope_stack.top.vars.has(decl.id.name))
                reportWarning(
                    `multiple var declarations for '${decl.id.name}' in this function.`,
                    this.filename,
                    n.loc
                );
        ...
        // declaration node.
        if (assignments.length > 1) assign_exp = b.sequenceExpression(assignments);
        else assign_exp = assignments[0];

        if (this.skipExpressionStatement) return assign_exp;
        else return b.expressionStatement(assign_exp);
    }
}
