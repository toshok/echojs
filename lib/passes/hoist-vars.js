...
function create_empty_declarator (decl_name) {
    return b.variableDeclarator(b.identifier(decl_name), b.nullLit());
}
...
export class HoistVars extends TransformPass {
    ...
    visitProgram (n) {
        ...
        n.body.unshift(b.letDeclaration(vars.keys().map( (varname) => create_empty_declarator(varname))));
        return n;
    }
    ...
    visitFunction (n) {
        ...
        n.body.body.unshift(b.letDeclaration(vars.keys().map( (varname) => create_empty_declarator(varname))));
        return n;
    }
    ...
    visitVariableDeclaration (n) {
        if (n.kind !== "var") return super(n); // we only need to do this for var decls.
