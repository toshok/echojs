...
export class DesugarLetLoopVars extends TransformPass {
    ...
    visitFor(n) {
        ...
        for (let loopvar in mappings) {
            ...
            // loop iteration
            new_body.body.push(
                b.variableDeclaration("let", b.identifier(loopvar), mappings[loopvar])
            );
            ...
            assignments.push(
                b.expressionStatement(
                    b.assignmentExpression(mappings[loopvar], "=", b.identifier(loopvar))
                )
            );
        }
    ...
    // for (let k in o) body   →   for (var %loop_k in o) { let k = %loop_k; body }
    //
    // unlike visitFor there's no copy-back through a finally: for-in
    // creates a fresh binding each iteration (nothing carries over), so a
    // fresh `let` initialized from the hoisted var is the whole story.
    // without this, closures created in the body all shared one binding
    // and saw the last key.
    visitForIn(n) {
        n.right = this.visit(n.right);
        if (
            n.left.type !== b.VariableDeclaration ||
            n.left.kind === "var" ||
        ...
        let kind = n.left.kind; // let or const
        let decl = n.left.declarations[0];
        let orig = decl.id;
        let loopvar = b.identifier(freshLoopVar(orig.name));
        decl.id = loopvar;
        n.left.kind = "var";

        let new_body = b.blockStatement();
        new_body.body.push(b.variableDeclaration(kind, orig, loopvar));
        new_body.body.push(n.body);
        n.body = this.visit(new_body);
...
class RemapIdentifiers extends TransformPass {
    ...
    visitObjectPattern(n) {
        for (let prop of n.properties) this.currentMapping()[prop.key] = null;
        super.visitObjectPattern(n);
    }
    ...
    visitFunction(n) {
        if (n.id) this.currentMapping()[n.id.name] = null;

        this.mappings.push(shallow_copy_object(this.currentMapping()));
        if (n.rest) this.currentMapping()[n.rest.name] = null;
