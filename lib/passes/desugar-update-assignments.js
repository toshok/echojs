...
export class DesugarUpdateAssignments extends TransformPass {
    ...
    visitProgram(n) {
        ...
        if (n.prepends.length > 0) n.body = n.prepends.concat(n.body);
        return n;
    }
    ...
    visitAssignmentExpression(n, parentBlock) {
        ...
        if (n.operator.length === 1) return n;

        if (n.left.type === b.Identifier) {
            // for identifiers we just expand a += b to a = a + b
            // strip the trailing '=' ("<<=" -> "<<", not "<")
            n.right = b.binaryExpression(n.left, n.operator.slice(0, -1), n.right);
            n.operator = "=";
            return n;
        }
        ...
        if (n.left.type === b.MemberExpression) {
            ...
            n.right = b.binaryExpression(
                b.memberExpression(n.left.object, n.left.property, n.left.computed),
                n.operator.slice(0, -1),
                n.right
            );
            n.operator = "=";
        ...
        reportError(
            Error,
            `unexpected expression type ${n.left.type} in update assign expression.`,
            this.filename,
            n.left.loc
        );
    }
