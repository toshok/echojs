...
function is_getset_intrinsic(n) {
    ...
    if (
        n.callee.name === slot_id.name ||
        n.callee.name === getLocal_id.name ||
        n.callee.name === getGlobal_id.name
    )
        return true;
    if (
        n.callee.name === setSlot_id.name ||
        n.callee.name === setLocal_id.name ||
        n.callee.name === setGlobal_id.name
    )
        return true;
    return false;
}
...
class Scope {
    ...
    getReferents(name) {
        return this.referents.get(name);
    }
    hasReferents(name) {
        return this.referents.has(name);
    }
    ...
    isFunctionBodyScope() {
        return this.location.block === this.location.func.body;
    }
    isAncestorOf(s) {
    ...
    differentFunction(otherscope) {
        return this.location.func !== otherscope.location.func;
    }

    debugString() {
        let str = `scope `;
        if (this.location.block.loc) str = `#{str} at line ${this.location.block.loc.start.line}`;
        if (this.isFunctionBodyScope()) str = `${str} : for function ${this.location.func.id.name}`;
        if (this.env) str = `${str} : environment = ${this.env.name}`;
        return str;
    }
}
...
class Environment {
    ...
    hasSlots() {
        return this.slot_map.size > 0;
    }
    hasSlot(name) {
        return this.slot_map.has(name);
    }
    getSlot(name) {
    ...
    addChild(env) {
        ...
        if (env.parentEnv && env.parentEnv !== this)
            throw new Error(
                `attempting to set parent of ${env.name} to ${this.name}, but it already has a a parent, ${env.parentEnv.name}`
            );
...
class SubstituteVariables extends TransformPass {
    ...
    env_name() {
        return this.current_scope.env.name;
    }
    env_slot(name) {
        return this.current_scope.env.getSlot(name);
    }
            ...
            for (let referent of referents) {
                if (referent.referencingScope.differentFunction(this.current_scope)) {
                    ...
                    return b.expressionStatement(
                        setSlotIntrinsic(
                            this.env_name(),
                            this.env_slot(decl.id.name),
                            decl.init || b.undefinedLit()
                        )
                    );
                }
            }
    ...
    visitCallExpression(n) {
        ...
        if (is_intrinsic(n)) {
            ...
            if (is_intrinsic(n, constructSuperApply_id.name))
                this.current_scope.location.func.scratch_size = Math.max(
                    this.current_scope.location.func.scratch_size,
    ...
    visitFunction(n) {
        ...
        if (n.id) {
            ...
            if (n.params[0].name === "%env_unused") intrinsic_id = makeClosureNoEnv_id;
            else intrinsic_args.push(b.identifier(n.params[0].name, n.loc));

            if (n.id.type === b.Identifier) intrinsic_args.push(b.literal(n.id.name));
            else intrinsic_args.push(b.literal(escodegen.generate(n.id)));
        } else {
            intrinsic_id = makeAnonClosure_id;
            if (n.params[0].name === "%env_unused") intrinsic_args.push(b.undefinedLit());
            else intrinsic_args.push(b.identifier(n.params[0].name, n.loc));
        }
        ...
        if (referents) {
            ...
            if (ref.binding.type === "local" || ref.binding.type === "arg") {
                ...
                if (declaringEnv && declaringEnv.hasSlot(leftname)) {
                    let rv = setSlotIntrinsic(
                        declaringEnv.name,
                        declaringEnv.getSlot(leftname),
                        rhs
                    );
                    rv.loc = n.loc;
                    return rv;
                ...
                if (leftname === "undefined")
                    reportError(
                        SyntaxError,
                        "reassigning 'undefined' not permitted.",
                        this.filename,
                        n.loc
                    );
                ...
                // rhs was already visited above; visiting it again
                // double-wraps closures (and the second visitFunction
                // resets the inner function's scratch_size)
                let rv = ref.binding.getStoreIntrinsic(rhs);
                rv.loc = n.loc;
                return rv;
            } else {
    ...
    visitIdentifier(n) {
        ...
        if (referents) {
            ...
            if (binding.type === "local" || binding.type === "arg") {
                ...
                if (export_info.constval) return export_info.constval;
    ...
    visitMemberExpression(n) {
        ...
        if (!is_intrinsic(n.object, "%moduleGetExotic")) return n;
        if (n.property.type !== b.Identifier && !is_string_literal(n.property)) return n;
...
class FlattenDeclarations extends TransformPass {
    ...
    visitBlock(n) {
        ...
        let new_body = [];
        for (let s of n.body) {
            if (decl_map.has(s)) new_body = new_body.concat(decl_map.get(s));
            else new_body.push(s);
        }
        n.body = new_body;
        return n;
    }
    ...
    visitVariableDeclaration(n, decl_map) {
        if (n.declarations.length == 1) return super.visitVariableDeclaration(n, decl_map);

        let decl_replacement = [];
        for (let decl of n.declarations)
            decl_replacement.push(
                b.variableDeclaration(
                    n.kind,
                    decl.id,
                    decl.init ? this.visit(decl.init) : b.undefinedLit()
                )
            );
...
class CollectScopeNestingInfo extends TransformPass {
    ...
    doWithScope(scope, fn) {
        ...
        if (this.current_scope) this.current_scope.children.push(scope);
            ...
            if (s.type === b.VariableDeclaration) {
                ...
                if (d.init) {
                    if (is_intrinsic(d.init, "%moduleGetSlot")) {
                        for_scope.addBinding(
                            new ModuleSlotBinding(
                                d.init.arguments[0],
                                d.init.arguments[1],
                                d.id.name
                            )
                        ...
                        for_scope.addBinding(
                            new ModuleExoticBinding(d.init.arguments[0], d.id.name)
                        );
                    } else {
                ...
                is_intrinsic(s.expression, "%moduleSetSlot")
            ) {
                let args = s.expression.arguments;
                // a re-export (`export { X }` where X is an import) emits a
                // moduleSetSlot whose name is already bound to the imported
                // module's slot.  registering our own module's binding over
                // it would make every reference to X -- including the
                // setSlot's own right-hand side -- read our uninitialized
                // slot instead of the import.
                let existing = for_scope.getBinding(args[1].value);
    ...
    visitBlock(n, initial_bindings) {
        ...
        if (this.root_scope === null) this.root_scope = this_scope;

        if (initial_bindings) for (let binding of initial_bindings) this_scope.addBinding(binding);
    ...
    function collapse_paths() {
        ...
        func_to_envs.forEach((func_envs, func) => {
            ...
            for (let idx = func_envs.length - 1; idx >= 1; idx--) {
                ...
                if (
                    !parent_envs.has(current_e) ||
                    parent_envs.get(current_e).level < prospective_parent.level
                ) {
    ...
    function walk_scope2(s, level) {
        ...
        if (s.env) {
            if (s.env.parentEnv && s.env.parentEnv.slotCount() > 0) {
                ...
                s.location.block.body.unshift(
                    b.expressionStatement(
                        setSlotIntrinsic(
                            s.env.name,
                            s.env.getSlot(s.env.parentEnv.name),
                            b.identifier(s.env.parentEnv.name)
                        )
                    )
                );
            }
            s.location.block.body.unshift(
                b.letDeclaration(
                    b.identifier(s.env.name),
                    intrinsic(makeClosureEnv_id, [b.literal(s.env.slot_map.size)])
                )
            );
        } else {
        ...
        if (s.isFunctionBodyScope()) {
            ...
            if (func_envs.length === 0) {
                ...
                while (last_idx >= 0) {
                    ...
                    let env_decl = b.constDeclaration(
                        b.identifier(func_envs[last_idx].name),
                        slotIntrinsic(
                            func_envs[last_idx + 1].name,
                            func_envs[last_idx + 1].getSlot(func_envs[last_idx].name)
                        )
                    );
        ...
        if (is_intrinsic(n, makeClosure_id.name) || is_intrinsic(n, makeAnonClosure_id.name)) {
            ...
            if (!is_undefined_literal(n.arguments[0])) {
                ...
                if (closure_env !== func_env) {
                    throw new Error(
                        `closure created using environment ${closure_env}, while function takes ${func_env}`
                    );
                }
            }
...
export class NewClosureConvert {
    ...
    visit(tree) {
        ...
        let collectScopes = new CollectScopeNestingInfo(
            this.options,
            this.filename,
            this.allModules
        );
        let substituteVariables = new SubstituteVariables(
            this.options,
            this.filename,
            this.allModules
        );
