...
class SubstituteVariables extends TransformPass {
    ...
    visitAssignmentExpression(n) {
        ...
        if (referents) {
            ...
            if (ref.binding.type === 'local' || ref.binding.type === 'arg') {
                ...
                return ref.binding.getStoreIntrinsic(this.visit(rhs));
            }
            else {
    ...
    visitMemberExpression (n) {
        ...
        if (!is_intrinsic(n.object, "%moduleGetExotic") || n.computed)
            return n;
        if (n.property.type !== Identifier && !is_string_literal(n.property))
            return n;

        let moduleString = n.object.arguments[0];
        let moduleExport = n.property.type === Identifier ? n.property.name : n.property.raw;
        ...
        return intrinsic(moduleGetSlot_id, [moduleString, b.literal(moduleExport)]);
    }
...
class CollectScopeNestingInfo extends TransformPass {
    ...
    createBindingsForScope(block, for_scope) {
        for (let s of block.body) {
            ...
            else if (s.type === ExpressionStatement && is_intrinsic(s.expression, "%moduleSetSlot")) {
