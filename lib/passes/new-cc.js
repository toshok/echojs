...
const superid                 = b.identifier("%super");
const makeClosureEnv_id       = b.identifier("%makeClosureEnv");
const makeClosure_id          = b.identifier("%makeClosure");
const makeAnonClosure_id      = b.identifier("%makeAnonClosure");
const setSlot_id              = b.identifier("%setSlot");
const slot_id                 = b.identifier("%slot");
const invokeClosure_id        = b.identifier("%invokeClosure");
const setLocal_id             = b.identifier("%setLocal");
const setGlobal_id            = b.identifier("%setGlobal");
const getLocal_id             = b.identifier("%getLocal");
const getGlobal_id            = b.identifier("%getGlobal");
const getArg_id               = b.identifier("%getArg");
const moduleGet_id            = b.identifier("%moduleGet");
const moduleImportBatch_id    = b.identifier("%moduleImportBatch");
const templateCallsite_id     = b.identifier("%templateCallsite");
const templateHandlerCall_id  = b.identifier("%templateHandlerCall");
const templateDefaultHandlerCall_id = b.identifier("%templateDefaultHandlerCall");
const createArgScratchArea_id       = b.identifier("%createArgScratchArea");
const setPrototypeOf_id       = b.identifier("%setPrototypeOf");
const objectCreate_id         = b.identifier("%objectCreate");
const gatherRest_id           = b.identifier("%gatherRest");
const arrayFromSpread_id      = b.identifier("%arrayFromSpread");
const argPresent_id           = b.identifier("%argPresent");
...
class SubstituteVariables extends TransformPass {
        ...
        if (referents) {
            for (let referent of referents) {
                if (referent.referencingScope.differentFunction(this.current_scope)) {
                    ...
                    return b.expressionStatement(setSlotIntrinsic(this.env_name(), this.env_slot(decl.id.name), decl.init || b.undefined()));
                }
            }
        }
        return n;
    ...
    visitCallExpression (n) {
        ...
        n = super(n);
        this.current_scope.location.func.scratch_size = Math.max(this.current_scope.location.func.scratch_size, n.arguments.length);
    ...
    visitFunction(n) {
        ...
        let intrinsic_args = [n.params[0].name === "%env_unused" ? b.undefined() : b.identifier(n.params[0].name, n.loc)];
    ...
    visitAssignmentExpression(n) {
        if (n.left.type !== Identifier) return super(n);
        ...
        if (referents) {
            for (let referent of referents) {
                if (referent.referencingScope.differentFunction(this.current_scope)) {
                    // it's closed over, so we need to set it in our allocated environment
                    return setSlotIntrinsic(this.env_name(), this.env_slot(leftname), rhs);
                }
            }
        }
    ...
    visitObjectExpression(n) {
        for (let property of n.properties) {
            if (property.key.type === ComputedPropertyKey)
...
class FlattenDeclarations extends TransformPass {
    ...
    visitVariableDeclaration(n, decl_map) {
        if (n.declarations.length == 1) return super(n,decl_map)

        let decl_replacement = [];
        for (let decl in n.declarations)
            decl_replacement.push(b.variableDeclaration(n.kind, decl.id, decl.init ? this.visit(decl.init) : b.undefined()));
        ...
        for (let s of n.body) {
            if (s.type === VariableDeclaration) {
                for (let d of s.declarations)
                    this_scope.addBinding(new LocalBinding(d.id.name, s.kind === 'const'));
            }
...
function placeEnvironments(root_scope) {
    ...
    function collapse_paths() {
        ...
        func_to_envs.forEach ((func_envs, func) => {
            ...
            let func_envs = func_envs.filter((a_env) => a_env);
...
function is_undefined_literal(e) {
    if (e.type === Literal && e.value === undefined) return true;
    return e.type === UnaryExpression && e.operator === "void" && e.argument.value === 0;
}
...
export class NewClosureConvert {
    ...
    visit(tree) {
        let flattenDecls = new FlattenDeclarations(this.options, this.filename);
        let collectScopes = new CollectScopeNestingInfo(this.options, this.filename);
        let substituteVariables = new SubstituteVariables(this.options, this.filename);
