 ...
 */
import { ComputedPropertyKey, BlockStatement } from '../ast-builder';
...
export class LocateEnv extends TransformPass {
    ...
    visitFunction (n) {
        ...
        let new_env = { exp: n, id: this.env_id, decls: n.ejs_decls ? n.ejs_decls : new Set(), closed: new Set(), parent: current_env };
        this.env_id ++;
        n.ejs_env = new_env;
    ...
    visitIdentifier (n) {
        ...
        if (env.decls && env.decls.has(n.name)) return n;

        let closed_over = false;
        // look up our environment stack for the decl for it.
        env = env.parent;
        let intervening_function = false;
        while (env) {
            if (env.decls && env.decls.has(n.name)) {
                ...
                if (env.exp.type !== BlockStatement) {
                    // if it's a function
