...
export class EqIdioms extends TreeVisitor {
    visitBinaryExpression(exp) {
        ...
        if (
            ...
            switch (typecheck) {
                case "object":
                    intrinsic = typeofIsObject_id;
                    break;
                case "function":
                    intrinsic = typeofIsFunction_id;
                    break;
                case "string":
                    intrinsic = typeofIsString_id;
                    break;
                case "symbol":
                    intrinsic = typeofIsSymbol_id;
                    break;
                case "number":
                    intrinsic = typeofIsNumber_id;
                    break;
                case "boolean":
                    intrinsic = typeofIsBoolean_id;
                    break;
                case "null":
                    intrinsic = isNull_id;
                    break;
                case "undefined":
                    intrinsic = isUndefined_id;
                    break;
                default:
            ...
            if (exp.operator[0] === "!") rv = b.unaryExpression("!", rv);
            return rv;
        }
        ...
        if (exp.operator === "==" || exp.operator === "!=") {
            if (
                ...
                is_undefined_literal(left) ||
                is_undefined_literal(right)
            ) {
                ...
                if (exp.operator === "!=") rv = b.unaryExpression("!", rv);
                return rv;
            }
        }
