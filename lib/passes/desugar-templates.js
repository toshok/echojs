...
export class DesugarTemplates extends TransformPass {
    visitBlock(n, callsites) {
        callsites = [];
        n = super.visitBlock(n, callsites);
    ...
    generateCreateCallsiteIdFunc(name, quasis, loc) {
        ...
        let raw = b.arrayExpression(raw_elements);
        let cooked = b.arrayExpression(cooked_elements);

        return b.functionDeclaration(
            b.identifier(`generate_${name}`),
            [],
            b.blockStatement(
                [
                    b.expressionStatement(
                        intrinsic(templateCallsite_id, [b.literal(name), raw, cooked])
                    ),
                ],
                loc
            ),
    ...
    visitTaggedTemplateExpression(n, callsites) {
        // visit the tag and the substitution expressions first: they can
        // contain templates of their own (e.g. inside arrow functions),
        // which would otherwise survive this pass undesugared.
        n.tag = this.visit(n.tag);
        n.quasi.expressions = n.quasi.expressions.map((e) => this.visit(e));

        let callsiteid_func_id = freshCallsiteId();
        let callsite_func = this.generateCreateCallsiteIdFunc(
            callsiteid_func_id,
            n.quasi.quasis,
            n.loc
        );
    ...
    visitTemplateLiteral(n) {
        // see visitTaggedTemplateExpression: substitutions must be visited
        let expressions = n.expressions.map((e) => this.visit(e));
        let cooked = b.arrayExpression(n.quasis.map((q) => b.literal(q.value.cooked)));
        let substitutions = b.arrayExpression(expressions);
