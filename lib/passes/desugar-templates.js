...
export class DesugarTemplates extends TransformPass {
    ...
    generateCreateCallsiteIdFunc (name, quasis, loc) {
        ...
        return b.functionDeclaration(b.identifier(`generate_${name}`), [],
                                     b.blockStatement([b.expressionStatement(intrinsic(templateCallsite_id, [b.literal(name), raw, cooked]))], loc), [], null, loc);
    }
    
    visitTaggedTemplateExpression (n, callsites) {
        let callsiteid_func_id = freshCallsiteId();
        let callsite_func = this.generateCreateCallsiteIdFunc(callsiteid_func_id, n.quasi.quasis, n.loc);
