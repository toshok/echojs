...
export class SubstituteVariables extends TransformPass {
    ...
    visitVariableDeclaration(n) {
        ...
        for (let decl of decls) {
            ...
            if (closed_over) {
                ...
                if (decl.init) {
                    // FIXME: we should also check for an explicit 'undefined' here
                    ...
                    rv.push(
                        b.expressionStatement(
                            b.assignmentExpression(
                                this.currentMapping()[decl.id.name],
                                "=",
                                decl.init
                            )
                        )
    ...
    visitProperty(n) {
        if (n.computed) n.key = this.visit(n.key);
        n.value = this.visit(n.value);
        return n;
    }

    visitBlock(n) {
        if (!n.ejs_env) return super.visitBlock(n);

        let this_env_id = b.identifier(`%env_${n.ejs_env.id}`);
        let parent_env_name;
        if (n.ejs_env.parent) parent_env_name = `%env_${n.ejs_env.parent.id}`;
        ...
        if (n.ejs_env.closed.empty() && !n.ejs_env.nested_requires_env) {
            ...
            env_prepends.push(
                b.letDeclaration(
                    this_env_id,
                    intrinsic(makeClosureEnv_id, [
                        b.literal(n.ejs_env.closed.size() + (n.ejs_env.parent ? 1 : 0)),
                    ])
                )
            );
            ...
            if (n.ejs_env.parent) {
                ...
                env_prepends.push(
                    b.expressionStatement(
                        intrinsic(setSlot_id, [
                            this_env_id,
                            b.literal(parent_env_slot),
                            b.literal(parent_env_name),
                            b.identifier(parent_env_name),
                        ])
                    )
                );
            }
            ...
            let prepend_environment = exps => {
                let obj = this_env_id;
                for (let prop of exps) obj = intrinsic(slot_id, [obj, prop]);
                return obj;
            };
            ...
            for (let mapped in new_mapping) {
                ...
                if (mapped !== "%slot_mapping")
                    new_mapping[mapped] = prepend_environment(
                        flatten_memberexp(val, n.ejs_env.slot_mapping)
                    );
            }
            ...
            n.ejs_env.closed.keys().forEach(sym => {
                new_mapping[sym] = intrinsic(slot_id, [
                    this_env_id,
                    b.literal(n.ejs_env.slot_mapping[sym]),
                    b.literal(sym),
                ]);
            });
        }
        ...
        if (n.ejs_decls) {
            new_mapping = reject(
                new_mapping,
                sym => n.ejs_decls.has(sym) && !n.ejs_env.closed.has(sym)
            );
        }

        this.mappings.push(new_mapping);
        super.visitBlock(n);
        if (env_prepends.length > 0) n.body = env_prepends.concat(n.body);
        this.mappings.pop();
        return n;
    }
    ...
    visitFunction(n) {
        try {
            ...
            if (n.ejs_env.closed.empty() && !n.ejs_env.nested_requires_env) {
                    ...
                    if (n.ejs_env.closed.has(param.name))
                        env_prepends.push(
                            b.expressionStatement(
                                intrinsic(setSlot_id, [
                                    this_env_id,
                                    b.literal(n.ejs_env.slot_mapping[param.name]),
                ...
                var flatten_memberexp = (exp, mapping) => {
                    if (exp.type !== CallExpression) {
                        ...
                        return flatten_memberexp(exp.arguments[0], mapping).concat([
                            exp.arguments[1],
                        ]);
                    }
                };
            ...
            if (env_prepends.length > 0) n.body.body = env_prepends.concat(n.body.body);
            this.mappings.pop();
            ...
            if (!n.toplevel) {
                if (n.type === FunctionDeclaration) {
                    ...
                    // n.type is FunctionExpression
                    let intrinsic_args = [];
                    intrinsic_args.push(
                        n.ejs_env.parent ? b.identifier(parent_env_name) : b.nullLit()
                    );
    ...
    visitCallExpression(n) {
        ...
        this.function_stack.top.scratch_size = Math.max(
            this.function_stack.top.scratch_size,
            n.arguments.length
        );
