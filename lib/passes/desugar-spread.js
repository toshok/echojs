...
import { arrayFromSpread_id, apply_id, constructSuperApply_id, constructApply_id } from "../common-ids";
...
export class DesugarSpread extends TransformPass {
    ...
    // both pipelines construct through the runtime's dense-array apply
    // (spread-new never compiled before)
    visitNewExpression(n) {
        n = super.visitNewExpression(n);
        if (!n.arguments.some((el) => el.type === b.SpreadElement)) return n;
        let chunks = spreadChunks(n.arguments);
