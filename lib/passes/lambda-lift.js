...
export class LambdaLift extends TransformPass {
        ...
        if (n.scratch_size > 0)
            n.body.body.unshift(
                b.expressionStatement(
                    intrinsic(createArgScratchArea_id, [b.literal(n.scratch_size)])
                )
            );
    }
    ...
    visitFunctionExpression(n) {
        let global_name;
        if (n.displayName) global_name = genGlobalFunctionName(n.displayName, this.filename);
        else if (n.id && n.id.name) global_name = genGlobalFunctionName(n.id.name, this.filename);
        else global_name = genAnonymousFunctionName(this.filename);
