...
function getModuleFile(module_info, platform) {
    if (typeof module_info.module_file == "string") return module_info.module_file;
    if (!module_info.module_file[platform])
        throw new Error(
            `module ${module_info.module_name} doesn't have a module file for platform ${platform}`
        );
    return module_info.module_file[platform];
}

function getModuleLinkFlags(module_info, platform) {
    if (typeof module_info.link_flags == "string") return module_info.link_flags;
    if (!module_info.link_flags[platform])
        throw new Error(
            `module ${module_info.module_name} doesn't have a link flags for platform ${platform}`
        );
...
function registerNativeModuleInfo(
    ...
    if (module_info.link_flags)
        link_flags = link_flags.concat(getModuleLinkFlags(module_info, platform));
    if (module_info.module_file)
        module_files = module_files.concat(getModuleFile(module_info, platform));
...
export function gatherAllModules(file_args, options) {
    ...
    gatherAllNativeModules(
        options.native_module_dirs,
        options.target_platform + "-" + options.target_arch
    );
    ...
    while (work_list.length !== 0) {
        ...
        let found = false;
        let jsfile = file;
        if (path.extname(jsfile) !== ".js") jsfile = jsfile + ".js";
