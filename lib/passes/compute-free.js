...
export class ComputeFree extends TransformPass {
    ...
    free_blocklike (exp,body) {
        ...
        if (body) {
            ...
            uses = setUnion.apply(null, body.map(this.call_free));
        }
        else {
            decls = [];
            uses = new Set();
    ...
    free (exp) {
        ...
        case Program: {
            ...
            let uses = setUnion.apply(null, exp.body.map(this.call_free));
            exp.ejs_decls = decls;
            exp.ejs_free_vars = uses.subtract(decls);
            break;
        }
        ...
        case TryStatement: {
            exp.ejs_free_vars = setUnion.apply(null, [this.free(exp.block)].concat(exp.handlers.map(this.call_free)));
            break;
        }
        case CatchClause: {
        ...
        case VariableDeclaration:
            exp.ejs_free_vars = setUnion.apply(null, exp.declarations.map(this.call_free));
            break;
        ...
        case ForStatement: {
            let decls = null;
            if (exp.init && exp.init.type === VariableDeclaration)
                decls = this.decl_names [exp.init];

            let uses = setUnion.call(null, this.free(exp.init), this.free(exp.test), this.free(exp.update), this.free(exp.body));
            exp.ejs_decls = decls;
            exp.ejs_free_vars = uses.subtract(decls);
            break;
        }
        case ForInStatement:
            exp.ejs_free_vars = setUnion.call(null, this.free(exp.left), this.free(exp.right), this.free(exp.body));
            break;
        case ForOfStatement:
            exp.ejs_free_vars = setUnion.call(null, this.free(exp.left), this.free(exp.right), this.free(exp.body));
            break;
        case WhileStatement:
            exp.ejs_free_vars = setUnion.call(null, this.free(exp.test), this.free(exp.body));
            break;
        case DoWhileStatement:
            exp.ejs_free_vars = setUnion.call(null, this.free(exp.test), this.free(exp.body));
            break;
        case SwitchStatement:
            exp.ejs_free_vars = setUnion.apply(null, [this.free(exp.discriminant)].concat(exp.cases.map(this.call_free)));
            break;
        case SwitchCase:
        ...
        case MemberExpression:
            ...
            break;
        case CallExpression:
            exp.ejs_free_vars = setUnion.apply(null, [this.free(exp.callee)].concat(exp.arguments.map(this.call_free)));
            break;
        case NewExpression:
            exp.ejs_free_vars = setUnion.apply(null, [this.free(exp.callee)].concat(exp.arguments.map(this.call_free)));
            break;
        case SequenceExpression:
            exp.ejs_free_vars = setUnion.apply(null, exp.expressions.map(this.call_free));
            break;
        case ConditionalExpression:
            exp.ejs_free_vars = setUnion.call(null, this.free(exp.test), this.free(exp.consequent), this.free(exp.alternate));
            break;
        ...
        case ObjectExpression:
            exp.ejs_free_vars = exp.properties.length === 0 ? new Set() : setUnion.apply(null, exp.properties.map(this.call_free));
            break;
        case ArrayExpression:
            exp.ejs_free_vars = exp.elements.length === 0 ? new Set() : setUnion.apply(null, exp.elements.map(this.call_free));
            break;
        case IfStatement:
            exp.ejs_free_vars = setUnion.call(null, this.free(exp.test), this.free(exp.consequent), this.free(exp.alternate));
            break;
