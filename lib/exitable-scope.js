...
export class ExitableScope {
    ...
    exitFore() {
        throw new Error("Exitable scope does not allow exitFore");
    }

    exitAft() {
        throw new Error("Exitable scope does not allow exitAft");
    }
...
export class TryExitableScope extends ExitableScope {
    ...
    getLandingPadBlock() {
        if (!this.landing_pad_block) this.landing_pad_block = this.create_landing_pad_bb();
        return this.landing_pad_block;
    }

    lookupDestinationIdForScope(scope, reason, label = null) {
        // the label rides along in the destination: when the destination
        // is ANOTHER finally scope (nested finallies — DesugarLetLoopVars
        // wraps every for-let body in one), its dispatch must re-exit
        // with the label or the continue/break lands on the innermost
        // loop instead of the labeled one
        for (let dest of this.destinations)
            if (dest.scope === scope && dest.reason === reason && dest.label === label)
                return dest.id;

        let id = consts.int32(this.destinations.length);
        this.destinations.unshift({ scope: scope, reason: reason, id: id, label: label });
        return id;
    }

    exitFore(label = null) {
        let scope;
        if (label) scope = LoopExitableScope.findLabeledOrFinally(label, this.parent);
        else scope = LoopExitableScope.findLoopOrFinally(this.parent);

        if (this.hasFinally) {
            let reason = this.lookupDestinationIdForScope(
                scope,
                TryExitableScope.REASON_CONTINUE,
                label
            );
            irbuilder.createStore(reason, this.cleanup_reason);
            irbuilder.createBr(this.cleanup_bb);
        } else {
            scope.exitFore(label);
        }
    }
    ...
    exitAft(fromBreak, label = null) {
        ...
        if (this.hasFinally) {
            let reason;
            if (fromBreak)
                reason = this.lookupDestinationIdForScope(
                    scope,
                    TryExitableScope.REASON_BREAK,
                    label
                );
            else reason = consts.int32(TryExitableScope.REASON_FALLOFF_TRY);

            irbuilder.createStore(reason, this.cleanup_reason);
            irbuilder.createBr(this.cleanup_bb);
        } else {
            if (fromBreak) scope.exitAft(fromBreak, label);
            else irbuilder.createBr(this.cleanup_bb);
        }
    }
}
TryExitableScope.REASON_FALLOFF_TRY = -2; // we fell off the end of the try block
TryExitableScope.REASON_EXCEPTION = -20; // an exception unwound into a finally-only try
...
export class LoopExitableScope extends ExitableScope {
    ...
    exitFore(label = null) {
        ...
        else irbuilder.createBr(this.fore_bb);
    }

    exitAft(fromBreak, label = null) {
        if (label && label !== this.label)
            LoopExitableScope.findLabeledOrFinally(label).exitAft(fromBreak, label);
        else irbuilder.createBr(this.aft_bb);
    }
...
export class LabeledStatementExitableScope extends ExitableScope {
    constructor(label, aft_bb) {
        super(label);
        this.aft_bb = aft_bb;
    }
    ...
    exitFore() {
        throw new Error("cannot continue this label");
    }
}
