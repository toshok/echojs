...
function isNotNull(n) {
    if (!n) throw new Error("assertion failed: value is null or undefined");
    return true;
}
function hasType(n) {
    if (!n.type)
        throw new Error(
            `assertion failed: value ${JSON.stringify(n)} does not have a 'type:' property`
        );
    return true;
}

function isast(n) {
    return isNotNull(n) && hasType(n) && n;
}

function isnullableast(n) {
    return (!n || hasType(n)) && n;
}

function isastarray(n) {
    if (!Array.isArray(n)) throw new Error("value must be an array");
    for (let el of n) isast(el);
    return n;
}

function isboolean(n) {
    if (typeof n !== "boolean") throw new Error("value must be a boolean");
    return n;
}

// esprima currently fails to parse arrow functions with default argument values, so we make those function expressions

export function arrayExpression(els = []) {
    return { type: ArrayExpression, elements: els };
}
export function arrowFunctionExpression(params, body, defaults = [], expression = false) {
    return {
        type: ArrowFunctionExpression,
        params: params.map(isast),
        defaults: defaults.map(isast),
        body: isast(body),
        expression: expression,
    };
}

export function assignmentExpression(l, op, r) {
    return { type: AssignmentExpression, operator: op, left: isast(l), right: isast(r) };
}
export function binaryExpression(l, op, r) {
    return { type: BinaryExpression, operator: op, left: isast(l), right: isast(r) };
}
export function blockStatement(stmts = [], loc = null) {
    return { type: BlockStatement, body: stmts.map(isast), loc: loc };
}
export function breakStatement(label) {
    return { type: BreakStatement, label: isast(label) };
}
export function callExpression(callee, args = []) {
    return { type: CallExpression, callee: isast(callee), arguments: args.map(isast) };
}
export function catchClause(param, body, guard) {
    return { type: CatchClause, body: body, param: isast(param), guard: isnullableast(guard) };
}
export function conditionalExpression(test, consequent, alternate) {
    return {
        type: ConditionalExpression,
        test: isast(test),
        consequent: isast(consequent),
        alternate: isast(alternate),
    };
}
export function continueStatement(label) {
    return { type: ContinueStatement, label: isnullableast(label) };
}
export function emptyStatement() {
    return { type: EmptyStatement };
}
export function expressionStatement(exp) {
    return { type: ExpressionStatement, expression: isast(exp) };
}
export function forInStatement(left, right, body) {
    return { type: ForInStatement, left: isast(left), right: isast(right), body: isast(body) };
}
export function forOfStatement(left, right, body) {
    return { type: ForOfStatement, left: isast(left), right: isast(right), body: isast(body) };
}
...
export function functionDeclaration(id, params, body, defaults = []) {
    return {
        type: FunctionDeclaration,
        id: isast(id),
        params: params.map(isast),
        body: isast(body),
        defaults: defaults.map(isnullableast),
        generator: false,
        expression: false,
    };
}
export function functionExpression(id, params, body, defaults = []) {
    return {
...
export function ifStatement(test, consequent, alternate) {
    return {
        ...
        test: isast(test),
        consequent: isast(consequent),
        alternate: isnullableast(alternate),
    };
}
export function labeledStatement(label, body) {
    return { type: LabeledStatement, label: isast(label), body: body.map(isast) };
}
export function literal(val) {
    return { type: Literal, value: val, raw: typeof val === "string" ? `\'${val}\'` : `${val}` };
}
export function logicalExpression(l, op, r) {
    return { type: LogicalExpression, left: isast(l), right: isast(r), operator: op };
}
export function memberExpression(obj, prop, computed = false) {
    return {
        type: MemberExpression,
        object: isast(obj),
        property: isast(prop),
        computed: isboolean(computed),
    };
}
export function metaProperty(meta, property) {
    return { type: MetaProperty, meta, property };
}
export function methodDefinition(key, value, kind = "init") {
    return { type: MethodDefinition, key: key, value: value, kind: kind };
}
export function objectExpression(properties) {
    return { type: ObjectExpression, properties: properties.map(isast) };
}
export function property(key, value, kind = "init", computed = false) {
    return {
        type: Property,
        key: isast(key),
        value: isast(value),
...
export function restElement(arg) {
    return { type: RestElement, argument: isast(arg) };
}
export function returnStatement(arg) {
    return { type: ReturnStatement, argument: isast(arg) };
}
export function sequenceExpression(expressions) {
    return { type: SequenceExpression, expressions: expressions.map(isast) };
}
...
export function switchCase(test, consequent) {
    return { type: SwitchCase, test: isnullableast(test), consequent: isastarray(consequent) };
}
export function thisExpression() {
    return { type: ThisExpression };
}
export function throwStatement(arg) {
    return { type: ThrowStatement, argument: isast(arg) };
}
export function tryStatement(block, handlers, finalizer) {
    return {
        type: TryStatement,
        block: block,
        handlers: handlers,
...
export function unaryExpression(op, arg) {
    return { type: UnaryExpression, operator: op, argument: isast(arg) };
}
...
export function constDeclaration(...rest) {
    return variableDeclaration("const", ...rest);
}
export function letDeclaration(...rest) {
    return variableDeclaration("let", ...rest);
}
export function varDeclaration(...rest) {
    return variableDeclaration("var", ...rest);
}

export function variableDeclarator(id, init = undefined) {
    return { type: VariableDeclarator, id: isast(id), init: init };
}
export function whileStatement(test, body) {
    return { type: WhileStatement, test: isast(test), body: isast(body) };
}

export function undefinedLit() {
