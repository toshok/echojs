...
// AST.
//
// DesugarClasses, then DesugarGeneratorFunctions, then DesugarSpread —
// the legacy relative order: super(...args) desugars into
// %constructSuper(ref, ...args) first, generator methods desugar as plain
// function expressions, and the spread pass then rewrites what remains.
// running these before DesugarImportExport means `export class Foo`
// reaches it as `export let Foo = <iife>(...)` — the same %moduleSetSlot
// store.
//
