...
// AST.
//
// DesugarClasses runs before DesugarSpread (the legacy relative order):
// super(...args) desugars into %constructSuper(ref, ...args) first, and
// the spread pass then rewrites what remains.  running these before
// DesugarImportExport means `export class Foo` reaches it as
// `export let Foo = <iife>(...)` — the same %moduleSetSlot store.
//
// DesugarSpread also stays in the main list below as a safety net for
// spreads synthesized by later passes (currently none).
const pre_eir_passes = [DesugarClasses, DesugarSpread, DesugarMetaProperties];
