...
//
// DesugarDefaults and DesugarRestParameters deliberately do NOT hoist:
// EIR handles both natively and strictly better.  DesugarDefaults
// rewrites EVERY parameter (defaulted or not) into an argc-guarded
// `let p = %getArg(i, dflt)` load, where EIR emits a conditional only
// for parameters that have defaults and keeps the rest as direct SSA
// values.  DesugarRestParameters' %arrayFromRest needs the legacy
// handler's scope mutation (it registers the rest name in the visitor's
// topScope), where EIR's rest_args op is a branch-free select.  Both
// passes serve only the legacy pipeline (fallback functions and the
// toplevel) and die with it in phase 4 rather than hoisting.
...
// same-name redeclarations collapse to the last one — which used to be
// per-function fallbacks in EIR; at the toplevel it also moves the
// closure slot stores to the top, where hoisting says they belong.
