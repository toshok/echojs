...
export class JSModuleInfo extends ModuleInfo {
    ...
    getExportGetter(ident) {
        ...
        if (export_info.promoted) return null;
        let function_id = b.identifier(`get_export_${ident}`);
        let loc = { start: { line: 0, column: 0 } };
        if (export_info.constval) {
            return b.functionExpression(
                function_id,
                [env_unused_id],
                b.blockStatement([b.returnStatement(export_info.constval)], loc),
                [],
                null,
                loc
            );
            ...
            return b.functionExpression(
                ...
                b.blockStatement(
                    [
                        b.returnStatement(
                            intrinsic(moduleGetSlot_id, [b.literal(this.path), b.literal(ident)])
                        ),
                    ],
                    loc
                ),
    ...
    getExportSetter(ident) {
        let export_info = this.exports.get(ident);
        if (export_info && export_info.promoted) return null;
        let function_id = b.identifier(`set_export_${ident}`);
        // we shouldn't generate a setter for const exports
        return b.functionExpression(
            function_id,
            [env_unused_id, value_id],
            b.blockStatement([
                intrinsic(moduleSetSlot_id, [b.literal(this.path), b.literal(ident), value_id]),
            ])
        );
    }
}
