...
test("lower: computed accessor keys raise LowerNotSupported", () => {
    // define_accessor takes an atom key; computed accessor keys don't
    // lower (object18.js xfails on this end to end)
    let threw = false;
    try {
        lowerFunctionNode(
            parseFnPreEIR("function t(k) { return { get [k]() { return 1; } }; }")
        );
    } catch (e) {
        threw = isLowerNotSupported(e);
    }
