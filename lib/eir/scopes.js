        ...
        switch (n.type) {
            ...
            case b.ObjectExpression:
                for (let p of n.properties) {
                    // accessor properties don't fit make_object's plain
                    // key/value shape — lowering one as an ordinary
                    // property would silently misbehave
                    if (p.kind && p.kind !== "init")
                        throw LowerNotSupported(`object literal ${p.kind}ter`, n.loc);
