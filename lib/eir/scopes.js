...
export class ScopeAnalysis {
    constructor() {
        ...
        // qualification alone isn't unique).  NOTE: initialized HERE, not
        // lazily at first use — the lazy-init + template-literal form of
        // this code miscompiled under the legacy pipeline (undistilled;
    ...
    walkStmt(n) {
        switch (n.type) {
            ...
            case b.ExportNamedDeclaration: {
                ...
                if (n.source) throw LowerNotSupported("re-export", n.loc);
                ...
                throw LowerNotSupported("export declaration form", n.loc);
            }
        ...
        switch (n.type) {
            ...
            case b.ObjectExpression:
                for (let p of n.properties) {
                    // accessors lower via define_accessor; computed
                    // accessor keys don't (rare, and the runtime call
                    // takes an atom)
                    if (p.kind && p.kind !== "init" && p.computed)
                        throw LowerNotSupported(`computed object literal ${p.kind}ter`, n.loc);
