...
export class ScopeAnalysis {
    ...
    enterFunction(fnNode, name) {
        ...
        if (fnNode.defaults && fnNode.defaults.some((d) => d))
            throw LowerNotSupported("default parameters", fnNode.loc);
        if (fnNode.rest)
            throw LowerNotSupported("rest parameter", fnNode.loc);
        if (fnNode.generator)
    ...
    walkExpr(n) {
        switch (n.type) {
            case b.Literal:
                // regex literals (and any other object-valued literal)
                // aren't lowerable; reject here so we fall back early
            ...
            case b.AssignmentExpression:
                // lowering only handles simple assignment; reject compound
                // forms here so we fall back early (a late lowering failure
                // abandons the whole file's EIR set)
                if (n.operator !== "=")
