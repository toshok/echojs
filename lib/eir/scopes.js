...
export class ScopeAnalysis {
    ...
    analyzeFunction(fnNode, name) {
        ...
        if (fnNode.id && fnNode.id.name) {
            ...
            selfBinding = new Binding(fnNode.id.name, "self", null);
            this.curScope.names.set(fnNode.id.name, selfBinding);
        }
    ...
    enterFunction(fnNode, name) {
        ...
        if (fnNode.defaults && fnNode.defaults.some((d) => d))
            throw new LowerNotSupported("default parameters", fnNode.loc);
        if (fnNode.rest)
            throw new LowerNotSupported("rest parameter", fnNode.loc);
        if (fnNode.generator)
            throw new LowerNotSupported("generator function", fnNode.loc);
    ...
    reference(idNode) {
        ...
        if (idNode.name === "arguments")
            throw new LowerNotSupported("the arguments object", idNode.loc);
        ...
        if (binding.kind === "self") {
            ...
            if (binding.fnInfo !== this.curFn)
                throw new LowerNotSupported("self-reference from a nested function", idNode.loc);
            return;
        }
        ...
        switch (n.type) {
            ...
            case b.FunctionDeclaration: {
                ...
                if (!this.curScope.isFnTop)
                    throw new LowerNotSupported("block-level function declaration", n.loc);
                if (this.curScope.names.has(n.id.name))
                    throw new LowerNotSupported(
