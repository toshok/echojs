...
//     the "%self" module global (const-literal exports fold);
...
//     back too.
// anything else (non-exported mutable module vars, namespace/default
// imports, siblings used as values, unsupported syntax) falls back per
// function via LowerNotSupported.
//
...
function collectModuleRefs(toplevelBody, module_infos, this_module_info) {
    ...
    // named imports from non-native modules
    if (module_infos) {
    ...
    if (this_module_info) {
        for (let wrapped of toplevelBody) {
            ...
            if (!decl || Array.isArray(decl) || decl.type !== b.VariableDeclaration) continue;
...
function candidateOf(wrapped) {
    ...
    // var f = function () { ... };  (single declarator only)
    if (
