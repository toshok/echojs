...
// `var` declarations hoist to module scope from ANY statement depth
// (`if (c) var print = console.log;` binds `print` at module scope, not
// as a global).  walk statements recursively — but not into nested
// functions, whose vars are their own.
function collectNestedVarNames(n, names) {
    if (!n || typeof n !== "object") return;
    if (Array.isArray(n)) {
        for (let el of n) collectNestedVarNames(el, names);
        return;
    }
    switch (n.type) {
        ...
        case b.VariableDeclaration:
            if (n.kind === "var")
                for (let d of n.declarations) collectPatternNames(d.id, names);
            return;
        default:
            for (let k of Object.keys(n)) {
                if (k === "loc") continue;
                collectNestedVarNames(n[k], names);
            }
            return;
    }
}
    ...
    for (let wrapped of toplevelBody) {
        ...
        switch (stmt.type) {
            ...
            case b.ImportDeclaration:
                ...
                break;
            default:
                // hoisted vars inside nested statements still bind at
                // module scope
                collectNestedVarNames(stmt, names);
                break;
        }
    }
    return names;
...
export function collectEIRFunctions(tree, filename, module_infos, this_module_info, options) {
    let exclude_fns = options && options.ir_exclude_fn;
    let toplevel = tree.body[0];
    let body = toplevel.body.body;
    ...
    if (succeeded.length > 0) {
        verifyModule(eir_module);
        if (dumpRequested(options)) dumpModule(filename, "candidate functions", eir_module);
    }
