...
export function printFunction(fn) {
    ...
    let header_str = header_params.join(", ");
    lines.push(`fn @${fn.name}(${header_str}) {`);

    // note: inner templates are hoisted out of outer template expressions
    // throughout this file; the legacy compiler miscompiles a template
    // inside an arrow inside another template's ${} (DesugarTemplates bug).
    let paramStr = (p) => `${nameOf(p)}: ${p.type}`;

    for (let b of fn.blocks) {
        let plist = "";
        if (b !== fn.entry && b.params.length > 0) {
            let inner = b.params.map(paramStr).join(", ");
            plist = `(${inner})`;
