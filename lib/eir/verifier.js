...
export function verifyFunction(fn) {
    let fail = (msg, inst) => {
        let where = inst ? ` at '${printInst(inst, (v) => `%v${v.id}`)}'` : "";
    ...
    for (let b of fn.blocks) {
        ...
        // edge argument counts match target params
        if (term.targets) {
            for (let t of term.targets) {
                if (t.args.length !== t.block.params.length)
                    fail(
                        `edge to ^${t.block.name} passes ${t.args.length} args, target has ${t.block.params.length} params`,
                        term
                    );
                for (let a of t.args)
