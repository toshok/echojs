...
// declarations and expressions, arrow functions that don't use `this`
// (full closure support), default/rest parameters, `arguments`, if/else,
// while, do-while, for, for-of, for-in, switch, break/continue, return,
// throw, try/catch (unwind edges), try/finally (finalizer duplication),
// and the %-intrinsic calls listed in intrinsics.js (produced by the
// pre-EIR desugar passes, e.g. %arrayFromSpread).
//
// Not yet: tagged templates, arrows using lexical `this`, closures over
// let/const loop variables (per-iteration envs), labeled break/continue,
// getters/setters.
...
class LowerFunction {
    ...
    functionExpr(n) {
        ...
        return this.b.emit("make_closure", [this.curEnvValue()], { fn: childInfo.name });
    }
    ...
    intrinsicCall(n) {
        ...
        let args = n.arguments.map((a) => this.expr(a));
        if (intr.op) return this.b.emit(intr.op, args, {});
        return this.b.emit("call_runtime", args, { name: intr.runtime });
    }
