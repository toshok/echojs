...
class LowerFunction {
    constructor(info, analysis, module, mod_ctx) {
        ...
        this.module = module;
        // module-scope interop: imports (name -> {module, slot, constval})
        // and sibling top-level EIR functions callable directly
        this.mod_ctx = mod_ctx || { imports: new Map(), siblings: new Map() };
    ...
    identifier(n) {
        ...
        if (binding === null || binding === undefined) {
            let imp = this.mod_ctx.imports.get(n.name);
            if (imp) {
                if (imp.constval !== undefined) return this.literal(imp.constval);
