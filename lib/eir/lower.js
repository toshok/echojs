...
class LowerFunction {
    constructor(info, analysis, module) {
            ...
            case b.ObjectExpression: {
                ...
                for (let p of n.properties) {
                    if (p.computed || (p.key.type !== b.Identifier && p.key.type !== b.Literal))
                        throw new LowerNotSupported("computed object key", n.loc);
    ...
    identifier(n) {
        ...
        if (binding.kind === "self")
            throw new LowerNotSupported("function self-reference as a value", n.loc);
    ...
    functionExpr(n) {
        ...
        lowerOneFunction(childInfo, this.analysis, this.module);
    ...
    assignment(n) {
        ...
        if (n.left.type === b.Identifier) {
            ...
            else this.writeBinding(binding, v);
            return v;
        }
    ...
    call(n) {
        ...
        if (n.callee.type === b.MemberExpression) {
            ...
            // direct recursion: call the current function without closure
            // dispatch, forwarding our own env
    ...
    stmt(n) {
        switch (n.type) {
            ...
            case b.FunctionDeclaration:
                ...
                lowerOneFunction(this.analysis.infoFor(n), this.analysis, this.module);
                return;
            ...
            case b.BreakStatement: {
                if (n.label || this.loops.length === 0)
                    throw new LowerNotSupported("break outside plain loop", n.loc);
                this.b.br(this.loops[this.loops.length - 1].breakTarget, []);
                return;
            }
            case b.ContinueStatement: {
                if (n.label || this.loops.length === 0)
                    throw new LowerNotSupported("continue outside plain loop", n.loc);
...
export function lowerAnalyzedFunction(info, analysis, module) {
    return lowerOneFunction(info, analysis, module);
}

function lowerOneFunction(info, analysis, module) {
    if (info.lowered) return info.fn;
    info.lowered = true;
    let lf = new LowerFunction(info, analysis, module);
    ...
    for (let child of info.children) lowerOneFunction(child, analysis, module);
    return info.fn;
}
