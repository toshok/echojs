...
import { convert as closure_convert, preEIRConvert as pre_eir_convert } from "./closure-conversion";
...
class LLVMIRVisitor extends TreeVisitor {
    constructor(module, filename, triple, options, abi, allModules, this_module_info, dibuilder, difile) {
        ...
        this.ejs_intrinsics = Object.create(null, {
            templateDefaultHandlerCall: {
                value: this.handleTemplateDefaultHandlerCall,
            },
            ...
            constructApply: { value: this.handleConstructApply },
            setConstructorKindDerived: {
                value: this.handleSetConstructorKindDerived,
            },
            setConstructorKindBase: {
                value: this.handleSetConstructorKindBase,
            },
        ...
        this.opencode_intrinsics = {
            ...
            isNullOrUndefined: false, // unused
            isUndefined: true,
            isNull: true,
        };
    ...
    emitModuleResolution(module_accessors) {
        ...
        for (let accessor of module_accessors) {
            let get_func =
                (accessor.getter && accessor.getter.ir_func) || consts.Null(types.EjsClosureFunc);
            let set_func =
                (accessor.setter && accessor.setter.ir_func) || consts.Null(types.EjsClosureFunc);
    ...
    beginCatch(result) {
        return this.createCall(
            this.ejs_runtime.begin_catch,
            [ir.createPointerCast(result, types.Int8Pointer, "")],
            "begincatch"
        );
    }
    endCatch() {
        return this.createCall(this.ejs_runtime.end_catch, [], "endcatch");
    }
    ...
    createLoad(ty, value, name) {
        let rv = ir.createLoad(ty, value, name);
        return rv;
    }
    ...
    storeBoolean(alloca, jsbool, name) {
        ...
        if (this.triple.pointerSize() === 64)
            return ir.createStore(
                consts.int64_lowhi(0xfff98000, jsbool ? 0x00000001 : 0x000000000),
                alloca_as_int64,
                name
            );
        else
            return ir.createStore(
                consts.int64_lowhi(0xffffff83, jsbool ? 0x00000001 : 0x000000000),
                alloca_as_int64,
                name
            );
    }
    ...
    storeToDest(dest, arg, name = "") {
        if (!arg) arg = { type: b.Literal, value: null };

        if (arg.type === b.Literal) {
            if (arg.value === null) return this.storeNull(dest, name);

            if (arg.value === undefined) return this.storeUndefined(dest, name);

            if (typeof arg.value === "number") return this.storeDouble(dest, arg.value, name);

            if (typeof arg.value === "boolean") return this.storeBoolean(dest, arg.value, name);
    ...
    storeGlobal(prop, value) {
        ...
        if (prop.type === b.Identifier) gname = prop.name; // prop.type is b.Literal
        else gname = prop.value;
        ...
        return this.createCall(
            this.ejs_runtime.global_setprop,
            [c, value],
            `globalpropstore_${gname}`
        );
    }
    ...
    loadGlobal(prop) {
        ...
        if (this.options.frozen_global)
            return ir.createLoad(types.EjsValue, this.ejs_globals[prop.name], `load-${gname}`);
    ...
    visitWithScope(scope, children) {
        ...
        for (let child of children) this.visit(child);
        this.scope_stack.pop();
    }

    findIdentifierInScope(ident) {
        for (let scope of this.scope_stack.stack) {
            if (scope.has(ident)) return scope.get(ident);
        }
        return null;
    }
    ...
    createPropertyStore(obj, prop, rhs, computed) {
        if (computed) {
            ...
            return this.createCall(
                this.ejs_runtime.object_setprop,
                [obj, this.visit(prop), rhs],
                "propstore_computed"
            );
        } else {
            var pname;
            ...
            // we store obj.prop, prop is an id
            if (prop.type === b.Identifier) pname = prop.name; // prop.type is b.Literal
            else pname = prop.value;
    ...
    createPropertyLoad(obj, prop, computed, canThrow = true) {
        if (computed) {
            ...
            if (this.options.record_types)
                this.createCall(
                    this.ejs_runtime.record_getprop,
                    [consts.int32(this.genRecordId()), obj, loadprop],
                    ""
                );

            return this.createCall(
                this.ejs_runtime.object_getprop,
                [obj, loadprop],
                "getprop_computed",
                canThrow
            );
            ...
            if (this.options.record_types)
                this.createCall(
                    this.ejs_runtime.record_getprop,
                    [consts.int32(this.genRecordId()), obj, pname],
                    ""
                );
    ...
    setDebugLoc(ast_node) {
        ...
        ir.setCurrentDebugLocation(
            llvm.DebugLoc.get(
                ast_node.loc.start.line,
                ast_node.loc.start.column,
                this.currentFunction.debug_info
            )
        );
    }
    ...
    visitOrNull(n) {
        return this.visit(n) || this.loadNullEjsValue();
    }
    visitOrUndefined(n) {
        return this.visit(n) || this.loadUndefinedEjsValue();
    }
    ...
    visitProgram(n) {
        ...
        for (let func of n.body) this.visit(func);
    }
    ...
    visitBlock(n) {
        ...
        if (iife_dest_bb) {
            ...
            let rv = this.createEjsValueLoad(
                this.findIdentifierInScope(iife_rv.name),
                "%iife_rv_load"
            );
            return rv;
        } else {
    ...
    visitSwitch(n) {
        ...
        case_checks.push({
            dest_check: defaultCase ? defaultCase.bb : merge_bb,
        });
        ...
        this.doInsideExitableScope(new SwitchExitableScope(merge_bb), () => {
            ...
            for (let casenum = 0; casenum < case_checks.length - 1; casenum++) {
                ...
                if (discTest._ejs_returns_ejsval_bool) {
                    disc_cmp = this.createEjsvalICmpEq(
                        discTest,
                        consts.ejsval_false(this.triple.pointerSize() === 32)
                    );
                } else {
                    disc_truthy = this.createCall(
                        this.ejs_runtime.truthy,
                        [discTest],
                        "disc_truthy"
                    );
                ...
                ir.createCondBr(
                    disc_cmp,
                    case_checks[casenum + 1].dest_check,
                    case_checks[casenum].body
                );
            ...
            for (let casenum = 0; casenum < case_bodies.length - 1; casenum++) {
                ...
                case_bodies[casenum].consequent.forEach((consequent) => {
    ...
    visitLabeledStatement(n) {
        if (
            n.body.type === b.ForInStatement ||
            n.body.type === b.ForStatement ||
            n.body.type === b.ForOfStatement ||
            n.body.type === b.DoWhileStatement ||
            n.body.type === b.WhileStatement
        ) {
            // loops are handled by the individual loop statement visit functions
        ...
        let labeled_bb = new llvm.BasicBlock("labeled_bb", insertFunc);
        let merge_bb = new llvm.BasicBlock("labeled_merge_bb", insertFunc);

        ir.createBr(labeled_bb);

        this.doInsideExitableScope(
            new LabeledStatementExitableScope(n.label.name, merge_bb),
            () => {
                this.doInsideBBlock(labeled_bb, () => {
                    this.visit(n.body);
    ...
    visitContinue(n) {
        ...
        // intercepts (findLabeledOrFinally returns the TRY scope), its
        // exitFore re-dispatches — without the label it continued the
        // INNERMOST loop instead of the labeled one
        if (n.label && n.label.name)
            return LoopExitableScope.findLabeledOrFinally(n.label.name).exitFore(n.label.name);
        else return LoopExitableScope.findLoopOrFinally().exitFore();
    }
    ...
    generateCondBr(exp, then_bb, else_bb) {
        ...
        if (exp.type === b.Literal && typeof exp.value === "boolean") {
            ...
            if (exp_value._ejs_returns_ejsval_bool) {
                ...
                let cond_truthy = this.createCall(
                    this.ejs_runtime.truthy,
                    [exp_value],
                    "cond_truthy"
                );
    ...
    visitFor(n) {
        ...
        this.doInsideBBlock(test_bb, () => {
            if (n.test) this.generateCondBr(n.test, body_bb, merge_bb);
            else ir.createBr(body_bb);
        });
    ...
    visitForIn(n) {
        ...
        let iterator = this.createCall(
            this.ejs_runtime.prop_iterator_new,
            [this.visit(n.right)],
            "iterator"
        );
        ...
        this.doInsideExitableScope(new LoopExitableScope(n.label, forin_bb, merge_bb), () => {
            ...
            this.doInsideBBlock(forin_bb, () => {
                let moreleft = this.createCall(
                    this.ejs_runtime.prop_iterator_next,
                    [iterator, consts.True()],
                    "moreleft"
                );
            ...
            //     goto forin_bb
            this.doInsideBBlock(body_bb, () => {
                let current = this.createCall(
                    this.ejs_runtime.prop_iterator_current,
                    [iterator],
                    "iterator_current"
                );
    ...
    visitForOf() {
        throw new Error(
            "internal compiler error.  for..of statements should have been transformed away by this point."
        );
    }
    ...
    visitUpdateExpression(n) {
        ...
        let temp = this.createCall(
            update_op,
            [argument, one],
            "update_temp",
            !update_op.doesNotThrow
        );
        ...
        return this.createEjsValueLoad(result, "%update_result_load");
    }
    ...
    visitIfOrCondExp(n, load_result) {
        ...
        ir.setInsertPoint(merge_bb);
        if (load_result) return this.createEjsValueLoad(cond_val, "cond_val_load");
        else return merge_bb;
    }
        ...
        if (this.iifeStack.top.iife_rv) {
            ...
            if (n.argument)
                ir.createStore(
                    ...
                    this.findIdentifierInScope(this.iifeStack.top.iife_rv.name)
                );
            ...
            if (this.finallyStack.length > 0) {
                if (!this.currentFunction.returnValueAlloca)
                    this.currentFunction.returnValueAlloca = this.createAlloca(
                ...
                ir.createStore(
                    consts.int32(ExitableScope.REASON_RETURN),
                    this.currentFunction.cleanup_reason
                );
                ir.createBr(this.finallyStack[0]);
            } else {
                let return_alloca = this.createAlloca(
                    this.currentFunction,
                    types.EjsValue,
                    "return_alloca"
                );
                ...
                this.createRet(this.createEjsValueLoad(return_alloca, "return_load"));
            }
        }
    }
    ...
    visitVariableDeclaration(n) {
        if (n.kind === "var")
            throw new Error(
                "internal compiler error.  var declarations should have been transformed to lets by this point."
            );

        let scope = this.scope_stack.top;

        let { allocas, new_allocas } = this.createAllocas(
            this.currentFunction,
            n.declarations,
            scope
        );
    ...
    storeValueInDest(rhvalue, lhs) {
        if (lhs.type === b.Identifier) {
            ...
            let result;
            if (dest) result = ir.createStore(rhvalue, dest);
            else result = this.storeGlobal(lhs, rhvalue);
            return result;
        } else if (lhs.type === b.MemberExpression) {
            return this.createPropertyStore(
                this.visit(lhs.object),
                lhs.property,
                rhvalue,
                lhs.computed
            );
            ...
            return this.createCall(
                this.ejs_runtime.global_setprop,
                [this.getAtom(gname), rhvalue],
                `globalpropstore_${lhs.arguments[0].name}`
            );
        } else if (is_intrinsic(lhs, "%moduleGetSlot")) {
            // a module-slot-bound variable as a store destination (e.g. a
            // for-in/for-of loop variable): store through the slot ref
            return ir.createStore(
                rhvalue,
                this.handleModuleSlotRef(lhs, this.opencode_intrinsics.moduleSetSlot)
            );
        } else {
    ...
    visitAssignmentExpression(n) {
        ...
        if (n.operator.length === 2)
            throw new Error(
                `binary assignment operators '${n.operator}' should not exist at this point`
            );

        if (this.options.record_types)
            this.createCall(
                this.ejs_runtime.record_assignment,
                [consts.int32(this.genRecordId()), rhvalue],
                ""
            );
        this.storeValueInDest(rhvalue, lhs);

        // we need to visit lhs after the store so that we load the value, but only if it's used
        if (!n.result_not_used) return rhvalue;
    }

    visitFunction(n) {
        if (n.eir_module) return n.toplevel ? this.emitEIRToplevel(n) : this.emitEIRThunk(n);

        if (!n.toplevel)
            debug.log(
                () =>
                    `        function ${n.ir_name} at ${this.filename}:${
                        n.loc ? n.loc.start.line : "<unknown>"
                    }`
            );
...
*/
    ...
    // an EIR-owned module toplevel: the legacy side keeps only the module
    // scaffolding — the initialized-flag check, literal initialization,
    // module registration, accessors and import resolution emitted by
    // emitModuleResolution — and the body block forwards straight into the
    // EIR-emitted toplevel function.
    emitEIRToplevel(n) {
        let insertBlock = ir.getInsertBlock();
    ...
    emitEIRThunk(n) {
        let insertBlock = ir.getInsertBlock();
        let saved_function = this.currentFunction;

        if (!this.eir_emitter) this.eir_emitter = new EIREmitter(this);
        // candidates in one file share an EIR module; emit it exactly once
        if (!this.eir_emitted) this.eir_emitted = new Map();
        let eir_fns = this.eir_emitted.get(n.eir_module);
        if (!eir_fns) {
            eir_fns = this.eir_emitter.emitModule(n.eir_module);
    ...
    visitUnaryExpression(n) {
        ...
        if (n.operator === "delete") {
            ...
            // computed keys (`delete o[k]`) evaluate the key expression;
            // the old fake-literal path read `.property.name` (undefined
            // for anything computed) and deleted the wrong property
            let prop;
            if (n.argument.computed) {
                prop = n.argument.property;
            } else {
                prop = {
                    type: b.Literal,
            ...
            return this.createCall(
                callee,
                [this.visitOrNull(n.argument.object), this.visit(prop)],
                "result"
            );
        } else if (n.operator === "!") {
            let arg_value = this.visitOrNull(n.argument);
            if (
                this.opencode_intrinsics.unaryNot &&
                this.triple.pointerSize() === 64 &&
                arg_value._ejs_returns_ejsval_bool
            ) {
                let cmp = this.createEjsvalICmpEq(
                    arg_value,
                    consts.ejsval_true(false),
                    "cmpresult"
                );
    ...
    visitSequenceExpression(n) {
        let rv = null;
        for (let exp of n.expressions) rv = this.visit(exp);
        return rv;
    }
    ...
    visitBinaryExpression(n) {
        ...
        if (!callee) throw new Error(`Internal error: unhandled binary operator '${n.operator}'`);
        ...
        if (this.options.record_types)
            this.createCall(
                this.ejs_runtime.record_binop,
                [
                    consts.int32(this.genRecordId()),
                    consts.string(ir, n.operator),
                    left_visited,
                    right_visited,
                ],
                ""
    ...
    visitLogicalExpression(n) {
        ...
        this.doInsideBBlock(left_bb, () => {
            ...
            else throw "Internal error 99.1";
            ir.createBr(merge_bb);
        });
        ...
        ir.setInsertPoint(merge_bb);
        return this.createEjsValueLoad(result, `result_${n.operator}_load`);
    }
    ...
    visitArgsForCall(callee, pullThisFromArg0, args) {
        ...
        if (callee.takes_builtins) {
            ...
            let this_alloca = this.createAlloca(
                this.currentFunction,
                types.EjsValue,
                "this_alloca"
            );
            ...
            if (args_length > 0) {
                const scratchAreaType = llvm.ArrayType.get(
                    types.EjsValue,
                    this.currentFunction.scratch_length
                );
                ...
                for (let i = 0; i < args_length; i++) {
                    let gep = ir.createGetElementPointer(
                        scratchAreaType,
                        this.currentFunction.scratch_area,
                ...
                let argsCast = ir.createGetElementPointer(
                    scratchAreaType,
                    this.currentFunction.scratch_area,
            ...
            for (let a of args) argv.push(this.visitOrNull(a));
        }
    ...
    visitArgsForConstruct(callee, args, this_loc, newTarget_loc) {
        ...
        if (args.length > 0) {
            ...
            let visited = [];
            for (let a of args) visited.push(this.visitOrNull(a));
    ...
    visitCallExpression(n) {
        ...
        debug.log(() => {
            return n.arguments
                .map((a, i) => `          arguments[${i}] =  ${JSON.stringify(a)}`)
                .join("");
        });
        ...
        if (!intrinsicHandler)
            throw new Error(
                `Internal error: callee should not be null in visitCallExpression (callee = '${n.callee.name}', arguments = ${n.arguments.length})`
            );
    ...
    visitThisExpression() {
        ...
        return this.createEjsValueLoad(
            this.createLoad(
                types.EjsValue.pointerTo(),
                this.findIdentifierInScope("%this"),
                "load_this_ptr"
            ),
            "load_this"
        );
    ...
    visitIdentifier(n) {
        ...
        if (source) {
            ...
            rv = this.createEjsValueLoad(source, `load_${val}`);
            return rv;
        }
        ...
        if (val === "arguments") {
            let arguments_alloca = this.createAlloca(
                this.currentFunction,
                types.EjsValue,
                "local_arguments_object"
            );
            let saved_insert_point = ir.getInsertBlock();
            ir.setInsertPoint(this.currentFunction.entry_bb);

            let load_argc = this.createLoad(
                types.Int32,
                this.currentFunction.topScope.get("%argc"),
                "argc_load"
            );
            let load_args = this.createLoad(
                types.EjsValue.pointerTo(),
                this.currentFunction.topScope.get("%args"),
                "args_load"
            );

            let args_new = this.ejs_runtime.arguments_new;
            let arguments_object = this.createCall(
                args_new,
                [load_argc, load_args],
                "argstmp",
                !args_new.doesNotThrow
            );
            ir.createStore(arguments_object, arguments_alloca);
            this.currentFunction.topScope.set("arguments", arguments_alloca);

            ir.setInsertPoint(saved_insert_point);
            return this.createEjsValueLoad(arguments_alloca, "load_arguments");
        }
    ...
    visitObjectExpression(n) {
        let obj_proto = ir.createLoad(
            types.EjsValue,
            this.ejs_globals.Object_prototype,
            "load_objproto"
        );
        let object_create = this.ejs_runtime.object_create;
        let obj = this.createCall(
            object_create,
            [obj_proto],
            "objtmp",
            !object_create.doesNotThrow
        );

        let accessor_map = new Map();

        // gather all properties so we can emit get+set as a single call to
        // define_accessor_prop.  non-computed keys map by NAME: keying by
        // the key AST node put a get/set pair in separate entries, and the
        // second define_accessor_prop (with an undefined getter) clobbered
        // the first.
        let keyFor = (property) =>
            property.computed
                ? property.key
                : property.key.type === b.Identifier
                  ? property.key.name
                  : String(property.key.value);

        for (let property of n.properties) {
            if (property.kind === "get" || property.kind === "set") {
                if (!accessor_map.has(keyFor(property))) accessor_map.set(keyFor(property), new Map());
                if (accessor_map.get(keyFor(property)).has(property.kind))
                    throw new SyntaxError(
                        `a '${property.kind}' method for '${escodegenerate(
                            property.key
                        )}' has already been defined.`
                    );
                if (accessor_map.get(keyFor(property)).has("init"))
                    throw new SyntaxError(
                        `${property.key.loc.start.line}: property name ${escodegenerate(
                            property.key
                        )} appears once in object literal.`
                    );
            } else if (property.kind === "init") {
                if (accessor_map.get(keyFor(property)))
                    throw new SyntaxError(
                        `${property.key.loc.start.line}: property name ${escodegenerate(
                            property.key
                        )} appears once in object literal.`
                    );
                accessor_map.set(keyFor(property), new Map());
            } else {
                throw new Error(`unrecognized property kind '${property.kind}'`);
            }

            if (property.computed) {
                accessor_map.get(keyFor(property)).set("computed", true);
            }
            accessor_map.get(keyFor(property)).set(property.kind, property);
        }
        ...
        accessor_map.forEach((prop_map, propkey) => {
            ...
            if (prop_map.has("computed")) propkey = this.visit(propkey);
            else propkey = this.getAtom(String(propkey)); // name-keyed

            if (prop_map.has("init")) {
                let val = this.visit(prop_map.get("init").value);
                this.createCall(
                    this.ejs_runtime.object_define_value_prop,
                    [obj, propkey, val, consts.int32(0x77)],
                    `define_value_prop_${propkey}`
                );
            } else {
                ...
                this.createCall(
                    this.ejs_runtime.object_define_accessor_prop,
                    [obj, propkey, get_method, set_method, consts.int32(0x19)],
                    `define_accessor_prop_${propkey}`
                );
            }
        });
    ...
    visitArrayExpression(n) {
        ...
        let obj = this.createCall(
            this.ejs_runtime.array_new,
            [consts.int64(n.elements.length), consts.bool(force_fill)],
            "arrtmp",
            !this.ejs_runtime.array_new.doesNotThrow
        );
        let i = 0;
        for (let el of n.elements) {
            // don't create property stores for array holes — but the index
            // still advances past them (an element after a hole used to
            // land at the hole's index)
            if (el != null) {
    ...
    visitLiteral(n) {
        ...
        if (typeof n.raw === "string" && n.raw[0] === "/") {
            ...
            let flags = consts.string(
                ir,
                `${n.value.global ? "g" : ""}${n.value.multiline ? "m" : ""}${
                    n.value.ignoreCase ? "i" : ""
                }`
            );

            let regexp_new_utf8 = this.ejs_runtime.regexp_new_utf8;
            var regexpcall = this.createCall(
                regexp_new_utf8,
                [source, flags],
                "regexptmp",
                !regexp_new_utf8.doesNotThrow
            );
    ...
    createCall(callee, argv, callname, canThrow = true) {
        ...
        if (TryExitableScope.unwindStack.depth === 0 || callee.doesNotThrow || !canThrow) {
            ...
            calltmp = this.abi.createInvoke(
                this.currentFunction,
                callee.type,
                callee,
                argv,
                normal_block,
                TryExitableScope.unwindStack.top.getLandingPadBlock(),
                callname
            );
    ...
    visitTry(n) {
        ...
        if (!this.currentFunction.cleanup_reason)
            this.currentFunction.cleanup_reason = this.createAlloca(
                this.currentFunction,
                types.Int32,
                "cleanup_reason"
            );
        ...
        let scope = new TryExitableScope(
            this.currentFunction.cleanup_reason,
            branch_target,
            () => new llvm.BasicBlock("exception", insertFunc),
            finally_block != null
        );
        this.doInsideExitableScope(scope, () => {
            ...
            if (n.finalizer) this.finallyStack.shift();
        ...
        if (scope.landing_pad_block) {
            ...
            this.doInsideBBlock(scope.landing_pad_block, () => {
                ...
                let caught_result = ir.createLandingPad(
                    types.EjsLandingPad,
                    clause_count,
                    "caught_result"
                );
                caught_result.addClause(
                    ir.createPointerCast(this.ejs_runtime.exception_typeinfo, types.Int8Pointer, "")
                );
                ...
                if (!insertFunc.hasPersonality()) {
                    insertFunc.setPersonality(
                        ir.createPointerCast(
                            this.ejs_runtime.personality,
                            types.Int8Pointer,
                            "personality"
                        )
                    );
                ...
                if (catch_block) ir.createBr(catch_block);
                else if (finally_block) {
                    // finally-only try: run the finalizer, then RETHROW.
                    // (branching straight to the finalizer with a stale
                    // cleanup_reason silently swallowed the exception --
                    // and DesugarLetLoopVars wraps every `for (let ...)`
                    // body in exactly this construct.)
                    if (!this.currentFunction.caught_exception_alloca)
                        this.currentFunction.caught_exception_alloca = this.createAlloca(
                            this.currentFunction,
                            types.EjsValue,
                            "caught_exception"
                        );
                    let catchval = this.beginCatch(exception);
                    ir.createStore(catchval, this.currentFunction.caught_exception_alloca);
                    this.endCatch();
                    ir.createStore(
                        consts.int32(TryExitableScope.REASON_EXCEPTION),
                        this.currentFunction.cleanup_reason
                    );
                    ir.createBr(finally_block);
                } else throw "this shouldn't happen.  a try{} without either a catch{} or finally{}";
                ...
                if (n.handlers.length > 0) {
                    this.doInsideBBlock(catch_block, () => {
                        ...
                        if (n.handlers[0].param && n.handlers[0].param.name) {
                            ...
                            let alloca = this.createAlloca(
                                this.currentFunction,
                                types.EjsValue,
                                `local_catch_${catch_name}`
                            );
                            catch_scope.set(catch_name, alloca);
                            ir.createStore(catchval, alloca);
                        }

                        if (n.finalizer) this.finallyStack.unshift(finally_block);
        ...
        if (n.finalizer) {
            this.doInsideBBlock(finally_block, () => {
                ...
                let cleanup_reason = this.createLoad(
                    types.Int32,
                    this.currentFunction.cleanup_reason,
                    "cleanup_reason_load"
                );
                ...
                if (this.currentFunction.returnValueAlloca) {
                    ...
                    this.doInsideBBlock(return_tramp, () => {
                        if (this.finallyStack.length > 0) {
                            ...
                            this.createRet(
                                this.createEjsValueLoad(
                                    this.currentFunction.returnValueAlloca,
                                    "rv"
                                )
                            );
                        }
                ...
                let switch_stmt = ir.createSwitch(
                    cleanup_reason,
                    merge_block,
                    scope.destinations.length + 1
                );
                if (this.currentFunction.returnValueAlloca)
                ...
                switch_stmt.addCase(
                    consts.int32(TryExitableScope.REASON_FALLOFF_TRY),
                    falloff_tramp
                );

                if (scope.landing_pad_block && !catch_block) {
                    // the exception remembered by the finally-only
                    // landingpad path above resumes here
                    let exception_tramp = new llvm.BasicBlock("exception_tramp", insertFunc);
                    this.doInsideBBlock(exception_tramp, () => {
                        let exc = this.createEjsValueLoad(
                            this.currentFunction.caught_exception_alloca,
                            "caught_exc"
                        );
                        // a fresh _ejs_throw of the saved value: the original
                        // C++ exception ended at the begin/end_catch pair in
                        // the landingpad (_ejs_rethrow needs an ACTIVE
                        // exception and would terminate)
                        this.createCall(this.ejs_runtime.throw, [exc], "", true);
                ...
                for (let s = 0, e = scope.destinations.length; s < e; s++) {
                    ...
                    this.doInsideBBlock(dest_tramp, () => {
                        // relay the label: the destination may itself be a
                        // finally scope (nested finallies) that must keep
                        // routing toward the labeled loop
                        if (dest.reason == TryExitableScope.REASON_BREAK)
                            dest.scope.exitAft(true, dest.label);
                        else if (dest.reason == TryExitableScope.REASON_CONTINUE)
                            dest.scope.exitFore(dest.label);
                    });
    ...
    handleTemplateDefaultHandlerCall(exp) {
        ...
        let concat_string = (s) => {
            if (!strval) strval = s;
            else strval = this.createCall(this.ejs_runtime.string_concat, [strval, s], "strconcat");
        };

        while (cooked_i < cooked_strings.length) {
            let c = cooked_strings[cooked_i];
            cooked_i += 1;
            if (c.length !== 0) concat_string(this.getAtom(c.value));
    ...
    handleTemplateCallsite(exp) {
        ...
        let callsite_alloca = this.createAlloca(
            this.currentFunction,
            types.EjsValue,
            `local_${callsite_id}`
        );

        let callsite_global = new llvm.GlobalVariable(
            this.module,
            types.EjsValue,
            callsite_id,
            llvm.Constant.getAggregateZero(types.EjsValue),
        ...
        let global_callsite_load = this.createEjsValueLoad(callsite_global, "load_global_callsite");
        ir.createStore(global_callsite_load, callsite_alloca);

        let callsite_load = ir.createLoad(types.EjsValue, callsite_alloca, "load_local_callsite");
        ...
        this.doInsideBBlock(then_bb, () => {
            ...
            let frozen_raw = this.createCall(
                this.ejs_runtime.object_freeze,
                [callsite_raw],
                "frozen_raw"
            );
            ...
            this.createCall(
                this.ejs_runtime.object_setprop,
                [callsite_cooked, this.visit(b.literal("raw")), frozen_raw],
                "propstore_raw"
            );

            let frozen_cooked = this.createCall(
                this.ejs_runtime.object_freeze,
                [callsite_cooked],
                "frozen_cooked"
            );
        ...
        return this.createRet(
            ir.createLoad(types.EjsValue, callsite_alloca, "load_local_callsite")
        );
    }
    ...
    handleModuleSlotRef(exp, opencode) {
        ...
        if (opencode && this.triple.pointerSize() === 64) {
            // must NOT be an inbounds gep: imported modules are declared
            // with the generic EJSModule type whose exports array has
            // length 1, so indexing slotnum > 0 through an inbounds gep is
            // poison and newer llvm optimizers (16+) miscompile the load.
            return ir.createGetElementPointer(
                types.EjsModule,
                module_global,
                [consts.int64(0), consts.int32(3), consts.int64(slotnum)],
                "slot_ref"
            );
        }

        return this.createCall(
            this.ejs_runtime.module_get_slot_ref,
            [module_global, consts.int32(slotnum)],
            "module_slot"
        );
    }
    ...
    handleModuleGetSlot(exp, opencode) {
        ...
        return ir.createLoad(types.EjsValue, slot_ref, "module_slot_load");
    }
    ...
    handleModuleSetSlot(exp, opencode) {
        ...
        return ir.createLoad(types.EjsValue, slot_ref, "load_slot"); // do we need this?  we don't need to keep assignment expression semantics for this
    }

    handleModuleGetExotic(exp) {
        let moduleString = exp.arguments[0].value;

        if (this.opencode_intrinsics.moduleGetExotic) {
            if (moduleString === this.this_module_info.path) {
        ...
        // version.
        return this.createCall(
            this.ejs_runtime.module_get,
            [this.visit(exp.arguments[0])],
            "get_module_exotic"
        );
    }

    handleGetNewTarget() {
        return this.createEjsValueLoad(this.findIdentifierInScope("%newTarget"), "new_target_load");
    }

    handleGetArgumentsObject() {
        let arguments_alloca = this.createAlloca(
            this.currentFunction,
            types.EjsValue,
            "local_arguments_object"
    ...
    handleGetLocal(exp) {
        return this.createEjsValueLoad(
            this.findIdentifierInScope(exp.arguments[0].name),
            `load_${exp.arguments[0].name}`
        );
    }
    handleGetGlobal(exp) {
    ...
    handleSetLocal(exp) {
        ...
        if (!dest) throw new Error(`identifier not found: ${exp.arguments[0].name}`);
        let arg = exp.arguments[1];
        this.storeToDest(dest, arg);
        return ir.createLoad(types.EjsValue, dest, "load_val");
    }
    ...
    handleSetGlobal(exp) {
        ...
        if (this.options.frozen_global)
            throw new SyntaxError(
                `cannot set global property '${exp.arguments[0].name}' when using --frozen-global`
            );
    ...
    emitEjsvalTo(val, type, prefix) {
        if (this.triple.pointerSize() === 64) {
            let payload = this.createEjsvalAnd(
                val,
                consts.int64_lowhi(0x7fff, 0xffffffff),
                `${prefix}_payload`
            );
    ...
    emitLoadSpecops(obj) {
        if (this.triple.pointerSize() === 64) {
            ...
            let specops_slot = ir.createInBoundsGetElementPointer(
                types.EjsObject,
                obj,
                [consts.int64(0), consts.int32(1)],
                "specops_slot"
            );
            return ir.createLoad(types.EjsValue, specops_slot, "specops_load");
        } else {
    ...
    emitThrowNativeError(errorCode, errorMessage) {
        this.createCall(
            this.ejs_runtime.throw_nativeerror_utf8,
            [consts.int32(errorCode), consts.string(ir, errorMessage)],
            "",
            true
        );
        return ir.createUnreachable();
    ...
    emitLoadEjsFunctionClosureFunc(closure) {
        if (this.triple.pointerSize() === 64) {
            let func_slot_gep = ir.createInBoundsGetElementPointer(
                types.EjsClosureEnv.pointerTo(),
                closure,
                [consts.int64(1)],
                "func_slot_gep"
            );
            let func_slot = ir.createBitCast(
                func_slot_gep,
                this.abi
                    .createFunctionType(types.EjsValue, [
                        types.EjsValue,
                        types.EjsValue,
                        types.Int32,
                        types.EjsValue.pointerTo(),
            ...
            return ir.createLoad(types.EjsValue, func_slot, "func_load");
        } else {
    ...
    emitLoadEjsFunctionClosureEnv(closure) {
        if (this.triple.pointerSize() === 64) {
            let env_slot_gep = ir.createInBoundsGetElementPointer(
                types.EjsClosureEnv.pointerTo(),
                closure,
                [consts.int64(1), consts.int32(1)],
                "env_slot_gep"
            );
            let env_slot = ir.createBitCast(env_slot_gep, types.EjsValue.pointerTo(), "env_slot");
            return ir.createLoad(types.EjsValue, env_slot, "env_load");
        } else {
    ...
    handleInvokeClosure(exp, opencode) {
        ...
        if (!this.currentFunction.scratch_area) {
            throw new Error(
                `Internal error: function ${this.currentFunction.name} has no scratch space and makes a [[Call]] call with ${exp.arguments.length} arguments (${escodegenerate(exp)})`
            );
        }
        ...
        if (opencode && this.triple.pointerSize() === 64) {
            ...
            var call_result_alloca = this.createAlloca(
                this.currentFunction,
                types.EjsValue,
                "call_result"
            );
            ...
            this.doInsideBBlock(candidate_is_object_bb, () => {
                ...
                this.doInsideBBlock(direct_invoke_bb, () => {
                    ...
                    let direct_call_result = this.createCall(
                        func_load,
                        [env_load, argv[1], argv[2], argv[3], argv[4], argv[5]],
                        "callresult"
                    );
                ...
                this.doInsideBBlock(runtime_invoke_bb, () => {
                    let runtime_call_result = this.createCall(
                        this.ejs_runtime.invoke_closure,
                        argv,
                        "callresult",
                        true
                    );
            ...
            return ir.createLoad(types.EjsValue, call_result_alloca, "call_result_load");
        } else {
            return this.createCall(this.ejs_runtime.invoke_closure, argv, "call", true);
        }
    }

    handleConstructClosure(exp) {
        ...
        if (!this.currentFunction.scratch_area) {
            throw new Error(
                `Internal error: function has no scratch space and makes a [[Construct]] call with ${exp.arguments.length} arguments`
            );
        }

        let argv = this.visitArgsForConstruct(
            this.ejs_runtime.construct_closure,
            exp.arguments,
            this_alloca
        );
    ...
    handleConstructSuper(exp) {
        let this_ptr = this.createLoad(
            types.EjsValue.pointerTo(),
            this.findIdentifierInScope("%this"),
            "this_ptr"
        );
        let newTarget = this.createEjsValueLoad(
            this.findIdentifierInScope("%newTarget"),
            "load_newTarget"
        );
    ...
    handleConstructSuperApply(exp) {
        ...
        let argv = this.visitArgsForConstruct(
            this.ejs_runtime.construct_closure_apply,
            exp.arguments,
            this_ptr,
            newTarget
        );
    ...
    // %constructApply(ctor, argsArray): new Foo(...args) — the runtime
    // spreads the dense array (newTarget = the constructor itself)
    handleConstructApply(exp) {
        let ctor = this.visit(exp.arguments[0]);
        let arr = this.visit(exp.arguments[1]);

        let this_alloca = this.createAlloca(this.currentFunction, types.EjsValue, "ctor_apply_this");
        this.storeUndefined(this_alloca);
        ...
        ir.createStore(arr, gep);

        return this.createCall(
            this.ejs_runtime.construct_closure_apply,
            [ctor, this_alloca, consts.int32(1), gep, ctor],
            "ctor_apply",
            true
        );
    }

    handleSetConstructorKindDerived(exp) {
        let ctor = this.visit(exp.arguments[0]);
        return this.createCall(this.ejs_runtime.set_constructor_kind_derived, [ctor], "");
    }

    handleSetConstructorKindBase(exp) {
        let ctor = this.visit(exp.arguments[0]);
        return this.createCall(this.ejs_runtime.set_constructor_kind_base, [ctor], "");
    }

    handleMakeGenerator(exp) {
    ...
    handleCreateArgScratchArea(exp) {
        ...
        this.currentFunction.scratch_area = this.createAlloca(
            this.currentFunction,
            argsArrayType,
            "args_scratch_area"
        );
    ...
    handleGetSlot(exp, opencode) {
        ...
        return ir.createLoad(types.EjsValue, slot_ref, "slot_ref_load");
    }

    handleSetSlot(exp, opencode) {
        let new_slot_val;

        if (exp.arguments.length === 4) new_slot_val = exp.arguments[3];
        else new_slot_val = exp.arguments[2];

        let slotref = this.handleSlotRef(exp, opencode);

        this.storeToDest(slotref, new_slot_val);

        return ir.createLoad(types.EjsValue, slotref, "load_slot");
    }
    ...
    handleSlotRef(exp, opencode) {
        ...
        if (opencode && this.triple.pointerSize() === 64) {
            ...
            return ir.createInBoundsGetElementPointer(
                types.EjsClosureEnv,
                envp,
                [consts.int64(0), consts.int32(2), consts.int64(slotnum)],
                "slot_ref"
            );
        } else {
            return this.createCall(
                this.ejs_runtime.get_env_slot_ref,
                [env, consts.int32(slotnum)],
                "slot_ref_tmp",
                false
            );
        }
    ...
    createEjsBoolSelect(val, falseval = false) {
        let rv = ir.createSelect(
            val,
            this.loadBoolEjsValue(!falseval),
            this.loadBoolEjsValue(falseval),
            "sel"
        );
        rv._ejs_returns_ejsval_bool = true;
    ...
    createEjsvalICmpUGt(arg, i64_const, name) {
        return ir.createICmpUGt(this.getEjsvalBits(arg), i64_const, name);
    }
    createEjsvalICmpULt(arg, i64_const, name) {
        return ir.createICmpULt(this.getEjsvalBits(arg), i64_const, name);
    }
    createEjsvalICmpEq(arg, i64_const, name) {
        return ir.createICmpEq(this.getEjsvalBits(arg), i64_const, name);
    }
    createEjsvalAnd(arg, i64_const, name) {
        return ir.createAnd(this.getEjsvalBits(arg), i64_const, name);
    }

    isObject(val) {
        if (this.triple.pointerSize() === 64) {
            return this.createEjsvalICmpUGt(
                val,
                consts.int64_lowhi(0xfffbffff, 0xffffffff),
                "cmpresult"
            );
        } else {
            let trunc = ir.createTrunc(this.getEjsvalBits(val), types.Int32, "trunc.i");
            return ir.createICmpEq(trunc, consts.int32(-120), "cmpresult");
        }
    }

    isObjectFunction(obj) {
        return ir.createICmpEq(
            this.emitLoadSpecops(obj),
            this.ejs_runtime.function_specops,
            "function_specops_cmp"
        );
    }
    ...
    isObjectSymbol(obj) {
        return ir.createICmpEq(
            this.emitLoadSpecops(obj),
            this.ejs_runtime.symbol_specops,
            "symbol_specops_cmp"
        );
    }
    ...
    isString(val) {
        if (this.triple.pointerSize() === 64) {
            let mask = this.createEjsvalAnd(
                val,
                consts.int64_lowhi(0xffff8000, 0x00000000),
                "mask.i"
            );
            return ir.createICmpEq(mask, consts.int64_lowhi(0xfffa8000, 0x00000000), "cmpresult");
        } else {
            let trunc = ir.createTrunc(this.getEjsvalBits(val), types.Int32, "trunc.i");
            return ir.createICmpEq(trunc, consts.int32(-123), "cmpresult");
        }
    }
    ...
    isBoolean(val) {
        if (this.triple.pointerSize() === 64) {
            ...
            let trunc = ir.createTrunc(this.getEjsvalBits(val), types.Int32, "trunc.i");
            return ir.createICmpEq(trunc, consts.int32(-125), "cmpresult");
        }
    }
    ...
    isUndefined(val) {
        if (this.triple.pointerSize() === 64) {
            return this.createEjsvalICmpEq(
                val,
                consts.int64_lowhi(0xfff90000, 0x00000000),
                "cmpresult"
            );
            ...
            let trunc = ir.createTrunc(this.getEjsvalBits(val), types.Int32, "trunc.i");
            return ir.createICmpEq(trunc, consts.int32(-126), "cmpresult");
        }
    }
    ...
    isNull(val) {
        if (this.triple.pointerSize() === 64) {
            ...
            let trunc = ir.createTrunc(this.getEjsvalBits(val), types.Int32, "trunc.i");
            return ir.createICmpEq(trunc, consts.int32(-121), "cmpresult");
        }
    }
    ...
    handleTypeofIsFunction(exp, opencode) {
        ...
        if (opencode && this.triple.pointerSize() === 64) {
            ...
            var typeofIsFunction_alloca = this.createAlloca(
                this.currentFunction,
                types.EjsValue,
                "typeof_is_function"
            );
            ...
            let rv = ir.createLoad(types.EjsValue, typeofIsFunction_alloca, "typeof_is_function");
            rv._ejs_returns_ejsval_bool = true;
            return rv;
        } else {
            return this.createCall(
                this.ejs_runtime.typeof_is_function,
                [arg],
                "is_function",
                false
            );
    ...
    handleTypeofIsSymbol(exp, opencode) {
        ...
        if (opencode && this.triple.pointerSize() === 64) {
            ...
            var typeofIsSymbol_alloca = this.createAlloca(
                this.currentFunction,
                types.EjsValue,
                "typeof_is_symbol"
            );
            ...
            let rv = ir.createLoad(types.EjsValue, typeofIsSymbol_alloca, "typeof_is_symbol");
            rv._ejs_returns_ejsval_bool = true;
            return rv;
        } else {
    ...
    handleIsNullOrUndefined(exp, opencode) {
        let arg = this.visitOrNull(exp.arguments[0]);
        if (opencode)
            return this.createEjsBoolSelect(
                ir.createOr(this.isNull(arg), this.isUndefined(arg), "or")
            );
        else
            return this.createCall(
                this.ejs_binops["=="],
                [this.loadNullEjsValue(), arg],
                "is_null_or_undefined",
                false
            );
    }

    handleBuiltinUndefined() {
        return this.loadUndefinedEjsValue();
    }
    ...
    handleSetPrototypeOf(exp) {
        ...
        return this.createCall(
            this.ejs_runtime.object_set_prototype_of,
            [obj, proto],
            "set_prototype_of",
            true
        );
    ...
    handleArrayFromRest(exp) {
        ...
        let rest_alloca = this.createAlloca(
            this.currentFunction,
            types.EjsValue,
            "local_rest_object"
        );
        ...
        let gep = ir.createInBoundsGetElementPointer(
            types.EjsValue.pointerTo(),
            load_args,
            [consts.int32(formal_params_length)],
            "rest_arg_gep"
        );
        load_argc = ir.createNswSub(load_argc, consts.int32(formal_params_length));
        load_argc = ir.createZExt(load_argc, types.Int64);
        let rest_value = this.createCall(
            this.ejs_runtime.array_new_copy,
            [load_argc, gep],
            "argstmp",
            !this.ejs_runtime.array_new_copy.doesNotThrow
        );
        ...
        rest_value = this.createCall(
            this.ejs_runtime.array_new,
            [consts.int64(0), consts.False()],
            "arrtmp",
            !this.ejs_runtime.array_new.doesNotThrow
        );
        ir.createStore(rest_value, rest_alloca);
        ...
        return ir.createLoad(types.EjsValue, rest_alloca, "load_rest");
    }
    ...
    handleArrayFromSpread(exp) {
        ...
        let visited = [];
        for (let a of exp.arguments) visited.push(this.visitOrNull(a));
    ...
    handleGetArg(exp) {
        ...
        let arg_value_alloca = this.createAlloca(
            this.currentFunction,
            types.EjsValue,
            `arg_${arg_num}_value`
        );
        ...
        this.doInsideBBlock(has_slot_bb, () => {
            ...
            let arg_ptr = ir.createGetElementPointer(
                types.EjsValue,
                load_args,
                [consts.int32(arg_num)],
                `arg${arg_num}_ptr`
            );
            let arg_load = this.createEjsValueLoad(arg_ptr, `arg${arg_num}`);
            if (
                exp.arguments.length > 1 &&
                (exp.arguments[1].type !== b.Literal || exp.arguments[1].value !== undefined)
            ) {
                // more complicated form, we need to check if the passed arg was undefined
                let arg_is_undefined = this.isUndefined(arg_load);
                let arg_select = ir.createSelect(
                    arg_is_undefined,
        ...
        ir.setInsertPoint(merge_bb);
        return ir.createLoad(types.EjsValue, arg_value_alloca, "load_arg_value");
    }
...
class AddFunctionsVisitor extends TreeVisitor {
    ...
    visitFunction(n) {
        if (n && n.id && n.id.name) n.ir_name = n.id.name;
        else n.ir_name = "_ejs_anonymous";
        ...
        if (this.dibuilder && this.difile)
            n.ir_func.debug_info = this.dibuilder.createFunction(
                this.difile,
                n.ir_name,
                n.displayName || n.ir_name,
                this.difile,
                lineno,
                false,
                true,
...
export function compile(tree, base_output_filename, source_filename, module_infos, options, triple) {
    ...
    if (options.ir) {
        let excluded =
            options.ir_exclude &&
            options.ir_exclude.some((pat) => source_filename.indexOf(pat) !== -1);
        if (excluded) {
            debug.log(1, `EIR: ${source_filename}: excluded via --ir-exclude`);
        } else {
            // whole-module or nothing: when the toplevel can't lower, the
            // module compiles on the legacy pipeline (per-function
            // candidate mode and its forwarding thunks are gone).  the
            // warning is the tell that legacy is still load-bearing
            // somewhere — those modules block the middle-end deletion.
            let toplevelLowered = collectEIRToplevel(tree, source_filename, module_infos, this_module_info, options);
            if (!toplevelLowered)
                console.warn(`ejs: ${source_filename}: module fell back to the legacy pipeline`);
        }
    }
    ...
    // use the suffix-stripped name: module bindings created during closure
    // conversion must use the same key the module was registered under in
    // module_infos (imports are always suffix-free; the main file isn't)
    tree = closure_convert(tree, module_filename, module_infos, options);
    ...
    this_module_info.exports.forEach((export_info, key) => {
        ...
        if (module_prop) module_accessors.push(module_prop);
    });
