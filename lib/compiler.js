...
import { Literal, Identifier, MemberExpression, ComputedPropertyKey, FunctionDeclaration, ClassDeclaration, VariableDeclaration, VariableDeclarator } from 'ast-builder';
...
module runtime from '../lib/runtime';
...
class ABI {
    ...
    createAlloca (func, type, name) {
        ...
        //        this.createCall this.llvm_intrinsics.gcroot(), [(ir.createPointerCast alloca, types.int8Pointer.pointerTo(), "rooted_alloca"), consts.nullConst types.int8Pointer], ""
...
class LLVMIRVisitor extends TreeVisitor {
    ...
    createPropertyStore (obj,prop,rhs,computed) {
        if (computed) {
            ...
            this.createCall(this.ejs_runtime.object_setprop, [obj, this.visit(prop), rhs], "propstore_computed");
	}
        else {
	    var pname;
            ...
            this.createCall(this.ejs_runtime.object_setprop, [obj, c, rhs], `propstore_${pname}`);
	}
    }
    ...
    visitBlock (n) {
        ...
        if (iife_dest_bb) {
            ...
            let rv = this.createLoad(this.findIdentifierInScope(n.ejs_iife_rv.name), "%iife_rv_load");
            return rv;
	}
        else {
            return n;
	...
	for (let _case of n.cases) {
		...
		let disc_cmp, disc_truthy;
                ...
                if (discTest._ejs_returns_ejsval_bool) {
                    ...
                    disc_cmp = ir.createICmpEq(disc_truthy, consts.falseConst(), "disccmpresult");
		}
	...
	let cmp, exp_value;
        if (exp.type === Literal && typeof exp.value === "boolean") {
            ...
            if (exp_value._ejs_returns_ejsval_bool) {
                ...
                cmp = ir.createICmpEq(cond_truthy, consts.falseConst(), "cmpresult");
	    }
	}
	...
	let lhs;
        ...
        this.doInsideExitableScope (new LoopExitableScope (n.label, forin_bb, merge_bb), () => {
            ...
            this.doInsideBBlock (forin_bb, () => {
                let moreleft = this.createCall(this.ejs_runtime.prop_iterator_next, [iterator, consts.trueConst()], "moreleft");
                let cmp = ir.createICmpEq(moreleft, consts.falseConst(), "cmpmoreleft");
	...
	for (let i = 0, e = n.params.length; i < e; i ++) {
        ...
        let args_load = this.createLoad(this.currentFunction.topScope.get("%args"), "args_load");
	for (let i = first_format_index, e = allocas.length; i < e; i ++)
	    this.storeUndefined(allocas[i]);
        ...
        if (n.formal_params.length > 0) {
            let load_argc = this.createLoad(this.currentFunction.topScope.get("%argc"), "argc");
                
	    for (let i = 0, e = n.formal_params.length; i < e; i ++) {
                let then_bb  = new llvm.BasicBlock ("arg_then", insertFunc);
		let else_bb  = new llvm.BasicBlock ("arg_else", insertFunc);
		let merge_bb = new llvm.BasicBlock ("arg_merge", insertFunc);

                let cmp = ir.createICmpSGt(load_argc, consts.int32(i), "argcmpresult");
                ...
                // args array
                let arg_ptr = ir.createGetElementPointer(args_load, [consts.int32(i)], `arg${i}_ptr`);
                let arg = this.createLoad(arg_ptr, `arg${i}_load`);
                let store = ir.createStore(arg, allocas[first_formal_index+i]);
                ir.createBr(merge_bb);
                ...
                if (n.defaults[i]) {
                    ...
                    arg = this.visit(n.defaults[i], `arg${i}_default_value`);
    ...
    visitUnaryExpression (n) {
		...
		throw new Error(`Internal error: unary operator '${n.operator}' not implemented`);
            this.createCall(callee, [this.visitOrNull(n.argument)], "result");
	}
    }
		...
		for (let a of args)
		    visited.push(this.visitOrNull(a));
                ...
                visited.forEach((a,i) => {
                    ...
                    ir.createStore(visited[i], gep, `argv[${i}]-store`);
		});
                ...
                argv.push(consts.nullConst(types.EjsValue.pointerTo()));
	    }
        }               
        else {
	    for (let a of args)
    ...
    visitArgsForConstruct (callee, args) {
        ...
        this.doInsideBBlock (creator_is_defined_bb, () => {
            let thisArg = this.createCall(this.ejs_runtime.invoke_closure, [creator, ctor, consts.int32(0), consts.nullConst(types.EjsValue.pointerTo())], "thisArg", true);
            ir.createStore(thisArg, this_alloca);
    ...
    visitArrayExpression (n) {
        let obj = this.createCall(this.ejs_runtime.array_new, [consts.int32(0), consts.falseConst()], "arrtmp", !this.ejs_runtime.array_new.doesNotThrow);
        let i = 0;
	...
	let cmp;
        ...
        if (opencode && this.options.target_pointer_size === 64) {
            ...
            if (call_result_is_object._ejs_returns_ejsval_bool) {
                ...
                cmp = ir.createICmpEq(cond_truthy, consts.trueConst(), "cmpresult");
	    }
	}
    ...
    handleGatherRest (exp) {
        ...
        rest_value = this.createCall(this.ejs_runtime.array_new, [consts.int32(0), consts.falseConst()], "arrtmp", !this.ejs_runtime.array_new.doesNotThrow);
        ir.createStore(rest_value, rest_alloca);
        ir.createBr(rest_merge_bb);
