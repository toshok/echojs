...
class LLVMIRVisitor extends TreeVisitor {
    ...
    visitFunction(n) {
        if (!n.toplevel)
            debug.log(
                () =>
                    `        function ${n.ir_name} at ${this.filename}:${n.loc
                        ? n.loc.start.line
                        : "<unknown>"}`
            );
...
*/
    ...
    visitCallExpression(n) {
        ...
        if (!intrinsicHandler)
            throw new Error(
                `Internal error: callee should not be null in visitCallExpression (callee = '${n
                    .callee.name}', arguments = ${n.arguments.length})`
            );
    ...
    visitLiteral(n) {
        ...
        if (typeof n.raw === "string" && n.raw[0] === "/") {
            ...
            let flags = consts.string(
                ir,
                `${n.value.global ? "g" : ""}${n.value.multiline ? "m" : ""}${n.value.ignoreCase
                    ? "i"
                    : ""}`
            );
    ...
    handleInvokeClosure(exp, opencode) {
        ...
        if (!this.currentFunction.scratch_area) {
            throw new Error(
                `Internal error: function has no scratch space and makes a [[Call]] call with ${exp
    ...
    handleConstructClosure(exp) {
        ...
        if (!this.currentFunction.scratch_area) {
            throw new Error(
                `Internal error: function has no scratch space and makes a [[Construct]] call with ${exp
                    .arguments.length} arguments`
            );
        }
