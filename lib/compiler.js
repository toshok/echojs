...
import { ExitableScope, TryExitableScope, SwitchExitableScope, LoopExitableScope, LabeledStatementExitableScope } from './exitable-scope';
...
class LLVMIRVisitor extends TreeVisitor {
    ...
    loadBoolEjsValue (n) {
        let rv = this.loadCachedEjsValue(n, (alloca) => {
            ...
            if (n)
                ir.createStore(consts.ejsval_true(this.options.target_pointer_size == 32), alloca_as_int64);
            else
                ir.createStore(consts.ejsval_false(this.options.target_pointer_size == 32), alloca_as_int64);
        });
        ...
        this.doInsideExitableScope (new SwitchExitableScope(merge_bb), () => {
            ...
            for (let casenum = 0; casenum < case_checks.length -1; casenum ++) {
                ...
                if (discTest._ejs_returns_ejsval_bool) {
                    disc_cmp = this.createEjsvalICmpEq(discTest, consts.ejsval_false(this.options.target_pointer_size === 32));
                }
                else {
    ...
    visitLabeledStatement (n) {
        if (n.body.type === b.ForInStatement || n.body.type === b.ForStatement || n.body.type === b.ForOfStatement ||
            n.body.type === b.DoWhileStatement || n.body.type === b.WhileStatement) {
        ...
        this.doInsideExitableScope (new LabeledStatementExitableScope(n.label.name, merge_bb), () => {
    ...
    generateCondBr (exp, then_bb, else_bb) {
        ...
        if (exp.type === b.Literal && typeof exp.value === 'boolean') {
            ...
            if (exp_value._ejs_returns_ejsval_bool) {
                cmp = this.createEjsvalICmpEq(exp_value, consts.ejsval_false(this.options.target_pointer_size === 32), 'cmpresult');
            }
    ...
    visitVariableDeclaration (n) {
        if (n.kind === 'var') throw new Error('internal compiler error.  var declarations should have been transformed to lets by this point.');
...
*/
    ...
    visitUnaryExpression (n) {
        ...
        if (n.operator === 'delete') {
            ...
            if (this.opencode_intrinsics.unaryNot && this.options.target_pointer_size === 64 && arg_value._ejs_returns_ejsval_bool) {
                let cmp = this.createEjsvalICmpEq(arg_value, consts.ejsval_true(false), 'cmpresult');
    ...
    visitTry (n) {
        ...
        if (scope.landing_pad_block) {
            ...
            this.doInsideBBlock (scope.landing_pad_block, () => {
                ...
                let caught_result = ir.createLandingPad(types.EjsLandingPad, clause_count, 'caught_result');
                ...
                if (!insertFunc.hasPersonality()) {
                    insertFunc.setPersonality(ir.createPointerCast(this.ejs_runtime.personality, types.Int8Pointer, 'personality'));
                }
    ...
    handleGetLocal  (exp) { return this.createLoad(this.findIdentifierInScope(exp.arguments[0].name), `load_${exp.arguments[0].name}`); }
    handleGetGlobal (exp) { return this.loadGlobal(exp.arguments[0]); }
    ...
    isTrue  (val) { return ir.createICmpEq(val, consts.ejsval_true(this.options.target_pointer_size === 32), 'cmpresult'); }
    isFalse (val) { return ir.createICmpEq(val, consts.ejsval_false(this.options.target_pointer_size === 32), 'cmpresult'); }
    ...
    handleBuiltinUndefined () { return this.loadUndefinedEjsValue(); }
    ...
    handleGetArg (exp) {
        ...
        this.doInsideBBlock(has_slot_bb, () => {
            ...
            if (exp.arguments.length > 1 && (exp.arguments[1].type !== b.Literal || exp.arguments[1].value !== undefined)) {
                ...
                let arg_select = ir.createSelect(arg_is_undefined, this.visit(exp.arguments[1]), arg_load, 'arg_select');
