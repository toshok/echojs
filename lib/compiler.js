...
class LLVMIRVisitor extends TreeVisitor {
    ...
    visitContinue (n) {
        if (n.label && n.label.name)
            return LoopExitableScope.findLabeledOrFinally().exitFore(n.label.name);
        else
    ...
    visitArgsForCall (callee, pullThisFromArg0, args) {
        ...
        if (callee.takes_builtins) {
            ...
            argv.push(consts.int32(0));            // %callFlags = EJS_CALL_FLAGS_CALL
    ...
    visitArgsForConstruct (callee, args, this_loc, newTarget_loc) {
        ...
        argv.push(consts.int32(1));       // %callFlags = EJS_CALL_FLAGS_CONSTRUCT
