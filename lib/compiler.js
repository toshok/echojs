...
import { collectEIRFunctions, collectEIRToplevel } from "./eir/integrate";
...
export function compile(tree, base_output_filename, source_filename, module_infos, options, triple) {
    ...
    if (options.ir) {
        ...
        if (excluded) {
            ...
            let toplevelLowered = false;
            if (options.ir_toplevel)
                toplevelLowered = collectEIRToplevel(tree, source_filename, module_infos, this_module_info, options);
            if (!toplevelLowered) {
                let { lowered, fellback } = collectEIRFunctions(tree, source_filename, module_infos, this_module_info, options);
                debug.log(1, `EIR: ${source_filename}: ${lowered} function(s) lowered, ${fellback} fell back`);
            }
        }
    }
