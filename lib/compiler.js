...
class LLVMIRVisitor extends TreeVisitor {
    ...
    loadBoolEjsValue(n) {
        let rv = this.loadCachedEjsValue(n, alloca => {
            ...
            if (n)
                ir.createStore(
                    consts.ejsval_true(this.options.target_pointer_size == 32),
                    alloca_as_int64
                );
            else
                ir.createStore(
                    consts.ejsval_false(this.options.target_pointer_size == 32),
                    alloca_as_int64
                );
        });
    ...
    storeGlobal(prop, value) {
        ...
        if (prop.type === b.Identifier)
            gname = prop.name; // prop.type is b.Literal
    ...
    createPropertyStore(obj, prop, rhs, computed) {
        if (computed) {
            ...
            // we store obj.prop, prop is an id
            if (prop.type === b.Identifier)
                pname = prop.name; // prop.type is b.Literal
    ...
    visitBlock(n) {
        ...
        if (iife_dest_bb) {
            ...
            let rv = this.createEjsValueLoad(this.findIdentifierInScope(iife_rv.name), "%iife_rv_load");
            return rv;
        } else {
            return n;
        }
...
*/
    ...
    visitUnaryExpression(n) {
        ...
        if (n.operator === "delete") {
            ...
            if (
                ...
                this.options.target_pointer_size === 64 &&
    ...
    visitArgsForCall(callee, pullThisFromArg0, args) {
        ...
        if (callee.takes_builtins) {
            ...
            if (args_length > 0) {
                const scratchAreaType = llvm.ArrayType.get(types.EjsValue, this.currentFunction.scratch_length);
    ...
    visitCallExpression(n) {
        ...
        if (!intrinsicHandler)
            throw new Error(
                `Internal error: callee should not be null in visitCallExpression (callee = '${
                    n.callee.name
    ...
    visitThisExpression() {
        ...
        return this.createEjsValueLoad(
            this.createLoad(types.EjsValue.pointerTo(), this.findIdentifierInScope("%this"), "load_this_ptr"),
            "load_this"
        );
    }
    ...
    visitObjectExpression(n) {
        let obj_proto = ir.createLoad(types.EjsValue, this.ejs_globals.Object_prototype, "load_objproto");
    ...
    createCall(callee, argv, callname, canThrow = true) {
        ...
        if (TryExitableScope.unwindStack.depth === 0 || callee.doesNotThrow || !canThrow) {
            ...
            calltmp = this.abi.createCall(this.currentFunction, callee.type, callee, argv, callname);
        } else {
                ...
                if (this.currentFunction.returnValueAlloca) {
                    ...
                    this.doInsideBBlock(return_tramp, () => {
                        if (this.finallyStack.length > 0) {
                            ...
                            this.createRet(
                                this.createEjsValueLoad(this.currentFunction.returnValueAlloca, "rv")
                            );
                        }
                    });
                }
    ...
    handleTemplateCallsite(exp) {
        ...
        ir.setInsertPoint(merge_bb);
        return this.createRet(ir.createLoad(types.EjsValue, callsite_alloca, "load_local_callsite"));
    }
    ...
    handleGetArgumentsObject() {
        ...
        let load_argc = this.createLoad(types.Int32, this.currentFunction.topScope.get("%argc"), "argc_load");
        let load_args = this.createLoad(types.EjsValue.pointerTo(), this.currentFunction.topScope.get("%args"), "args_load");
    ...
    emitEjsvalFromPtr(ptr, prefix) {
        if (this.options.target_pointer_size === 64) {
            ...
            ir.createLoad(types.EjsValue, fromptr_alloca, `${prefix}_load`);
        } else {
            throw new Error("emitEjsvalTo not implemented for this case");
        }
    }
    ...
    handleInvokeClosure(exp, opencode) {
        ...
        if (!this.currentFunction.scratch_area) {
            throw new Error(
                `Internal error: function has no scratch space and makes a [[Call]] call with ${
    ...
    handleConstructClosure(exp) {
        ...
        if (!this.currentFunction.scratch_area) {
            throw new Error(
                `Internal error: function has no scratch space and makes a [[Construct]] call with ${
                    exp.arguments.length
                } arguments`
            );
        }
    ...
    handleConstructSuper(exp) {
        let this_ptr = this.createLoad(types.EjsValue.pointerTo(), this.findIdentifierInScope("%this"), "this_ptr");
        let newTarget = this.createEjsValueLoad(this.findIdentifierInScope("%newTarget"), "load_newTarget");
    ...
    handleGetArg(exp) {
        ...
        let load_argc = this.createLoad(types.Int32, this.currentFunction.topScope.get("%argc"), "argc_n_load");
...
export function compile(tree, base_output_filename, source_filename, module_infos, options) {
    let abi =
        options.target_arch === "armv7" ||
        options.target_arch === "armv7s" /* || options.target_arch === 'x86'*/
            ? new SRetABI()
            : new ABI();
