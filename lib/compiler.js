...
const EjsFuncType = llvm.FunctionType.get (EjsValueType, [EjsContextType, EjsValueType, llvm.Type.getInt32Ty()]).pointerTo;

const stringType = llvm.Type.getInt8Ty().pointerTo;
const boolType = llvm.Type.getInt8Ty();
const voidType = llvm.Type.getVoidTy();
...
NodeVisitor.prototype.visitIf = function(n) {
  ...
  llvm.IRBuilder.createCall(this.ejs.truthy, [this.visit(n.test), truthy_stackalloc], "cond_truthy");
  var cond_truthy = llvm.IRBuilder.createLoad(truthy_stackalloc, "truthy_load");

  var insertBlock = llvm.IRBuilder.getInsertBlock();
  var insertFunc = insertBlock.parent;
...
NodeVisitor.prototype.visitVariableDeclaration = function(n) {
  if (n.kind == "var") {
    ...
    for (var i = 0, j = n.declarations.length; i < j; i++) {
      ...
      var init_needs_closure = (n.declarations[i].init.type === esprima.Syntax.FunctionExpression);

      if (init_needs_closure) {
  	initializer = llvm.IRBuilder.createCall(this.ejs.closure_new, [llvm.IRBuilder.createPointerCast(initializer, EjsFuncType, "castfunc"), llvm.Constant.getNull(EjsValueType)/*XXX environment*/], "closure");
      }
...
NodeVisitor.prototype.visitAssignmentExpression = function(n) {
  ...
  if (lhs.type === esprima.Syntax.Identifier) {
    var result = llvm.IRBuilder.createStore (this.visit(rhs), findIdentifierInScope(lhs.value, this.current_scope));
    return result;
  }
...
NodeVisitor.prototype.visitFunction = function(n) {
  ...
  for (var i = 0, j = n.params.length; i < j; i++) {
    if (n.params[i].type !== esprima.Syntax.Identifier) {
      debug ("we don't handle destructured/defaulted parameters yet");
  ...
  for (var i = 0, j = n.params.length; i < j; i++) {
    if (n.params[i].type === esprima.Syntax.Identifier) {
      ...
      debug ("we don't handle destructured args at the moment.");
...
NodeVisitor.prototype.visitBinaryExpression = function(n) {
  debug ("operator = '" + n.operator + "'");
  ...
  var args = [this.visit(n.left), this.visit(n.right), result];
  // call the add method
  var rv = llvm.IRBuilder.createCall(callee, args, "result");
  // load and return the result
  return llvm.IRBuilder.createLoad(result, "result_"+builtin+"_load");
};

NodeVisitor.prototype.visitLogicalExpression = function(n) {
  debug ("operator = '" + n.operator + "'");
  var builtin = "logop" + n.operator;
  var callee = this.ejs[builtin];
  // allocate space on the stack for the result
  var result = this.createAlloca(this.currentFunction, EjsValueType, "result_" + builtin);
  // call the add method
...
NodeVisitor.prototype.createLoadThis = function () {
  var _this = findIdentifierInScope(EJS_THIS_NAME, this.current_scope);
  return llvm.IRBuilder.createLoad (_this, "load_this");
};

NodeVisitor.prototype.createLoadContext = function () {
  var _context = findIdentifierInScope(EJS_CONTEXT_NAME, this.current_scope);
  return llvm.IRBuilder.createLoad (_context, "load_context");
};

NodeVisitor.prototype.visitCallExpression = function(n) {
  debug ("visitCall " + JSON.stringify(n));

  var callee;

  if (n.callee.type === esprima.Syntax.MemberExpression) {
    debug ("creating property load!");
    callee = this.createPropertyLoad (n.callee.object, n.callee.property);
  }
  else {
  ...
  if (callee_is_closure) {
    ...
    debug ("loading this and context");
    // we insert the extra BUILTIN_ARGS since we're calling a JS function
    args.unshift(this.createLoadThis());
    args.unshift(this.createLoadContext());

    compiled_arg_start = 2;
    debug ("done loading this and context");
  }
  else {
    debug ("ffi function, not loading this and context");
    compiled_arg_start = 0;
  }

  debug ("callee = " + callee.toString());
  ...
  debug ("done visiting args");
  if (callee_is_closure) {
    debug (1);
    var invoke_fun = this.ejs.invoke_closure[argv.length-compiled_arg_start];
    debug ("2 invoke_fun = this.ejs.invoke_closure[" + (argv.length-compiled_arg_start) + "]");
    ...
    debug ("callee == " + callee);
    return llvm.IRBuilder.createCall(callee, argv, callee.returnType && callee.returnType.isVoid() ? "" : "calltmp");
  }
};
...
NodeVisitor.prototype.visitPropertyAccess = function(n) {
  debug ("property access: " + nc[1].value); // NC-USAGE
  return n;
};
...
NodeVisitor.prototype.visitExpressionStatement = function (n) {
  var rv = this.visit (n.expression);
  return rv;
};

NodeVisitor.prototype.visitLiteral = function(n) {
  if (typeof (n.value) === "string") {
    debug ("string = " + n.value);
...
NodeVisitor.prototype.visit = function(n) {
  ...
  switch (n.type) {
  case syntax.Program: rv = this.visitProgram(n); break;
  case syntax.FunctionDeclaration: rv = this.visitFunction(n); break; // XXX esprima distinguishes between these two.  hallelujah
  case syntax.FunctionExpression: rv = this.visitFunction(n); break;  // XXX
  // XXX case GETTER: rv = this.visitGetter(n); break;
  // XXX case SETTER: rv = this.visitSetter(n); break;
  // XXX case SCRIPT: rv = this.visitScript(n); break;
  case syntax.BlockStatement: rv = this.visitBlock(n); break;
  // XXX case LET_BLOCK: rv = this.visitBlock(n); break;
  case syntax.ExpressionStatement: rv = this.visitExpressionStatement(n); break;
  case syntax.SwitchStatement: rv = this.visitSwitch(n); break;
  case syntax.SwitchCase: rv = this.visitCase(n); break;
  case syntax.ForStatement: rv = this.visitFor(n); break;
  case syntax.WhileStatement: rv = this.visitWhile(n); break;
  case syntax.IfStatement: rv = this.visitIf(n); break;
  case syntax.ForInStatement: rv = this.visitForIn(n); break;
  case syntax.DoWhileStatement: rv = this.visitDo(n); break;
  case syntax.BreakStatement: rv = this.visitBreak(n); break;
  case syntax.ContinueStatement: rv = this.visitContinue(n); break;
  case syntax.TryStatement: rv = this.visitTry(n); break;
  case syntax.ThrowStatement: rv = this.visitThrow(n); break;
  case syntax.ReturnStatement: rv = this.visitReturn(n); break;
  case syntax.YieldStatement: rv = this.visitYield(n); break;
  case syntax.WithStatement: rv = this.visitWith(n); break;
  case syntax.VariableDeclaration: rv = this.visitVariableDeclaration(n); break;
  case syntax.LabeledStatement: rv = this.visitLabeledState(n); break;
  case syntax.AssignmentExpression: rv = this.visitAssignmentExpression(n); break;
  case syntax.LogicalExpression: rv = this.visitLogicalExpression(n); break;
  case syntax.NewExpression: rv = this.visitNewExpression(n); break;
  case syntax.ThisExpression: rv = this.visitThisExpression(n); break;
  case syntax.BinaryExpression: rv = this.visitBinaryExpression(n); break;
  case syntax.RelationalExpression: rv = this.visitRelationalExpression(n); break;
  case syntax.Identifier: rv = this.visitIdentifier(n); break;
  case syntax.Literal: rv = this.visitLiteral(n); break;
  case syntax.CallExpression: rv = this.visitCallExpression(n); break;
  default:
  ...
  case INCREMENT:
  case DECREMENT:
  case DOT: rv = this.visitPropertyAccess(n); break;
  case INDEX: break;
  case NEW_WITH_ARGS:
  case ARRAY_INIT:
  case ARRAY_COMP:
