            ...
            this.doInsideBBlock(finally_block, () => {
                ...
                for (let s = 0, e = scope.destinations.length; s < e; s++) {
                    ...
                    this.doInsideBBlock(dest_tramp, () => {
                        if (dest.reason == TryExitableScope.REASON_BREAK) dest.scope.exitAft(true);
    ...
    if (options.ir) {
        ...
        if (excluded) {
            ...
            if (options.ir_toplevel)
                toplevelLowered = collectEIRToplevel(tree, source_filename, module_infos, this_module_info);
            if (!toplevelLowered) {
                let { lowered, fellback } = collectEIRFunctions(tree, source_filename, module_infos, this_module_info, options.ir_exclude_fn);
